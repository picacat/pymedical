...
# 藥袋格式1 241mm x 93mm
# 2024.01.08 脈悅
class PrintPrescriptionBagForm3:
    # 初始化
    ...
    def _print_patient(self, painter, medical_record):
        ...
        painter.drawText(105, lines[0], medical_record['case_date'])
        painter.drawText(335, lines[0], medical_record['patient_key'])

        font = QtGui.QFont(self.font_name, 16, QtGui.QFont.PreferQuality)
        painter.setFont(font)
        painter.drawText(105, lines[1], medical_record['patient_name'])
        painter.drawText(335, lines[1], f"{medical_record['gender']} / {medical_record['age']}")
    ...
    def _print_dosage(self, painter, medical_record):
        ...
        painter.drawText(550, lines[1], medical_record['pres_days'])
    ...
    def _print_prescript(self, painter):
        ...
        for row_no, row in enumerate(rows):
            ...
            painter.drawText(medicine_name_position, lines[0]+top, string_utils.xstr(row['MedicineName']))
            if number_utils.get_float(row['Dosage']) > 0:
                painter.drawText(
                    QtCore.QRect(dosage_position, lines[0]+top-16, 70, character_height),
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter,
                    f"{row['Dosage']:.1f}")
                painter.drawText(unit_position, lines[0]+top, string_utils.xstr(row['Unit']))
