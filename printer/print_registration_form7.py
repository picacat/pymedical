...
class PrintRegistrationForm7:
    ...
    def print_painter(self):
        ...
        painter.drawText(
            20, 50, medical_record['clinic_name'] + ' ' + medical_record['clinic_id'])

        font = QtGui.QFont(self.font_name, 10, QtGui.QFont.PreferQuality)
        painter.setFont(font)
        painter.drawText(
            20, 66, medical_record['clinic_telephone'] + ' ' + medical_record['clinic_address'])

        font = QtGui.QFont(self.font_name, 12, QtGui.QFont.PreferQuality)
        painter.setFont(font)
        painter.drawText(10, lines[1], medical_record['patient_key'])
        painter.drawText(
            130, lines[1], medical_record['patient_name'] + f"({medical_record['gender']})")

        font = QtGui.QFont(self.font_name, 10, QtGui.QFont.PreferQuality)
        painter.setFont(font)
        painter.drawText(130, lines[1]+14, medical_record['birthday'])

        font = QtGui.QFont(self.font_name, 12, QtGui.QFont.PreferQuality)
        painter.setFont(font)
        painter.drawText(270, lines[1], medical_record['visit'])
        painter.drawText(400, lines[1], medical_record['registration_no'])

        painter.drawText(10, lines[2]-5, f'{medical_record["room"]}診')
        painter.drawText(0, lines[2]+15, f'{medical_record["massager"]}醫師')

        painter.drawText(100, lines[2], medical_record['ins_type'])
        painter.drawText(210, lines[2], string_utils.xstr(medical_record['regist_fee']))
        painter.drawText(320, lines[2], string_utils.xstr(medical_record['diag_share_fee']))
        painter.drawText(450, lines[2], string_utils.xstr(medical_record['deposit_fee']))

        font = QtGui.QFont(self.font_name, 10, QtGui.QFont.PreferQuality)
        painter.setFont(font)
        painter.drawText(0, lines[3], medical_record['case_date'])

        remark = medical_record['remark']
        massage_fee = charge_utils.get_misc_fee(self.database, remark)
        if massage_fee > 0:
            painter.drawText(0, lines[3]+16, remark)

        font = QtGui.QFont(self.font_name, 12, QtGui.QFont.PreferQuality)
        painter.setFont(font)
        painter.drawText(100, lines[3], medical_record['case_time'])

        painter.drawText(210, lines[3], medical_record['card'])

        traditional_health_care_fee = medical_record['traditional_health_care_fee']
        if traditional_health_care_fee > 0:
            painter.drawText(300, lines[3], f'自費: {traditional_health_care_fee}')

        painter.drawText(400, lines[3], medical_record['registrar'])
    ...
    def _get_medical_record(self):
        ...
        traditional_health_care_fee = self._get_traditional_health_care_fee()
        ...
        medical_record['remark'] = string_utils.xstr(row['Remark'])
        medical_record['case_times'] = case_utils.get_case_times_this_month(
            self.database, medical_record['patient_key'])

        if medical_record['visit'] == '複診':
            medical_record['visit'] += f'({medical_record["case_times"]})'
        ...
        medical_record['traditional_health_care_fee'] = traditional_health_care_fee
