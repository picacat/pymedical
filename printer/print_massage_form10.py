...
# 民俗調理收據格式10 3"套表掛號單 (天璽)
# 2023.09.14
class PrintMassageForm10:
    # 初始化
    ...
    def _get_medical_record(self):
        ...
        SELECT
            ...
            patient.Birthday, patient.Gender, patient.DiscountType
        FROM cases
    ...
    def print_painter(self):
        ...
        painter.drawText(120, lines[1], medical_record['patient_name'] + f'({medical_record["gender"]})')
        ...
        if '針' in medical_record['treat_type']:
            treat_type = '針灸+民俗'
        elif '傷' in medical_record['treat_type']:
            treat_type = '傷科+民俗'
        else:
            treat_type = '內科+民俗'

        painter.drawText(190, lines[3], treat_type)

        traditional_health_care_fee = charge_utils.get_traditional_health_care_fee_from_case(
            self.database, self.case_key)
        painter.drawText(300, lines[3], '推拿師:' + string_utils.xstr(medical_record['massager']))
        painter.drawText(300, lines[3]+18, f'{massage_title}:' + str(traditional_health_care_fee))
