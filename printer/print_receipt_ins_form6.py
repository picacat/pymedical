...
class PrintReceiptInsForm6:
    ...
    def _get_prescript_html(self, row):
        ...
        prescript_record = printer_utils.get_prescript_html2(
            ...
            '費用收據', blocks=2, instruction=self.additional, max_line=4, is_print_dosage=self.print_dosage)
        ...
        if len(rows) > 0:
          ...
          if self.system_settings.field('列印印花稅總繳') == 'Y' and regist_fee >= 250:
    ...
    def _get_ins_fees_html(row):
        ...
        total_fee = regist_fee + diag_share_fee + drug_share_fee + deposit_fee - return_fee
