...
class PrintReceiptInsForm29:
    ...
    def _html(self):
        ...
        prescript_html, self.prescript_count = printer_utils.get_prescript_html24(
        ...
        pres_days = case_utils.get_pres_days(self.database, self.case_key, medicine_set=1)
        packages = case_utils.get_packages(self.database, self.case_key, medicine_set=1)
        instruction = case_utils.get_instruction(self.database, self.case_key, medicine_set=1)
        dosage_mode = case_utils.get_dosage_mode(self.database, self.case_key, medicine_set=1)

        total_fee = case_utils.get_total_fee(self.database, self.case_key, medicine_set=1)
        ...
        if pres_days > 0 and self.additional is None:
            ...
            _, total_dosage, _, single_day_dosage = case_utils.get_prescript_html_data(
                self.database, self.system_settings, self.case_key, medicine_set=1
            )
                ...
                <table width="100%" cellspacing="0" style="margin-left: 10px;">
                    ...
                    <tr>
                       <td>
                            ...
                            生日: {birthday_str}{tab}保險: {share_type} {card}
                       </td>
                    </tr>
                </table>
