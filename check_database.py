...
class CheckDatabase(QtWidgets.QDialog):
        ...
        if self.call_from == 'pymedical':
            process_list = [
                self.database.check_field_exists(
                    'patient', 'add', 'FamilyPatientKey', 'varchar(10) AFTER PrivateInsurance'),
                self.database.check_field_exists(
                    'patient', 'add', 'EmergencyContact', 'varchar(20) AFTER FamilyPatientKey'),
                self.database.check_field_exists(
                    'patient', 'add', 'EmergencyContactPhone', 'varchar(40) AFTER EmergencyContact'),
                self.database.check_field_exists(
                    'patient', 'add', 'EmergencyRelevant', 'varchar(100) AFTER EmergencyContactPhone'),
                self.database.check_field_exists(
            ...
            process_list = [
                ...
                self.database.check_field_exists('patient', 'add', 'NursingHome', 'varchar(50) after Allergy'),
                self.database.check_field_exists('patient', 'add', 'NursingHomeID', 'varchar(20) after NursingHome'),
                self.database.check_field_exists(
                    'patient', 'add', 'NursingHomeInDate', 'varchar(10) after NursingHomeID'),
            ]
    ...
    def _check_cases(self):
        if self.call_from == 'pymedical':
            process_list = [
                ...
                self.database.check_field_exists(
                    'cases', 'add', 'RegistPaymentType', 'varchar(20) DEFAULT "現金" AFTER DebtFee'),
                self.database.check_field_exists(
                    'cases', 'add', 'ChargePaymentType', 'varchar(20) DEFAULT "現金" AFTER RegistPaymentType'),
                self.database.check_field_exists('cases', 'add', 'DiseaseCode4', 'varchar(10) AFTER DiseaseName3'),
                self.database.check_field_exists('cases', 'add', 'DiseaseName4', 'varchar(40) AFTER DiseaseCode4'),
                self.database.check_field_exists('cases', 'add', 'DiseaseCode5', 'varchar(10) AFTER DiseaseName4'),
                self.database.check_field_exists('cases', 'add', 'DiseaseName5', 'varchar(40) AFTER DiseaseCode5'),
                self.database.check_field_exists('cases', 'add', 'RegistTypex', 'varchar(10) AFTER RegistType'),
                self.database.check_field_exists(
                    'cases', 'add', 'DrugPickupDone',
                    'ENUM("False", "True") NOT NULL DEFAULT "False" AFTER DrugDone'),
                self.database.check_field_exists(
                    'cases', 'add', 'IsClosed', 'tinyint(1) NOT NULL DEFAULT 0 AFTER Security'),
            ]
        else:
            process_list = [
                ...
                self.database.check_field_exists('cases', 'add', 'InvoiceNo', 'varchar(20) AFTER Security'),
                self.database.check_field_exists(
                ...
                self.database.check_field_exists('cases', 'add', 'CurativeEffect', 'int AFTER Cure'),
                self.database.check_field_exists('cases', 'add', 'TreatType', 'varchar(100) AFTER RegistType'),
                self.database.check_field_exists('cases', 'change', ['TreatType', 'TreatType'], 'varchar(100)'),
                self.database.check_field_exists('cases', 'change', ['Treatment', 'Treatment'], 'varchar(100)'),
                self.database.check_field_exists('cases', 'add', 'NursingAssistant', 'varchar(20) AFTER Cashier'),
                self.database.check_field_exists('cases', 'add', 'MassageReferrer', 'varchar(20) AFTER Cashier'),
                self.database.check_field_exists('cases', 'change', ['Card', 'Card'], 'varchar(15)'),
                self.database.check_field_exists('cases', 'change', ['Injury', 'Injury'], 'varchar(50)'),
                self.database.check_field_exists('cases', 'change', ['Share', 'Share'], 'varchar(50)'),
            ]
    ...
    def _check_dosage(self):
        if self.call_from == 'pymedical':
            process_list = [
                self.database.check_field_exists('dosage', 'add', 'TotalDosage', 'double(6,2) AFTER Days'),
                self.database.check_field_exists('dosage', 'add', 'TotalDosage', 'double(6,2) AFTER Days'),
                self.database.check_field_exists('dosage', 'add', 'FreeInsMedicine', 'varchar(4) AFTER Amount'),
                self.database.check_field_exists('dosage', 'add', 'NoPharmacy', 'varchar(4) AFTER FreeInsMedicine'),
            ]
        else:
            process_list = [
                self.database.check_field_exists('dosage', 'add', 'SelfTotalFee', 'int AFTER Instruction'),
                self.database.check_field_exists(
                    'dosage', 'add', 'DiscountRate', 'int DEFAULT 100 AFTER SelfTotalFee'),
    ...
    def _check_prescript(self):
        if self.call_from == 'pymedical':
            ...
            process_list = [
                ...
                self.database.check_field_exists(
                    'prescript', 'change', ['MedicineType', 'MedicineType'], 'varchar(10)'),
                self.database.check_field_exists('prescript', 'add', 'DiscountFee', 'decimal(10,2) AFTER Price'),
                self.database.check_field_exists('prescript', 'add', 'Promotion', 'varchar(10) AFTER Amount'),
                self.database.check_field_exists('prescript', 'add', 'Debt', 'decimal(10,2) AFTER DiscountFee'),
                self.database.check_field_exists('prescript', 'add', 'Dealer', 'varchar(10) AFTER Amount'),
                self.database.check_field_exists('prescript', 'add', 'Remark', 'varchar(200) AFTER Promotion'),
            ]
    ...
    def _check_medicine(self):
        if self.call_from == 'pymedical':
            process_list = [
                ...
                self.database.check_field_exists('medicine', 'add', 'NoDosage', 'varchar(4) AFTER Charged'),  # 不計算總量
                self.database.check_field_exists('medicine', 'add', 'NonNHI', 'varchar(4) AFTER NoDosage'),   # 僅用於自費
            ...
            process_list = [
                ...
                self.database.check_field_exists('medicine', 'add', 'Deactivate', 'varchar(50) AFTER HitRate'),
                self.database.check_field_exists('medicine', 'change', ['location', 'Location'], 'varchar(20)'),
                self.database.check_field_exists(
                    'medicine', 'change', ['MedicineType', 'MedicineType'], 'varchar(10)'),
                self.database.check_field_exists(
                    'medicine', 'change', ['MedicineCode', 'MedicineCode'], 'varchar(15)'),
                self.database.check_field_exists(
                    'medextend', 'change', ['MedExtendType', 'ExtendType'], 'varchar(10)'),
    ...
    def _check_person(self):
        if self.call_from == 'pymedical':
            ...
            process_list = [
                ...
                self.database.check_field_exists('person', 'add', 'Room', 'int AFTER Position'),
            ]
    ...
    def _check_reserve(self):
        if self.call_from == 'pymedical':
            process_list = [
                self.database.check_field_exists(
                    'reserve', 'add', 'Arrival', 'enum("False", "True") not null AFTER Doctor'),
                self.database.check_field_exists(
                    'doctor_month_schedule', 'add', 'CanReservation', 'ENUM("False", "True") NOT NULL AFTER Doctor'),
                self.database.check_field_exists(
                    'reservation_table', 'add', 'ReserveType', 'varchar(10) AFTER ReservationTableKey'),
                self.database.check_field_exists(
            ...
            process_list = [
                ...
                self.database.check_field_exists('reserve', 'add', 'CreateTime', 'datetime AFTER ReserveDate'),
            ]
    ...
    def _check_wait(self):
        if self.call_from == 'pymedical':
            process_list = [
                self.database.check_field_exists('wait', 'add', 'VHCReqCode', 'varchar(100) AFTER Remark'),
                self.database.check_field_exists('wait', 'change', ['Card', 'Card'], 'varchar(6)'),
                self.database.check_field_exists('wait', 'modify', 'VHCReqCode', 'varchar(256)'),
                self.database.check_field_exists(
                    'wait', 'add', 'DrugPickupDone',
                    'ENUM("False", "True") NOT NULL DEFAULT "False" AFTER DrugDone'),
            ]
        else:
            process_list = [
                ...
                self.database.check_field_exists('wait', 'change', ['TreatType', 'TreatType'], 'varchar(100)'),
                self.database.check_field_exists('wait', 'change', ['Share', 'Share'], 'varchar(50)'),
            ]
    ...
    def _check_ins_apply(self):
        if self.call_from == 'pymedical':
            process_list = [
                self.database.check_field_exists('insapply', 'add', 'DiagShareFee', 'int AFTER ShareFee'),
                self.database.check_field_exists('insapply', 'add', 'DrugShareFee', 'int AFTER DiagShareFee'),
                self.database.check_field_exists('insapply', 'add', 'ExamShareFee', 'int AFTER DrugShareFee'),
                self.database.check_field_exists(
                    'insapply', 'add', 'Identifier', 'varchar(20) AFTER Name'),
                self.database.check_field_exists(
                    'insapply', 'add', 'ActualIdentifier', 'varchar(20) AFTER Identifier'),
                self.database.check_field_exists(
                    'insapply', 'add', 'OriginalIdentifier', 'varchar(20) AFTER ActualIdentifier'),

                self.database.check_field_exists('insreply', 'add', 'ReplySeq', 'int AFTER Reason2'),
                self.database.check_field_exists('insreply', 'add', 'ReplyInsCode', 'varchar(12) AFTER ReplySeq'),
            ]
        else:
            process_list = [
                ...
                self.database.check_field_exists('insapply', 'add', 'CaseKey7', 'int(11) AFTER Percent6'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode7', 'varchar(12) AFTER CaseKey7'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee7', 'int AFTER TreatCode7'),
                self.database.check_field_exists('insapply', 'add', 'Percent7', 'int AFTER TreatFee7'),

                self.database.check_field_exists('insapply', 'add', 'CaseKey8', 'int AFTER Percent7'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode8', 'varchar(12) AFTER CaseKey8'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee8', 'int AFTER TreatCode8'),
                self.database.check_field_exists('insapply', 'add', 'Percent8', 'int AFTER TreatFee8'),

                self.database.check_field_exists('insapply', 'add', 'CaseKey9', 'int AFTER Percent8'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode9', 'varchar(12) AFTER CaseKey9'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee9', 'int AFTER TreatCode9'),
                self.database.check_field_exists('insapply', 'add', 'Percent9', 'int AFTER TreatFee9'),

                self.database.check_field_exists('insapply', 'add', 'CaseKey10', 'int AFTER Percent9'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode10', 'varchar(12) AFTER CaseKey10'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee10', 'int AFTER TreatCode10'),
                self.database.check_field_exists('insapply', 'add', 'Percent10', 'int AFTER TreatFee10'),

                self.database.check_field_exists('insapply', 'add', 'CaseKey11', 'int AFTER Percent10'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode11', 'varchar(12) AFTER CaseKey11'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee11', 'int AFTER TreatCode11'),
                self.database.check_field_exists('insapply', 'add', 'Percent11', 'int AFTER TreatFee11'),

                self.database.check_field_exists('insapply', 'add', 'CaseKey12', 'int AFTER Percent11'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode12', 'varchar(12) AFTER CaseKey12'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee12', 'int AFTER TreatCode12'),
                self.database.check_field_exists('insapply', 'add', 'Percent12', 'int AFTER TreatFee12'),

                self.database.check_field_exists('insapply', 'add', 'CaseKey13', 'int AFTER Percent12'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode13', 'varchar(12) AFTER CaseKey13'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee13', 'int AFTER TreatCode13'),
                self.database.check_field_exists('insapply', 'add', 'Percent13', 'int AFTER TreatFee13'),

                self.database.check_field_exists('insapply', 'add', 'CaseKey14', 'int AFTER Percent13'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode14', 'varchar(12) AFTER CaseKey14'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee14', 'int AFTER TreatCode14'),
                self.database.check_field_exists('insapply', 'add', 'Percent14', 'int AFTER TreatFee14'),

                self.database.check_field_exists('insapply', 'add', 'CaseKey15', 'int AFTER Percent14'),
                self.database.check_field_exists('insapply', 'add', 'TreatCode15', 'varchar(12) AFTER CaseKey15'),
                self.database.check_field_exists('insapply', 'add', 'TreatFee15', 'int AFTER TreatCode15'),
                self.database.check_field_exists('insapply', 'add', 'Percent15', 'int AFTER TreatFee15'),

                self.database.check_field_exists('insapply', 'add', 'DiseaseCode4', 'varchar(10) AFTER DiseaseCode3'),
                self.database.check_field_exists('insapply', 'add', 'DiseaseCode5', 'varchar(10) AFTER DiseaseCode4'),
            ]
    ...
    def _check_clinic(self):
        if self.call_from == 'pymedical':
            process_list = [
                self.database.check_field_exists('clinic', 'change', ['InputCode', 'InputCode'], 'varchar(10)'),
            ]
        else:
            process_list = [
                ...
                self.database.check_field_exists('clinic', 'change', ['ClinicName', 'ClinicName'], 'varchar(200)'),
            ]
    ...
    def _check_others(self):
        if self.call_from == 'pymedical':
            process_list = [
                self.database.check_field_exists('bulletin', 'add', 'Title', 'varchar(200) AFTER BulletinKey'),
                self.database.check_field_exists('reserve_cancel', 'add', 'ReserveBackup', 'TEXT AFTER Remark'),
                self.database.check_field_exists('stockinitems', 'change', ['UnitPrice', 'UnitPrice'], 'decimal(10, 2)'),
                self.database.check_field_exists('stockinitems', 'change', ['Amount', 'Amount'], 'decimal(10, 2)'),
                self.database.check_field_exists('stockin', 'change', ['Amount', 'Amount'], 'decimal(10, 2)'),
                self.database.check_field_exists('backup_records', 'modify', 'JSON', 'MEDIUMTEXT'),
                self.database.check_field_exists(
                    'stockinventory_items', 'add', 'Location', 'varchar(20) AFTER MedicineName'),
                # self.database.check_field_exists('bulletin', 'modify', 'Title', 'MEDIUMTEXT'),                
            ]
        else:
            process_list = [
                self.database.check_field_exists(
                    'dict_groups', 'add', 'DictOrderNo', 'varchar(10) AFTER DictGroupsKey'),
                ...
                self.database.check_field_exists('hospid', 'add', 'Telephone', 'varchar(50) AFTER HospName'),
                self.database.check_field_exists('hospid', 'add', 'Address', 'varchar(100) AFTER Telephone'),
                ...
                self.database.check_field_exists('images', 'change', ['Filename', 'Filename'], 'varchar(200)'),
                self.database.check_field_exists('temp_patient', 'add', 'Address', 'varchar(100) AFTER PhoneNo'),
                self.database.check_field_exists('temp_patient', 'add', 'Symptom', 'blob AFTER Address'),
                self.database.check_field_exists('temp_patient', 'add', 'Remark', 'blob AFTER Symptom'),
                self.database.check_field_exists('temp_patient', 'add', 'Cellphone', 'varchar(20) AFTER PhoneNo'),
                self.database.check_field_exists('hosts', 'add', 'Function', 'varchar(100) AFTER HISVersion'),
                self.database.check_field_exists('presextend', 'add', 'TimeStamp', 'TIMESTAMP AFTER Content'),
                self.database.check_field_exists(
                    'temporary_schedule', 'add', 'ScheduleType', 'varchar(10) AFTER CaseDate'),
                self.database.check_field_exists('backup_records', 'add', 'Editor', 'varchar(50) AFTER Deleter'),
                self.database.check_field_exists('extension_json', 'change', ['KeyValue', 'KeyValue'], 'varchar(50)'),
                self.database.check_field_exists(
                    'doctor_agent', 'add', 'OriginalDoctor', 'varchar(20) AFTER AgentDate'),
                self.database.check_field_exists(
                    'doctor_month_schedule', 'add', 'ProgressID', 'varchar(100) AFTER ScheduleID'),
                self.database.check_field_exists(
                    'temporary_schedule', 'add', 'Agent', 'varchar(10) AFTER Name'),
                self.database.check_field_exists(
                    'hosts', 'add', 'ImageDir', 'varchar(100) AFTER Function'),
                self.database.check_field_exists('pregnant', 'change', ['OvulateDATE', 'OvulateDate'], 'date'),
                self.database.check_field_exists(
    ...
    def _correct_records(self):
        ...
        if len(rows) > 0:
            self.database.exec_sql('UPDATE icd10 SET ICDCode = "N899" WHERE ChineseName = "白帶" and ICDCode = "12"')
    ...
    def _check_covid_post_icd_10(self):
        ...
        data = [icd_code, 'COVID', 'COVID-19後的病況，未明示', 'Post COVID-19 condition, unspecified']
    ...
    def _check_infectious_share(self):
        ...
        fields = ['ChargeType', 'InsType', 'ItemName', 'ShareType',
                  'TreatType', 'Course', 'InsCode', 'Amount', 'Remark']

        data = [
            '門診負擔', '健保', '行政協助法定傳染病通報且隔離案件',
            '法定傳染病通報隔離', '內科', '首次', '914', 0, '本次就醫醫療費用全部由疾管署支付'
        ]
    ...
    def _check_infectious_drug_share(self):
        ...
        fields = ['ChargeType', 'InsType', 'ItemName', 'ShareType', 'InsCode', 'Amount', 'Remark']

        data = [
            '藥品負擔', '健保', '行政協助法定傳染病通報且隔離案件',
            '法定傳染病通報隔離', '914', 0, '本次就醫醫療費用全部由疾管署支付'
        ]
        self.database.insert_record('charge_settings', fields, data)

    def _check_infectious_medicine(self):
        self._check_infectious_medicine_extra('(富田)台灣清冠一號濃縮顆粒', '1110022062')
    ...
    def _check_infectious_ins_charge(self):
        ...
        if len(rows) >= 2:
            self.database.exec_sql(f'DELETE FROM charge_settings WHERE InsCode = "{ins_code}"')

        fields = ['ChargeType', 'ItemName', 'InsCode', 'Amount', 'Remark']

        data = [
            '處置費', '台灣清冠一號藥品補助費', ins_code, 300,
