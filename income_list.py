...
class IncomeList(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_income = class_utils.get_table_widget(self.ui.tableWidget_income, self.database)
    ...
    def _set_table_widget_total(self):
        ...
        ins_heading = ['健保', '掛號費', '門診負擔', '藥品負擔', '欠卡費', '還卡費', '掛號欠款', '健保合計', '民俗調理', '現金總計']
        for col_no, heading in enumerate(ins_heading):
            self.ui.tableWidget_total.setItem(0, col_no, QtWidgets.QTableWidgetItem(heading))
            self.ui.tableWidget_total.item(0, col_no).setTextAlignment(QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter)
            self.ui.tableWidget_total.item(0, col_no).setBackground(QtGui.QColor('lightGray'))

        self_heading = ['自費', '掛號費', '自費金額', '應收合計', '欠款', '實收合計', '還款', '自費合計', '民俗調理', '現金總計']
        for col_no, heading in enumerate(self_heading):
            self.ui.tableWidget_total.setItem(2, col_no, QtWidgets.QTableWidgetItem(heading))
            self.ui.tableWidget_total.item(2, col_no).setTextAlignment(QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter)
            self.ui.tableWidget_total.item(2, col_no).setBackground(QtGui.QColor('lightGray'))

    # 設定信號
    def _set_signal(self):
        self.ui.tableWidget_income.doubleClicked.connect(self.open_medical_record)
        self.ui.tableWidget_income.horizontalHeader().sectionClicked.connect(self._header_clicked)
        self.ui.toolButton_set_traditional_health_case.clicked.connect(
            lambda: self._set_traditional_health_case(case_key=None))
        self.ui.toolButton_set_batch_traditional_health_case.clicked.connect(self._set_batch_traditional_health_case)
    ...
    def _set_table_width(self):
        width = [
            100, 50, 180, 50, 75, 90, 50, 90, 90, 90, 80, 50,
            70, 70, 70, 70, 70, 70, 70, 70, 50, 70, 80,
            80, 80, 60,
        ]
    ...
    def _set_check_box(self):
        for row_no in range(self.ui.tableWidget_income.rowCount()):
            ...
            self.ui.tableWidget_income.setCellWidget(
                row_no, col_no, check_box_remark)
    ...
    def _set_table_widget_total_value(self):
        ...
        diag_share_fee = number_utils.get_integer(
            self.ui.tableWidget_income.item(row_no, self.columns['diag_share_fee']).text())
        drug_share_fee = number_utils.get_integer(
            self.ui.tableWidget_income.item(row_no, self.columns['drug_share_fee']).text())
        ...
        ins_total = ins_regist_fee + diag_share_fee + drug_share_fee + deposit_fee + refund_fee + regist_debt
        ins_fee_list = [
            ins_regist_fee, diag_share_fee, drug_share_fee, deposit_fee, refund_fee, regist_debt, ins_total,
        ]

        self_regist_fee = self._get_regist_fee_from_table_widget('自費')
        self_total_fee = number_utils.get_integer(
            self.ui.tableWidget_income.item(row_no, self.columns['self_total_fee']).text())
        ...
        self_fee_list = [
            self_regist_fee, self_total_fee, total_fee, debt, receipt_fee, repayment, self_total]

        for col_no, ins_fee in enumerate(ins_fee_list):
            self.ui.tableWidget_total.setItem(1, col_no+1, QtWidgets.QTableWidgetItem(string_utils.xstr(ins_fee)))
            self.ui.tableWidget_total.item(1, col_no+1).setTextAlignment(
                QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
            )

        for col_no, self_fee in enumerate(self_fee_list):
            self.ui.tableWidget_total.setItem(3, col_no+1, QtWidgets.QTableWidgetItem(string_utils.xstr(self_fee)))
            self.ui.tableWidget_total.item(3, col_no+1).setTextAlignment(
                QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
            )

        self.ui.tableWidget_total.setItem(1, 8, QtWidgets.QTableWidgetItem(string_utils.xstr(massage_fee)))
        self.ui.tableWidget_total.item(1, 8).setTextAlignment(
            QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter
        )

        self.ui.tableWidget_total.setItem(1, 9, QtWidgets.QTableWidgetItem(string_utils.xstr(cash_total)))
    ...
    def _get_regist_fee_from_table_widget(self, in_ins_type):
        ...
        for row_no in range(self.ui.tableWidget_income.rowCount()-1):
            ...
            regist_fee += number_utils.get_integer(
                self.ui.tableWidget_income.item(row_no, self.columns['regist_fee']).text())
    ...
    def _table_widget_income_patient_key_exists(self, patient_key, name, ins_type):
        row_no_exists = None
        for row_no in range(self.ui.tableWidget_income.rowCount()):
            patient_key_item = self.ui.tableWidget_income.item(row_no, self.columns['patient_key'])
            name_item = self.ui.tableWidget_income.item(row_no, self.columns['name'])
            ins_type_item = self.ui.tableWidget_income.item(row_no, self.columns['ins_type'])
            if (patient_key_item is not None and patient_key_item.text() == patient_key and
                    name_item is not None and name_item.text() == name):
    ...
    def _get_registration_item(self, registration_row_no):
        cell_data = [
            [self.columns['case_key'], self.tableWidget_registration.item(registration_row_no, 0)],
            [self.columns['case_date'], self.tableWidget_registration.item(registration_row_no, 1)],
            [self.columns['period'], self.tableWidget_registration.item(registration_row_no, 2)],
            [self.columns['patient_key'], self.tableWidget_registration.item(registration_row_no, 3)],
            [self.columns['name'], self.tableWidget_registration.item(registration_row_no, 4)],
            [self.columns['ins_type'], self.tableWidget_registration.item(registration_row_no, 5)],
            [self.columns['share_type'], self.tableWidget_registration.item(registration_row_no, 6)],
            [self.columns['treat_type'], self.tableWidget_registration.item(registration_row_no, 7)],    # 負擔類別
            [self.columns['discount_type'], self.tableWidget_registration.item(registration_row_no, 8)],    # 優待類別

            [self.columns['card'], self.tableWidget_registration.item(registration_row_no, 9)],    # 卡序

            [self.columns['regist_fee'], self.tableWidget_registration.item(registration_row_no, 10)],  # 掛號費
            [self.columns['diag_share_fee'], self.tableWidget_registration.item(registration_row_no, 11)],  # 門診負擔
            [self.columns['deposit_fee'], self.tableWidget_registration.item(registration_row_no, 12)],  # 欠卡費
            [self.columns['refund_fee'], self.tableWidget_registration.item(registration_row_no, 13)],  # 還卡費

            [self.columns['repayment'], self.tableWidget_registration.item(registration_row_no, 15)],  # 自費還款
            [self.columns['regist_debt'], self.tableWidget_registration.item(registration_row_no, 14)],  # 掛號欠款

            [self.columns['registrar'], self.tableWidget_registration.item(registration_row_no, 17)],  # 掛號者
            [self.columns['regist_no'], self.tableWidget_registration.item(registration_row_no, 19)],  # 診號
        ]
    ...
    def _insert_registration_item(self, income_row_no, registration_row_no):
        ...
        for cell in cell_data:
            ...
            if cell[0] in [self.columns['refund_fee'], self.columns['repayment']]:  # 還卡費,還款另外計算 (當天可能還兩筆)
                continue
        ...
        total_regist_fee = number_utils.get_integer(
            self.ui.tableWidget_income.item(income_row_no, self.columns['regist_fee']).text())
        regist_fee = number_utils.get_integer(
            self.tableWidget_registration.item(registration_row_no, 10).text())  # 櫃台結帳分析表

        total_refund_fee = number_utils.get_integer(
            self.ui.tableWidget_income.item(income_row_no, self.columns['refund_fee']).text())
        refund_fee = number_utils.get_integer(
            self.tableWidget_registration.item(registration_row_no, 13).text())  # 櫃台結帳分析表
        ...
        total_repayment = number_utils.get_integer(
            self.ui.tableWidget_income.item(income_row_no, self.columns['repayment']).text())
        repayment = number_utils.get_integer(
            self.tableWidget_registration.item(registration_row_no, 15).text())  # 櫃台結帳分析表
    ...
    def _get_charge_item(self, charge_row_no, ins_type=None, treat_type=None):
        ...
        cell_data = [
            ...
            [self.columns['patient_key'], self.tableWidget_charge.item(charge_row_no, 3)],
            ...
            [self.columns['share_type'], self.tableWidget_charge.item(charge_row_no, 6)],
            [self.columns['treat_type'], self.tableWidget_charge.item(charge_row_no, 7)],
            [self.columns['discount_type'], self.tableWidget_charge.item(charge_row_no, 8)],  # 優待類別

            [self.columns['card'], self.tableWidget_charge.item(charge_row_no, 9)],    # 卡序
            [self.columns['pres_days'], self.tableWidget_charge.item(charge_row_no, 10)],   # 藥日

            [self.columns['drug_share_fee'], self.tableWidget_charge.item(charge_row_no, 12)],  # 藥品負擔
            [self.columns['self_total_fee'], total_fee_item],  # 自費應收
            [self.columns['debt'], self.tableWidget_charge.item(charge_row_no, 15)],  # 自費欠款
            [self.columns['receipt_fee'], self.tableWidget_charge.item(charge_row_no, 16)],  # 實收現金

            [self.columns['cashier'], self.tableWidget_charge.item(charge_row_no, 17)],  # 醫師
            [self.columns['regist_no'], self.tableWidget_charge.item(charge_row_no, 19)],  # 診號
    ...
    def _insert_charge_item(self, income_row_no, charge_row_no):
        ...
        cell_data = self._get_charge_item(charge_row_no, ins_type=ins_type, treat_type=treat_type)

        for cell in cell_data:
            col_no = cell[0]
            if (self.ui.tableWidget_income.item(income_row_no, col_no) is not None and
    ...
    def _separate_massage_fee(self):
        ...
        for row_no in range(row_count):
            ...
            ins_type_item = self.ui.tableWidget_income.item(row_no, self.columns['ins_type'])
            if ins_type_item is None or ins_type_item.text() == '健保':  # 只統計自費民俗調理
                continue
            ...
            case_key = self.ui.tableWidget_income.item(row_no, self.columns['case_key']).text()
            ...
            self.ui.tableWidget_income.setItem(row_no, self.columns['self_total_fee'], item)
            self.ui.tableWidget_income.item(
                row_no, self.columns['self_total_fee']).setTextAlignment(
                QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)

            item = QtWidgets.QTableWidgetItem()
            item.setData(QtCore.Qt.EditRole, massage_fee)
            self.ui.tableWidget_income.setItem(row_no, self.columns['massage_fee'], item)
            self.ui.tableWidget_income.item(
                row_no, self.columns['massage_fee']).setTextAlignment(
                QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
    ...
    def _calculate_subtotal(self):
        ...
        for row_no in range(row_count):
            subtotal = 0
            for col_no in range(self.columns['regist_fee'], self.columns['massage_fee']+1):  # regist_fee...massage_fee
            ...
            self.ui.tableWidget_income.item(
                row_no, self.columns['receipt_fee']).setTextAlignment(
                QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
            )
    ...
    def _calculate_total(self):
        ...
        for row_no in range(row_count):
            treat_type = self.ui.tableWidget_income.item(row_no, self.columns['treat_type']).text()
            ...
            receipt_fee += number_utils.get_integer(
                self.ui.tableWidget_income.item(row_no, self.columns['receipt_fee']).text())
    ...
    def _merge_traditional_health_care(self):
        for row_no in range(self.ui.tableWidget_income.rowCount(), -1, -1):
            ...
            if treat_type in ['民俗調理', '自費健保']:
                case_key = self.ui.tableWidget_income.item(row_no, self.columns['case_key']).text()
                patient_key = self.ui.tableWidget_income.item(row_no, self.columns['patient_key']).text()
                ...
                if replace_row_no is not None:
                    ...
                    massage_fee = self.ui.tableWidget_income.item(row_no, self.columns['receipt_fee'])
                    if massage_fee is not None:
                        massage_fee = massage_fee.text()
                    else:
                        massage_fee = 0

                    self._merge_fee(replace_row_no, self.columns['massage_fee'], massage_fee)
    ...
    def _read_return_goods(self):
        ...
        for row in rows:
            ...
            for col_no, cell in enumerate(cells):
                ...
                if col_no in [3]:
                    self.ui.tableWidget_income.item(
                        row_count-1, col_no).setTextAlignment(
                        QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter
                    )
                elif col_no in [4]:
                    self.ui.tableWidget_income.item(
    ...
    def _merge_fee(self, row_no, col_no, fee):
        ...
        self.ui.tableWidget_income.setItem(
            row_no, col_no, QtWidgets.QTableWidgetItem(string_utils.xstr(origin_fee + number_utils.get_integer(fee))))
        self.ui.tableWidget_income.item(
            row_no, col_no).setTextAlignment(
            QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
        )
    ...
    def export_to_excel(self):
        ...
        export_utils.export_income_list(
            self.system_settings, excel_file_name, self.ui.tableWidget_income, self.columns)
    ...
    def _set_batch_traditional_health_case(self):
        ...
        for row_no in range(row_count):
            ...
            check_box = self.ui.tableWidget_income.cellWidget(row_no, self.columns['remark'])
            if check_box is None or not check_box.isChecked():
                continue

            case_key_item = self.ui.tableWidget_income.item(row_no, self.columns['case_key'])
            if case_key_item is None:
                continue

            case_key = case_key_item.text()
            ins_type = self.ui.tableWidget_income.item(row_no, self.columns['ins_type']).text()
            treat_type = self.ui.tableWidget_income.item(row_no, self.columns['treat_type']).text()
            self._set_traditional_health_case(
                case_key=case_key, ins_type=ins_type, treat_type=treat_type,
                traditional_health_care_fee=traditional_health_care_fee,
                set_massager=False, refresh_record=False)
    ...
    def _set_traditional_health_case(
            self, case_key=None, ins_type=None, treat_type=None,
            set_massager=True, traditional_health_care_fee=None, refresh_record=True):
        if case_key is None:
            case_key = self.table_widget_income.field_value(self.columns['case_key'])
            ins_type = self.table_widget_income.field_value(self.columns['ins_type'])
            treat_type = self.table_widget_income.field_value(self.columns['treat_type'])
        ...
        if massager is None and set_massager:
            ...
            input_dialog = dialog_utils.get_dialog(
                '推拿師選擇', '請選擇民俗調理的推拿師父', None, QInputDialog.TextInput, 320, 200
            )
        ...
        if traditional_health_care_fee is None:
            ...
            input_dialog = dialog_utils.get_dialog(
                '設定費用', '請設定民俗調理費', string_utils.xstr(fee), QInputDialog.IntInput, 320, 200
            )
            if not input_dialog.exec_():
                return
        ...
        if ins_type == '健保' or (ins_type == '自費' and treat_type != '民俗調理'):
            case_utils.write_traditional_health_care(
                ...
                traditional_health_care_fee=traditional_health_care_fee, massager=massager
            )
        else:
