...
from libs import (case_utils, class_utils, cshis_utils, date_utils,
                  dialog_utils, nhi_utils, number_utils, patient_utils,
...
# 健保ICD卡 2018.03.31
class CSHIS:
    def __init__(self, parent, database, system_settings, ic_card_type='健保卡', qrcode=None):
        self.parent = parent
    ...
    def input_hpc_pin(self):
        ...
        system_utils.show_message_box(
            ...
            title=title, text=message, informative=hint,
        )
    ...
    def get_hc_signature(self, service_type):
        ...
        # if hc_mode == 0:  # 未置入卡片
        #     cshis_utils.show_ic_card_message(1102, '讀取健保卡簽章')  # 未置入卡片
        #     return None
    ...
    def read_basic_data(self, show_error=True):
        ...
        response = self._get_requests_response(service_path, 'POST', data, local_url=False)
    ...
    def _get_qrcode(self):
        ...
        input_dialog = dialog_utils.get_dialog(
            '虛擬健保卡', '請讀取新版虛擬健保卡 QRCode 2.0',
            None, QInputDialog.TextInput, 600, 200
        )
        ok = input_dialog.exec_()
        if not ok:
            return None
    ...
    def read_register_basic_data_by_vhc(self):
        ...
        self.basic_data = {
            ...
            'insured_mark': cshis_utils.get_insured_mark(card_content['identityStatus']),
    ...
    def get_card_status(self):
        ...
        available_date = date_utils.nhi_date_to_west_date(response.json()['cardValidity'])
    ...
    def write_treatment_code_fee_thread(
            self, out_queue, registration_datetime, treat_after_check,
            disease_code1, disease_code2, disease_code3, disease_code4, share_fee):
    ...
    def write_treatment_code_fee(
            self, registration_datetime,
            treat_after_check, disease_code1, disease_code2, disease_code3, disease_code4, share_fee):
        title = '寫入診察資料'
        ...
        msg_queue = Queue()
        QtCore.QCoreApplication.processEvents()
        t = Thread(target=self.write_treatment_code_fee_thread,
                   args=(msg_queue, registration_datetime, treat_after_check,
                         disease_code1, disease_code2, disease_code3, disease_code4, share_fee))
        t.start()
        (error_code) = msg_queue.get()
        msg_box.close()
    ...
    def upload_data(self, upload_type, xml, case_count):
        ...
        t = Thread(target=self.upload_data_thread, args=(
            msg_queue, upload_type, xml, case_count))
        t.start()
    ...
    def write_ic_card_abnormal(self, patient_id):
        ...
        self.treat_data = cshis_utils.decode_cshis6_no_ic_card_treat_data(response.json())
    ...
    def write_medicine_signature(self, case_row, patient_row, prescript_rows, dosage_row):
        ...
        for row in prescript_rows:
            ...
            try:
                total_dosage = round(
                    number_utils.get_float(row['Dosage']) * number_utils.get_integer(dosage_row['Days']), 2)
            except TypeError:
                total_dosage = 0
        ...
        prescript_sign_list, hex_prescript_sign_list = self.write_multi_prescript_sign(reg_datetime, prescriptions)
    ...
    def write_treat_signature(self, case_row, dosage_row, patient_row):
        ...
        treat_sign, hex_treat_sign = self.write_multi_prescript_sign(reg_datetime, prescription)
    ...
    def write_ic_treatment(self, case_key, treat_after_check):
        ...
        if not self.write_treatment_code_fee(
            reg_datetime, treat_after_check,
