...
class CSHIS(QThread):
    ...
    def run(self):
        self.update_hc(show_message=False)
    ...
    def get_seq_number_256(self, treat_item, baby_treat, treat_after_check):
        ...
        t = Thread(
            target=self.get_seq_number_256_thread, args=(msg_queue, treat_item, baby_treat, treat_after_check, )
        )
        t.start()
    ...
    # ic卡寫卡
    def write_ic_card(self, write_type, patient_key, course, share_type, treat_after_check=None):
    ...
    # ic 醫令寫卡
    def rewrite_ic_prescript(self, case_key, treat_after_check):
        self.write_prescript_signature(case_key)  # 寫入醫令簽章
        case_utils.update_xml(
            self.database, 'cases', 'Security', 'prescript_sign_time',
            date_utils.now_to_str(), 'CaseKey', case_key
