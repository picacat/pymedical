...
class ICUploadXML2:
    def __init__(
        self, parent, database, system_settings, tableWidget_medical_record, upload_type
    ):
        self.parent = parent
    ...
    def get_upload_type(self, medical_record):
        upload_type = case_utils.extract_security_xml(
            medical_record["Security"], "資料格式"
        )
    ...
    def add_mb1(self, mb, medical_record, upload_type):
        ...
        if upload_type in ["A"]:
            clinic_id = case_utils.extract_security_xml(
                medical_record["Security"], "院所代號"
            )
            card = case_utils.extract_security_xml(
                medical_record["Security"], "健保卡序"
            )
        ...
        treat_after_check = case_utils.extract_security_xml(
            medical_record["Security"], "補卡註記"
        )
        if treat_after_check in ["", None]:
            treat_after_check = "1"

        treat_item = cshis_utils.get_treat_item(
            medical_record["Continuance"], medical_record["Share"]
        )
        if (
            card in ["", None]
            and treat_item == "03"
            and medical_record["TreatType"] in nhi_utils.HOME_CARE
        ):
            treat_item = "AH"

        if upload_type in ["A"] and treat_item in [
            "AA",
            "AD",
        ]:  # 正常上傳療程及職傷不要放就醫序號
            card = ""
        ...
        if (
            medical_record["CaseDate"].strftime("%Y-%m-%d") <= "2024-12-31"
        ...
        share_fee = number_utils.get_integer(
            medical_record["DiagShareFee"]
        ) + number_utils.get_integer(medical_record["DrugShareFee"])

        if upload_type in ["A"]:
            sam_id = case_utils.extract_security_xml(
                medical_record["Security"], "安全模組"
            )
        ...
        m04.text = string_utils.xstr(
            date_utils.west_date_to_nhi_date(patient_record["Birthday"])
        )
        m05 = ET.SubElement(mb1, "M05")
        m05.text = string_utils.xstr(clinic_id)
        m06 = ET.SubElement(mb1, "M06")
        m06.text = string_utils.xstr(
        ...
        security_signature = case_utils.extract_security_xml(
            medical_record["Security"], "安全簽章"
        )
        ...
        if treat_item in ["AA", "AH"]:  # 連續療程
            ...
            if original_security is None:
                ...
                original_identification = case_utils.extract_security_xml(
        ...
        if pres_days > 0:
            ...
            if upload_type in ["A"]:
                medicine_order_count = nhi_utils.get_medicine_order_count(
        ...
        if treat_after_check == "2":
            ...
            registered_date = case_utils.get_case_extend(
                self.database, case_key, "實際就醫日期"
            )
        ...
        if treat_after_check == "2":
            identification = case_utils.get_case_extend(
    ...
    def add_mb2(self, mb, medical_record, upload_type):
        ...
        if tour_area is not None:
            ...
            if correction_area_code is not None:
                self._add_correction_area_order(
                    mb, medical_record, upload_type, correction_area_code
                )
    ...
    def _add_correction_area_order(
        self, mb, medical_record, upload_type, correction_area_code
    ):
    ...
    def add_medicine(self, mb, medical_record, upload_type):
        ...
        if (
            "G000" in string_utils.xstr(medical_record["Card"]) and len(rows) > 0
        ):  # 新特約院所開藥增加虛擬碼 R005, 避免雲端用藥重複被核扣
            self._add_virtual_order(mb, medical_record, upload_type)

        ic_card_type = case_utils.get_ic_card_type(self.database, case_key)
        if (
            number_utils.get_integer(medical_record["Continuance"]) >= 2
            and ic_card_type == "虛擬健保卡"
        ):
            self._add_vhc_ic_card(mb, medical_record, upload_type)

        if (
            string_utils.xstr(medical_record["RegistType"]) in nhi_utils.TELECOM_TYPE
            or string_utils.xstr(medical_record["RegistType"])
            in nhi_utils.INFECTIOUS_INJURY_TYPE
            or string_utils.xstr(medical_record["Share"])
            in nhi_utils.INFECTIOUS_INJURY_TYPE
            or string_utils.xstr(medical_record["Injury"])
            in nhi_utils.INFECTIOUS_INJURY_TYPE
        ):
    ...
    def _add_covid19(self, mb, medical_record, upload_type):
        ...
        prescript_type = "G"  # G-虛擬醫令
        if (
            string_utils.xstr(medical_record["RegistType"])
            in ["視訊門診"] + nhi_utils.INFECTIOUS_INJURY_TYPE
        ):
            ins_code = "ViT-COVID19"
        elif (
            string_utils.xstr(medical_record["Injury"])
            in nhi_utils.INFECTIOUS_INJURY_TYPE
        ):
            ins_code = "ViT-COVID19"
        elif (
            string_utils.xstr(medical_record["Share"])
            in nhi_utils.INFECTIOUS_INJURY_TYPE
        ):
            ins_code = "ViT-COVID19"
    ...
    def add_medicine_rows(
        self, mb, medical_record, prescript_row, dosage_row, case_key, upload_type
    ):
        mb2 = ET.SubElement(mb, "MB2")
