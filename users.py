...
class Users(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_users = class_utils.get_table_widget(self.ui.tableWidget_users, self.database)
    ...
    def _set_table_width(self):
        width = [
            100,
            80, 100, 130, 50, 130, 150, 100, 50, 60, 200, 180, 120,
    ...
    def _read_users(self, show_all=False):
        ...
        sql = f'''
            SELECT * FROM person
                {condition}
            ORDER BY FIELD(
                Position, "醫師", "支援醫師", "藥師", "護士", "護理師", "職員", "理療師", "推拿師父", NULL),
                Code, Name, PersonKey
        '''
    ...
    def _set_user_data(self, row_no, row):
        ...
        if (self.system_settings.field('使用者') == '超級使用者' or
                personnel_utils.get_permission(
                    self.database, self.program_name, '查看使用者密碼', self.user_name) == 'Y'):
        ...
        for column in range(len(user_row)):
            self.ui.tableWidget_users.setItem(
                row_no, column,
                QtWidgets.QTableWidgetItem(user_row[column])
            )
    ...
    def _open_permission_dialog(self):
        ...
        dialog = dialog_utils.get_dialog_permission(self, self.database, self.system_settings, person_key)
        dialog.exec_()
        dialog.deleteLater()
