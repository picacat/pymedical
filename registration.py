...
from libs import (alleypin_utils, case_utils, charge_utils, class_utils,
                  ...
                  string_utils, system_utils, ui_utils, validator_utils,
                  vhc_utils, web_utils)
...
class Registration(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, *args):
        ...
        if self.system_settings.field('使用讀卡機') == 'N' and self.system_settings.field('新特約期間使用晶片讀卡機') == 'Y':
    ...
    def _set_ui(self):
        ...
        # )
        self.table_widget_wait = class_utils.get_table_widget(self.ui.tableWidget_wait, self.database)
        ...
        if self.system_settings.field('掛號名單顯示民俗調理費') != 'Y':  # 顯示速度太慢
            self.table_widget_wait.set_column_hidden([self.wait_column['SelfMassageFee']])
            self.table_widget_wait_completed.set_column_hidden([self.wait_done_column['MassageFee']])

        self.table_widget_first_visit = class_utils.get_table_widget(self.ui.tableWidget_first_visit, self.database)
    ...
    def _set_tab_widget_corner_widget(self):
        ...
        self.ui.tabWidget_list.setCornerWidget(self.tab_corner_widget, QtCore.Qt.TopRightCorner)
    ...
    def _set_signal(self):
        ...
        self.ui.action_vhc_ic_card.triggered.connect(lambda: self._registration_by_vhc_ic_card(None))
        ...
        self.ui.action_quick_write_ic_card.triggered.connect(self._action_quick_write_ic_treatment)
        ...
        self.ui.action_write_ic_treatment_by_qrcode.triggered.connect(self.write_ic_treatment_by_qrcode)
        self.ui.action_rewrite_vhc_card.triggered.connect(lambda: self.rewrite_vhc_card_by_qrcode(None))
        self.ui.action_rewrite_vhc_prescript.triggered.connect(self.rewrite_vhc_prescript_by_qrcode)

        self.ui.toolButton_query.clicked.connect(self.query_patient)
        self.ui.toolButton_delete_wait.clicked.connect(lambda: self.delete_wait_list(show_warning=True))
        ...
        self.ui.action_rewrite_vhc_card_all.triggered.connect(lambda: self.rewrite_ic_card('虛擬健保卡'))
        self.ui.toolButton_rewrite_ic_prescript.clicked.connect(self.rewrite_ic_prescript)
        self.ui.toolButton_quick_ic_card.clicked.connect(self._quick_write_ic_treatment)
        self.ui.toolButton_request_req_code.clicked.connect(lambda: self._request_req_code(show_message=True))
        ...
        self.ui.comboBox_patient_discount.currentIndexChanged.connect(self._set_regist_fee)
    ...
    def _check_first_visit_reservation(self, ic_card):
        ...
        if len(rows) > 0:
            ...
            if arrival:
                self._parent.close_tab_by_name('新病患資料')  # 先關閉可能已開啟的初診登錄頁面
    ...
    def _modify_patient(self):
        ...
        if tab_name == '候診名單':
            ...
            patient_key = self.ui.tableWidget_wait_completed.item(
                self.ui.tableWidget_wait_completed.currentRow(), self.wait_done_column['PatientKey']
            )
    ...
    def _set_table_width(self):
        width = [
            100, 100,
            45, 60, 70, 85, 20, 45, 45,
            50, 90, 90, 100, 50, 80, 30, 80, 80, 80, 80, 80, 80
        ]
        # self.table_widget_wait.set_table_heading_width(width)

        width = [
            100, 100, 70, 90, 45, 50, 80, 100, 40, 50, 65, 30, 30, 50, 60,
            80,
            80, 80, 80, 80, 400,
        ]
    ...
    def _read_first_visit(self):
        ...
        last_month = (datetime.date(today.year, today.month, 1) - datetime.timedelta(1)).replace(day=1)
    ...
    def _set_first_visit_data(self, row_no, row):
        ...
        for col_no in range(len(first_visit_row)):
            ...
            if col_no in [1, 2, 3]:
                self.ui.tableWidget_first_visit.item(
    ...
    def _set_wait_data(self, row_no, row):
        ...
        case_utils.set_in_progress_icon(self.ui.tableWidget_wait, row_no, self.wait_column['Progress'], in_progress)
    ...
    def _set_reg_mode(self, enabled, ic_card=None):
        ...
        if ic_card:
            try:
                ...
                self.ui.groupBox_patient.setTitle(f'病患資料 - (健保卡有效期限至: {valid_date} 可用次數: {available_count}次)')
            except Exception:
    ...
    def _set_combo_box(self):
        ...
        ui_utils.set_combo_box(self.ui.comboBox_reg_type, nhi_utils.REG_TYPE, '一般門診')
        ui_utils.set_combo_box(self.ui.comboBox_injury_type, nhi_utils.INJURY_TYPE, '普通疾病')
        ui_utils.set_combo_box(self.ui.comboBox_share_type, nhi_utils.SHARE_TYPE, '基層醫療')
        ...
        ui_utils.set_combo_box(
            self.ui.comboBox_doctor,
            personnel_utils.get_person(self.database, '醫師', exclude_person='值班醫師'))
        ui_utils.set_combo_box(
    ...
    def _get_card_sequence(self):
        ...
        if self.system_settings.field('掛號類別') == '居家醫療':
            ...
            card_sequence = nhi_utils.get_home_care_card(self.database, patient_key, today)
    ...
    def _selection_changed(self):
        ...
        if sender_name == 'comboBox_patient_share':
            ...
            elif self.ui.comboBox_reg_type.currentText() in nhi_utils.SPECIAL_PHARMACY_TYPE:
                system_utils.show_message_box(
                    QMessageBox.Warning,
                    '領取長期藥品',
            ...
            if self.ui.comboBox_reg_type.currentText() in nhi_utils.INFECTIOUS_TYPE:
                self.ui.comboBox_share_type.setCurrentText(nhi_utils.INFECTIOUS_INJURY_TYPE[0])
                self.ui.comboBox_injury_type.setCurrentText(nhi_utils.INFECTIOUS_INJURY_TYPE[0])

                if '修正存檔' in self.ui.action_save.text() or '修正存檔不印' in self.ui.action_save_no_print.text():
                    case_key = self.ui.groupBox_registration.title().split('-')[-1]
                    infectious_date = case_utils.get_case_extend(self.database, case_key, '確診日期')
                else:
                ...
                if infectious_date is None:
                    infectious_date = date_utils.get_dialog_date(
                        ...
                        date_type='date', call_from=self.program_name
                    )
                    if infectious_date is not None:
                        self.ui.comboBox_remark.setCurrentText(
                            f'確診日期: {infectious_date.toString("yyyy-MM-dd")}'
                        )
            elif self.ui.comboBox_injury_type.currentText() not in nhi_utils.OCCUPATIONAL_INJURY_TYPE:
            ...
            self._check_area()
            if self.ui.comboBox_share_type.currentText() in nhi_utils.MAIN_OCCUPATIONAL_INJURY_TYPE:
                if self.ui.comboBox_injury_type.currentText() not in nhi_utils.MAIN_OCCUPATIONAL_INJURY_TYPE:
                    self.ui.comboBox_injury_type.setCurrentText(nhi_utils.MAIN_OCCUPATIONAL_INJURY_TYPE[0])
                if card[:4] != nhi_utils.OCCUPATIONAL_INJURY_CARD:
                    self.ui.comboBox_card.setCurrentText(
                        nhi_utils.INJURY_CARD_DICT[nhi_utils.OCCUPATIONAL_INJURY_CARD])
            elif self.ui.comboBox_share_type.currentText() in nhi_utils.INFECTIOUS_INJURY_TYPE:
                if self.ui.comboBox_injury_type.currentText() not in nhi_utils.INFECTIOUS_INJURY_TYPE:
                    self.ui.comboBox_injury_type.setCurrentText(nhi_utils.INFECTIOUS_INJURY_TYPE[0])
                if card[:4] != nhi_utils.INFECTIOUS_INJURY_CARD:
                    self.ui.comboBox_card.setCurrentText(
                        nhi_utils.INFECTIOUS_CARD_DICT[nhi_utils.INFECTIOUS_INJURY_CARD])
            ...
            if self.ui.comboBox_injury_type.currentText() in nhi_utils.MAIN_OCCUPATIONAL_INJURY_TYPE:
                if self.ui.comboBox_share_type.currentText() != nhi_utils.MAIN_OCCUPATIONAL_INJURY_TYPE[0]:
                    self.ui.comboBox_share_type.setCurrentText(nhi_utils.MAIN_OCCUPATIONAL_INJURY_TYPE[0])
                if card[:4] != nhi_utils.OCCUPATIONAL_INJURY_CARD:
                    self.ui.comboBox_card.setCurrentText(
                        nhi_utils.INJURY_CARD_DICT[nhi_utils.OCCUPATIONAL_INJURY_CARD])
            elif self.ui.comboBox_injury_type.currentText() in nhi_utils.INFECTIOUS_INJURY_TYPE:
                if self.ui.comboBox_share_type.currentText() != nhi_utils.INFECTIOUS_INJURY_TYPE[0]:
                    self.ui.comboBox_share_type.setCurrentText(nhi_utils.INFECTIOUS_INJURY_TYPE[0])
                if card[:4] != nhi_utils.INFECTIOUS_INJURY_CARD:
                    self.ui.comboBox_card.setCurrentText(
                        nhi_utils.INFECTIOUS_CARD_DICT[nhi_utils.INFECTIOUS_INJURY_CARD])
            else:
                if card[:4] != nhi_utils.OCCUPATIONAL_INJURY_CARD and self.ui.comboBox_share_type.currentText() != \
            ...
            self._set_regist_fee()
            self._set_diag_share_fee()
            if self.ui.comboBox_treat_type.currentText() in ['內科'] + nhi_utils.CARE_TREAT:
                card_sequence = self._get_card_sequence()
                self.ui.comboBox_card.setCurrentText(card_sequence)
            ...
            if card == '欠卡':
                ...
                if self.ui.comboBox_treat_type.currentText() not in nhi_utils.CARE_TREAT:
            ...
            if card[:4] == nhi_utils.OCCUPATIONAL_INJURY_CARD:
                if self.ui.comboBox_injury_type.currentText() not in nhi_utils.MAIN_OCCUPATIONAL_INJURY_TYPE:
                    self.ui.comboBox_injury_type.setCurrentText(nhi_utils.MAIN_OCCUPATIONAL_INJURY_TYPE[0])
            elif card[:4] == nhi_utils.INFECTIOUS_INJURY_CARD and \
                self.ui.comboBox_reg_type.currentText() not in \
                    nhi_utils.INFECTIOUS_INJURY_TYPE + nhi_utils.TELECOM_TYPE:
                msg_box = QMessageBox()
                msg_box.setIcon(QMessageBox.Warning)
                ...
                if covid19:
                    # self.ui.comboBox_injury_type.setCurrentText(nhi_utils.INFECTIOUS_INJURY_TYPE[0])
                    self.ui.comboBox_reg_type.setCurrentText(nhi_utils.INFECTIOUS_INJURY_TYPE[0])
                else:
            ...
            if '掛號存檔' in self.ui.action_save.text():  # 門診掛號非修正時, 重新取得診號
            ...
            if '掛號存檔' in self.ui.action_save.text():  # 門診掛號
                ...
                current_room = self.table_widget_wait.field_value(self.wait_column['Room'])
                current_regist_no = self.table_widget_wait.field_value(self.wait_column['RegistNo'])
                ...
                if current_room != room:  # 換診間重新取得診號
                    ...
                    current_doctor = self.table_widget_wait.field_value(self.wait_column['Doctor'])
            ...
            schedule_doctor = registration_utils.get_schedule_doctor(self.database, room, period)
    ...
    def _set_self_fee(self, item_name):
        self_fee = charge_utils.get_charge_settings_fee(self.database, '自費', '自費', item_name)
        if self_fee is None:
            return
    ...
    def _get_patient(self, keyword, ic_card=None, use_patient_key=True):
        ...
        row = patient_utils.search_patient(
            ...
            verify_keyword, use_patient_key=use_patient_key)
    ...
    def _modify_wait(self):
        ...
        if tab_name == '候診名單':
            ...
            case_key = self.ui.tableWidget_wait_completed.item(
                self.ui.tableWidget_wait_completed.currentRow(), self.wait_done_column['CaseKey']
            )
            in_use = None
    ...
    def _check_ic_card_basic_data(self, ic_card, row, use_vhc_ic_card=False):
        ...
        if patient_modified and not use_vhc_ic_card:
            row['Birthday'] = datetime.datetime.strptime(ic_card.basic_data['birthday'], '%Y-%m-%d').date()
    ...
    def _check_reservation_exists(self, patient_key):
        ...
        if len(rows) > 0:
            ...
            if ready_to_arrival:
                ...
                self._parent.open_reservation(reserve_key, None, None, vhc_ic_card=vhc_ic_card)
    ...
    def _auto_completion_course(self, patient_key):
        ...
        if self.ui.comboBox_reg_type.currentText() in nhi_utils.TOUR_TYPE + nhi_utils.LONG_TERM_CARE:  # 巡迴醫療及中醫長照除外
            return default_card, None

        today = datetime.date.today()
        last_treat_date = (today - datetime.timedelta(days=30-1)).strftime('%Y-%m-%d 00:00:00')

        if self.ui.comboBox_treat_type.currentText() in nhi_utils.PREGNANT_CARE_TREAT:  # 2024.12.13 助孕照護，保胎照護，上次有開藥，不要續療程
            sql = f'''
        ...
        if self.system_settings.field('療程中斷不續療程') == 'Y':
            ...
            row = rows[0]
            if number_utils.get_integer(row['Continuance']) <= 0:  # 上次病歷內科，不要續療程
                return default_card, None
        ...
        if treat_type in nhi_utils.TOUR_TYPE + nhi_utils.LONG_TERM_CARE:  # 巡迴醫療及中醫長照除外
            return default_card, None
        ...
        card = string_utils.xstr(row['Card'])
        if card[:4] in nhi_utils.MANUAL_CARD_LIST and number_utils.get_integer(row['Continuance']) >= 6:  # 異常卡序療程已滿
            if self.system_settings.field('新特約期間') != 'Y' and card[:4] in nhi_utils.ABNORMAL_CARD:  # 非特約期間異常卡序不傲自動連號
                return default_card, None
            else:
                if len(card) >= 5:
                    index = int(card[4])
            ...
            if treat_type in nhi_utils.PREGNANT_CARE_TREAT:  # 助孕照護，保胎照護，其他照護
                self.ui.comboBox_treat_type.setCurrentText(treat_type)
    ...
    def _registration_precheck(self, patient_key):
        ...
        message = registration_utils.check_card_yesterday(self.database, patient_key)   # 檢查隔日過卡
        if message is not None:
            warning_message.append(message)
    ...
    def _check_deposit(self, patient_key, ic_card):
        ...
        if ic_card is not None and message is not None:  # 讀健保卡且上次未還卡
            ...
            ''')
            msg_box.setInformativeText("請注意！如果欠卡日期尚未過期, 請至 [健保卡欠還卡]作業執行[健保還卡]後，再繼續掛號!")
            msg_box.addButton(QPushButton("取消掛號，先讓我完成還卡作業"), QMessageBox.NoRole)  # 0
    ...
    def _set_combo_box_doctor(self, current_doctor):
        ...
        registration_utils.set_temporary_doctor_schedule(self.database, period, in_duty_doctor_list)
    ...
    def _set_registration_data(self, patient_key, medical_record=None):
        ...
        if medical_record is None:  # 門診掛號
            ...
            no_return_days = number_utils.get_integer(self.system_settings.field('未回診天數'))
            if visit == '複診':
                if patient_utils.is_two_years_ago_visit(self.database, patient_key):
                    ...
                    if self.system_settings.field('申報初診照護') == 'Y':
                        visit = '初診'
                elif no_return_days > 0 and \
                        patient_utils.is_no_return_days(self.database, patient_key, no_return_days):
                    system_utils.show_message_box(
                        QMessageBox.Critical,
            ...
            period = registration_utils.get_current_period(self.system_settings)
            doctor = registration_utils.get_schedule_doctor(self.database, room, period)  # 醫師要先取得，以便確定是否佔用預約號
            ...
            if self.ui.spinBox_reg_no.value() == 0:  # 取得診號  (已在room, therapist, period on changed時取得, 不需要重複再取)
                reg_no = registration_utils.get_reg_no(
                    self.database, self.system_settings, room, doctor, period, self.reserve_key,
                )
    ...
    def _set_combo_box_color(self):
        ...
        if self.ui.comboBox_reg_type.currentText() in nhi_utils.TOUR_TYPE_WITH_GOTO_LACK_AREA:
        ...
        elif self.ui.comboBox_reg_type.currentText() in nhi_utils.INFECTIOUS_TYPE + nhi_utils.TELECOM_TYPE:
            self.ui.comboBox_reg_type.setStyleSheet('background-color: lightgreen')
        ...
        elif self.ui.comboBox_injury_type.currentText() in nhi_utils.OCCUPATIONAL_INJURY_TYPE:
            self.ui.comboBox_injury_type.setStyleSheet('background-color: lightblue')

    def _check_area(self):
        if self.system_settings.field('資源類別') in ['一般', '資源不足開業'] and \
                self.system_settings.field('掛號類別') not in nhi_utils.TOUR_TYPE + nhi_utils.CORRECTION_REG_TYPE:
    ...
    def delete_wait_list(self, show_warning=None):
        ...
        if tab_name == '候診名單':
            ...
            wait_key = self.table_widget_wait_completed.field_value(self.wait_done_column['WaitKey'])
            ...
            card = self.table_widget_wait_completed.field_value(self.wait_done_column['Card'])
            room = self.table_widget_wait_completed.field_value(self.wait_done_column['Room'])
            doctor = self.table_widget_wait_completed.field_value(self.wait_done_column['Doctor'])
        ...
        if self.system_settings.field('alleypin') == 'Y':  # 在刪除病歷檔前推播，否則資料會找不到
            try:
                alleypin_utils.cancel_outpatient_alleypin_appointments(self.database, self.system_settings, case_key)
                alleypin_utils.update_progresses(self.database, self.system_settings, case_key)
            except Exception:
                system_utils.show_message_box(
                    QMessageBox.Critical,
                    '翔評主機連線錯誤',
    ...
    def cancel_ic_card(self):
        ...
        try:
            case_key = table_widget_wait.item(row_no, self.wait_column['CaseKey']).text()
            wait_key = table_widget_wait.item(row_no, self.wait_column['WaitKey']).text()
            ...
            if vhc_req_code is not None:
                ...
                if qrcode is None:
                    system_utils.show_message_box(
                        QMessageBox.Critical,
                        '無法寫卡',
                        f'<font size="5" color="red"><b>無法使用虛擬健保卡寫卡, 無法取得授權.</b></font>',
                        '請重新取得授權.'
                    )
                    return

            ic_card = class_utils.get_vhccshis(self, self.database, self.system_settings, qrcode)
        else:
            ic_card = class_utils.get_cshis(self, self.database, self.system_settings)
    ...
    def _check_ins_ok_before_save(self):
        ...
        if number_utils.get_integer(course) <= 1 and self.system_settings.field('隔日過卡不能存檔') == 'Y':
            message = registration_utils.check_card_yesterday(self.database, patient_key)   # 檢查隔日過卡
            if message is not None:
                system_utils.show_message_box(
                    QMessageBox.Critical,
    ...
    def _save_records(self):
        if self.system_settings.field('掛號診號不可重複') == 'Y' and self._is_regist_no_exists():
            system_utils.show_message_box(
                QMessageBox.Critical,
                '診號重複',
        ...
        if '掛號存檔' in self.ui.action_save.text() and not self._verify_registration_data(card):
            return

        ic_card, req_code = self._process_ic_card(card)
        if ic_card is None:  # 不須讀卡
            if self.ui.comboBox_ins_type.currentText() == '健保' and \
                    not self.ui.checkBox_request_token.isChecked() and card == '自動取得':
                system_utils.show_message_box(
                    QMessageBox.Critical,
                    '卡序有誤',
        ...
        if self.ui.checkBox_request_token.isChecked() and (not req_code or req_code == 'error'):
            system_utils.show_message_box(
                QMessageBox.Critical,
                '請求錯誤',
        ...
        if self._is_today():  # 掛號今日門診才執行下列程式
            ...
            total_amount = number_utils.get_integer(self.ui.lineEdit_total_amount.text())  # 應收金額非實收金額
            if self.system_settings.field('掛號收據無金額不列印') == 'Y' and total_amount <= 0:  # 金正中醫 2022.12.27
                pass
            elif sender_name == 'action_save' and '掛號存檔' in self.ui.action_save.text():
                self.print_regist(
                    self.ui.comboBox_ins_type.currentText(),
                    self.ui.comboBox_treat_type.currentText(),
                    '系統設定', case_key)
    ...
    def _verify_registration_data(self, card):
        ...
        if self.ui.comboBox_reg_type.currentText() in nhi_utils.TOUR_TYPE + nhi_utils.LONG_TERM_CARE:  # 巡迴醫療不檢查
            return True
        ...
        if self.system_settings.field('掛號療程14日未完成提醒') == 'Y':
            message = registration_utils.check_course_complete_in_days(  # 檢查療程14日未完成
    ...
    def _process_ic_card(self, card):
        ...
        if '修正存檔' in self.ui.action_save.text() or '修正存檔不印' in self.ui.action_save_no_print.text():
            if card not in ['自動取得', '欠卡'] + nhi_utils.ABNORMAL_CARD:  # 掛號修正存檔, 卡序若為自動取得或欠卡及異常卡序, 要產生卡序及安全簽章
                return None, None

        card_abnormal = string_utils.xstr(self.ui.comboBox_card_abnormal.currentText()).split(' ')[0]
        if card == '欠卡' or card[:4] in nhi_utils.ABNORMAL_CARD or card_abnormal in nhi_utils.ABNORMAL_CARD:
            if card[:4] in ['A020', 'F000'] or card_abnormal in ['A020', 'F000']:  # 2024-12-31 A020網路不通無法取得就醫識別碼
                return None, None
    ...
    def _write_ic_card(self, treat_after_check):
        ...
        if self.ui.comboBox_treat_type.currentText() == '居家醫療' and card != '自動取得':
            treat_type = '居家醫療'
        else:
            treat_type = None
    ...
    def _write_vhc_ic_card(self, treat_after_check):
        ...
        card = self._get_card(self.ui.comboBox_card.currentText(), ' ')
        if ic_card_ok:
            seq_number = string_utils.xstr(self.vhc_ic_card.treat_data['seq_number'])  # 有產生卡號才更新
            if card != 'IC06' and seq_number != '':
                self.ui.comboBox_card.setCurrentText(seq_number)
    ...
    def _save_patient(self):
        ...
        if self.ui.lineEdit_name.isModified() or \
                self.ui.lineEdit_chart_no.isModified() or \
        ...
        fields = ['ChartNo', 'Name', 'ID', 'Birthday', 'Telephone', 'Cellphone', 'Address', 'InsType',
        ...
        self.database.update_record('patient', fields, 'PatientKey', self.ui.lineEdit_patient_key.text(), data)
    ...
    def _set_traffic_allowance(self, case_key):
        ...
        case_utils.insert_traditional_health_care_prescript(
            self.database, self.system_settings, case_key, traffic_allowance_fee,
    ...
    def _check_traditional_health_care_fee(self, ins_type, case_key):
        ...
        if ins_type == '自費':
            if traditional_health_care_fee > 0:
                if treat_type in nhi_utils.TRI_HEAT_TREAT:
                    fields = ['TreatType', 'SMassageFee', 'SelfTotalFee', 'TotalFee', 'ReceiptFee']
                    data = [
                        treat_type,
                        traditional_health_care_fee,
                        traditional_health_care_fee,
                    ...
                    case_utils.remove_traditional_health_care_prescript(
                        self.database, self.system_settings, case_key, folk_massage_name=treat_type)
                    case_utils.insert_traditional_health_care_prescript(
                        self.database, self.system_settings, case_key,
                        traditional_health_care_fee, folk_massage_name=treat_type)
                ...
                case_utils.remove_traditional_health_care_prescript(
                    self.database, self.system_settings, case_key)
                case_utils.insert_traditional_health_care_prescript(
                    self.database, self.system_settings, case_key, traditional_health_care_fee)
        ...
        elif self.ui.comboBox_massager.currentText() != '' or traditional_health_care_fee > 0:
            write_health_care = True

        if write_health_care:
            try:
                self._write_traditional_health_care(ins_type, case_key, traditional_health_care_fee)
            except ValueError:
                pass
    ...
    def _insert_medical_record(self, ic_card=None):
        ...
        card_abnormal = self._get_card(self.ui.comboBox_card_abnormal.currentText(), ' ')
        ins_type = self.ui.comboBox_ins_type.currentText()
        ...
        charge_date = None
        charge_period = None
        if self.ui.comboBox_ins_type.currentText() == '自費' and \
                self.ui.comboBox_treat_type.currentText() == '民俗調理' and \
                self.system_settings.field('候診名單顯示自費民俗調理') != 'Y':
            doctor_done = 'True'
        ...
        fields = [
            'CaseDate', 'PatientKey', 'Name', 'Visit', 'RegistType', 'TourArea', 'Injury',
            'TreatType', 'Share', 'InsType', 'Card', 'Continuance', 'XCard', 'Period',
            'Room', 'RegistNo', 'Massager', 'Register', 'DesignatedDoctor', 'DesignatedMassager',
            'ApplyType', 'PharmacyType',
            'RegistFee', 'DiagShareFee', 'SDiagShareFee', 'DepositFee', 'Security', 'Remark',
            'Doctor', 'RegistPaymentType',
            'DoctorDone', 'DoctorDate', 'ChargeDone', 'ChargeDate', 'ChargePeriod',
        ]
        ...
        data = [
            ...
            doctor,
            payment_type,
            doctor_done, doctor_date,
            charge_done, charge_date, charge_period,
        ]
        ...
        if self.ui.comboBox_share_type.currentText() in nhi_utils.INFECTIOUS_INJURY_TYPE:
            self._insert_infectious_case(case_key, share_type, treat_type, course)

        if self.ui.comboBox_treat_type.currentText() in nhi_utils.HOME_CARE:  # 居家醫療不寫入民俗調理 2024-10-20 善揚
    ...
    def _insert_infectious_case(self, case_key, share, treatment, course):
        ...
        if ok and item:
            ins_code, medicine_name, dosage = prescript_utils.get_infectious_drug_factory(item)

        ins_code = charge_utils.get_ins_code_from_charge_settings(self.database, '處置費', '台灣清冠一號藥品補助費')
        case_date, _ = case_utils.get_case_date(self.database, case_key)
        treat_fee = charge_utils.get_ins_fee_from_ins_code(self.database, ins_code, case_date=case_date)
        diag_fee = charge_utils.get_ins_fee_from_ins_code(self.database, 'E5204C', case_date=case_date)
        medicine_set = 1
        packages = 3
        pres_days = 5
        ...
        agent_fee = charge_utils.get_ins_agent_fee(
            self.database, self.system_settings, share, treatment, course, drug_fee,
        )

        fields = [
            'DiseaseCode1', 'DiseaseName1', 'DiagFee', 'InterDrugFee', 'InsApplyFee', 'InsTotalFee', 'AgentFee'
        ]
        data = ['U071', '確認COVID-19病毒感染', diag_fee, inter_drug_fee, ins_apply_fee,  ins_apply_fee, agent_fee]
        self.database.update_record('cases', fields, 'CaseKey', case_key, data)

        if medicine_name is not None:
            fields = [
                'PrescriptNo', 'CaseKey', 'CaseDate', 'MedicineSet',
                'MedicineType', 'InsCode', 'MedicineName', 'DosageMode',
                'Dosage', 'Unit'
            ]
            data = [
                1, case_key, date_utils.now_to_str(), medicine_set,
                '複方', ins_code, medicine_name, '日劑量', dosage, '克'
            ]
    ...
    def _write_traditional_health_care(self, ins_type, in_case_key, traditional_health_care_fee):
        treat_type = self.ui.comboBox_treat_type.currentText()

        if ins_type == '健保' or (ins_type == '自費' and treat_type != '民俗調理'):
            case_utils.write_traditional_health_care(
                self.database, self.system_settings, in_case_key,
    ...
    def _get_wait_key(self, case_key, table_widget_wait):
        ...
        for row_no in range(table_widget_wait.rowCount()):
            if table_widget_wait.item(row_no, self.wait_column['CaseKey']).text() == case_key:
                wait_key = table_widget_wait.item(row_no, self.wait_column['WaitKey']).text()
                break
    ...
    def insert_wait(self, case_key, req_code):
        if self.ui.checkBox_request_token.isChecked() and self.ui.comboBox_card.currentText() == '自動取得':
            card = '請求授權'
        else:
        ...
        fields = [
            ...
            'Room', 'RegistNo', 'Doctor', 'Massager', 'Remark', 'VHCReqCode',
            'DoctorDone',
        ]
        data = [
            case_key,
    ...
    def _get_wait_list_field(self, case_key, field_no):
        ...
        for row_no in range(self.ui.tableWidget_wait.rowCount()):
            case_key_item = self.ui.tableWidget_wait.item(row_no, self.wait_column['CaseKey']).text()
    ...
    def _update_medical_record(self, ic_card=None):
        ...
        charge_utils.calculate_ins_fee(
            self.database, self.system_settings, case_key, ins_type=ins_type)  # 自費也要將健保批價歸零 2023.09.13 太初
        ...
        fields = [
            'CaseDate', 'Name', 'Visit', 'RegistType', 'TourArea', 'Injury',
            'TreatType', 'Share', 'InsType', 'Card', 'Continuance', 'XCard', 'Period',
            'Room', 'RegistNo', 'Doctor', 'Massager', 'DesignatedDoctor', 'DesignatedMassager',
            'ApplyType', 'PharmacyType',
            'RegistFee', 'DiagShareFee', 'SDiagShareFee', 'DepositFee', 'Remark', 'RegistPaymentType',
        ]
        ...
        if ic_card is not None:  # 重新產生卡序
            ...
            data = [security]
            self.database.update_record('cases', fields, 'CaseKey', case_key, data)
            origin_card = self._get_card(self._get_wait_list_field(case_key, self.wait_column['Card']))
            if origin_card == '欠卡':
                self.database.delete_record('deposit', 'CaseKey', case_key)
    ...
    def _exam_precheck(self):
        ...
        dialog = dialog_utils.get_dialog_exam_precheck(
            self, self.database, self.system_settings, case_key, '掛號作業',
        )
        dialog.exec_()
        dialog.deleteLater()
    ...
    def print_wait(self):
        ...
        if tab_name == '候診名單':
            ...
            case_key = self.table_widget_wait_completed.field_value(self.wait_done_column['CaseKey'])
            ins_type = self.table_widget_wait_completed.field_value(self.wait_done_column['InsType'])
            treat_type = self.table_widget_wait_completed.field_value(self.wait_done_column['TreatType'])
    ...
    def print_regist(self, ins_type, treat_type, printable, case_key=False):
        ...
        if ins_type == '健保':
            ...
            if case_utils.is_traditional_health_case(self.database, case_key) or \
                    case_utils.is_self_traditional_health_case(self.database, case_key):
            ...
            if treat_type == '民俗調理' or \
                    case_utils.is_traditional_health_case(self.database, case_key) or \
                    case_utils.is_self_traditional_health_case(self.database, case_key):
                self.print_massage_form(printable, case_key)
    ...
    def print_registration_form(self, printable, case_key=None):
        ...
        printer_utils.print_regist_form(self, self.database, self.system_settings, case_key, printable)

    # 列印民俗調理單
    def print_massage_form(self, printable, case_key=False):
        if self.ui.comboBox_treat_type.currentText() in nhi_utils.HOME_CARE:  # 居家醫療不列印 2024-10-20 善揚
            return
    ...
    def reservation_arrival(self, reserve_key, late=False,
                            late_remark='預約到報過號', vhc_ic_card=None):
        """預約報到."""
        self.reserve_key = None
        sql = f'''
            SELECT * FROM reserve
    ...
    def _set_wait_completed_data(self, row_no, row):
        ...
        if ins_type != '健保' or card == '欠卡' or \
                card[:4] in nhi_utils.ABNORMAL_CARD or \
                xcard in nhi_utils.ABNORMAL_CARD:
            ic_wrote = '略'
        ...
        receipt_fee = (
            regist_fee + diag_share_fee + drug_share_fee +
            deposit_fee + total_fee + traditional_health_care_fee
        )
    ...
    def _wait_completed_table_item_changed(self):
        ...
        ic_wrote = self.ui.tableWidget_wait_completed.item(row_no, self.wait_done_column['WriteCard'])
        if ic_wrote is not None:
            ic_wrote = ic_wrote.text()

        card = self.ui.tableWidget_wait_completed.item(row_no, self.wait_done_column['Card'])
        if card is not None:
            card = card.text()
    ...
    def _request_req_code(self, show_message=True):
        ...
        if tab_name == '候診名單':
            ...
            wait_key = self.table_widget_wait_completed.field_value(self.wait_done_column['WaitKey'])
            patient_key = self.table_widget_wait_completed.field_value(self.wait_done_column['PatientKey'])
    ...
    def _vhc_get_seq_number(self):
        ...
        card = self._get_card(self.table_widget_wait.field_value(self.wait_column['Card']))
        ...
        ic_card = class_utils.get_cshis(self, self.database, self.system_settings)
        qrcode = ic_card.get_response_token(vhc_req_code)
        self.vhc_ic_card = class_utils.get_vhccshis(self, self.database, self.system_settings, qrcode)
        ...
        ic_card_ok = self.vhc_ic_card.write_ic_card(
            '掛號寫卡', patient_key, course, share_type, cshis_utils.NORMAL_CARD, treat_type=treat_type)
        if not ic_card_ok:
            return
    ...
    def _quick_write_ic_treatment(self):
        ...
        if self.table_widget_wait_completed.field_value(self.wait_done_column['WriteCard']) == '是':
        ...
        case_key = self.table_widget_wait_completed.field_value(self.wait_done_column['CaseKey'])
        card = self._get_card(self.table_widget_wait_completed.field_value(self.wait_done_column['Card']))
    ...
    def write_ic_treatment(self):
        ...
        share_type = self.table_widget_wait_completed.field_value(self.wait_done_column['ShareType'])
    ...
    def rewrite_ic_card(self, ic_card_type=None):
        ...
        if self.system_settings.field('讀卡機控制軟體版本') == 'cshis6':
            ic_card.write_ic_medical_record(case_key, cshis_utils.NORMAL_CARD, reset_vhc_card=False)
        else:
            ic_card.write_ic_medical_record(case_key, cshis_utils.NORMAL_CARD)
    ...
    def rewrite_vhc_card_by_qrcode(self, qrcode=None):
        ...
        card = self._get_card(self.table_widget_wait_completed.field_value(self.wait_done_column['Card']))
        course = number_utils.get_integer(self.table_widget_wait_completed.field_value(
