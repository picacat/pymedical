...
class StatisticsInsApplyYear(QtWidgets.QMainWindow):
    ...
    def _set_table_data(self, ins_apply_data):
        ...
        for col_no in range(len(ins_apply_data)):
            ...
            if col_no in [0]:
                self.ui.tableWidget_ins_apply.item(
    ...
    def _calculate_total(self):
        ...
        for row_no in range(self.ui.tableWidget_ins_apply.rowCount()):
            total_diag_count += number_utils.get_integer(self.ui.tableWidget_ins_apply.item(row_no, 1).text())
            diag_fee += number_utils.get_integer(self.ui.tableWidget_ins_apply.item(row_no, 2).text())
            drug_fee += number_utils.get_integer(self.ui.tableWidget_ins_apply.item(row_no, 3).text())
            pharmacy_fee += number_utils.get_integer(self.ui.tableWidget_ins_apply.item(row_no, 4).text())
            treat_fee += number_utils.get_integer(self.ui.tableWidget_ins_apply.item(row_no, 5).text())
            ins_apply_total += number_utils.get_integer(self.ui.tableWidget_ins_apply.item(row_no, 6).text())
            share_fee += number_utils.get_integer(self.ui.tableWidget_ins_apply.item(row_no, 7).text())
            ins_apply_fee += number_utils.get_integer(self.ui.tableWidget_ins_apply.item(row_no, 8).text())

        total_row = [
            '合計',
            total_diag_count, diag_fee, drug_fee, pharmacy_fee, treat_fee,
            ins_apply_total, share_fee, ins_apply_fee,
        ]
    ...
    def _create_bar_chart(self):
        ...
        for row in range(row_count - 1):  # 最後一列是「合計」，不統計
            ...
            item_text = self.ui.tableWidget_ins_apply.item(row, 8).text().replace(",", "").strip()
    ...
    def _export_to_excel(self):
        ...
        excel_filename = os.path.join(last_dir, f"{self.dialog_setting['year']}年度健保申報分項表.xlsx")
        excel_filename, _ = QFileDialog.getSaveFileName(
            self.parent,
            "匯出分項表",excel_filename,
        ...
        export_utils.export_table_widget_to_excel(
            excel_filename, self.ui.tableWidget_ins_apply,
            None, [1, 2, 3, 4, 5, 6, 7, 8], f"{self.dialog_setting['year']}健保申報年度分列項目表",
        )
