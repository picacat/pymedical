...
class InsAppeal(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_appeal = class_utils.get_table_widget(
            self.ui.tableWidget_appeal, self.database
        )
        self.table_widget_appeal.set_column_hidden([0, 1])
        self.table_widget_appeal_items = class_utils.get_table_widget(
            self.ui.tableWidget_appeal_items, self.database
        )
    ...
    def _set_signal(self):
        ...
        self.ui.tableWidget_appeal.itemSelectionChanged.connect(
            self._ins_appeal_changed
        )
    ...
    def open_dialog(self):
        ...
        if not self._check_apply_data_exists():
            self.ui.label_year.setText(
                f"{self.apply_year}年{self.apply_month}月 申報資料不存在"
            )
    ...
    def _remove_ins_appeal(self):
        ...
        self.database.exec_sql(
            f"DELETE FROM insappeal WHERE InsAppealKey = {ins_appeal_key}"
        )
        self.database.exec_sql(
            f"DELETE FROM insappeal_items WHERE InsAppealKey = {ins_appeal_key}"
        )
    ...
    def _save_ins_appeal_reason(self):
        ...
        self.database.update_record(
            "insappeal", fields, "InsAppealKey", ins_appeal_key, data
        )
    ...
    def _set_appeal_items_data(self, row_no, row):
        ...
        button_delete.clicked.connect(
            lambda: self._remove_appeal_items(ins_appeal_items_key)
        )
        ...
        button_edit.clicked.connect(
            lambda: self._edit_appeal_items(ins_appeal_items_key)
        )
    ...
    def _upload_data(self):
        ...
        msg_box.setText(
            f"<font size='4' color='red'><b>確定上傳健保申復檔案?<br>申報檔名: {zip_file}</b></font>"
        )
    ...
    def _remove_appeal_items(self, ins_appeal_items_key):
        ...
        self.database.exec_sql(
            f"DELETE FROM insappeal_items WHERE InsAppealItemsKey = {ins_appeal_items_key}"
        )
