...
from libs import (case_utils, charge_utils, class_utils, cshis_utils,
                  date_utils, nhi_utils, number_utils, personnel_utils,
...
class Cashier(QtWidgets.QMainWindow):
    ...
    def _set_signal(self):
        ...
        self.ui.action_print_prescription.triggered.connect(lambda: self._print_prescript(None))
        ...
        self.ui.tableWidget_charge_list.itemSelectionChanged.connect(self._pharmacy_list_changed)
        ...
        self.ui.comboBox_payment_type2.currentTextChanged.connect(self._set_payment_type2)
    ...
    def _show_medical_record(self, case_key):
        ...
        if case_row['ChargeDone'] == 'True':
            self.ui.lineEdit_receipt_drug_share_fee.setText(string_utils.xstr(s_drug_share_fee))
        else:
        ...
        self.ui.label_fee_error.setVisible(False)
        if s_diag_fee + s_drug_fee + s_herb_fee + s_expensive_fee + \
                s_acupuncture_fee + s_massage_fee + s_dislocate_fee + s_material_fee + s_exame_fee != self_total_fee:
    ...
    def _set_charge_all_fee(self, case_row):
        ...
        if self.system_settings.field('掛號收費批價進行') == 'Y':
            total_fee = regist_fee + diag_share_fee + drug_share_fee + deposit_fee + self_total_fee
        else:
        ...
        self.ui.comboBox_payment_type2.setCurrentText(self.ui.comboBox_payment_type.currentText())
    ...
    def _apply_charge(self):
        ...
        msg_box.setInformativeText(f"注意！批價存檔後, 此筆資料將歸檔至已批價名單!<br>{debt_message}")
        ...
        if sender_name == 'action_save':  # 批價列印
            ...
            if self.system_settings.field('電子處方箋路徑') not in ['', None]:
                prescript_utils.save_electrical_prescript(self.database, self.system_settings, case_key)
    ...
    def _calculate_receipt_fee2(self):
        ...
        if receipt_fee < total_fee:
            self.ui.lineEdit_debt2.setText(
    ...
    def _save_records(self, wait_key, case_key):
        ...
        if self.ui.groupBox_charge_all.isVisible():
            ...
            s_diag_share_fee = number_utils.get_integer(self.ui.lineEdit_diag_share_fee.text())            
            s_drug_share_fee = number_utils.get_integer(self.ui.lineEdit_drug_share_fee2.text())
            ...
            drug_share_fee = number_utils.get_integer(self.ui.lineEdit_drug_share_fee.text())
            ...
            if drug_share_fee > 0 and debt > 0 and s_drug_share_fee <= 0:  # 收入與欠款沖銷平衡
    ...
    def _save_debt(self, debt, case_key):
        ...
        rows = self.database.select_record(f'SELECT * FROM cases WHERE CaseKey = {case_key}')
        if len(rows) <= 0:
            return
    ...
    def _modify_income(self):
        ...
        msg_box.setInformativeText(f"注意！實收金額若小於應收金額, 將會產生一筆欠款資料")
        msg_box.addButton(QPushButton("取消"), QMessageBox.NoRole)
        msg_box.addButton(QPushButton("確定"), QMessageBox.YesRole)
        apply_charge = msg_box.exec_()
        if not apply_charge:
    ...
    def _calculate_total_fee(self):
        ...
        self.ui.lineEdit_total.setText(
            string_utils.xstr(drug_share_fee + total_fee)
        )
