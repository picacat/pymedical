...
class CheckCourse(QtWidgets.QMainWindow):
    ...
    def _check_no_first_course(self, row_no):
        course = number_utils.get_integer(
            self.ui.tableWidget_errors.item(row_no, 7).text()
        )
        if course == 1:
            return
        ...
        if (
            ...
            if card == previous_card:
                ...
                previous_case_date = date_utils.str_to_date(
                    case_date
    ...
    def _check_last_course_not_full(self, row_no):
        ...
        if previous_course == 1:
            ...
            if (
                delta.days < 30
                and regist_type not in ["照護機構中醫照護"]
                and card[:4] != nhi_utils.OCCUPATIONAL_INJURY_CARD
            ):  # 職災不檢查
                error_message.append("上次療程僅首次且未滿30日開新療程")
        elif previous_course <= 5:
            delta = nhi_utils.get_first_course_delta(
                self.ui.tableWidget_errors,
                row_no,
                patient_key,
                previous_card,
                previous_course,
    ...
    def _check_complicated_treat_level(self, row_no):
        ...
        current_treat = string_utils.xstr(
            self.ui.tableWidget_errors.item(row_no, 10).text()
        )
        previous_treat = string_utils.xstr(
            self.ui.tableWidget_errors.item(row_no - 1, 10).text()
        )

        error_message = []
        if (
            previous_treat
            in nhi_utils.GENERAL_ACUPUNCTURE_TREAT + nhi_utils.GENERAL_MASSAGE_TREAT
        ):
            if current_treat in nhi_utils.COMPLICATED_ACUPUNCTURE_TREAT:
                error_message.append("針傷複雜度不可高於上次")
        elif (
            previous_treat
            in nhi_utils.MODERATE_COMPLICATED_ACUPUNCTURE_LIST
            + nhi_utils.MODERATE_COMPLICATED_MASSAGE_TREAT
        ):
            if (
                current_treat
                in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_LIST
        ...
        if (
            current_treat
            in nhi_utils.MODERATE_COMPLICATED_MASSAGE_TREAT
            + nhi_utils.HIGHLY_COMPLICATED_MASSAGE_TREAT
            and course >= 2
        ):
    ...
    def _check_pres_days_over_28days(self, row_no):
        patient_key = self.ui.tableWidget_errors.item(row_no, 3).text()
        card = self.ui.tableWidget_errors.item(row_no, 6).text()
        total_pres_days = number_utils.get_integer(
            self.ui.tableWidget_errors.item(row_no, 11).text()
        )
        ...
        for i in range(6, 0, -1):
            ...
            if (
                previous_patient_key is None
                or previous_patient_key.text() != patient_key
            ):
                continue
            ...
            pres_days = number_utils.get_integer(
                self.ui.tableWidget_errors.item(row_no - i, 11).text()
            )
            if pres_days > 0:
    ...
    def _check_previous_course(self, row_no):
        ...
        previous_patient_key = self.ui.tableWidget_errors.item(row_no - 1, 3)
        if (
            previous_patient_key is None or previous_patient_key.text() != patient_key
        ):  # 不同人
            return
        ...
        previous_disease_code = (
            self.ui.tableWidget_errors.item(row_no - 1, 8).text()[:6].strip()
        )
    ...
    def _check_course(self):
        ...
        for row_no in range(row_count):
            case_date = self.ui.tableWidget_errors.item(row_no, 1).text()
            if (
                date_utils.str_to_date(case_date).month != self.apply_month
            ):  # 上月不檢查
                progress_dialog.setValue(row_no)
                continue

            share_type = self.ui.tableWidget_errors.item(
                row_no, 5
            ).text()  # 山地離島每次都是新療程
    ...
    def _set_row_error_message(self, row_no, col_no, error_message):
        self.ui.tableWidget_errors.setItem(
            row_no, col_no, QtWidgets.QTableWidgetItem(", ".join(error_message))
        )
    ...
    def _set_row_color(self, row_no, color):
        for column_no in range(self.ui.tableWidget_errors.columnCount()):
            self.ui.tableWidget_errors.item(row_no, column_no).setForeground(
                QtGui.QColor(color)
            )
    ...
    def _remove_full_course(self):
        for row_no in range(self.ui.tableWidget_errors.rowCount()):
            ...
            if date_utils.str_to_date(case_date).month != self.apply_month:
                ...
                last_case_date = case_date
                for i in range(1, 7):
                    next_patient_key_item = self.ui.tableWidget_errors.item(
                        row_no + i, 3
                    )
                    ...
                    next_case_date = self.ui.tableWidget_errors.item(
                        row_no + i, 1
                    ).text()
                    ...
                    if (
                        patient_key == next_patient_key
                        and card == next_card
                        and next_course > course
                    ):
    ...
    def _remove_last_month_single_course(self):
        for row_no in range(self.ui.tableWidget_errors.rowCount()):
            ...
            while True:
                ...
                if next_patient_key_item is None:
                    if (
                        date_utils.str_to_date(current_case_date).month
                        == self.apply_month
                    ):
                        at_this_month = True
    ...
    def _insert_record(self, row, row_no=None):
        ...
        for col_no in range(len(error_row)):
            ...
            self.ui.tableWidget_errors.setItem(row_no, col_no, item)

            if col_no in [7]:
                self.ui.tableWidget_errors.item(row_no, col_no).setTextAlignment(
    ...
    def _check_highly_massage_course(self, row_no):
        ...
        if first_treat in nhi_utils.HIGHLY_COMPLICATED_MASSAGE_TREAT:
            error_message.append("高度傷科療程2-6次不能執行複雜性針傷")
