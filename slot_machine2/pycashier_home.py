...
from libs import date_utils, registration_utils, string_utils, ui_utils, web_utils
...
class PyCashierHome(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        if self.system_settings.field("院所名稱") == "新生堂中醫診所":
            style = """
                QMainWindow#WindowHome
                {border-image: url(./images/pycashier_home4.jpg);}
            """
            self.ui.setStyleSheet(style)
            self.ui.label_doctor3.setVisible(False)
            self.ui.label_doctor5.setVisible(False)
            style = """
                QMainWindow#WindowHome
                {border-image: url(./images/pycashier_home4.jpg);}
            """
            self.ui.pushButton_reservation.move(243, 755)
    ...
    def _open_reservation_web(self):
        if self.system_settings.field("院所名稱") == "站前新生堂中醫診所":
            web_utils.open_address("http://zencmc.idv.tw")
        else:
            web_utils.open_address("http://114.32.131.111:5000/")
