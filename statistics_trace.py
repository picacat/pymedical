...
class StatisticsTrace(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_trace = class_utils.get_table_widget(self.ui.tableWidget_trace, self.database)
    ...
    def open_dialog(self):
        dialog = dialog_utils.get_dialog_date_period(self, self.database, self.system_settings)
    ...
    def create_bar_chart(self, data_rows):
        ...
        for i in reversed(range(self.ui.horizontalLayout_trace.count())):
            ...
            widget = item.widget()
            if widget and widget not in [self.ui.tableWidget_trace, self.ui.tableWidget_patient_list]:
    ...
    def _get_row_no(self, patient_key):
        for row_no in range(self.ui.tableWidget_patient_list.rowCount()):
            if self.ui.tableWidget_patient_list.item(row_no, 0).text() == string_utils.xstr(patient_key):
                return row_no
