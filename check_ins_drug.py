...
class CheckInsDrug(QtWidgets.QMainWindow):
    ...
    def _update_ins_code(self):
        ...
        for row_no in range(record_count):
            progress_dialog.setValue(row_no)
            error_message = self.ui.tableWidget_prescript.item(
                row_no, self.column["ErrorMessage"]
            )
            if error_message is None:
                continue

            prescript_key = self.ui.tableWidget_prescript.item(
                row_no, self.column["PrescriptKey"]
            ).text()
            medicine_key = self.ui.tableWidget_prescript.item(
                row_no, self.column["MedicineKey"]
            )
            medicine_type = self.ui.tableWidget_prescript.item(
                row_no, self.column["MedicineType"]
            ).text()
            medicine_name = self.ui.tableWidget_prescript.item(
                row_no, self.column["MedicineName"]
            ).text()
            ins_code = self.ui.tableWidget_prescript.item(
                row_no, self.column["InsCode"]
            ).text()
            if medicine_key in [None, ""] or ins_code in [None, ""]:
                self._update_ins_code_by_name(
                    prescript_key, medicine_type, medicine_name
                )
                continue

            medicine_key = medicine_key.text()
            ins_code = prescript_utils.get_medicine_field(
