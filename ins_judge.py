...
class InsJudge(QtWidgets.QMainWindow):
    ...
    def _set_signal(self):
        ...
        self.ui.action_export_ins_order_pdf.triggered.connect(self._export_ins_order_pdf)
        self.ui.action_export_medical_record_pdf.triggered.connect(self._export_medical_record_pdf)
        self.ui.action_export_medical_chart_pdf.triggered.connect(self._export_medical_chart_pdf)
    ...
    def open_dialog(self):
        ...
        enabled = False
        if self.ui.tabWidget_ins_data.count() > 0 and self.tab_ins_apply_tab.tabWidget_ins_apply.count() > 0:
            enabled = True
    ...
    def _add_ins_apply_tab(self, ins_list=None):
        ...
        self.tab_ins_apply_tab = module_utils.get_ins_apply_tab(
            ...
            self.period, self.apply_type, self.clinic_id, ins_list=ins_list,
        )
    ...
    def _upload_emr(self):
        ...
        ins_emr = module_utils.get_ins_upload_emr(
            self, self.database, self.system_settings,
            self.apply_date, self.apply_type, self.period, self.clinic_id, self.apply_upload_date,
        )
    ...
    def _print_ins_order(self, print_type='print'):
        ...
        for row_no, row in enumerate(rows):
            ...
            printer_utils.print_form_ins_apply_order(
                self, self.database, self.system_settings,
                self.apply_year, self.apply_month,
                self.apply_type, ins_apply_key, print_type
            )
        progress_dialog.setValue(record_count)
        progress_dialog.deleteLater()
    ...
    def _open_csv_file(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, "開啟抽審檔",
            '',  # ← 建議路徑先清空，才能顯示副檔名過濾
            "所有檔案 (*);;zip檔 (*.zip);;csv檔 (*.csv)", options=options
        )
        if not filename:
            return
