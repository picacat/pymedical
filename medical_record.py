...
from libs import (alleypin_utils, case_utils, charge_utils, class_utils,
                  cshis_utils, date_utils, db_utils, dialog_utils, log_utils,
                  module_utils, nhi_utils, number_utils, personnel_utils,
                  prescript_utils, printer_utils, registration_utils,
                  string_utils, system_utils, ui_utils, vhc_utils, web_utils)
...
class MedicalRecord(QtWidgets.QMainWindow):
    ...
    def _set_case_closed(self):
        ...
        for widget in self.findChildren((
            ...
            QtWidgets.QAction, QtWidgets.QToolButton, QtWidgets.QRadioButton,
        ...
        self.ui.action_force_save.triggered.connect(lambda: self.save_medical_record(force_save=True))
    ...
    def _read_case_extend(self):
        if case_utils.get_case_extend(self.database, self.case_key, '整合醫療照護') == 'Y':
            try:
        ...
        if case_utils.get_case_extend(self.database, self.case_key, '不申報慢性病') == 'Y':
            try:
                ...
                self.check_box_chronic_code.setStyleSheet('color: darkGreen; font-weight: bold')
            except Exception:
                pass
    ...
    def _prompt_injury(self):
        ...
        msg_box.setText("<font size='5' color='blue'><b>上次門診為主訴職災，請問是否繼續?</b></font>")
    ...
    def _set_patient_large_font(self):
        self.ui.label_case_date.setStyleSheet(self.ui.label_case_date.styleSheet() + '; font-size: 24px;')
        self.ui.label_ins_type.setStyleSheet(self.ui.label_ins_type.styleSheet() + '; font-size: 24px;')
        self.ui.label_patient_name.setStyleSheet(self.ui.label_patient_name.styleSheet() + '; font-size: 24px;')
        self.ui.label_regist_no.setStyleSheet(self.ui.label_regist_no.styleSheet() + '; font-size: 24px;')
        self.ui.label_share_type.setStyleSheet(self.ui.label_share_type.styleSheet() + '; font-size: 24px;')
        self.ui.label_card.setStyleSheet(self.ui.label_card.styleSheet() + '; font-size: 24px;')
    ...
    def _set_ui(self):
        ...
        if self.system_settings.field('健保自費分開') == 'Y' or self.call_from == '病歷查詢健保病歷':
            pass
        else:
        ...
        self.medical_record['DiagDate'] = self.tab_registration.ui.lineEdit_diag_start_time.text()
        if self.medical_record['DiagDate'] == '':
            self.medical_record['DiagDate'] = self.medical_record['CaseDate']
        else:
            self.medical_record['DiagDate'] = date_utils.str_to_datetime(self.medical_record['DiagDate'])
        ...
        if (self.call_from == '醫師看診作業' and
                personnel_utils.get_permission(
                    self.database, '醫師看診作業', '候診病歷非主治醫師不可存檔', self.user_name) == 'Y' and
                self.user_name != doctor):
    ...
    def _set_pulse(self):
        max_pulse = 10
        default_pulse_list = ['左', '右', '浮', '沉', '滑', '弦', '細', '濡', '遲', '數']
        custum_pulse_list = []
    ...
    def _set_memo_button(self):
        ...
        self.ui.tabWidget_past_record.setCornerWidget(self.add_extra_memo_button, QtCore.Qt.TopRightCorner)
    ...
    def _set_background_color(self):
        ...
        elif self.tab_registration.comboBox_reg_type.currentText() in nhi_utils.TELECOM_TYPE:
            background_color = ui_utils.VIDEO_DIAG_COLOR
        elif (self.tab_registration.comboBox_share_type.currentText() in nhi_utils.INFECTIOUS_TYPE and
              self.tab_registration.comboBox_injury_type.currentText() in nhi_utils.INFECTIOUS_TYPE):
    ...
    def _open_medical_record_collection(self):
        dialog = dialog_utils.get_dialog_medical_record_collection(
            self, self.database, self.system_settings, self.medical_record['CaseDate'],
        )
        dialog.exec_()
        dialog.deleteLater()
    ...
    def _get_current_medicine_set(self):
        ...
        if current_tab_text == '健保':
            ...
            medicine_set = number_utils.get_integer(current_tab_text.split('自費')[1]) + 1
    ...
    def _open_medical_record_experience(self):
        ...
        dialog = dialog_utils.get_dialog_medical_record_experience(
            self, self.database, self.system_settings, self.medical_record['CaseDate'], medicine_set,
        )
        dialog.exec_()
        dialog.deleteLater()
    ...
    def _set_signal(self):
        ...
        self.ui.action_reference_prescript.triggered.connect(self._open_reference_prescript)
        ...
        self.ui.action_medical_record_experience.triggered.connect(self._open_medical_record_experience)
        ...
        self.ui.action_medical_record_version_history.triggered.connect(self._medical_record_version_history)
        ...
        self.ui.toolButton_blood_measure.clicked.connect(self._tool_button_blood_measure_clicked)
        self.ui.toolButton_symptom.clicked.connect(self._tool_button_dictionary_clicked)
        self.ui.toolButton_symptom_kt.clicked.connect(self._tool_button_dictionary_clicked)
        ...
        self.ui.toolButton_symptom_picker.clicked.connect(self._tool_button_picker_clicked)
        self.ui.toolButton_tongue_picker.clicked.connect(self._tool_button_picker_clicked)
        ...
        self.ui.toolButton_disease4.clicked.connect(self._tool_button_dictionary_clicked)
        ...
        self.ui.pushButton_disease4.clicked.connect(self._tool_button_dictionary_clicked)
        ...
        self.ui.lineEdit_disease_code4.returnPressed.connect(self.disease_code_return_pressed)
        self.ui.lineEdit_disease_code4.editingFinished.connect(self.disease_code_editing_finished)
    ...
    def _integrate_care_clicked(self):
        ...
        self.check_box_integrate_care.setStyleSheet('color: red; font-weight: bold')
        dialog = dialog_utils.get_dialog_integrate_care(self, self.database, self.system_settings, self.case_key)
        dialog.exec_()
    ...
    def _set_integrate_care_symptom(self, symptom):
        self.insert_text(self.ui.textEdit_symptom, symptom + '\n', '', insert_comma=False)
    ...
    def _set_tab_cornor_widget(self):
        ...
        if self.call_from in ['醫師看診作業', '新增自費病歷', '加購自費病歷'] and \
                self.system_settings.field('顯示看診計時器') == 'Y':
            self._set_timer(tab_corner_widget, h_layout)

        self.ui.tabWidget_medical.setCornerWidget(tab_corner_widget, QtCore.Qt.TopRightCorner)
    ...
    def _set_regist_typex_corner_widget(self, tab_corner_widget, h_layout):
        ...
        ui_utils.set_combo_box(self.combo_box_regist_typex, nhi_utils.REGIST_TYPEX, regist_typex)
    ...
    def _set_integrate_care(self, tab_corner_widget, h_layout):
        ...
        if self.call_from == '醫師看診作業' and treat_type in nhi_utils.IMPROVE_CARE_TREAT:  # 試辦計畫不可申報
            self.check_box_integrate_care.setEnabled(False)
    ...
    def _chronic_code_clicked(self):
        self.tab_registration.ui.checkBox_no_special_code.setChecked(self.check_box_chronic_code.isChecked())
    ...
    def _set_prescript_tab_cornor_widget(self):
        ...
        self.ui.tabWidget_prescript.setCornerWidget(tab_corner_widget, QtCore.Qt.TopRightCorner)

    def _prescript_zoom_clicked(self):
        self.ui.groupBox_diagnostic.setVisible(not self.ui.groupBox_diagnostic.isVisible())
            ...
            for i in range(len(disease_list)):
                ...
                if disease_list[i][0].text() == '' and disease_list[i+1][0].text() != '':
                    ...
                    case_utils.set_disease_tool_tip(
                        self.database, disease_list[i][0], disease_list[i][1], self.parent.complicated_treat_list)
    ...
    def check_chronic_disease(self):
        ...
        for icd_field in disease_list:
            ...
            if rows and not self.tab_registration.ui.checkBox_no_special_code.isChecked():
    ...
    def check_complicated_treat_disease(self, disease_code):
        ...
        second_treatment = string_utils.xstr(self.tab_list[0].comboBox_second_treatment.currentText())

        if treatment in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_LIST or \
                second_treatment in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_LIST:
            return

        hint = '以上提醒僅供參考'

        hint_list = []
        if disease_code in self.parent.moderate_complicated_acupuncture_list and \
                treatment not in nhi_utils.MODERATE_COMPLICATED_ACUPUNCTURE_LIST and \
                second_treatment not in nhi_utils.MODERATE_COMPLICATED_ACUPUNCTURE_LIST:
            hint_list.append('<font size="5" color="blue"><b>可申報中度複雜性針灸!</b></font>')

        if disease_code in self.parent.highly_complicated_acupuncture_list and \
                treatment not in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_LIST and \
                second_treatment not in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_LIST:
            hint_list.append('<font size="5" color="blue"><b>可申報高度複雜性針灸!</b></font>')

        if disease_code in self.parent.moderate_complicated_massage_list and \
                treatment not in nhi_utils.MODERATE_COMPLICATED_MASSAGE_LIST and \
                second_treatment not in nhi_utils.MODERATE_COMPLICATED_MASSAGE_LIST:
            hint_list.append('<font size="5" color="blue"><b>可申報中度複雜性傷科!</b></font>')

        if disease_code in self.parent.highly_complicated_massage_list and \
                treatment not in nhi_utils.HIGHLY_COMPLICATED_MASSAGE_LIST and \
                second_treatment not in nhi_utils.HIGHLY_COMPLICATED_MASSAGE_LIST:
            hint_list.append('<font size="5" color="blue"><b>可申報高度複雜性傷科!</b></font>')
        ...
        try:
            ...
            ''')
            if len(rows) > 0:
                hint_list.append('<font size="5" color="red"><b>此診斷碼為慢性病</b></font>')
        except Exception:
            pass


        if len(hint_list) > 0:
            if self.system_settings.field('輸入病名後不要彈出複雜性針傷提示視窗') == 'Y':
                pass
            else:
                system_utils.show_message_box(
                    QMessageBox.Information,
    ...
    def disease_code_return_pressed(self):
        ...
        if icd_code.isdigit():
            ...
            english_name_script = ' AND '.join(
                [f'UPPER(EnglishName) LIKE "%{k.upper()}%"' for k in keyword_list if len(icd_code) >= 5])
    ...
    def _set_disease(self, line_edit_disease_code, line_edit_disease_name, icd_code, disease_name):
        ...
        try:
            case_utils.set_disease_tool_tip(
                self.database, line_edit_disease_code, line_edit_disease_name, self.parent.complicated_treat_list)
        except Exception:
            pass
    ...
    def _insert_template_patient(self):
        ...
        self.database.exec_sql('UPDATE patient SET PatientKey = 0 WHERE Name = "參考病歷"')
    ...
    def _set_permission(self):
        ...
        if personnel_utils.get_permission(self.database, '病患查詢', '調閱資料', self.user_name) != 'Y':
            self.ui.action_patient.setEnabled(False)
    ...
    def _set_in_progress(self, in_progress):
        ...
        if in_progress != 'NULL' and self.system_settings.field('alleypin') == 'Y':
            alleypin_utils.update_progresses(self.database, self.system_settings, self.case_key)
    ...
    def _set_patient_data(self):
        self.age_year = None
        if self.case_key is None:
            self.ui.label_case_date.setText(datetime.datetime.now().strftime("%Y-%m-%d %H:%M"))
        ...
        if self.patient_record is None:
            ...
            self.ui.label_case_date.setText(self.medical_record['CaseDate'].strftime('%Y-%m-%d %H:%M'))
        ...
        self.ui.groupBox_patient.setTitle(f'病患基本資料 (病歷號: {patient_key}, {treat_type})')
        ...
        if self.system_settings.field('日期格式') == '民國年':
            ...
            try:
                birthday = date_utils.date_to_zh_tw_date(string_utils.xstr(self.patient_record['Birthday']))
            except Exception:
                pass
        ...
        self.ui.label_patient_name.setText(string_utils.xstr(name))
        self.ui.label_regist_no.setText(string_utils.xstr(f'{regist_no} ({regist_type})'))
    ...
    def record_modified(self):
        ...
        try:
            if self.ui.textEdit_symptom.document().isModified() or \
                    ...
                    self.ui.textEdit_patient_remark.document().isModified() or \
                    ...
                    self.ui.lineEdit_disease_code4.isModified() or \
    ...
    def open_dictionary(self, medicine_set, dialog_type=None):
            ...
            if self.ui.textEdit_symptom.hasFocus():
                ...
                for i in range(len(self.tab_list)):
                    if self.tab_list[i] is not None and self.tab_list[i].ui.tableWidget_prescript.hasFocus():
                        ...
                        medicine_set = i + 1
                    elif self.tab_list[0] is not None and self.tab_list[0].ui.tableWidget_treat.hasFocus():
                        dialog_type = '健保處置'
                        medicine_set = 1
        ...
        if dialog_type in ['主訴', '舌診', '脈象', '備註']:
            ...
            dialog = dialog_utils.get_dialog_symptom_kt(
                self, self.database, self.system_settings, text_edit[dialog_type], text_edit['舌診'])
        elif dialog_type in ['辨證']:
            dialog = dialog_utils.get_dialog_diagnosis(
                self, self.database, self.system_settings, dialog_type, text_edit[dialog_type], text_edit['治則']
            )
            ...
            if dialog_type == '病名2' and self.sender().objectName() == 'toolButton_disease2':
        ...
        elif dialog_type in ['健保處置', '自費處置', '健保針灸處置', '健保傷科處置'] and medicine_set is not None:
            ...
            dialog = dialog_utils.get_dialog_medicine(
                self, self.database, self.system_settings,
                self.tab_list[medicine_set-1].tableWidget_prescript, medicine_set, treat,
            )
    ...
    def _set_diagnostic(self, row):
        ...
        for i, disease_field in enumerate(disease_field_list):
            ...
            case_utils.set_disease_tool_tip(
                self.database, disease_field[0], disease_field[1], self.parent.complicated_treat_list)

        self.ui.lineEdit_distinguish.setText(string_utils.get_str(row['Distincts'], 'utf8'))
    ...
    def _check_infectious_date(self):
        if (string_utils.xstr(self.medical_record['InsType']) != '健保' or
           string_utils.xstr(self.medical_record['Share']) not in nhi_utils.INFECTIOUS_TYPE or
           string_utils.xstr(self.medical_record['Injury']) not in nhi_utils.INFECTIOUS_TYPE):
            return

        infectious_date = case_utils.get_case_extend(self.database, self.case_key, '確診日期')
    ...
    def _read_recently_history(self):
        ...
        self.tab_medical_record_recently_history = \
            module_utils.get_medical_record_recently_history(
                self, self.database, self.system_settings, self.case_key, self.patient_key, self.call_from
            )
    ...
    def _read_fees(self):
        ...
        self.tab_medical_record_fees = module_utils.get_medical_record_fees(
            self, self.database, self.system_settings,
            self.medical_record, self.case_key, self.patient_key, self.call_from)
        self.ui.tabWidget_past_record.addTab(self.tab_medical_record_fees, tab_name)
        tab_index = self._get_tab_index(tab_name)
        self.ui.tabWidget_past_record.tabBar().setTabTextColor(tab_index, QtGui.QColor('darkCyan'))
    ...
    def save_medical_record(self, print_form=False, force_save=False):
        ...
        if self.is_doctor_done():
            ...
            try:
                case_utils.backup_medical_record(
                    self.database, self.case_key, '編輯備份',
                    backup_date,
                    self.user_name,
                )  # 編輯前備份資料
            except Exception:
        ...
        if (self.ins_type == '健保' and
            ...
            ins_apply_fee = charge_utils.get_table_widget_item_fee(
                ...
                self.tab_medical_record_fees.INS_COLUMN['InsApplyFee'], 0
            )
            deposit_fee = charge_utils.get_table_widget_item_fee(
                self.tab_medical_record_fees.ui.tableWidget_cash_fees,
                self.tab_medical_record_fees.SELF_COLUMN['DepositFee'], 0
            )
        ...
        if self.ins_type == '自費' and string_utils.xstr(self.medical_record['TreatType']) != '自購':
        ...
        if ((self.ins_type == '健保') and
                ...
                (card[:4] not in nhi_utils.ABNORMAL_CARD) and
                (xcard[:4] not in nhi_utils.ABNORMAL_CARD) and
                (card != '欠卡')):
            if ic_card_type == '虛擬健保卡' and self.system_settings.field('虛擬健保卡統一在掛號作業') == 'Y':
                pass
            else:
                if ic_card_type == '虛擬健保卡':
                    qrcode = None
                    vhc_req_code = vhc_utils.get_vhc_req_code_from_wait(self.database, self.wait_key)
                ...
                while True:
                    if ic_card.insert_correct_ic_card(self.patient_key):
                        if card == 'IC' or \
                                self.system_settings.field('產生安全簽章位置') == '診療' and \
                                card != '欠卡' and \
                                card[:4] not in nhi_utils.ABNORMAL_CARD:
                    ...
                    msg_box.setText("<font size='4' color='red'><b>讀卡機作業失敗, 是否繼續存檔?</b></font>")
                    msg_box.setInformativeText("注意！繼續存檔, 不會影響當日IC卡資料上傳!")
                    msg_box.addButton(QPushButton("再試一次寫卡"), QMessageBox.NoRole)
                    msg_box.addButton(QPushButton("放棄寫卡, 繼續存檔"), QMessageBox.YesRole)
                    msg_box.addButton(QPushButton("取消存檔, 繼續編輯"), QMessageBox.RejectRole)
        ...
        if '存檔列印' in menu_item or print_form:
            ...
            if self.system_settings.field('電子處方箋路徑') not in ['', None]:
                prescript_utils.save_electrical_prescript(self.database, self.system_settings, self.case_key)
        elif menu_item == '存檔後選擇列印處方':
    ...
    def _write_ic_card(self, ic_card):
        ...
        ic_card_ok = ic_card.write_ic_card(
            '掛號寫卡',
            self.patient_key,
            number_utils.get_integer(self.tab_registration.ui.comboBox_course.currentText()),
        ...
        card = string_utils.xstr(self.medical_record['Card'])
        if card == 'IC':
            card = string_utils.xstr(ic_card_ok.treat_data['seq_number'])  # 有產生卡號才更新
    ...
    def _check_fees(self):
        ...
        old_total_fee = number_utils.get_integer(
            charge_utils.get_table_widget_item_fee(
                self.tab_medical_record_fees.ui.tableWidget_cash_fees,
                self.tab_medical_record_fees.SELF_COLUMN['TotalFee'], 0
            )
        )
        ...
        if new_total_fee != old_total_fee:
            ...
            msg_box.addButton(QPushButton(f"使用目前的自費金額 ${old_total_fee}"), QMessageBox.NoRole)
            msg_box.addButton(QPushButton(f"使用電腦自動批價的金額 ${new_total_fee}"), QMessageBox.YesRole)
    ...
    def _set_self_treat_type(self):
        exclude_list = nhi_utils.HOME_CARE + nhi_utils.TRADITIONAL_TREAT + nhi_utils.PURCHASE
    ...
    def _set_treatment_and_course(self):
        if self.tab_registration.ui.comboBox_ins_type.currentText() in ['自費']:  # 自費不要變更
            return

        if self.tab_registration.ui.comboBox_treat_type.currentText() in nhi_utils.HOME_CARE:  # 居家醫療不要變更
        ...
        primary_treatment = string_utils.xstr(self.tab_list[0].comboBox_treatment.currentText())
        second_treatment = string_utils.xstr(self.tab_list[0].comboBox_second_treatment.currentText())
        course = number_utils.get_integer(self.tab_registration.ui.comboBox_course.currentText())
        ...
        if treatment not in ['', None]:
            ...
            if self.tab_registration.ui.comboBox_treat_type.currentText() != '醫療諮詢' and \
                    treat_type not in ['慢性腎病照護', '助孕照護', '保胎照護']:
    ...
    def update_medical_record(self, set_doctor_done=False, check_prescript=True):
        ...
        if self.medical_record['InsType'] == '健保':
            if self.check_box_integrate_care.isChecked():
                ...
                case_utils.clear_case_extend(self.database, self.case_key, '整合醫療照護')
    ...
    def _set_charge_done(self, case_key=None, charge_done='True'):
        ...
        if self.system_settings.field('在掛號機批價繳費') == 'Y' and \
                string_utils.xstr(self.medical_record['Register']) == '掛號機':  # 2024.10.01 經由掛號機掛號來的病患，由掛號機批價
            return

        if self.system_settings.field('視訊診療須經過批價作業') == 'Y':
            injury = string_utils.xstr(self.medical_record['Injury'])
            regist_type = string_utils.xstr(self.medical_record['RegistType'])
            if injury in nhi_utils.INFECTIOUS_INJURY_TYPE or regist_type in nhi_utils.TELECOM_TYPE:
                return
    ...
    def save_prescript(self, check_prescript=True):
        ...
        for medicine_set, tab_prescript in zip(range(1, max_tab+1), self.tab_list):
            if tab_prescript is not None:
                try:
                    if not tab_prescript.save_prescript(check_prescript=check_prescript):
                        check_ok = False
                except RuntimeError:  # 關閉處方頁, 刪除整個處方
                    self.remove_prescript(medicine_set)
            else:
    ...
    def _open_past_history(self):
        dialog = dialog_utils.get_dialog_medical_record_past_history(
            self, self.database, self.system_settings, self.case_key, self.patient_key, '病歷登錄'
        )
        dialog.exec_()
        dialog.deleteLater()
        ...
        if key in [
            ...
            if key in [Qt.Key_Enter, Qt.Key_Return]:
                ...
                if self.input_code != '':
                    ...
                    QtCore.QTimer.singleShot(50, lambda: self._query_diagnostic_dict(
    ...
    def _tool_button_picker_clicked(self):
        ...
        dialog = dialog_utils.get_dialog_diagnostic_picker(
            self, self.database, self.system_settings, text_edit, clinic_type, '',
        )
    ...
    def _check_highly_massage_course(self):
        ...
        treatment = registration_utils.get_first_course_treatment(
            self.database, self.system_settings, case_date, self.patient_key, card, course
        ) 
        ...
        if treatment in nhi_utils.HIGHLY_COMPLICATED_MASSAGE_TREAT:
            system_utils.show_message_box(
                QMessageBox.Warning,
                '高度複雜傷科後續治療提醒',
                f'<h3><font color="red">本療程第一次為高度複雜性傷科，今日只能執行一般針灸或一般傷科治療。</font></h3>',
    ...
    def _insert_today(self):
        try:
            today = date_utils.west_date_to_nhi_date(self.medical_record['CaseDate'].date(), '/')
        except Exception:
    ...
    def _insert_calendar(self):
        ...
        self.insert_text(self.ui.textEdit_symptom, insert_date + ' ', '', insert_comma=False)
        self.ui.textEdit_symptom.setFocus(True)
    ...
    def _add_symptom_dict(self):
        selected_text = self.ui.textEdit_symptom.textCursor().selectedText().strip()[:40]
    ...
    def capture_image(self):
        ...
        dialog = dialog_utils.get_dialog_capture_image(
            self, self.database, self.system_settings, self.case_key, self.patient_key, '病歷影像',
        )
    ...
    def _exam_precheck(self):
        dialog = dialog_utils.get_dialog_exam_precheck(
            self, self.database, self.system_settings, self.case_key, '病歷登錄',
        )
        dialog.exec_()
        if dialog.ui.checkBox_copy_to_remark.isChecked():
            remark = dialog.get_remark()
            self.insert_text(self.ui.textEdit_remark, remark + '\n', '', insert_comma=False)
    ...
    def _conflict_drug(self):
        ...
        dialog = dialog_utils.get_dialog_conflict_drug(
            self, self.database, self.system_settings, doctor_id, patient_id, table_widget,
        )
        dialog.exec_()
        dialog.deleteLater()
    ...
    def _restore_medical_record_json(self, backup_records_key):
        ...
        max_medicine_set = case_utils.get_max_medicine_set_from_json(prescript_json_rows)
        ...
        for medicine_set in range(1, max_medicine_set+1):
            self.tab_list[medicine_set-1].copy_prescript_from_json(
                backup_records_key, json_medical_record, prescript_json_rows, medicine_set)
    ...
    def refresh_wait(self):
        ...
        self.ui.groupBox_diagnostic.setTitle(f'診斷  (主治醫師: {doctor}){waiting_count}')
    ...
    def _get_prescript_rows(self, medicine_set, tab_prescript):
        ...
        for row_no in range(table_widget_prescript.rowCount()):
            ...
            dosage_mode = table_widget_prescript.item(
                row_no, prescript_utils.INS_PRESCRIPT_COL_NO['DosageMode']).text()
            unit = table_widget_prescript.item(
        ...
        if medicine_set == 1:
            ...
            for row_no in range(table_widget_treat.rowCount()):
                ...
                medicine_type = table_widget_treat.item(
                    row_no, prescript_utils.INS_TREAT_COL_NO['MedicineType']).text()
                medicine_name = table_widget_treat.item(
                    row_no, prescript_utils.INS_TREAT_COL_NO['MedicineName']).text()
                ins_code = table_widget_treat.item(
                    row_no, prescript_utils.INS_TREAT_COL_NO['InsCode']).text()
                row = {}
                row['medicine_set'] = medicine_set
                row['medicine_key'] = medicine_key
                row['medicine_type'] = medicine_type
