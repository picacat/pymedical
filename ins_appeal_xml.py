...
class InsAppealXML(QtWidgets.QMainWindow):
    ...
    def _get_ins_total_points(self):
        ...
        for row in rows:
            ...
            ins_appeal_items_total_points = self._get_ins_appeal_items_total_points(ins_appeal_key)
    ...
    def _get_reject_total_points(self):
        ...
        for row in rows:
            ins_appeal_key = row['InsAppealKey']
            ins_appeal_items_reject_points = self._get_ins_appeal_items_total_points(ins_appeal_key)
    ...
    def _add_tdata(self, root):
        ...
        general_case, general_points, special_case, special_points = self._get_ins_total_points()
    ...
    def _add_dbody(self, ddata, row):
        ...
        point3 = number_utils.get_integer(row['Point3'])  # 電腦核減點數
        if point3 > 0:
