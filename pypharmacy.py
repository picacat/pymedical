...
class PyPharmacy(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, *args):
        super(PyPharmacy, self).__init__(parent)
        self.parent = parent
        ...
        self.scale_time = round(number_utils.get_float(self.system_settings.field('電子秤測重時間')), 1)
    ...
    def _set_signal(self):
        ...
        self.ui.action_print_drug_bag.triggered.connect(lambda: self._print_prescript_bag(None))
        self.ui.action_medicine_settings.triggered.connect(self._open_dialog_medicine_settings)
        ...
        self.ui.tableWidget_pharmacy_list.itemSelectionChanged.connect(self._pharmacy_list_changed)
    ...
    def open_dialog_pharmacy(self, data):
        ...
        is_druged = prescript_utils.get_pres_extend_value(self.database, prescript_key, '調劑完成')
        if is_druged == '是':
            system_utils.show_message_box(
                QMessageBox.Critical,
                '已經配過此藥了',
        ...
        dialog = dialog_utils.get_dialog_pharmacy_dosage(
            self, self.database, self.system_settings, prescript_key,
            medicine_key, medicine_code, medicine_set, self.scale_time)
    ...
    def _open_dialog_medicine_settings(self):
        dialog = dialog_utils.get_dialog_medicine_settings(self, self.database, self.system_settings)
        dialog.exec_()
    ...
    def _filter_pharmacy_list(self):
        ...
        for row_no in range(self.ui.tableWidget_pharmacy_list.rowCount()-1, -1, -1):
            ...
            if dispensing == '已調劑' and label_image is None:
                self.ui.tableWidget_pharmacy_list.removeRow(row_no)
            elif dispensing == '未調劑' and label_image is not None:
                self.ui.tableWidget_pharmacy_list.removeRow(row_no)
    ...
    def _set_pharmacy_list(self, row_no, row):
        ...
        for col_no in range(len(wait_row)):
            self.ui.tableWidget_pharmacy_list.setItem(
                row_no, col_no,
                QtWidgets.QTableWidgetItem(wait_row[col_no])
            )
            if col_no in [2, 3, 4, 5, 6, 8]:
                self.ui.tableWidget_pharmacy_list.item(
    ...
    def _set_pharmacy_ok(self, case_key, medicine_set, row_no, col_no):
        if not self._ready_to_serve(case_key, medicine_set):
            ...
            self._set_table_widget_image(self.ui.tableWidget_pharmacy_list, row_no, col_no, image_file)
    ...
    def _get_medicine_set(self):
        ...
        if pharmacy_type == '健保處方':
            ...
            medicine_set = number_utils.get_integer(pharmacy_type.replace('自費處方', '')) + 1
    ...
    def _read_prescript(self, case_key):
        ...
        if not self._ready_to_serve(case_key, medicine_set):
            ...
            for row_no in range(self.ui.tableWidget_prescript.rowCount()):
                ...
                self._set_table_widget_image(self.ui.tableWidget_prescript, row_no, 6, image_file)
    ...
    def _set_prescript_ok(self, prescript_key, row_no, col_no):
        ...
        self._set_table_widget_image(self.ui.tableWidget_prescript, row_no, col_no, image_file)
    ...
    def _save_records(self, wait_key, case_key, drug_done='True'):
        if wait_key not in ['', None]:
            self.database.exec_sql(f'UPDATE wait SET DrugDone = "{drug_done}" WHERE WaitKey = {wait_key}')
    ...
    def _set_scale_time(self):
        ...
        scale_time, ok = input_dialog.getDouble(
            self, '設定測重時間', '請輸入電子秤測重時間', self.scale_time, 0.5, 10, 1, QtCore.Qt.WindowFlags(), 0.1)
        if not ok:
            return
...
def handle_login(py_pharmacy):
    login_dialog = module_utils.get_login(py_pharmacy, py_pharmacy.database, py_pharmacy.system_settings)
    login_dialog.exec_()
...
def setup_user_environment(py_pharmacy, user_name, position):
    ...
    py_pharmacy.system_settings.post('登入日期', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
