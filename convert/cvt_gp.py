...
from libs import nhi_utils, number_utils, patient_utils, string_utils
...
class CvtGP:
    def __init__(self, parent, *args):
        ...
        self.conn = pyodbc.connect(
            f'DSN={self.parent.lineEdit_dsn_gp.text()};UID="";PWD=""'
        )
    ...
    def convert(self):
        if self.parent.ui.label_connection_status_gp == "未連線":
            ...
            msg_box.setText(
                "<font size='4' color='Red'><b>尚未執行連線測試, 請執行連線測試後再執行轉檔作業.</b></font>"
            )
    ...
    def _convert_registration(self, row, medical_record_row):
        ...
        medical_record_row["Share"] = self._get_share_type(
            medical_record_row["PatientKey"]
        )
    ...
    def _convert_diag(self, row, medical_record_row):
        ...
        medical_record_row["DiseaseCode1"] = self._get_field_value(
            row["DISE_STANDARD1"]
        )
        medical_record_row["DiseaseName1"] = self._get_field_value(row["DISE_NAME1"])
        medical_record_row["SpecialCode"] = self._get_field_value(
            row["DISE_SLOW_CODE_1"]
        )
        medical_record_row["DiseaseCode2"] = self._get_field_value(
            diag_row["DISE_STANDARD2"]
        )
        medical_record_row["DiseaseName2"] = self._get_field_value(
            diag_row["DISE_NAME2"]
        )
        medical_record_row["DiseaseCode3"] = self._get_field_value(
            diag_row["DISE_STANDARD3"]
        )
        medical_record_row["DiseaseName3"] = self._get_field_value(
            diag_row["DISE_NAME3"]
        )
        ...
        if treat_type is not None:
            treat_type = treat_type[:3]
            if treat_type in ["D01", "D02"]:
                medical_record_row["Treatment"] = "一般針灸"
            elif treat_type in ["D05", "D06"]:
                medical_record_row["Treatment"] = "中度複雜性針灸"
            elif treat_type in ["D07", "D08"]:
    ...
    def _convert_prescript(self, case_key, medical_record_row, dm_row):
        ...
        for row in rows:
            ...
            medicine_key, price = self._get_medicine_key(
                self._get_field_value(row["DRUG_ID"])
            )
            try:
                amount = number_utils.get_float(price) * number_utils.get_float(dosage)
            except Exception:
                amount = None
    ...
    def _convert_fees(self, case_key, dm_row):
        ...
        diag_share_fee = number_utils.get_integer(
            row["REG_INS_PART"]
        ) + number_utils.get_integer(row["HURT_INS_PART"])
        ...
        s_drug_fee = number_utils.get_integer(
            row["INN_MED_SELF"]
        ) + number_utils.get_integer(row["HEAL_MED_SELF"])
        s_herb_fee = number_utils.get_integer(row["DRINK_MED_SELF"])
        s_massage_fee = number_utils.get_integer(row["MASSAGE_SELF"])
        s_material_fee = number_utils.get_integer(
            row["MATERIAL_SELF"]
        ) + number_utils.get_integer(row["OTHER_SELF"])
    ...
    def _cvt_compound_title(self):
        ...
        fields = ["MedicineType", "MedicineMode", "InputCode", "MedicineName", "Unit"]
    ...
    def _cvt_compound(self):
        ...
        for row in rows:
            ...
            for drug_slip_row in drug_slip_rows:
                medicine_row = self._get_medicine_row(
                    self._get_field_value(drug_slip_row["Slip_Drug_Id"])
                )
                if medicine_row is None:
                    continue
