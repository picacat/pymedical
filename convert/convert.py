...
from convert import cvt_cm, cvt_cm2, cvt_gp, cvt_kthis, cvt_tm, cvt_utec, cvt_vis
...
class DialogConvert(QtWidgets.QDialog):
    ...
    def _set_combo_box(self):
        ...
        ui_utils.set_combo_box(
            self.ui.comboBox_lack_area, nhi_utils.TOUR_AREA_LEVEL["資源不足"], None
        )
        ui_utils.set_combo_box(
            self.ui.comboBox_correction_area,
            nhi_utils.CORRECTION_AREA_DICT[self.system_settings.field("健保業務")],
            None,
        )
    ...
    def set_vis_path(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            self,
            "選擇展望資料庫路徑",
            self.ui.lineEdit_vis_db_path.text(),
            options=options,
        )
        if directory:
    ...
    def accepted_button_clicked(self):
        tab_name = self.ui.tabWidget_convert.tabText(
            self.ui.tabWidget_convert.currentIndex()
        )
        if tab_name == "友杏":
    ...
    def test_connection(self):
        if string_utils.xstr(self.ui.lineEdit_database.text()) == "":
            ...
            msg_box.setText(
                "<font size='4' color='Red'><b>連線至資料庫主機失敗! 資料庫名稱空白.</b></font>"
            )
        ...
        msg_box.setText(
            "<font size='4' color='Blue'><b>恭喜您! 連線至資料庫主機成功.</b></font>"
        )
        ...
        if string_utils.xstr(self.ui.lineEdit_old_database.text()) != "":
            ...
            if self.utec_db.connected():
                ...
                msg_box.setWindowTitle("連線成功")
                msg_box.setText(
                    "<font size='4' color='Blue'><b>恭喜您! 連線至友杏資料庫主機成功.</b></font>"
                )
                msg_box.setInformativeText("連線成功, 可以執行轉檔作業.")
                msg_box.addButton(QPushButton("確定"), QMessageBox.YesRole)
                msg_box.exec_()
    ...
    def test_connection_tm(self):
        ...
        if dsn == "":
            ...
            msg_box.setWindowTitle("連線失敗")
            msg_box.setText(
                "<font size='4' color='Red'><b>連線至資料庫主機失敗! dsn名稱空白.</b></font>"
            )
        ...
        try:
            ...
            msg_box.setWindowTitle("連線失敗")
            msg_box.setText(
                "<font size='4' color='red'><b>連線至資料庫主機失敗.</b></font>"
            )
    ...
    def change_encoding(self):
        ...
        msg_box.setText(
            "<font size='4' color='Blue'><b>恭喜您! 資料庫編碼更改成功.</b></font>"
        )
