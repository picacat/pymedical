...
class CheckErrors(QtWidgets.QMainWindow):
    ...
    def _set_table_widget(self):
        self.table_widget_errors = class_utils.get_table_widget(
            self.ui.tableWidget_errors, self.database
        )
    ...
    def _insert_error_record(self, row, error_messages):
        ...
        card = (
            ...
            else string_utils.xstr(row["Card"])
            + "-"
        ...
        for column_no in range(len(error_record)):
            self.ui.tableWidget_errors.setItem(
                row_no, column_no, QtWidgets.QTableWidgetItem(error_record[column_no])
            )
            if column_no in [3, 11, 12, 13, 14, 15, 16, 17]:
                self.ui.tableWidget_errors.item(row_no, column_no).setTextAlignment(
    ...
    def _check_patient_error_exists(self, patient_key):
        for row_no in range(self.ui.tableWidget_errors.rowCount()):
            if self.ui.tableWidget_errors.item(row_no, 3).text() == string_utils.xstr(
                patient_key
            ):
                return True
    ...
    def _check_medical_record(self, row):
        ...
        if course >= 1:
            ...
            if (
                ...
                and treatment
                in nhi_utils.ACUPUNCTURE_TREAT_2021
                ...
                start_date >= nhi_utils.INS_TREAT_2021_DATE
                and treatment
                in nhi_utils.ACUPUNCTURE_TREAT_2020
            ...
            if course >= 2:
                if treatment in [
                    ...
                    error_messages.append(
                        "療程2-6次不得申報中度複雜性傷科首次或高度複雜性傷科起始次"
                    )
            else:
                if treatment in [
                    "中度針灸合併中度傷科療程2-6次",
            ...
            elif treatment in ["高度針灸合併高度傷科療程2-6次"]:
                error_messages.append("請改為高度針灸合併高度傷科後續治療")
            elif treatment in ["中度針灸合併高度傷科"]:
                error_messages.append("請改為中度針灸合併高度傷科起始次")
            elif treatment in ["高度針灸合併高度傷科"]:
        ...
        if card == "Z000" and (
            share_type in nhi_utils.INFECTIOUS_TYPE
            or injury_type in nhi_utils.INFECTIOUS_TYPE
        ):
        ...
        if (
            treatment in nhi_utils.INS_TREAT
            and string_utils.xstr(row["TreatType"]) in nhi_utils.INS_TREAT
            and number_utils.get_integer(row["Continuance"]) < 1
        ):
            error_messages.append("無療程序號")

        if (
            number_utils.get_integer(row["Continuance"]) >= 1
            and treatment not in nhi_utils.INS_TREAT
        ):
            error_messages.append("療程內無處置")

        if (
            string_utils.xstr(row["TreatType"]) in nhi_utils.TREAT_DICT
            and string_utils.xstr(row["Treatment"]) not in nhi_utils.INS_TREAT
        ):
            error_messages.append("就醫類別錯誤")
        ...
        if string_utils.xstr(row["DiseaseCode1"]) == "":
            ...
            for i in range(1, nhi_utils.MAX_DISEASE_CODE + 1):
                ...
                if disease_code != "" and disease_code[0] in [
        ...
        if (
            injury_type in nhi_utils.INFECTIOUS_TYPE
            or share_type in nhi_utils.INFECTIOUS_TYPE
        ):
            if string_utils.xstr(row["DiseaseCode1"]) != "U071":
                error_messages.append(
                    "非COVID-19確診病名部份負擔類別不得申報法定傳染病隔離通報"
                )
        ...
        if (
            self.check_empty_symptom
            and string_utils.get_str(row["Symptom"], "utf-8") == ""
        ):
        ...
        for i in range(1, 5):
            ...
            if not case_utils.is_disease_code_neat(self.database, disease_code):
                disease_code = case_utils.correct_neat_disease(
                    self.database, case_key, i
                )
                if disease_code is None:
    ...
    def _check_prescript(self, row):
        ...
        if (
            treatment
            in nhi_utils.COMPLICATED_ACUPUNCTURE_TREAT
        ...
        for prescript_row in prescript_rows:
            ...
            if "治療開始:" in medicine_name:
                try:
                    treat_start_times = datetime.datetime.strptime(
                        medicine_name.replace("治療開始:", ""), "%H:%M"
                    )
                except Exception:
                    pass
            ...
            if medicine_type in ["單方", "複方"] and "清冠一號" not in medicine_name:
                if ins_code != "" and ins_code[0] != "A":
                    error_messages.append(
                        f"""健保碼非中藥:{medicine_name}({ins_code})"""
                    )
        ...
        if require_treat_times:
            ...
            if (
                treat_start_times is not None
                and treat_end_times is not None
                and treat_times is not None
            ):
                try:
                ...
                if treat_end_times != correct_time_str:
                    correct_time = treat_start_times + datetime.timedelta(
                        minutes=minutes + 1
                    )  # 有一分鐘的誤差 (友杏)
                    correct_time_str = f"治療結束:{correct_time.strftime('%H:%M')}"
                    if treat_end_times != correct_time_str:
        ...
        if (
            string_utils.xstr(row["Treatment"]) in nhi_utils.ACUPUNCTURE_TREAT
            and acupuncture_treat <= 0
        ):
            error_messages.append("針灸治療無穴位記錄")
        elif (
            string_utils.xstr(row["Treatment"]) in nhi_utils.MASSAGE_TREAT
            and massage_treat <= 0
        ):
        ...
        if total_ins_medicine > 0:
            if (
                (
                    row["Share"] not in nhi_utils.INFECTIOUS_TYPE
                    and row["Injury"] not in nhi_utils.INFECTIOUS_TYPE
                )
                and pres_days >= 1
                and pres_days < 2
            ):
    ...
    def _check_charge(self, row):
        ...
        infectious_drug = prescript_utils.get_infectious_drug(
            self.database, row["CaseKey"]
        )
        isolation_position = case_utils.get_case_extend(
            self.database, row["CaseKey"], "隔離處所"
        )
    ...
    def _correct_errors(self):
        ...
        for row_no in range(self.ui.tableWidget_errors.rowCount()):
            ...
            if (
                "請改為中度針灸合併高度傷科起始次" in error_message
                or "請改為中度針灸合併高度傷科後續治療" in error_message
                or "請改為高度針灸合併高度傷科起始次" in error_message
                or "請改為高度針灸合併高度傷科後續治療" in error_message
            ):
            ...
            if (
                string_utils.xstr(row["Treatment"]) in nhi_utils.INS_TREAT
                and string_utils.xstr(row["TreatType"]) in nhi_utils.INS_TREAT
                and number_utils.get_integer(row["Continuance"]) < 1
            ):
            ...
            if (
                special_code != ""
                and string_utils.xstr(row["SpecialCode"]).strip() == ""
            ):
    ...
    def _correct_treatment_course_1(self, case_key):
        ...
        self.database.exec_sql(
            f'UPDATE cases SET Treatment = "{treatment}" WHERE CaseKey = {case_key}'
        )
    ...
    def _correct_name_character(self, case_key):
        ...
        self.database.exec_sql(
            f'UPDATE cases SET Name = "{name}" WHERE CaseKey = {case_key}'
        )
    ...
    def _check_care(self, row):
        ...
        if (
            treat_type
            not in ["小兒氣喘", "小兒腦性麻痺"] + nhi_utils.IMPROVE_CARE_TREAT
        ):
            return error_messages
    ...
    def _correct_home_care_card(self, case_key):
        ...
        self.database.exec_sql(
            f'UPDATE cases SET Card = "{correct_card}" WHERE CaseKey = {case_key}'
        )
