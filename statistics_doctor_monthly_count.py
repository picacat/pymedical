...
class StatisticsDoctorMonthlyCount(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, *args):
        super(StatisticsDoctorMonthlyCount, self).__init__(parent)
        self.parent = parent
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_STATISTICS_DOCTOR_MONTHLY_COUNT, self)
    ...
    def _set_statistics_table_heading(self):
        ...
        self._set_heading('內科', ['<=3天', '4-7天', '8-14天', '>=15天', '人數', '金額'])
        self._set_heading('針灸類給藥', ['<=3天', '4-7天', '8-14天', '>=15天', '人數', '金額'])
        self._set_heading('傷科類給藥', ['<=3天', '4-7天', '8-14天', '>=15天', '人數', '金額'])
    ...
    def _set_internal_cases(self, row, row_no, pres_days):
        ...
        try:
            case_count = number_utils.get_integer(self.ui.tableWidget_doctor_monthly.item(row_no, col_no).text()) + 1
            self._set_item_data(row_no, col_no, case_count)
        except Exception:
            pass

        try:
            case_subtotal = number_utils.get_integer(
                self.ui.tableWidget_doctor_monthly.item(row_no, 5).text()) + 1
    ...
    def _set_acupuncture_medicine_cases(self, row, row_no, pres_days):
        ...
        case_subtotal = number_utils.get_integer(
            self.ui.tableWidget_doctor_monthly.item(row_no, 11).text()) + 1
    ...
    def _set_massage_medicine_cases(self, row, row_no, pres_days):
        ...
        case_subtotal = number_utils.get_integer(
            self.ui.tableWidget_doctor_monthly.item(row_no, 17).text()) + 1
    ...
    def _set_general_acupuncture_cases(self, row, row_no, course):
        ...
        case_subtotal = number_utils.get_integer(
            self.ui.tableWidget_doctor_monthly.item(row_no, 21).text()) + 1
    ...
    def _set_moderate_acupuncture_cases(self, row, row_no, course):
        ...
        case_subtotal = number_utils.get_integer(
            self.ui.tableWidget_doctor_monthly.item(row_no, 25).text()) + 1
    ...
    def _set_highly_acupuncture_cases(self, row, row_no, course):
        ...
        case_subtotal = number_utils.get_integer(
            self.ui.tableWidget_doctor_monthly.item(row_no, 29).text()) + 1
    ...
    def _set_general_massage_cases(self, row, row_no, course):
        ...
        case_subtotal = number_utils.get_integer(
            self.ui.tableWidget_doctor_monthly.item(row_no, 33).text()) + 1
    ...
    def _set_moderate_massage_cases(self, row, row_no):
        case_count = number_utils.get_integer(self.ui.tableWidget_doctor_monthly.item(row_no, 35).text()) + 1
    ...
    def _set_highly_massage_cases(self, row, row_no):
        case_count = number_utils.get_integer(self.ui.tableWidget_doctor_monthly.item(row_no, 37).text()) + 1
    ...
    def _set_home_care_cases(self, row, row_no):
        case_count = number_utils.get_integer(self.ui.tableWidget_doctor_monthly.item(row_no, 39).text()) + 1
    ...
    def _calculate_subtotal(self):
        ...
        self.ui.tableWidget_doctor_monthly.setItem(0, case_col_no, QtWidgets.QTableWidgetItem('合計'))
        ...
        self.ui.tableWidget_doctor_monthly.setItem(1, case_col_no, QtWidgets.QTableWidgetItem('人數'))
        self.ui.tableWidget_doctor_monthly.item(1, case_col_no).setTextAlignment(
            QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter
        )
        self.ui.tableWidget_doctor_monthly.setItem(1, fee_col_no, QtWidgets.QTableWidgetItem('申報金額'))
        ...
        for row_no in range(2, row_count-1):
            total_case_count = 0
            total_fee = 0
            for col_no in range(1, self.ui.tableWidget_doctor_monthly.columnCount()-2):
