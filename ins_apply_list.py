...
from libs import (class_utils, date_utils, dialog_utils, export_utils,
                  nhi_utils, number_utils, personnel_utils, printer_utils,
...
class InsApplyList(QtWidgets.QMainWindow):
    ...
    def _set_signal(self):
        ...
        self.ui.toolButton_print_order.clicked.connect(lambda: self.print_order(print_type=None))
        self.ui.toolButton_print_medical_records.clicked.connect(
            lambda: self.print_medical_records(print_type=None))
        self.ui.toolButton_print_medical_chart.clicked.connect(lambda: self.print_medical_chart(print_type=None))
        self.ui.toolButton_print_patient_new_care.clicked.connect(self._print_patient_new_care)
        self.ui.toolButton_edit_ins_list.clicked.connect(self._edit_ins_list)
        self.ui.toolButton_print_order_bookmark.clicked.connect(self._print_order_bookmark)
        self.ui.toolButton_export.clicked.connect(self._export_ins_apply)
        self.ui.tableWidget_ins_apply_list.horizontalHeader().sectionClicked.connect(self._header_clicked)
    ...
    def _header_clicked(self, col_no):
        ...
        for row_no in range(row_count):
            try:
                check_box = self.ui.tableWidget_ins_apply_list.cellWidget(row_no, col_no)
    ...
    def open_medical_record(self):
        ...
        case_key_list = [
            ...
            row['CaseKey7'], row['CaseKey8'], row['CaseKey9'], row['CaseKey10'], row['CaseKey11'],
            row['CaseKey12'], row['CaseKey13'], row['CaseKey14'], row['CaseKey15'],
        ]
    ...
    def _open_medical_record_dialog(self, row, case_key_list):
        ...
        dialog.ui.label_header.setText(f'案件分類:{case_type}-{sequence:0>4} {name}的療程病歷明細')
        dialog.exec_()
    ...
    def _get_judge_sql(self, case_type=None):
        ...
        for row in self.ins_list:
            ...
            sequence = row[6]
            if case_type is None or (case_type is not None and current_case_type == case_type):
                sql_condition.append(f'(ApplyDate = "{apply_date}" AND CaseType = "{current_case_type}" AND Sequence = {sequence})')
    ...
    def print_order(self, print_type=None):
        ...
        printer_utils.print_form_ins_apply_order(
            ...
            self.apply_type, ins_apply_key, print_type=print_type
        )
    ...
    def print_medical_records(self, print_type=None):
        ...
        printer_utils.print_form_medical_records(
            self, self.database, self.system_settings,
            patient_key, None, start_date, end_date, print_type=print_type
        )
    ...
    def print_medical_chart(self, print_type=None):
        ...
        printer_utils.print_form_medical_chart(
            self, self.database, self.system_settings,
            patient_key, self.apply_date, print_type=print_type
        )
    ...
    def _print_patient_new_care(self):
        ...
        printer_utils.print_form_patient_new_care(
            self, self.database, self.system_settings,
            patient_key, case_key, self.apply_date,
        )
    ...
    def _find_error(self):
        self.table_widget_ins_apply_list.find_error(self.ui.tableWidget_ins_apply_list.columnCount()-1)
    ...
    def _edit_ins_list(self):
        ...
        dialog = dialog_utils.get_dialog_ins_list_edit(
            self, self.database, self.system_settings, ins_apply_key, sequence, case_date, end_date,
        )
    ...
    def _export_ins_apply(self):
        ...
        export_utils.export_table_widget_to_excel(
            excel_file_name, self.ui.tableWidget_ins_apply_list, [0],
    ...
    def _set_item(self, row_no, col_no, value, align=None):
        ...
        if align is not None:
            self.ui.tableWidget_ins_apply_statistics.item(row_no, col_no).setTextAlignment(
    ...
    def calculate_data(self):
        ...
        for doctor, items in doctor_summary.items():
            for row_no, item in enumerate(items):
                ...
                self._set_item(current_row_no, 5, total_price, align=QtCore.Qt.AlignRight)
    ...
    def _get_doctor_list(self):
        ...
        for row_no in range(self.ui.tableWidget_ins_apply_list.rowCount()):
            ...
            if all(code not in special_values \
