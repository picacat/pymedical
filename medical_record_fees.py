...
class MedicalRecordFees(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_ins_fees = class_utils.get_table_widget(
            self.ui.tableWidget_ins_fees, self.database
        )
        self.table_widget_cash_fees = class_utils.get_table_widget(
            self.ui.tableWidget_cash_fees, self.database
        )
    ...
    def _set_payment_type(self):
        ui_utils.set_combo_box(
            self.ui.comboBox_regist_payment_type, nhi_utils.PAYMENT_TYPE
        )
        ui_utils.set_combo_box(
            self.ui.comboBox_charge_payment_type, nhi_utils.PAYMENT_TYPE
        )
    ...
    def _set_signal(self):
        self.ui.toolButton_calculate_fees.clicked.connect(
            lambda: self.calculate_fees(auto_calculate_fees=True)
        )
        self.ui.tableWidget_cash_fees.keyPressEvent = (
            self._table_widget_cash_fees_key_press
        )
    ...
    def _table_widget_cash_fees_key_press(self, event):
        ...
        if key in [
            ...
            if current_row not in [self.SELF_COLUMN["ReceiptFee"]]:
                ...
                if current_row not in [self.SELF_COLUMN["DiscountFee"]] and key in [
                    QtCore.Qt.Key_Return,
                    QtCore.Qt.Key_Enter,
                ]:
        ...
        return QtWidgets.QTableWidget.keyPressEvent(
            self.ui.tableWidget_cash_fees, event
        )
    ...
    def _read_fees(self):
        ...
        if string_utils.xstr(row["TreatType"]) in nhi_utils.HOME_CARE:
            ...
            self.ui.tableWidget_cash_fees.setVerticalHeaderLabels(
                self.self_table_headers
            )
    ...
    def _set_cash_fee_editable(self):
        ...
        if user_name == "超級使用者":
            ...
            personnel_utils.get_permission(
                self.database, "病歷資料", "更改自費實收金額", user_name
            )
            != "Y"
        ):
    ...
    def _adjust_table_widget_align(self):
        for row_no in range(self.ui.tableWidget_ins_fees.rowCount()):
            ...
            self.ui.tableWidget_ins_fees.item(row_no, 0).setTextAlignment(
            ...
            if row_no in [
                ...
                self.ui.tableWidget_ins_fees.item(row_no, 0).setForeground(
                    QtGui.QColor("red")
                )
        ...
        for row_no in range(self.ui.tableWidget_cash_fees.rowCount()):
            ...
            self.ui.tableWidget_cash_fees.item(row_no, 0).setTextAlignment(
                QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
            )

            if row_no in [self.SELF_COLUMN["DiscountFee"]]:
                self.ui.tableWidget_cash_fees.item(row_no, 0).setForeground(
                    QtGui.QColor("red")
                )
    ...
    def calculate_fees(self, auto_calculate_fees=False):
        ins_type = string_utils.xstr(
            self.parent.tab_registration.comboBox_ins_type.currentText()
        )
        if ins_type == "健保":
            self.calculate_ins_fees()

        self.calculate_self_fees(
            self.parent.tab_list, auto_calculate_fees=auto_calculate_fees
        )
    ...
    def calculate_ins_fees(self):
        ...
        if (
            self.parent.tab_registration.ui.comboBox_apply_type.currentText()
            == "補報差額"
        ):
            return
        ...
        try:
            pres_days = number_utils.get_integer(
                self.parent.tab_list[0].ui.comboBox_pres_days.currentText()
            )
        except ValueError:
        ...
        isolation_position = (
            self.parent.tab_registration.ui.comboBox_isolation_position.currentText()
        )

        primary_treatment = self.parent.tab_list[0].comboBox_treatment.currentText()
        secondary_treatment = self.parent.tab_list[
            0
        ].comboBox_second_treatment.currentText()
        ...
        try:
            integrate_care = (
                self.parent.check_box_integrate_care.isChecked()
            )  # 有可能還沒create widget, 會造成不會批價
        except Exception:
        ...
        ins_fee = charge_utils.get_ins_fee(
            ...
            reg_type=string_utils.xstr(
                self.parent.tab_registration.comboBox_reg_type.currentText()
            ),
            treat_type=string_utils.xstr(
                self.parent.tab_registration.comboBox_treat_type.currentText()
            ),
            share=string_utils.xstr(
                self.parent.tab_registration.comboBox_share_type.currentText()
            ),
            course=course,
            pres_days=pres_days,
            pharmacy_type=string_utils.xstr(
                self.parent.tab_registration.comboBox_pharmacy_type.currentText()
            ),
            treatment=treatment,
            second_treatment=self.parent.tab_list[
                0
            ].comboBox_second_treatment.currentText(),
    ...
    def _set_ins_fee(self, ins_fee):
        ...
        discount_type = patient_utils.get_patient_discount_type(
            self.database, self.patient_key
        )
        ...
        if (
            self.system_settings.field("部份負擔連動") == "Y"
            or self.system_settings.field("檢查門診負擔多退少補") == "Y"
        ):
    ...
    def _set_receipt_drug_share_fee(self, discount_type, receipt_drug_share_fee):
        ...
        self.ui.tableWidget_cash_fees.setItem(  # 實收藥品負擔
            self.SELF_COLUMN["ReceiptDrugShare"],
            0,
    ...
    def _set_receipt_diag_share_fee(self, discount_type, receipt_diag_share_fee):
        ...
        self.ui.tableWidget_cash_fees.setItem(  # 實收門診負擔
            self.SELF_COLUMN["ReceiptDiagShare"],
            0,
    ...
    def get_total_fee(self):
        self_fee = charge_utils.get_self_fee(
            self.parent, self.database, self.system_settings, self.parent.tab_list
        )
        ...
        if self.system_settings.field("自費折扣方式") == "統一折扣":
            self_fee["discount_fee"] = self_total_fee - (
                self_total_fee * self.ui.spinBox_discount.value() / 100
            )
    ...
    def calculate_self_fees(self, tab_list, auto_calculate_fees=False):
        ...
        self_fee = charge_utils.get_self_fee(
            self.parent, self.database, self.system_settings, tab_list
        )
        ins_type = string_utils.xstr(
            self.parent.tab_registration.comboBox_ins_type.currentText()
        )
        ...
        if self_total_fee > 0 and ins_type == "自費" and self_fee["diag_fee"] == 0:
            visit = string_utils.xstr(
                self.parent.tab_registration.comboBox_visit.currentText()
            )
        ...
        if self.parent.tab_registration.ui.comboBox_treat_type.currentText() in [
            "加購"
        ]:
            self_fee["diag_fee"] = 0
    ...
    def save_record(self, case_key=None):
        ...
        if self.call_from in ["醫師看診作業"]:
            ...
            if (
                ins_type == "健保"
                and self.system_settings.field("檢查門診負擔多退少補") == "Y"
            ):
    ...
    def _check_receipt_diag_share_fee(self):
        ...
        receipt_diag_share_fee = number_utils.get_integer(
            self.medical_record["SDiagShareFee"]
        )
        current_diag_share_fee = number_utils.get_integer(
            charge_utils.get_table_widget_item_fee(
                self.ui.tableWidget_ins_fees, self.INS_COLUMN["DiagShareFee"], 0
            )
        )
    ...
    def _update_ins_fees_data(self):
        ...
        data = [
            ...
            self.ui.tableWidget_ins_fees.item(
                self.INS_COLUMN["InterDrugFee"], 0
            ).text(),
            self.ui.tableWidget_ins_fees.item(self.INS_COLUMN["PharmacyFee"], 0).text(),
            self.ui.tableWidget_ins_fees.item(
                self.INS_COLUMN["AcupunctureFee"], 0
            ).text(),
            self.ui.tableWidget_ins_fees.item(self.INS_COLUMN["MassageFee"], 0).text(),
            self.ui.tableWidget_ins_fees.item(
                self.INS_COLUMN["DislocateFee"], 0
            ).text(),
            self.ui.tableWidget_ins_fees.item(self.INS_COLUMN["ExamFee"], 0).text(),
            self.ui.tableWidget_ins_fees.item(self.INS_COLUMN["InsTotalFee"], 0).text(),
            self.ui.tableWidget_ins_fees.item(
                self.INS_COLUMN["DiagShareFee"], 0
            ).text(),
            self.ui.tableWidget_ins_fees.item(
                self.INS_COLUMN["DrugShareFee"], 0
            ).text(),
    ...
    def _check_cash_fees(self, receipt_regist_fee, receipt_diag_share_fee):
        ...
        if receipt_diag_share_fee != diag_share_fee:
            remark = (
                f"門診負擔應收: {diag_share_fee}元, 實收: {receipt_diag_share_fee}元"
            )
            if receipt_diag_share_fee > diag_share_fee:
    ...
    def _update_cash_fees_data(self, case_key=None):
        ...
        if (
            self.system_settings.field("病歷存檔檢查補退掛號費用") == "Y"
            and not self.parent.is_doctor_done()
        ):  # 醫師候診中才檢查
            regist_fee, receipt_diag_share_fee = self._check_cash_fees(
                regist_fee, receipt_diag_share_fee
            )
        ...
        data = [
            ...
            self.ui.tableWidget_cash_fees.item(
                self.SELF_COLUMN["SExpensiveFee"], 0
            ).text(),
            self.ui.tableWidget_cash_fees.item(
                self.SELF_COLUMN["SAcupunctureFee"], 0
            ).text(),
            self.ui.tableWidget_cash_fees.item(
                self.SELF_COLUMN["SMassageFee"], 0
            ).text(),
            self.ui.tableWidget_cash_fees.item(
                self.SELF_COLUMN["SMaterialFee"], 0
            ).text(),
