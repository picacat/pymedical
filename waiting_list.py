...
class WaitingList(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_waiting_list = class_utils.get_table_widget(
            self.ui.tableWidget_waiting_list, self.database
        )
        ...
        wait_completed_list_order_type = self.settings.value(
            "wait_completed_list", "升冪"
        )
    ...
    def _get_current_room(self):
        default_room = personnel_utils.get_person_field_value(
            self.database, self.user_name, "Room"
        )
    ...
    def _set_tab_widget_corner_widget(self):
        ...
        self.ui.tabWidget_waiting_list.setCornerWidget(
    ...
    def _set_doctor_corner_widget(self, tab_corner_widget, h_layout):
        ...
        position = personnel_utils.get_person_field_value(
            self.database, self.user_name, "Position"
        )
    ...
    def _set_hainachuan_corner_widget(self, tab_corner_widget, h_layout):
        ...
        self._seq_number_update_timer.timeout.connect(
            self._seq_number_value_changed
        )  # 確保只連接一次
        self.spinBox_seq_number.valueChanged.connect(
            self._start_debounce_seq_number_update
        )
    ...
    def _set_calling_bulletin_led(self, tab_corner_widget, h_layout):
        ...
        self._led_number_update_timer.timeout.connect(
            self._led_number_value_changed
        )  # 確保只連接一次
        self.spinBox_led_number.valueChanged.connect(
            self._start_debounce_led_number_update
        )
    ...
    def _refresh_waiting_list_color(self):
        ...
        for row_no in range(self.ui.tableWidget_waiting_list.rowCount()):
            item = self.ui.tableWidget_waiting_list.item(
                row_no, WAITING_LIST_COL_NO["RegistNo"]
            )
            if item is None:
                continue
            ...
            for col_no in range(self.ui.tableWidget_waiting_list.columnCount()):
                self.ui.tableWidget_waiting_list.item(row_no, col_no).setForeground(
                    color
                )

    def _set_combo_box_doctor(self):
        tab_name = self.ui.tabWidget_waiting_list.tabText(
    ...
    def _set_signal(self):
        ...
        self.ui.tableWidget_wait_completed.doubleClicked.connect(
            self.open_medical_record
        )
        self.ui.tabWidget_waiting_list.currentChanged.connect(
        ...
        self.ui.action_broadcast_led.triggered.connect(
            lambda: self._send_led(custom=False)
        )
        self.ui.action_custom_broadcast_led.triggered.connect(
            lambda: self._send_led(custom=True)
        )
        self.ui.tableWidget_reservation_list.itemSelectionChanged.connect(
            self._show_last_medical_record
        )
        self.ui.toolButton_print_prescript.clicked.connect(
            lambda: self._print_prescript("選擇列印")
        )
        self.ui.toolButton_print_receipt.clicked.connect(
            lambda: self._print_receipt("選擇列印")
        )
        self.ui.toolButton_print_misc.clicked.connect(
            lambda: self._print_misc("選擇列印")
        )
        ...
        self.ui.tableWidget_waiting_list.keyPressEvent = (
            self._table_widget_waiting_list_key_press
        )
        ...
        self.ui.comboBox_order_type.currentTextChanged.connect(
            self._combo_box_order_type_changed
        )
    ...
    def _set_permission(self):
        ...
        if (
            personnel_utils.get_permission(
                self.database, self.program_name, "病歷登錄", self.user_name
            )
            != "Y"
        ):
        ...
        if (
            personnel_utils.get_permission(
                self.database, self.program_name, "非醫師病歷登錄", self.user_name
            )
            == "Y"
        ):
    ...
    def _get_room_script(self, table_name, doctor=None):
        if self.system_settings.field("候診名單顯示診別") == "指定診別":
            ...
            self.system_settings.field("候診名單顯示診別") == "醫師診別"
            and doctor != "全部"
        ):
            if doctor is None:
                doctor = self.user_name
    ...
    def _display_waiting_message(self):
        ...
        for row_no in range(self.ui.tableWidget_waiting_list.rowCount()):
            item = self.ui.tableWidget_waiting_list.item(
                row_no, WAITING_LIST_COL_NO["InsType"]
            )
            if item is None:
                continue
        ...
        self.ui.label_waiting_message.setText(
            f"目前候診人數: 健保 {ins_count} 人, 預約未報到 {reserve_count} 人"
        )
    ...
    def _set_table_data(self, row_no, row):
        ...
        case_utils.set_in_progress_icon(
            ...
            row_no,
            WAITING_LIST_COL_NO["InProgress"],
            in_progress,
        )
        ...
        for col_no in range(len(wait_row)):
            ...
            if col_no in [
                WAITING_LIST_COL_NO["PatientKey"],
                WAITING_LIST_COL_NO["Age"],
                WAITING_LIST_COL_NO["Room"],
                WAITING_LIST_COL_NO["RegistNo"],
                WAITING_LIST_COL_NO["WaitTime"],
            ]:
                self.ui.tableWidget_waiting_list.item(row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
            elif col_no in [
                WAITING_LIST_COL_NO["Gender"],
                WAITING_LIST_COL_NO["Visit"],
            ]:
                self.ui.tableWidget_waiting_list.item(row_no, col_no).setTextAlignment(
    ...
    def _get_waiting_list_font_color(self, row_no):
        ...
        ins_type = self.ui.tableWidget_waiting_list.item(
            row_no, WAITING_LIST_COL_NO["InsType"]
        ).text()
        try:
            course = (
                self.ui.tableWidget_waiting_list.item(
                    row_no, WAITING_LIST_COL_NO["Card"]
                )
                .text()
                .split("-")[1]
            )
        ...
        in_progress = self.ui.tableWidget_waiting_list.cellWidget(
            row_no, WAITING_LIST_COL_NO["InProgress"]
        )
    ...
    def _set_reservation_data(self, row_no, row):
        ...
        for col_no in range(len(reservation_row)):
            ...
            if patient_key == "網路初診":
                self.ui.tableWidget_reservation_list.item(row_no, col_no).setForeground(
                    QtGui.QColor("green")
                )
    ...
    def _combo_box_order_type_changed(self):
        self.settings.setValue(
            "wait_completed_list", self.ui.comboBox_order_type.currentText()
        )
        self._read_wait_completed()
    ...
    def _set_statistics_list(self, doctor=None):
        ...
        statistics_list["本日健保已就診人數"] = (
            statistics_utils.get_count_by_treat_type(
                self.database,
                "wait",
                "當日",
                ["全部"],
        ...
        statistics_list["本日健保傷科人數"] = statistics_utils.get_count_by_treat_type(
            ...
            "wait",
            "當日",
            nhi_utils.MASSAGE_TREAT + nhi_utils.DISLOCATE_TREAT,
            doctor,
        )

        statistics_list["本日健保中度複針人數"] = (
            statistics_utils.get_count_by_treat_type(
                self.database,
                "wait",
                "當日",
        ...
        statistics_list["本日健保針傷合併人數"] = (
            statistics_utils.get_count_by_treat_type(
                self.database,
                "wait",
                "當日",
                nhi_utils.MERGE_TREAT_LIST,
                doctor,
            )
        )
        statistics_list["本日健保中度複傷人數"] = (
            statistics_utils.get_count_by_treat_type(
                self.database,
                "wait",
                "當日",
                nhi_utils.MODERATE_COMPLICATED_MASSAGE_TREAT
                + nhi_utils.MODERATE_COMPLICATED_MASSAGE_TREAT,
                doctor,
            )
        )
        statistics_list["本日健保高度複傷人數"] = (
            statistics_utils.get_count_by_treat_type(
                self.database,
                "wait",
                "當日",
                nhi_utils.HIGHLY_COMPLICATED_MASSAGE_TREAT,
                doctor,
            )
        )
        statistics_list["本日健保脫臼整復人數"] = (
            statistics_utils.get_count_by_treat_type(
                self.database,
                "wait",
                "當日",
        ...
        if self.system_settings.field("統計針傷給藥人數") == "Y":
            statistics_list["本日健保針傷給藥人數"] = (
                statistics_utils.get_treat_drug_count(
                    self.database,
                    "當日",
                    doctor,
                )
        ...
        statistics_list["第一段診察費合理量"] = self.statistics_dicts[
            "第一段診察費合理量"
        ]
        statistics_list["本月健保診察費人數"] = self.statistics_dicts[
            "本月健保診察費人數"
        ]
        statistics_list["本月健保針傷限量"] = self.statistics_dicts["本月健保針傷限量"]
        statistics_list["本月健保針傷合計"] = (
            statistics_list["本月健保針灸人數"] + statistics_list["本月健保傷科人數"]
        )
        statistics_list["本月健保中度複針限量"] = self.statistics_dicts[
            "本月健保中度複針限量"
        ]
        statistics_list["本月健保中度複針人數"] = self.statistics_dicts[
            "本月健保中度複針人數"
        ]
        statistics_list["本月健保高度複針限量"] = self.statistics_dicts[
            "本月健保高度複針限量"
        ]
        statistics_list["本月健保高度複針人數"] = self.statistics_dicts[
            "本月健保高度複針人數"
        ]
        statistics_list["本月健保針傷合併限量"] = self.statistics_dicts[
            "本月健保針傷合併限量"
        ]
        statistics_list["本月健保針傷合併人數"] = self.statistics_dicts[
            "本月健保針傷合併人數"
        ]

        statistics_list["本月健保中度複傷人數"] = self.statistics_dicts[
            "本月健保中度複傷人數"
        ]
        statistics_list["本月健保高度複傷人數"] = self.statistics_dicts[
            "本月健保高度複傷人數"
        ]
        statistics_list["本月健保脫臼整復人數"] = self.statistics_dicts[
            "本月健保脫臼整復人數"
        ]

        # 每次更新異動
        statistics_list["本月健保針傷合併人數"] += statistics_list[
            "本日健保針傷合併人數"
        ]
        statistics_list["本月健保中度複針人數"] += statistics_list[
            "本日健保中度複針人數"
        ]
        statistics_list["本月健保高度複針人數"] += statistics_list[
            "本日健保高度複針人數"
        ]
        statistics_list["本月健保中度複傷人數"] += statistics_list[
            "本日健保中度複傷人數"
        ]
        statistics_list["本月健保高度複傷人數"] += statistics_list[
            "本日健保高度複傷人數"
        ]
        statistics_list["本月健保脫臼整復人數"] += statistics_list[
            "本日健保脫臼整復人數"
        ]

        if (
            self.system_settings.field("統計針傷給藥人數") == "Y"
        ):  # 2024-10-28 看診完畢回到候診名單讀取速度會變慢
            statistics_list["本月健保針傷給藥人數"] = self.statistics_dicts[
                "本月健保針傷給藥人數"
            ]
            statistics_list["本月健保針傷給藥人數"] += statistics_list[
                "本日健保針傷給藥人數"
            ]
    ...
    def _table_widget_waiting_list_key_press(self, event):
        ...
        return QtWidgets.QTableWidget.keyPressEvent(
            self.ui.tableWidget_waiting_list, event
        )
    ...
    def _get_voice_sentence(self, voice_dict=None):
        ...
        if self.system_settings.field("叫號包含下一位請準備") == "Y":
            ...
            next_regist_no = self.ui.tableWidget_waiting_list.item(
                current_row_no + 1, 3
            )
            if next_regist_no is not None:
                next_regist_no = next_regist_no.text()
                if self.system_settings.field("叫號包含病患姓名") == "Y":
                    next_name = self.ui.tableWidget_waiting_list.item(
                        current_row_no + 1, 5
                    ).text()
                else:
                    next_name = ""
    ...
    def _write_ic_treatment(self):
        card = string_utils.xstr(
            self.table_widget_wait_completed.field_value(WAIT_DONE_LIST_COL_NO["Card"])
        )
        if card in ["", "IC", "自動取得"]:
    ...
    def _wait_completed_table_item_changed(self):
        ...
        case_key = self.ui.tableWidget_wait_completed.item(
            row_no, WAIT_DONE_LIST_COL_NO["CaseKey"]
        )
        ic_wrote = self.ui.tableWidget_wait_completed.item(
            row_no, WAIT_DONE_LIST_COL_NO["WriteCard"]
        )
    ...
    def _set_late(self):
        ...
        remark = self.table_widget_waiting_list.field_value(
            WAITING_LIST_COL_NO["Remark"]
        )
    ...
    def _start_flashing(self, widget):
        ...
        if not hasattr(self, "flash_timer"):
            ...
            self.flash_duration_timer.timeout.connect(
                lambda: self._stop_flashing(widget)
            )
    ...
    def _extra_purchase(self):
        ...
        patient_name = self.table_widget_wait_completed.field_value(
            WAIT_DONE_LIST_COL_NO["Name"]
        )
