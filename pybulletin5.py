...
class PyBulletin5(QtWidgets.QWidget):
    ...
    def _show_title(self):
        self.ui.label_title.setText(
            f"{self.system_settings.field('院所名稱')} 候診資訊系統"
        )
    ...
    def _set_marquee_list(self):
        ...
        if not rows:
            self.marquee_list.append(
                f"{self.system_settings.field('院所名稱')} 關心您的健康"
            )
            return
    ...
    def _get_waiting_rows(self, room, current_page=None):
        ...
        limit_script = (
            f"LIMIT {current_page * MAX_WAITING_ROWS}, {MAX_WAITING_ROWS}"
            if current_page
            else ""
        )
        sql = f'''
