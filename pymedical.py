...
from libs import (class_utils, date_utils, db_utils, dialog_utils, log_utils,
                  ...
                  string_utils, system_utils, ui_utils, web_utils)
...
def collect_traceback(exctype, value, tb):
    ...
    # 3. 組合通知內容
    mail_content = f'**異常類型:** {exctype.__name__}\n**異常值:** {value}\n**追蹤:**\n{stack}'
...
class PyMedical(QtWidgets.QMainWindow):
    ...
    def _check_single_instance(self):
        ...
        for i in top_windows:
            if self.clinic_name is not None and self.clinic_name + ' 醫療資訊管理系統' in i[1]:
                hwnd = i[0]
    ...
    def get_treatment_list_from_db(self):
        ...
        self.moderate_complicated_acupuncture_list = nhi_utils.get_moderate_complicated_acupuncture_list(self.database)
        progress_dialog.setValue(1)
        self.highly_complicated_acupuncture_list = nhi_utils.get_highly_complicated_acupuncture_list(self.database)
        progress_dialog.setValue(2)
        self.moderate_complicated_massage_list = nhi_utils.get_moderate_complicated_massage_list(self.database)
        progress_dialog.setValue(3)
        self.highly_complicated_massage_list = nhi_utils.get_highly_complicated_massage_list(self.database)
        progress_dialog.setValue(4)
        self.special_disease_list = nhi_utils.get_moderate_complicated_massage_with_special_disease_list(self.database)
    ...
    def closeEvent(self, event: QtGui.QCloseEvent):
        ...
        if quit_app:
            if self.user_name != '超級使用者':
                ...
                dialog.ui.dateEdit_start_date.setDate(
                    datetime.datetime.now().date())
                dialog.ui.dateEdit_end_date.setDate(
                    datetime.datetime.now().date())
                ...
                if len(rows) > 0:
                    ...
                    msg_box.addButton(QPushButton("不要關, 我要上傳"),
                                      QMessageBox.NoRole)  # 0
                    msg_box.addButton(QPushButton("我知道了"),
                                      QMessageBox.AcceptRole)  # 1
                    quit_app = msg_box.exec_()
                    if not quit_app:
                        event.ignore()
                        return

                backup_process = module_utils.get_backup(self, self.database, self.system_settings)
    ...
    def _turn_off_led(self):
        ...
        if (led_ip not in [None, '']):
            led_port = number_utils.get_integer(self.system_settings.field('叫號燈port'))
            try:
                system_utils.send_to_tcpip(led_ip, led_port, b'\xed\xed\x0f\x0f\x0f\x0f\x7f\x00\x00')
            except Exception:
                pass
    ...
    def check_system_db(self):
        """檢查資料庫是否需要補充."""
        mysql_path = './mysql'
        mysql_files = [f for f in os.listdir(
            mysql_path) if os.path.isfile(os.path.join(mysql_path, f))]
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_PY_MEDICAL, self, native_menu_bar=True)
    ...
    def _set_images(self):
        ...
        self.ui.label_line_qrcode.setPixmap(
            QtGui.QPixmap('./images/line_qrcode.jpg'))
    ...
    def set_plugin(self):
        ...
        massage_host_dict = db_utils.get_host_database_dict(
            self.database, '養生館')
    ...
    def _set_signal(self):
        ...
        self.ui.pushButton_registration.clicked.connect(
            self._open_subroutine)           # 掛號作業
        self.ui.pushButton_reservation.clicked.connect(
            lambda: self.open_reservation(None, None, None))  # 預約掛號
        self.ui.pushButton_cashier.clicked.connect(
            self._open_subroutine)                # 批價作業
        ...
        self.ui.toolButton_nhi_vpn_new.clicked.connect(self._open_hyper_link)           # 健保VPN (新版)
        self.ui.toolButton_mohw.clicked.connect(self._open_hyper_link)                  # 醫事入口網
        self.ui.toolButton_hca.clicked.connect(self._open_hyper_link)                   # 醫事憑證管理中心
        ...
        self.ui.action_import_medical_record.triggered.connect(self.open_import_medical_record)  # 匯入JSON
        ...
        self.ui.action_reservation.triggered.connect(lambda: self.open_reservation(None, None, None))   # 預約掛號
        ...
        self.ui.action_export_emr_xml.triggered.connect(self.export_emr_xml)                # 匯出電子病歷
        self.ui.action_export_medical_record_json.triggered.connect(self.export_medical_record_json)   # 匯出病歷
        self.ui.action_export_all_database.triggered.connect(self.export_all_database)      # 匯出整個資料庫
        self.ui.action_import_all_database.triggered.connect(self.import_all_database)      # 匯出整個資料庫
        ...
        self.ui.action_statistics_ins_pregnant.triggered.connect(self._open_subroutine)     # 孕產照護報表
        self.ui.action_statistics_nursing_home.triggered.connect(self._open_subroutine)     # 照護機構院民資料報表
        ...
        self.ui.action_exam_result.triggered.connect(self.open_exam_result)              # 檢驗報告查詢
        ...
        self.ui.action_bulletin_settings.triggered.connect(self.open_bulletin_settings)   # 候診資訊系統設定
        self.ui.action_cashier_machine_settings.triggered.connect(self.open_cashier_machine_settings)   # 掛號機零錢箱設定
        ...
        self.ui.action_statistics_ins_apply_year.triggered.connect(self._open_subroutine)
        ...
        self.ui.action_statistics_doctor_monthly.triggered.connect(self._open_subroutine)
        self.ui.action_statistics_doctor_amount.triggered.connect(self._open_subroutine)
        self.ui.action_statistics_doctor_medicine.triggered.connect(self._open_subroutine)
        self.ui.action_statistics_return_rate.triggered.connect(self._open_subroutine)
        self.ui.action_statistics_no_return_rate.triggered.connect(self._open_subroutine)
        ...
        self.ui.action_statistics_period_count.triggered.connect(self._open_subroutine)  # 診數統計

        self.ui.action_statistics_branch_project.triggered.connect(self._open_subroutine)
        self.ui.action_statistics_branch_daily.triggered.connect(self._open_subroutine)

        self.ui.action_statistics_commission.triggered.connect(self._open_subroutine)  # 自費抽成統計
        ...
        self.ui.action_case_template.triggered.connect(self._open_case_template)            # 參考病歷
        self.ui.action_commission_summary.triggered.connect(self._open_subroutine)          # 業績抽成統計
        self.ui.action_sales_summary.triggered.connect(self._open_subroutine)               # 業績金額統計

        self.ui.action_course_accomplish.triggered.connect(self._open_subroutine)           # 自費療程實現
        self.ui.action_export_table_to_json.triggered.connect(self._export_table_to_json)   # 匯出指定資料至json

        self.ui.action_statistics_correction_reg.triggered.connect(self._open_subroutine)   # 矯正機關內門診報表
        self.ui.action_import_home_care.triggered.connect(self._open_import_home_care)      # 居家藍芽資料匯入
        self.ui.action_supplier.triggered.connect(self._open_subroutine)                    # 廠商資料 2022.09.08 安聲
        ...
        self.ui.action_stock_replenishment.triggered.connect(self._open_subroutine)         # 補貨
        ...
        self.ui.action_physiotherapy.triggered.connect(self._open_subroutine)               # 物理治療預約
        self.ui.action_stamp_duty.triggered.connect(self._open_subroutine)                  # 自費印花稅統計

        self.ui.action_create_cshis6_shortcut.triggered.connect(self._create_shortcut)   # 產生讀卡機控制軟體6.0捷徑

    # 設定 css style
    def _set_style(self):
        self.ui.label_system_name.setText(
            f"<b>{self.clinic_name} 醫療資訊管理系統</b>"
        )
    ...
    def _open_subroutine(self, action=None):
        ...
        if tab_name == '醫師看診作業':
            ...
            self._add_tab(
                tab_name, self.database, self.system_settings,
    ...
    def open_examination(self, examination_key):
        ...
        self._add_tab(tab_name, self.database,
                      self.system_settings, examination_key, None, None)
    ...
    def open_return_card(self, patient_key):
        tab_name = '健保卡欠還卡'
        self._add_tab(tab_name, self.database,
                      self.system_settings, patient_key)
    ...
    # 開啟病歷資料
    def open_medical_record(self, case_key, call_from=None, current_user=None, archive_database=None):
        ...
        if call_from == '醫師看診作業' and self.user_name not in position_list:
            if personnel_utils.get_permission(
                    database, call_from, '非醫師病歷登錄', self.user_name) != 'Y':
                system_utils.show_message_box(
                    QMessageBox.Critical,
                    '使用者非醫師',
            ...
            for i in range(self.ui.tabWidget_window.count()):
                if self.ui.tabWidget_window.tabText(i) == tab_name:
                    ...
                    if doctor is not None and patient_key is not None:
                        ...
                        current_tab.set_doctor_and_patient(None, None, vhc_ic_card=vhc_ic_card)
    ...
    # 預約掛號
    def registration_arrival(self, reserve_key, late=False, late_remark='預約報到過號', vhc_ic_card=None):
        tab_name = '門診掛號'
        self._add_tab(tab_name, self.database, self.system_settings)
        ...
        current_tab.reservation_arrival(reserve_key, late, late_remark=late_remark, vhc_ic_card=vhc_ic_card)
    ...
    def open_settings(self):
        dialog = dialog_utils.get_dialog_system_settings(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()
    ...
    def open_menu_setting(self):
        dialog = dialog_utils.get_dialog_menu_setting(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    # 教學影片
    def open_tutorial_videos(self):
        dialog = dialog_utils.get_dialog_tutorial_videos(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    # 候診系統設定
    def open_bulletin_settings(self):
        dialog = dialog_utils.get_dialog_bulletin_settings(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    # 掛號機零錢箱設定
    def open_cashier_machine_settings(self):
        dialog = dialog_utils.get_dialog_cashier_machine_settings(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    # 分院資料設定
    def open_hosts_settings(self):
        dialog = dialog_utils.get_dialog_hosts(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    # 匯入病歷
    def open_import_medical_record(self):
        dialog = dialog_utils.get_dialog_import_medical_record(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    # 匯入居家藍芽病歷
    def _open_import_home_care(self):
        dialog = dialog_utils.get_dialog_import_home_care(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    # 健保卡讀卡機
    def open_ic_card(self):
        dialog = dialog_utils.get_dialog_ic_card(self, self.database, self.system_settings)
    ...
    def export_emr_xml(self):
        dialog = dialog_utils.get_dialog_export_emr_xml(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    def export_medical_record_json(self):
        dialog = dialog_utils.get_dialog_export_medical_record_json(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()
   ...
   # 就醫診療資料寫入作業
    def export_database(self, export_filename):
        title = '匯出全部資料庫'
        message = '<font size="5" color="red"><b>正在匯出全部資料庫中, 請稍後...</b></font>'
        hint = '匯出的時間會依照資料庫的大小, 花費一些時間.'
        msg_box = dialog_utils.message_box(title, message, hint)
        msg_box.show()
        msg_queue = Queue()
        QtCore.QCoreApplication.processEvents()
        t = Thread(target=self._export_database_thread, args=(msg_queue, export_filename))
        t.start()
    ...
    def import_all_database(self):
        ...
        msg_box.setText("<font size='4' color='red'><b>確定匯入全部資料庫, 並取代原來的資料庫?</b></font>")
        msg_box.setInformativeText("注意！資料匯入後, 原來的病歷資料將會全部消失, 並以新的匯入資料庫取代!")
    ...
    def import_database(self, import_filename):
        title = '匯入全部資料庫'
        message = '<font size="5" color="red"><b>正在匯入全部資料庫中, 請稍後...</b></font>'
        hint = '匯入的時間會依照資料庫的大小, 花費一些時間.'
        msg_box = dialog_utils.message_box(title, message, hint)
        msg_box.show()
        msg_queue = Queue()
        QtCore.QCoreApplication.processEvents()
        t = Thread(target=self._import_database_thread, args=(msg_queue, import_filename))
        t.start()
        (error_code) = msg_queue.get()
        msg_box.close()
    ...
    def set_permission(self):
        ...
        for item in person_list:
            ...
            if action_name in [
                    '關閉掛號作業', '關閉診療作業', '關閉醫務行政', '關閉自費管理', '關閉申報作業',
                    '關閉統計表', '關閉分院統計', '關閉進銷存管理', '關閉設定', '關閉系統作業']:
                if personnel_utils.get_permission(
                        self.database, string_utils.xstr(item[0]), action_name, self.user_name) == 'Y':
    ...
    ############################################## 例外狀況 ############################################################
            if personnel_utils.get_permission(
                    self.database, string_utils.xstr(item[0]), '輸入成方資料', self.user_name) == 'Y':
        ...
        if call_from in ['門診掛號']:
            if current_tab_text in ['門診掛號']:
                ...
                if doctor is not None and room is not None:
                    if (refresh_wait_option == '醫師診別' and self.user_name == doctor) or \
                       (refresh_wait_option == '指定診別' and room_no == room) or \
                       (refresh_wait_option == '所有診別'):
                        tab.read_wait()
    ...
    def refresh_status_bar(self):
        ...
        self.label_user_name.setText(
            f"使用者: {self.system_settings.field('使用者')}")
        self.label_station_no.setText(
            f"工作站編號: {self.system_settings.field('工作站編號')}")
    ...
    # 登出
    def logout(self):
        login_dialog = module_utils.get_login(self, self.database, self.system_settings, 'pymedical')
        login_dialog.exec_()
    ...
    def _set_login_statistics(self, user_name):
        statistics = module_utils.get_login_statistics(self, self.database, self.system_settings, user_name)
    ...
    def _update_files(self):
        ...
        dialog = module_utils.get_system_update(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()

    # 資料庫修復
    def _database_repair(self):
        dialog = dialog_utils.get_dialog_database_repair(self, self.database, self.system_settings)
        dialog.exec_()
        dialog.deleteLater()
    ...
    def restart_pymedical(self):
        if sys.platform == 'win32':
            os.execv(
                sys.executable,
                [sys.executable,
                 os.path.join(sys.path[0], __file__)] + sys.argv[1:])
        else:
    ...
    def _export_table_to_json(self):
        ...
        json_file_name, _ = QFileDialog.getSaveFileName(
            self, "匯出資料表JSON檔案", f'{table_name}.json',
            "json檔案 (*.json)",
            options=options
        )
...
def handle_login(py_medical):
    login_dialog = module_utils.get_login(py_medical, py_medical.database, py_medical.system_settings)
    login_dialog.exec_()
    if not login_dialog.login_ok:
        login_dialog.deleteLater()
...
def setup_user_environment(py_medical, user_name, position):
    ...
    if position in ['醫師', '支援醫師']:
        statistics = module_utils.get_login_statistics(py_medical, py_medical.database, py_medical.system_settings, user_name)
        statistics.start_statistics()
...
def configure_main_window(py_medical, config):
    ...
    if py_medical.system_settings.field('自動開啟雲端安全模組主控台') == 'Y' or \
            py_medical.system_settings.field('讀卡機控制軟體版本') == 'cshis6':
