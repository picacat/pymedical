...
class CertificatePayment(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        total_fee_field_name = self.system_settings.field(
            "醫療費用證明自費金額欄位名稱"
        )
        ...
        column_header = [
            ...
            "自付金額",
            "健保申報",
            medicine_fee_field_name,
            treat_fee_field_name,
            misc_fee_field_name,
            "折扣",
            total_fee_field_name,
            "自付合計",
            "診治醫師",
        ]
    ...
    def _set_signal(self):
        ...
        self.ui.action_print_certificate_cash.triggered.connect(
            self._print_certificate_cash
        )

        self.ui.action_print_certificate_cash2.triggered.connect(
            self._print_certificate_cash2
        )
        ...
        self.ui.action_print_certificate_total.triggered.connect(
            self._print_certificate_total
        )
        self.ui.action_print_certificate_cash_total.triggered.connect(
            self._print_certificate_cash_total
        )
        self.ui.action_print_certificate_receipt.triggered.connect(
            self._print_certificate_receipt
        )
        ...
        self.ui.action_print_certificate_ins_fee.triggered.connect(
            self._print_certificate_ins_fee
        )

        self.ui.action_print_certificate_pdf.triggered.connect(
            self._print_certificate_pdf
        )
        self.ui.action_print_certificate_cash_pdf.triggered.connect(
            self._print_certificate_cash_pdf
        )
        self.ui.action_print_certificate_total_pdf.triggered.connect(
            self._print_certificate_total_pdf
        )
        self.ui.action_print_certificate_prescript_pdf.triggered.connect(
            self._print_certificate_prescript_pdf
        )

        self.ui.tableWidget_certificate_list.itemSelectionChanged.connect(
            self._table_item_changed
        )
        self.ui.tableWidget_certificate_items.itemChanged.connect(
            self._cert_item_changed
        )
        self.ui.tableWidget_certificate_items.doubleClicked.connect(
    ...
    def _table_item_changed(self):
        ...
        sql = f"""
            ...
            ORDER BY CaseDate
        """
        self.table_widget_certificate_items.set_db_data(
            sql, self._set_certificate_items_data, set_focus=False
        )
    ...
    def _calculate_items_subtotal(self, row_no):
        regist_fee = number_utils.get_integer(
            self.ui.tableWidget_certificate_items.item(row_no, 4).text()
        )
        diag_share_fee = number_utils.get_integer(
            self.ui.tableWidget_certificate_items.item(row_no, 5).text()
        )
        drug_share_fee = number_utils.get_integer(
            self.ui.tableWidget_certificate_items.item(row_no, 6).text()
        )
        cash_fee = regist_fee + diag_share_fee + drug_share_fee

        ins_apply_fee = number_utils.get_integer(
            self.ui.tableWidget_certificate_items.item(row_no, 8).text()
        )
        self_drug_fee = number_utils.get_integer(
            self.ui.tableWidget_certificate_items.item(row_no, 9).text()
        )
        treat_fee = number_utils.get_integer(
            self.ui.tableWidget_certificate_items.item(row_no, 10).text()
        )
        misc_fee = number_utils.get_integer(
            self.ui.tableWidget_certificate_items.item(row_no, 11).text()
        )
        discount_fee = number_utils.get_integer(
            self.ui.tableWidget_certificate_items.item(row_no, 12).text()
        )
        ...
        self.ui.tableWidget_certificate_items.item(row_no, 7).setTextAlignment(
            QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
        )
        self.ui.tableWidget_certificate_items.item(row_no, 13).setTextAlignment(
            QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
        )
        self.ui.tableWidget_certificate_items.item(row_no, 14).setTextAlignment(
            QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
        )
    ...
    def _calculate_items_total(self):
        ...
        for row_no in range(row_count):
            ...
            cash_fee += number_utils.get_integer(
                self.ui.tableWidget_certificate_items.item(row_no, 7).text()
            )
            ins_apply_fee += number_utils.get_integer(
                self.ui.tableWidget_certificate_items.item(row_no, 8).text()
            )
            ...
            total_fee += number_utils.get_integer(
                self.ui.tableWidget_certificate_items.item(row_no, 13).text()
            )
            cash_total += number_utils.get_integer(
                self.ui.tableWidget_certificate_items.item(row_no, 14).text()
            )
        ...
        for col_no in range(len(total_fee_row)):
            self.ui.tableWidget_certificate_items.setItem(
                row_count, col_no, QtWidgets.QTableWidgetItem(total_fee_row[col_no])
            )
            if col_no >= 4:
                self.ui.tableWidget_certificate_items.item(
                    row_count, col_no
    ...
    def _set_medical_record_certificate_fee(self, certificate_fee):
        ...
        self._insert_prescript(
            case_key, row["CaseDate"], max_medicine_set + 1, certificate_fee
        )

    def _set_certificate_items_fee(self, certificate_fee):
        certificate_items_key = self.ui.tableWidget_certificate_items.item(
            0, 0
        )  # 抓第一筆來開立
    ...
    def _remove_item(self):
        ...
        self.database.delete_record(
            "certificate_items", "CertificateItemsKey", certificate_item_key
        )
        self._table_item_changed()
    ...
    def _calculate_fees(self):
        ...
        for row_no in range(self.ui.tableWidget_certificate_items.rowCount()):
            ...
            if self.system_settings.field("手動批價") == "Y":
                ...
                self_total_fee = charge_utils.get_self_total_fee(
    ...
    def _export_case_excel(self):
        name = self.ui.tableWidget_certificate_list.item(
            self.ui.tableWidget_certificate_list.currentRow(), 4
        ).text()
        start_date = self.ui.tableWidget_certificate_list.item(
            self.ui.tableWidget_certificate_list.currentRow(), 7
        ).text()
        end_date = self.ui.tableWidget_certificate_list.item(
            self.ui.tableWidget_certificate_list.currentRow(), 8
        ).text()
    ...
    def _export_to_excel(
        self, excel_filename, year=None, month=None, list_type="by_month"
    ):
    ...
    def _add_medical_record(self):
        case_date = date_utils.get_dialog_date(
    ...
    def _change_doctor(self):
        ...
        doctor, ok = input_dialog.getItem(
            self, "變更主治醫師", "請選擇主治醫師", doctor_list, 0, False
        )
