...
from libs import (case_utils, charge_utils, class_utils, date_utils, log_utils,
                  nhi_utils, number_utils, string_utils, system_utils,
                  ui_utils)
...
class CheckMedicalRecordCount(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, *args):
        ...
        self.long_term_care = '(' + ','.join(f"'{x}'" for x in nhi_utils.LONG_TERM_CARE) + ')'        
    ...
    def _set_ui(self):
        ...
        self.ui.label_merge_limit.setText(
            f'針傷合併限量: {self.merge_limit}次'
        )
    ...
    def _set_signal(self):
        ...
        self.ui.tableWidget_moderate_case.doubleClicked.connect(self.open_medical_record)
        self.ui.tableWidget_highly_case.doubleClicked.connect(self.open_medical_record)
        self.ui.tableWidget_moderate_massage_case.doubleClicked.connect(self.open_medical_record)
        self.ui.tableWidget_highly_massage_case.doubleClicked.connect(self.open_medical_record)
        self.ui.tableWidget_merge_case.doubleClicked.connect(self.open_medical_record)
        self.ui.tableWidget_treat_drug_case.doubleClicked.connect(self.open_medical_record)
        ...
        self.ui.tableWidget_moderate_doctor.itemSelectionChanged.connect(self._moderate_doctor_changed)
        self.ui.tableWidget_highly_doctor.itemSelectionChanged.connect(self._highly_doctor_changed)
        self.ui.tableWidget_moderate_massage_doctor.itemSelectionChanged.connect(self._moderate_massage_doctor_changed)
        self.ui.tableWidget_highly_massage_doctor.itemSelectionChanged.connect(self._highly_massage_doctor_changed)
        self.ui.tableWidget_merge_doctor.itemSelectionChanged.connect(self._merge_doctor_changed)
        self.ui.tableWidget_treat_drug_doctor.itemSelectionChanged.connect(self._treat_drug_doctor_changed)

        self.ui.tableWidget_patient_treat.keyPressEvent = self._patient_treat_key_press
        self.ui.tableWidget_medical_record_treat.keyPressEvent = self._medical_record_treat_key_press
        ...
        self.ui.toolButton_acupuncture1.clicked.connect(lambda: self._set_acupuncture('中度複雜性針灸'))
        self.ui.toolButton_acupuncture2.clicked.connect(lambda: self._set_acupuncture('高度複雜性針灸'))
        self.ui.toolButton_massage1.clicked.connect(lambda: self._set_massage('中度複雜性傷科'))  # 2025-04-27
        self.ui.toolButton_massage2.clicked.connect(lambda: self._set_massage('高度複雜性傷科'))  # 2025-04-27
        self.ui.toolButton_cancel_merge.clicked.connect(self._cancel_merge)

        self.ui.toolButton_complicated1.clicked.connect(
            lambda: self._set_complicated_acupuncture('中度複雜性針灸'))  # 恢復中度複雜性針灸
        self.ui.toolButton_complicated2.clicked.connect(
            lambda: self._set_complicated_acupuncture('高度複雜性針灸'))  # 恢復高度複雜性針灸
        self.ui.toolButton_complicated3.clicked.connect(
            lambda: self._set_complicated_massage('中度複雜性傷科'))  # 恢復中度複雜性傷科 2025-04-27
        self.ui.toolButton_complicated4.clicked.connect(
            lambda: self._set_complicated_massage('高度複雜性傷科'))  # 恢復高度複雜性傷科 2025-04-27
    ...
    def _medical_record_treat_key_press(self, event):
        ...
        return QtWidgets.QTableWidget.keyPressEvent(self.ui.tableWidget_medical_record_treat, event)
    ...
    def _patient_treat_key_press(self, event):
        ...
        return QtWidgets.QTableWidget.keyPressEvent(self.ui.tableWidget_patient_treat, event)
    ...
    def _calculate_avg_moderate_acupuncture(self):
        doctor_count, person_count, avg_count = self._get_avg_count(self.ui.tableWidget_moderate_doctor)
    ...
    def _calculate_avg_highly_acupuncture(self):
        doctor_count, person_count, avg_count = self._get_avg_count(self.ui.tableWidget_highly_doctor)
    ...
    def _calculate_avg_moderate_massage(self):
        doctor_count, person_count, avg_count = self._get_avg_count(self.ui.tableWidget_moderate_massage_doctor)
    ...
    def _calculate_avg_highly_massage(self):
        doctor_count, person_count, avg_count = self._get_avg_count(self.ui.tableWidget_highly_massage_doctor)
    ...
    def _calculate_avg_merge(self):
        doctor_count, person_count, avg_count = self._get_avg_count(self.ui.tableWidget_merge_doctor)
    ...
    def _calculate_avg_treat_drug(self):
        doctor_count, person_count, avg_count = self._get_avg_count(self.ui.tableWidget_treat_drug_doctor)
    ...
    def _set_patient_treat_data(self, row_no, row):
        ...
        for col_no in range(len(patient_row)):
            ...
            if exceed_count > 0:
                self.ui.tableWidget_patient_treat.item(row_no, col_no).setForeground(QtGui.QColor('red'))
    ...
    def _set_patient_diag_data(self, row_no, row):
        ...
        for col_no in range(len(patient_row)):
            ...
            if exceed_count > 0:
                self.ui.tableWidget_patient_diag.item(row_no, col_no).setForeground(QtGui.QColor('red'))
    ...
    def _set_medical_record_treat_color(self):
        ...
        for row_no in range(self.ui.tableWidget_medical_record_treat.rowCount()):
            ...
            if (case_date < datetime.datetime.strptime(self.start_date, '%Y-%m-%d %H:%M:%S')):
                self.table_widget_medical_record_treat.set_row_color(row_no, QtGui.QColor('darkGray'))
            else:
                record_count += 1

            self.table_widget_medical_record_treat.set_row_color(row_no, QtGui.QColor('black'))
            ...
            if apply_type == '不申報':
                ...
                self.table_widget_medical_record_treat.set_row_color(row_no, QtGui.QColor('lightSlateGrey'))
    ...
    def _read_moderate_acupuncture(self):
        ...
        self.table_widget_moderate_doctor.set_db_data(sql, self._set_moderate_doctor_data)
    ...
    def _set_moderate_doctor_data(self, row_no, row):
        ...
        for col_no in range(len(doctor_row)):
            ...
            self.ui.tableWidget_moderate_doctor.setItem(
                row_no, col_no, item
            )
            if col_no in [2]:
    ...
    def _set_highly_doctor_data(self, row_no, row):
        ...
        for col_no in range(len(doctor_row)):
            item = QtWidgets.QTableWidgetItem()
            item.setData(QtCore.Qt.EditRole, doctor_row[col_no])
            self.ui.tableWidget_highly_doctor.setItem(
                row_no, col_no, item
            )
            if col_no in [2]:
                self.ui.tableWidget_highly_doctor.item(
                    row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
    ...
    def _set_merge_doctor_data(self, row_no, row):
        ...
        for col_no in range(len(doctor_row)):
            item = QtWidgets.QTableWidgetItem()
            item.setData(QtCore.Qt.EditRole, doctor_row[col_no])
            self.ui.tableWidget_merge_doctor.setItem(
                row_no, col_no, item
            )
            if col_no in [2]:
                self.ui.tableWidget_merge_doctor.item(
                    row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
    ...
    def _read_treat_drug(self):
        sql = self._get_doctor_count_sql()
        self.table_widget_treat_drug_doctor.set_db_data(sql, self._set_treat_drug_doctor_data)
    ...
    def _set_treat_drug_doctor_data(self, row_no, row):
        ...
        for col_no in range(len(doctor_row)):
            item = QtWidgets.QTableWidgetItem()
            item.setData(QtCore.Qt.EditRole, doctor_row[col_no])
            self.ui.tableWidget_treat_drug_doctor.setItem(
                row_no, col_no, item
            )
            if col_no in [2]:
    ...
    def _set_moderate_case_data(self, row_no, row):
        ...
        for column in range(len(moderate_case_data)):
            ...
            if column in [3, 8, 13, 14]:
                self.ui.tableWidget_moderate_case.item(
                    row_no, column).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
            elif column in [7]:
    ...
    def _set_highly_case_data(self, row_no, row):
        ...
        for column in range(len(highly_case_data)):
            ...
            if column in [3, 8, 13, 14]:
                self.ui.tableWidget_highly_case.item(
                    row_no, column).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
            elif column in [7]:
    ...
    def _set_merge_case_data(self, row_no, row):
        ...
        for col_no in range(len(merge_case_data)):
            ...
            if col_no in [3, 8]:
                self.ui.tableWidget_merge_case.item(
                    row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
            elif col_no in [7]:
    ...
    def _set_treat_count_icon(self):
        ...
        for row_no in range(self.ui.tableWidget_patient_treat.rowCount()):
            exceed_count = number_utils.get_integer(self.ui.tableWidget_patient_treat.item(row_no, 3).text())
            if exceed_count > 0:
                self._error_count += 1
                icon = ui_utils.ICON_NO
                break
    ...
    def _set_diag_count_icon(self):
        ...
        for row_no in range(self.ui.tableWidget_patient_diag.rowCount()):
            exceed_count = number_utils.get_integer(self.ui.tableWidget_patient_diag.item(row_no, 3).text())
            if exceed_count > 0:
                self._error_count += 1
                icon = ui_utils.ICON_NO
                break
    ...
    def _set_moderate_massage_count_icon(self):
        ...
        _, _, avg_count = self._get_avg_count(self.ui.tableWidget_moderate_massage_doctor)
    ...
    def _set_acupuncture(self, origin_treat_type):
        ...
        log_utils.write_event_log(
            self.database, self.user_name, '病歷修正', '申報檢查-門診次數檢查',
        ...
        table_widget_case.set_item_text(row_no, 11, treatment, align=QtCore.Qt.AlignLeft)
    ...
    def _cancel_merge(self):
        ...
        table_widget_merge_case.set_item_text(row_no, 5, treatment, align=QtCore.Qt.AlignLeft)
        table_widget_merge_case.set_item_text(row_no, 11, treatment, align=QtCore.Qt.AlignLeft)
    ...
    def _set_complicated_acupuncture(self, treat_type):
        ...
        row_no = table_widget_case.current_row()
        table_widget_case.set_item_text(row_no, 5, treat_type, align=QtCore.Qt.AlignLeft)
        table_widget_case.set_item_text(row_no, 11, treat_type, align=QtCore.Qt.AlignLeft)
    ...
    def _read_moderate_massage(self):
        sql = self._get_doctor_count_sql()
        self.table_widget_moderate_massage_doctor.set_db_data(sql, self._set_moderate_massage_doctor_data)
    ...
    def _set_moderate_massage_doctor_data(self, row_no, row):
        ...
        for col_no in range(len(doctor_row)):
            item = QtWidgets.QTableWidgetItem()
            item.setData(QtCore.Qt.EditRole, doctor_row[col_no])
            self.ui.tableWidget_moderate_massage_doctor.setItem(
                row_no, col_no, item
            )
            if col_no in [2]:
    ...
    def _read_highly_massage(self):
        sql = self._get_doctor_count_sql()
        self.table_widget_highly_massage_doctor.set_db_data(sql, self._set_highly_massage_doctor_data)
    ...
    def _set_highly_massage_doctor_data(self, row_no, row):
        ...
        for col_no in range(len(doctor_row)):
            item = QtWidgets.QTableWidgetItem()
            item.setData(QtCore.Qt.EditRole, doctor_row[col_no])
            self.ui.tableWidget_highly_massage_doctor.setItem(
                row_no, col_no, item
            )
            if col_no in [2]:
    ...
    def _set_moderate_massage_case_data(self, row_no, row):
        ...
        for column in range(len(moderate_massage_case_data)):
            self.ui.tableWidget_moderate_massage_case.setItem(
                row_no, column, QtWidgets.QTableWidgetItem(moderate_massage_case_data[column]))
            if column in [3, 8, 13, 14]:
    ...
    def _set_highly_massage_case_data(self, row_no, row):
        ...
        for column in range(len(highly_massage_case_data)):
            self.ui.tableWidget_highly_massage_case.setItem(
                row_no, column, QtWidgets.QTableWidgetItem(highly_massage_case_data[column]))
            if column in [3, 8, 13, 14]:
