...
class PyCashier(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, *args):
        ...
        self.cpay = None
        self.cpay = class_utils.get_cpay(self.system_settings)  # todo
        if not self.cpay.connected:
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_PY_CASHIER2, self)
    ...
    def _set_pycashier_home(self):
        self.widget_home = module_utils.get_pycashier2_home(self, self.database, self.system_settings, self.ic_card)
        self.ui.stackedWidget.addWidget(self.widget_home)

    def _set_pycashier_registration(self):
        self.widget_registration = module_utils.get_pycashier2_registration(
    ...
    def _set_pycashier_payment(self):
        self.widget_payment = module_utils.get_pycashier2_payment(
            self, self.database, self.system_settings, self.ic_card, self.cpay
        )
        self.ui.stackedWidget.addWidget(self.widget_payment)

    def _set_pycashier_completed(self):
        self.widget_completed = module_utils.get_pycashier2_completed(
            self, self.database, self.system_settings, self.ic_card,
        )
