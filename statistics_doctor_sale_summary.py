...
class StatisticsDoctorSaleSummary(QtWidgets.QMainWindow):
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_STATISTICS_DOCTOR_SALE_SUMMARY, self)
    ...
    def _calculate_sales_volume_subtotal(self):
        ...
        for row_no in range(self.ui.tableWidget_prescript.rowCount()):
            ...
            self._set_table_widget_cell_value(row_no, subtotal_col_no, subtotal, QtCore.Qt.AlignRight)
        ...
        for medicine_type_row_no, medicine_type in enumerate(self.medicine_type_list):
            for col_no in range(2, self.ui.tableWidget_prescript.columnCount()):
                ...
                for row_no in range(self.ui.tableWidget_prescript.rowCount()):
                    ...
                    current_medicine_type_item = self.ui.tableWidget_prescript.item(row_no, 1)
                ...
                self._set_table_widget_cell_value(total_row_no, col_no, total, QtCore.Qt.AlignRight)
    ...
    def _set_table_doctor(self, rows):
        for row in rows:
            ...
            self.ui.tableWidget_prescript.setSpan(start_row_no, 0, len(self.medicine_type_list), 1)
    ...
    def _calculate_sales_volume_by_doctor(self, start_row_no, doctor):
        ...
        for row_no in range(start_row_no, start_row_no + len(self.medicine_type_list)):
            ...
            for offset, date in enumerate(self.date_range):
                ...
                self._set_table_widget_cell_value(row_no, col_no, total_amount, QtCore.Qt.AlignRight)
    ...
    def _get_total_amount(self, in_medicine_type, in_case_date, rows):
        ...
        for row in rows:
            ...
            days = case_utils.get_pres_days(self.database, row['CaseKey'], medicine_set=row['MedicineSet'])
            if days <= 0:
                days = 1
    ...
    def _set_table_widget_cell_value(self, row_no, col_no, value, align):
        ...
        self.ui.tableWidget_prescript.item(row_no, col_no).setTextAlignment(align | QtCore.Qt.AlignVCenter)
    ...
    def print_sale_summary(self):
        printer_utils.print_doctor_sale_summary(
            self, self.database, self.system_settings, self.start_date, self.end_date,
