...
class KioskRegistration(QtWidgets.QMainWindow):
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_KIOSK_REGISTRATION, self)
        style = """
    ...
    def set_registration_data(self):
        ...
        self.basic_data = self.card_observer.get_nhi_basic_data()
        if self.basic_data is None:
            return

        patient_id = self.basic_data["patient_id"]
        patient_name = self.basic_data["name"]
    ...
    def _charge_fee(self, case_row):
        ...
        self.parent.open_kiosk_payment(self.patient_key, "批價繳費", self.case_key)
    ...
    def _outpatient_registration(self, patient_row):
        ...
        doctor_list = self._set_doctor_buttons()
        if len(doctor_list) == 1:
            doctor = doctor_list[0]
            self.parent.open_kiosk_payment(self.patient_key, "門診掛號", doctor)
    ...
    def _set_doctor_buttons(self):
        ...
        for i, doctor in enumerate(doctor_list):
            ...
            self.button_list[i].setText(f"{doctor}醫師")
    ...
    def _set_reservation_arrival_data(self):
        self.parent.open_kiosk_payment(self.patient_key, "預約報到", self.reserve_key)
