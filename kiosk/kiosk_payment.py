...
class KioskPayment(QtWidgets.QMainWindow):
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_KIOSK_PAYMENT, self)
        style = '''
    ...
    def _charge_payment(self, case_key):
        ...
        self.total_amount = drug_share_fee + self_total_fee
        ...
        self.ui.label_total_fee.setText(string_utils.xstr(self.total_amount))
        self.ui.label_received_fee.setText('0')

        if self.total_amount > 0:  # todo
            self.start_charge_cash()
        else:
            pass
            # self.cpay.cancel_payment()
    ...
    def start_charge_cash(self):
        ...
        self.comm.display_change.connect(self.display_change)
        ...
        self.charge_cash_thread = threading.Thread(
            ...
            args=(self.total_amount, self.comm.update_display, self.comm.display_change, self.stop_event)
        )
    ...
    def update_display(self, receipt_total):
        self.inserted_cash = receipt_total
        self.ui.label_received_fee.setText(f'{receipt_total}')

    def display_change(self, remain_change):
        if remain_change < 0:
            return

        if self.regist_type == '門診掛號':
            payment_type = '掛號繳費'
        elif self.regist_type == '批價繳費':
            payment_type = '批價繳費'
        else:
            payment_type = '掛號繳費'

        self._open_kiosk_completed(payment_type)

    def set_payment_data(self, patient_key, regist_type, keyword):
        self.regist_type = regist_type
        ...
        regist_fee = charge_utils.get_regist_fee(
            ...
            string_utils.xstr(patient_row['Birthday']), discount_type, '健保', share_type, treat_type, course,
        )
        ...
        self.total_amount = regist_fee + diag_share_fee
    ...
    def _open_kiosk_completed(self, payment_type):
        self.inserted_cash = 0

        self.parent.open_kiosk_completed(
