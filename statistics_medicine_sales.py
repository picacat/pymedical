...
class StatisticsMedicineSales(QtWidgets.QMainWindow):
    ...
    def _set_table_width(self):
        ...
        self.table_widget_medical_record.set_table_heading_width(
            [100, 130, 80, 100, 70, 70]
        )

    # 設定信號
    def _set_signal(self):
        self.ui.tableWidget_medicine_sales.itemSelectionChanged.connect(
    ...
    def _open_medical_record(self):
        if (
            ...
            and personnel_utils.get_permission(
                self.database, "病歷查詢", "調閱病歷", self.user_name
            )
            != "Y"
        ):
            return
    ...
    def _get_sql(
        self, assign_medicine=None, group_by_medicine=False, return_goods=False
    ):
        # ins_type_condition = ''
        # if self.ins_type != '全部':
        ...
        if assign_medicine is not None:
            medicine_name_condition = (
                f'AND prescript.MedicineName = "{assign_medicine}" '
            )
        ...
        sql = f'''
            SELECT
                ...
                prescript.MedicineSet, prescript.DosageMode,
        ...
        if group_by_medicine:
            ...
            sql += (
                "GROUP BY cases.CaseKey ORDER BY DATE(cases.CaseDate), cases.PatientKey"
            )
    ...
    def _set_table_data(self, row_no, row):
        total_dosage = number_utils.get_float(row["TotalDosage"])
        dosage_mode = string_utils.xstr(row["DosageMode"])
        if dosage_mode == "次劑量":
            medicine_set = number_utils.get_integer(row["MedicineSet"])
            case_key = number_utils.get_float(row["CaseKey"])
    ...
    def _set_return_goods(self):
        ...
        for row in rows:
            medicine_name = string_utils.xstr(row["MedicineName"])
            real_medicine_name = re.sub(
                r"[（(](退貨|換貨)[）)]", "", medicine_name
            ).strip()
            ...
            for row_no in range(self.ui.tableWidget_medicine_sales.rowCount()):
                ...
                if (
                    real_medicine_name
                    == self.ui.tableWidget_medicine_sales.item(row_no, 0).text()
                ):
                    total_dosage = number_utils.get_float(row["TotalDosage"])
                    ...
                    self._set_cell(
                        row_no + 1, 0, medicine_name, align=QtCore.Qt.AlignLeft
                    )
                    self._set_cell(
                        row_no + 1,
                        1,
                        self.ui.tableWidget_medicine_sales.item(row_no, 1).text(),
                    )
                    self._set_cell(row_no + 1, 2, total_dosage)
                    self._set_cell(
                        row_no + 1,
                        3,
                        self.ui.tableWidget_medicine_sales.item(row_no, 3).text(),
    ...
    def _set_cell(self, row_no, col_no, value, align=QtCore.Qt.AlignRight):
        ...
        self.ui.tableWidget_medicine_sales.item(row_no, col_no).setTextAlignment(
            align | QtCore.Qt.AlignVCenter
        )
        self.ui.tableWidget_medicine_sales.item(row_no, col_no).setForeground(
            QtGui.QColor("red")
        )
