...
from PyQt5.QtWidgets import QMessageBox, QPushButton, QFileDialog, QInputDialog
import datetime
...
class PatientList(QtWidgets.QMainWindow):
    ...
    def _set_signal(self):
        ...
        self.ui.action_export_medical_record_excel.triggered.connect(self.export_medical_record_excel)
        ...
        self.ui.action_print_patient_new_care.triggered.connect(self._print_patient_new_care)
        ...
        self.ui.tableWidget_patient_list.horizontalHeader().sectionClicked.connect(self._header_clicked)

        self.ui.action_change_discount.triggered.connect(self._change_discount_type)
        self.ui.action_replace_discount_type.triggered.connect(self._replace_discount_type)
        self.ui.action_purge_temp_patient.triggered.connect(self._purge_temp_patient)
        self.ui.action_clear_invalid_patient_name.triggered.connect(self._clear_invalid_patient_name)
    ...
    def _set_table_width(self):
        width = [30, 80, 100, 60, 40, 120, 120, 60, 80, 80, 180, 120, 120, 120, 120, 120]
        self.table_widget_patient_list.set_table_heading_width(width)

    # 讀取病歷
    def open_dialog(self):
        dialog = dialog_utils.get_dialog_patient_list(self.ui, self.database, self.system_settings)
        if dialog.exec_():
    ...
    def _set_photo(self, patient_key, row_no):
        filename = personnel_utils.get_personal_photo_filename(self.image_file_path, patient_key)
        if filename is None:
            self.ui.tableWidget_patient_list.setCellWidget(
                row_no, self.column['Photo'],
                None
            )
            return
        ...
        ui_utils.set_table_widget_field_icon(
            self.ui.tableWidget_patient_list, row_no, self.column['Photo'], gtk_icon_file,
            'has_image', property_value, self._photo_button_clicked)
    ...
    def delete_patient_record(self):
        ...
        patient_key = self.table_widget_patient_list.field_value(self.column['PatientKey'])
    ...
    def export_patient_list(self):
        ...
        export_utils.export_table_widget_to_excel(
            excel_file_name, self.ui.tableWidget_patient_list,
        )
    ...
    def export_medical_record_excel(self):
        dialog = dialog_utils.get_dialog_patient_medical_record(
            self.ui, self.database, self.system_settings, self.ui.tableWidget_patient_list)
    ...
    def _print_chart_pdf(self):
        ...
        printer_utils.print_form_medical_chart(
            self, self.database, self.system_settings,
            patient_key, today, 'pdf_by_dialog'
        )
    ...
    def _change_discount_type(self):
        ...
        discount_type, ok = input_dialog.getItem(
            self, "更改優待類別", "請選擇優待類別, 更改前請先註記病患資料", items, 0, False
        )
        ...
        for row_no in range(self.ui.tableWidget_patient_list.rowCount()):
            ...
            change_mark = self.ui.tableWidget_patient_list.cellWidget(row_no, self.column['PrintMark'])
    ...
    def _purge_temp_patient(self):
        dialog = dialog_utils.get_dialog_purge_temp_patient(self.ui, self.database, self.system_settings)
        if not dialog.exec_():
            dialog.deleteLater()
            return
        ...
        for row_no in range(dialog.ui.tableWidget_temp_patient.rowCount()):
            ...
            self.database.delete_record('temp_patient', 'TempPatientKey', temp_patient_key)
