...
# 業績成長統計-月統計 2023.04.22
class StatisticsGrowthMonth(QtWidgets.QMainWindow):
    # 初始化
    def __init__(self, parent=None, *args):
        super(StatisticsGrowthMonth, self).__init__(parent)
        self.parent = parent
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_STATISTICS_GROWTH_MONTH, self)
    ...
    def _calculate_ins_data(self):
        ...
        progress_dialog = QtWidgets.QProgressDialog(
            '正在統計資料中, 請稍後...', '取消', 0, max_year, self
        )

        progress_dialog.setWindowModality(QtCore.Qt.WindowModal)
        progress_dialog.setValue(0)
        i = 0
        for year in range(this_year, this_year-max_year, -1):
            i += 1
            progress_dialog.setValue(i)
            sql = f'''
                SELECT COUNT(*) AS Count FROM cases
                WHERE
                    Year(CaseDate) = {year} AND
                    Month(CaseDate) = {self.month} AND
                    InsType = "健保"
            '''
            ...
            medical_record_count = [f'{year}年{self.month:0>2}月', ins_count, self_count]
        ...
        progress_dialog.setValue(max_year)
        progress_dialog.deleteLater()

    def _calculate_past_data(self):
    ...
    def _plot_ins_month_chart(self):
        ...
        for row_no in range(self.ui.tableWidget_medical_record.rowCount()-1, -1, -1):
            bar_set = []
            year = self.ui.tableWidget_medical_record.item(row_no, 0).text()[:4]
            ins_count = number_utils.get_integer(
                self.ui.tableWidget_medical_record.item(row_no, 1).text()
            )
            bar_set.append(QtChart.QBarSet(year))
