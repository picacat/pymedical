...
class InsApplyAdjustFee(QtWidgets.QMainWindow):
    ...
    def _adjust_pharmacy_fee(self):
        if number_utils.get_integer(self.system_settings.field('藥師人數')) <= 0:  # 無藥師, 不需調整
            return
        ...
        fields = ['PharmacistID', 'PharmacyCode', 'PharmacyFee', 'InsTotalFee', 'InsApplyFee']
        for row in rows:
            ...
            for i in range(nhi_utils.MAX_COURSE):
                ...
                on_duty, pharmacist = nhi_utils.pharmacist_schedule_on_duty(self.database, case_key)
                if on_duty:
                    code = '1'
                    new_pharmacist_id = personnel_utils.get_person_field_value(self.database, pharmacist, 'ID')
                else:
                    code = '2'
                ...
                fee = charge_utils.get_ins_fee_from_ins_code(self.database, ins_code, case_date=case_date)
                new_pharmacy_fee += fee

            ins_apply_key = row['InsApplyKey']
            ins_total_fee = number_utils.get_integer(row['InsTotalFee']) - pharmacy_fee + new_pharmacy_fee
            ins_apply_fee = number_utils.get_integer(row['InsApplyFee']) - pharmacy_fee + new_pharmacy_fee
    ...
    def _adjust_nurse_diag_fee(self):
        if number_utils.get_integer(self.system_settings.field('護士人數')) <= 0:  # 無護理師, 不需調整
            return
        ...
        for ins_calculated_row in self.ins_calculated_table:
            ...
            for row in rows:
                for course in range(1, nhi_utils.MAX_COURSE+1):
                    ...
                    if treat_code in nhi_utils.MODERATE_COMPLICATED_ACUPUNCTURE_CODE:  # 2023-05-09 中度複針不調整, 只調整一般針灸比較划算
                        continue

                    if treat_code in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_CODE:  # 高度複針不調整
                        continue
    ...
    def _adjust_first_visit_fee(self):
        ...
        for row_no, row in zip(range(1, len(rows)+1), rows):
            ...
            if row_no <= first_visit_limit:
                ...
                first_visit_fee = charge_utils.get_ins_fee_from_ins_code(self.database, 'A90', case_date=case_date)
