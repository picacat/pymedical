...
class StockInData(QtWidgets.QMainWindow):
    ...
    def insert_prescript_row(self, row):
        ...
        medicine_row = [
            None, medicine_key, medicine_type, medicine_name, row['Unit'],
            product_no, product_name, product_unit, unit_quantity,
        ]
        ...
        for col_no, field in enumerate(medicine_row):
            ...
            if col_no in [self.item_col_no['medicine_unit'], self.item_col_no['unit']]:
                self.ui.tableWidget_stock_in_items.item(row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter
                )
            elif col_no in [self.item_col_no['unit_quantity']]:
                self.ui.tableWidget_stock_in_items.item(row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )

        self.ui.tableWidget_stock_in_items.setCurrentCell(row_no, self.item_col_no['product_no'])
    ...
    def _auto_completion_product(self, row_no, medicine_key):
        ...
        self.ui.tableWidget_stock_in_items.setCurrentCell(row_no, self.item_col_no['quantity'])
        ...
        if col_no in [
            ...
            try:
                unit_price = number_utils.get_float(
                    self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['unit_price']).text())
            except Exception:
                unit_price = 0
                return
            ...
            try:
                quantity = number_utils.get_float(
                    self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['quantity']).text())
            except Exception:
                quantity = 0
            ...
            self.ui.tableWidget_stock_in_items.setItem(row_no, self.item_col_no['amount'], amount_item)
    ...
    def _calculate_total_amount(self):
        ...
        for row_no in range(row_count):
            amount = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['amount'])
            if amount is None:
                continue
    ...
    def _update_stock_in(self):
        fields = [
            'StockInDate', 'Supplier', 'OrderNo', 'Attn', 'Remark', 'InvoiceNo', 'PaymentType', 'Amount']
        data = [
        ...
        self.database.update_record('stockin', fields, 'StockInKey', self.stock_in_key, data)
    ...
    def _insert_stock_in_items(self, stock_in_key):
        fields = [
            'StockInKey', 'MedicineKey', 'MedicineName', 'ProductNo',  'ProductName',
            'Unit', 'UnitQuantity', 'Quantity', 'UnitPrice', 'Amount', 'Remark',
        ]

        for row_no in range(self.ui.tableWidget_stock_in_items.rowCount()):
            medicine_key = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['medicine_key']).text()
            medicine_name = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['medicine_name']).text()
            try:
                product_no = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['product_no']).text()
            except Exception:
                product_no = None
            ...
            try:
                unit = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['unit']).text()
            except Exception:
                unit = None
            ...
            try:
                unit_price = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['unit_price']).text()
            except Exception:
                unit_price = None

            try:
                quantity = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['quantity']).text()
            except Exception:
                quantity = None

            try:
                amount = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['amount']).text()
            except Exception:
                amount = None

            try:
                remark = self.ui.tableWidget_stock_in_items.item(row_no, self.item_col_no['remark']).text()
            except Exception:
                remark = None

            data = [
                stock_in_key, medicine_key, medicine_name, product_no, product_name,
