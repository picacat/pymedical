...
class Pharmacy(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, *args):
        super(Pharmacy, self).__init__(parent)
        self.parent = parent
    ...
    def _set_font_size(self):
        font_size = 18
        self.ui.tableWidget_charge_list.setStyleSheet(
            f'font-size: 15pt;'
        )
        self.ui.textEdit_prescript.setStyleSheet(
    ...
    def _set_table_data(self, row_no, row):
        ...
        self.ui.tableWidget_charge_list.setCellWidget(
            row_no, 10, check_box_charge_done)
    ...
    def _set_drug_done(self):
        ...
        check_box_drug_done = self.ui.tableWidget_charge_list.cellWidget(current_row_no, 11)
    ...
    def _set_drug_pickup_done(self):
        current_row_no = self.ui.tableWidget_charge_list.currentRow()
        check_box_drug_pickup_done = self.ui.tableWidget_charge_list.cellWidget(current_row_no, 12)

        wait_key = self.table_widget_charge_list.field_value(0)
        case_key = self.table_widget_charge_list.field_value(1)

        if check_box_drug_pickup_done is not None and check_box_drug_pickup_done.isChecked():
            self._save_drug_pickup_records(wait_key=wait_key, case_key=case_key)
        else:
            self._save_drug_pickup_records(wait_key=wait_key, case_key=case_key, drug_pickup_done='False')
    ...
    def _show_medical_record(self, case_key):
        ...
        prescript_record = case_utils.get_prescript_record(
            self.database, self.system_settings, case_key, display_total_dosage=True, display_location=True)
    ...
    def _save_drug_pickup_records(self, wait_key, case_key, drug_pickup_done='True'):
        if wait_key not in ['', None]:
            self.database.exec_sql(f'UPDATE wait SET DrugPickupDone = "{drug_pickup_done}" WHERE WaitKey = {wait_key}')
