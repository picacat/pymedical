...
class ICRecordUpload(QtWidgets.QMainWindow):
    ...
    def _set_signal(self):
        ...
        self.ui.action_create_ic_upload_xml_2.triggered.connect(
            lambda: self._upload_xml_file_2_0(
                "A2", clear_record=False, show_message=True
            )
        )
        ...
        self.ui.tableWidget_ic_record.horizontalHeader().sectionClicked.connect(
    ...
    def _set_table_data(self, row_no, row):
        ...
        security_row = case_utils.get_treat_data_xml_dict(
            string_utils.get_str(row["Security"], "utf-8")
        )
        ...
        for column in range(len(medical_record)):
            ...
            if column in [6, 15]:
                self.ui.tableWidget_ic_record.item(row_no, column).setTextAlignment(
            ...
            if error_message != "":
                self.ui.tableWidget_ic_record.item(row_no, column).setForeground(
                    QtGui.QColor("red")
                )
    ...
    def _set_check_box(self, row_no, check):
        ...
        self.ui.tableWidget_ic_record.setCellWidget(row_no, col_no, check_box)
        self.ui.tableWidget_ic_record.item(row_no, col_no).setTextAlignment(
            QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
        )
    ...
    def _check_error(self, row, security_row):
        ...
        if (
            patient_record is not None
            and string_utils.xstr(patient_record["Birthday"]) == ""
        ):
        ...
        if (
            row["CardNo"] is not None
            and len(row["CardNo"]) >= 1
            and len(row["CardNo"]) != 12
        ):
    ...
    def _upload_by_cshis6(
        self, upload_type, xml, case_count, prescript_count, show_message
    ):
        ic_card = class_utils.get_cshis(self, self.database, self.system_settings)
    ...
    def _upload_by_cshis5(
        self, upload_type, xml, case_count, prescript_count, show_message
    ):
        cshis_x = class_utils.get_cshisx(self.database, self.system_settings)
        rtn_code, op_code = cshis_x.VNHI_Upload(
            upload_type, xml, case_count, prescript_count
        )
    ...
    def _upload_xml_file_2_0(self, upload_type, clear_record=True, show_message=True):
        ...
        if self.cshis_version == "cshis6":
            ...
            upload_ok = self._upload_by_cshis6(
                upload_type, xml, case_count, prescript_count, show_message
            )
        else:
            upload_ok = self._upload_by_cshis5(
                upload_type, xml, case_count, prescript_count, show_message
            )
