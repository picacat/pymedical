...
class KioskPayment(QtWidgets.QMainWindow):
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(os.path.join(self.parent.UI_DIR, 'kiosk_payment.ui'), self)

    def _set_table_layout(self):
        image = system_utils.set_image(
            self, os.path.join(self.parent.IMAGE_DIR, 'ok.png'), 40, 530, width=self.ICON_W, height=self.ICON_H)
        self._bring_to_front(image)

        self.label_title = system_utils.set_label(
            self, '請確認XXX今日的繳費明細', 130, 530,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN)
        self._bring_to_front(self.label_title)

        button_cancel = system_utils.set_button(
            self, '取消繳費', 'white', 130, 1200,
            self.parent.BUTTON_FONT, self.parent.RED, self.parent.BUTTON_FONT_SIZE,
            340, self.parent.BUTTON_HEIGHT, self._back_to_home)
        self._bring_to_front(button_cancel)

        button_ok = system_utils.set_button(
            self, '確認並開始繳費', 'white', 600, 1200,
            self.parent.BUTTON_FONT, self.parent.DARK_GREEN, self.parent.BUTTON_FONT_SIZE,
            360, self.parent.BUTTON_HEIGHT, self._ready_to_payment)
    ...
    def set_payment_table(self):
        ...
        header = system_utils.set_label(
            self, '看診醫師', 75, self.ROW1, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(header)

        self.label_doctor = system_utils.set_label(
            self, '黃子玶', 345, self.ROW1, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(self.label_doctor)
        self._align_right(self.label_doctor, self.COL1, self.ROW1)

        header = system_utils.set_label(
            self, '掛號費', 602, self.ROW1, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(header)

        self.label_regist_fee = system_utils.set_label(
            self, '250元', 0, self.ROW1, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(self.label_regist_fee)
        self._align_right(self.label_regist_fee, self.COL2, self.ROW1)

        header = system_utils.set_label(
            self, '門診負擔', 75, self.ROW2, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(header)

        self.label_diag_share_fee = system_utils.set_label(
            self, '50元', 0, self.ROW2, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(self.label_diag_share_fee)
        self._align_right(self.label_diag_share_fee, self.COL1, self.ROW2)

        header = system_utils.set_label(
            self, '藥品負擔', 585, self.ROW2, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(header)

        self.label_drug_share_fee = system_utils.set_label(
            self, '40元', 0, self.ROW2, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(self.label_drug_share_fee)
        self._align_right(self.label_drug_share_fee, self.COL2, self.ROW2)

        header = system_utils.set_label(
            self, '自費金額', 75, self.ROW3, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(header)

        self.label_total_fee = system_utils.set_label(
            self, '160元', 345, self.ROW3, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(self.label_total_fee)
        self._align_right(self.label_total_fee, self.COL1, self.ROW3)

        header = system_utils.set_label(
            self, '費用總計', 75, self.ROW4, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(header)
        
        self.label_total_amount = system_utils.set_label(
            self, '500元整', 345, self.ROW4, self.parent.TEXT_FONT, self.HEADER_SIZE, self.HEADER_COLOR)
        self._bring_to_front(self.label_total_amount)

    def _set_bottom_image(self):
        image = system_utils.set_image(self, os.path.join(self.parent.IMAGE_DIR, 'bottom.png'), -23, 1773)
        image.raise_()
        image.show()

        image = system_utils.set_image(self, os.path.join(self.parent.IMAGE_DIR, 'scan_me.png'), 838, 1628)
        image.raise_()
        image.show()
    ...
    def _set_home_image(self):
        self.home_image = system_utils.set_image(self, os.path.join(self.parent.IMAGE_DIR, 'home.png'), 0, 0)
    ...
    def _ready_to_payment(self):
        ...
        dialog = module.DialogPayment(
            self.parent, self.database, self.system_settings, self.ic_card, self.case_key, self.total_amount)
        dialog.exec_()
