...
class DialogMessageBox(QtWidgets.QDialog):
    ...
    def set_no_iccard(self):
        system_utils.set_label(
            self, '系統未偵測到健保卡！', 0, self.TITLE_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, '請重新插入健保卡進行預約報到', 0, self.LINE1_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        system_utils.set_label(
            self, '（未帶健保卡請至櫃檯進行報到）', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.LIGHT_GREEN, center=True)
    ...
    def set_not_on_time(self):
        ...
        system_utils.set_label(
            self, '您已超過預約截止時間報到！', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, '請找現場服務人員協助', 0, self.LINE3_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.LIGHT_GREEN, center=True)
        self._set_back_home_button('返回首頁')
    ...
    def set_write_iccard_error(self, errror_code):
        ...
        system_utils.set_label(
            self, '健保卡寫卡錯誤！', 0, self.TITLE_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, f'錯誤原因: {error_message}', 0, self.LINE1_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        system_utils.set_label(
            self, '（請帶健保卡請至櫃檯進行報到）', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.LIGHT_GREEN, center=True)
        self._set_back_home_button('返回首頁')

    def set_no_patient(self):
        system_utils.set_label(
            self, '系統找不到您的資料！', 0, self.TITLE_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, '請攜帶健保卡至櫃檯辦理初診掛號', 0, self.LINE1_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')
    ...
    def set_no_reservation(self):
        ...
        system_utils.set_image(
            self, png_filename, 0, self.ICON_Y, width=self.ICON_W, height=self.ICON_W, center=True)
        system_utils.set_label(
            self, '預約系統查無您的預約記錄！', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, '請攜帶健保卡找現場服務人員協助', 0, self.LINE3_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')

    def set_already_arrival(self):
        system_utils.set_label(
            self, '您已經完成預約報到的作業！', 0, self.TITLE_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, '如有問題，請找現場服務人員協助', 0, self.LINE1_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')
    ...
    def set_not_doctor_done(self):
        ...
        system_utils.set_image(
            self, png_filename, 0, self.ICON_Y, width=self.ICON_W, height=self.ICON_W, center=True)
        system_utils.set_label(
            self, '您的批價作業尚未完成', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, '請稍後再進行現金繳費', 0, self.LINE3_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')
    ...
    def set_already_payment(self):
        ...
        system_utils.set_image(
            self, png_filename, 0, self.ICON_Y, width=self.ICON_W, height=self.ICON_H, center=True)
        system_utils.set_label(
            self, '您已經完成批價作業', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, '請勿再次繳費，謝謝', 0, self.LINE3_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')
    ...
    def set_arrival_done(self):
        ...
        system_utils.set_image(
            self, png_filename, 0, self.ICON_Y, width=self.ICON_W, height=self.ICON_H, center=True)
        system_utils.set_label(
            self, '預約報到完成，請查看候診螢幕', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')
    ...
    def set_payment_done(self):
        ...
        system_utils.set_image(
            self, png_filename, 0, self.ICON_Y, width=self.ICON_W, height=self.ICON_H, center=True)
        system_utils.set_label(
            self, '繳費已完成，請取回繳費收據及', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        system_utils.set_label(
            self, '健保卡，並稍後櫃台通知領藥', 0, self.LINE3_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')
    ...
    def arrival_checkin(self, reserve_row):
        ...
        for i, data in enumerate(reserve_data):
            system_utils.set_label(
                self, data, x, y + i * height, self.parent.TEXT_FONT, self.parent.FONT_SIZE, font_color)

        cancel_seconds = 10
        button_text = '取消預約報到'
        push_button = system_utils.set_button(
            self, f'{button_text}({cancel_seconds}s)', 'white', 100, self.BUTTON_Y,
            self.parent.BUTTON_FONT, self.parent.RED, self.parent.BUTTON_FONT_SIZE,
            430, self.parent.BUTTON_HEIGHT, lambda: self._set_arrival(False))
        ...
        system_utils.set_button(
            self, '確定預約報到', 'white', 560, self.BUTTON_Y,
            self.parent.BUTTON_FONT, self.parent.DARK_GREEN, self.parent.BUTTON_FONT_SIZE,
            340, self.parent.BUTTON_HEIGHT, lambda: self._set_arrival(True))
    ...
    def query_self_pay_case(self, reserve_row, start_date, end_date, pres_days, remain_days):
        ...
        push_button = system_utils.set_button(
            self, f'{button_text}({cancel_seconds}s)', 'white', 100, self.BUTTON_Y,
            self.parent.BUTTON_FONT, self.parent.RED, self.parent.BUTTON_FONT_SIZE,
            430, self.parent.BUTTON_HEIGHT, lambda: self._set_self_pay(False))
        push_button.clicked.connect(self.close)
        ...
        system_utils.set_button(
            self, '接受自費看診', 'white', 560, self.BUTTON_Y,
            self.parent.BUTTON_FONT, self.parent.DARK_GREEN, self.parent.BUTTON_FONT_SIZE,
            340, self.parent.BUTTON_HEIGHT, lambda: self._set_self_pay(True))
    ...
    def set_in_progress(self):
        ...
        system_utils.set_label(
            self, '健保卡讀卡中...', 0, self.LINE3_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        system_utils.set_label(
            self, '請勿取出您的健保卡', 0, self.LINE4_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
    ...
    def set_cancel_not_today(self):
        ...
        system_utils.set_image(
            self, png_filename, 0, self.ICON_Y, width=self.ICON_W, height=self.ICON_W, center=True)
        system_utils.set_label(
            self, '預約系統不開放取消當日預約', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.RED, center=True)
        system_utils.set_label(
            self, '請聯繫現場服務人員', 0, self.LINE3_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')
    ...
    def cancel_reservation(self, reserve_row):
        ...
        push_button = system_utils.set_button(
            self, f'{button_text}({cancel_seconds}s)', 'white', 100, self.BUTTON_Y,
            self.parent.BUTTON_FONT, self.parent.DARK_GREEN, self.parent.BUTTON_FONT_SIZE,
            430, self.parent.BUTTON_HEIGHT, lambda: self._set_cancel(False))
        ...
        system_utils.set_button(
            self, '確定取消預約', 'white', 560, self.BUTTON_Y,
            self.parent.BUTTON_FONT, self.parent.RED, self.parent.BUTTON_FONT_SIZE,
            340, self.parent.BUTTON_HEIGHT, lambda: self._set_cancel(True))
    ...
    def set_cancel_done(self):
        ...
        system_utils.set_image(
            self, png_filename, 0, self.ICON_Y, width=self.ICON_W, height=self.ICON_H, center=True)
        system_utils.set_label(
            self, '您的預約取消完成，請取回健保卡', 0, self.LINE2_Y,
            self.parent.TEXT_FONT, self.parent.FONT_SIZE, self.parent.DARK_GREEN, center=True)
        self._set_back_home_button('返回首頁')