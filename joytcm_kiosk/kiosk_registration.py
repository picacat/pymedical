...
class KioskRegistration(QtWidgets.QMainWindow):
    ...
    def set_registration_data(self):
        ...
        start_date, end_date, pres_days, remain_days = registration_utils.check_prescription_finished(       # 檢查上次健保給藥是否服藥完畢
            self.database, self.system_settings, None, patient_key, manual_message=True
        )
        if start_date is not None and remain_days is not None and remain_days >= 2:  # 上次開藥還有兩天
            if self._query_self_pay_case(reserve_row, start_date, end_date, pres_days, remain_days):  # 改掛自費
    ...
    def _reservation_arrival(self, reserve_row, ins_type='健保'):
        self._check_ic_card_basic_data(reserve_row)  # 自費也順便檢查基本資料是否與健保卡一致

        if ins_type == '健保':
            patient_key = reserve_row['PatientKey']
            self.treat_type, self.card, self.course = self._auto_completion_course(patient_key)
    ...
    def _check_ic_card_basic_data(self, reserve_row):
        ...
        if string_utils.xstr(patient_row['InsType']) != self.ic_card.basic_data['insured_mark']:
    ...
    def _insert_medical_record(self, reserve_row, ins_type):
        ...
        try:
            age_year, _ = date_utils.get_age(patient_row['Birthday'], datetime.datetime.now())
        except Exception:
            age_year = None
        ...
        if ins_type == '健保':
            ...
            diag_share_fee = charge_utils.get_diag_share_fee(
                self.database, self.system_settings, share_type, treat_type, course, reg_type
            )
