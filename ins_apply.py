...
class InsApply(QtWidgets.QMainWindow):
    ...
    def open_dialog(self):
        ...
        dialog = dialog_utils.get_dialog_ins_apply(self.ui, self.database, self.system_settings)
    ...
    def _check_ins_xml_file(self):
        ...
        self.tab_ins_apply_fee_performance = module_utils.get_ins_apply_fee_performance(
            self, self.database, self.system_settings,
            self.apply_year, self.apply_month, '全部',
            self.start_date, self.end_date,
            self.period, self.apply_type, exclude_c5=False
        )
    ...
    def _check_ins_indicator(self):
        self.tab_indicator = module_utils.get_ins_apply_indicator(
            self, self.database, self.system_settings,
            self.apply_date, self.period, self.apply_type_code, self.clinic_id,
        )
    ...
    def _upload_data(self):
        ...
        msg_box.setText(f"<font size='4' color='red'><b>確定上傳健保申報檔案?<br>申報檔名: {xml_file}</b></font>")
