...
# 收費設定 2018.04.14
class DictCure(QtWidgets.QMainWindow):
    # 初始化
    def __init__(self, parent=None, *args):
        super(DictCure, self).__init__(parent)
        self.parent = parent
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_DICT_CURE, self)
        ...
        self.table_widget_dict_cure = class_utils.get_table_widget(self.ui.tableWidget_dict_cure, self.database)
        self.table_widget_dict_cure.set_column_hidden([0])
    ...
    def _set_signal(self):
        ...
        self.ui.toolButton_add_cure.clicked.connect(self._add_cure)
        self.ui.toolButton_remove_cure.clicked.connect(self._remove_cure)
        self.ui.toolButton_edit_cure.clicked.connect(self._edit_cure)
        self.ui.tableWidget_dict_cure.doubleClicked.connect(self._edit_cure)

    # 設定欄位寬度
    def _set_table_width(self):
        dict_groups_width = [100, 180]
        dict_cure_width = [100, 180, 180, 750]
        ...
        self.table_widget_dict_cure.set_table_heading_width(dict_cure_width)
    ...
    def dict_groups_name_changed(self):
        ...
        self.ui.groupBox_dict_cure.setTitle(f'{self.dict_type}資料 - [{dict_groups_name}]')
        self._read_dict_cure(dict_groups_name)
        self.ui.tableWidget_dict_groups_name.setFocus(True)

    def _read_dict_cure(self, dict_groups_name):
        sql = f'''
            SELECT * FROM clinic
            WHERE
        ...
        '''
        self.table_widget_dict_cure.set_db_data(sql, self._set_dict_cure_data)

    def _set_dict_cure_data(self, rec_no, rec):
        dict_cure_rec = [
        ...
        for column in range(len(dict_cure_rec)):
            self.ui.tableWidget_dict_cure.setItem(
                rec_no, column, QtWidgets.QTableWidgetItem(dict_cure_rec[column])
            )
    ...
    def _edit_dict_groups(self):
        ...
        input_dialog = dialog_utils.get_dialog(
            f'{self.dict_type}類別', f'請輸入{self.dict_type}類別',
            old_groups,
    ...
    def _add_groups_name(self):
        ...
        data = [
            f'{self.dict_type}', dict_groups, groups_name
        ]
        self.database.insert_record('dict_groups', field, data)
    ...
    def _add_cure(self):
        ...
        result = dialog.exec_()
        if result != 0:
            current_row = self.ui.tableWidget_dict_cure.rowCount()
            self.ui.tableWidget_dict_cure.insertRow(current_row)
            dict_groups_name = self.table_widget_dict_groups_name.field_value(1)
    ...
    def _remove_cure(self):
        cure_name = self.table_widget_dict_cure.field_value(3)
        msg_box = dialog_utils.get_message_box(
            f'刪除{self.dict_type}資料',
            QMessageBox.Warning,
            f'<font size="5" color="red"><b>確定刪除{self.dict_type}: "{cure_name}"?</b></font>',
            '注意！資料刪除後, 將無法回復!'
        )
        remove_record = msg_box.exec_()
        if not remove_record:
        ...
        key = self.table_widget_dict_cure.field_value(0)
        self.database.delete_record('clinic', 'ClinicKey', key)
        self.ui.tableWidget_dict_cure.removeRow(self.ui.tableWidget_dict_cure.currentRow())

    # 更改主訴
    def _edit_cure(self):
        clinic_key = self.table_widget_dict_cure.field_value(0)
