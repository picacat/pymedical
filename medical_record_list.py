...
from libs import (case_utils, class_utils, cshis_utils, date_utils, db_utils,
                  dialog_utils, export_utils, log_utils, nhi_utils,
                  number_utils, personnel_utils, printer_utils,
                  stock_utils, string_utils, system_utils, ui_utils)
...
class MedicalRecordList(QtWidgets.QMainWindow):
    ...
    def _set_permission(self):
        ...
        if personnel_utils.get_permission(self.database, self.program_name, '關檔', self.user_name) != 'Y':
    ...
    def _set_signal(self):
        ...
        self.ui.action_open_record.triggered.connect(lambda: self.open_medical_record(case_key=None))
        ...
        self.ui.action_print_non_dosage_receipt.triggered.connect(self._print_non_dosage_receipt)
        ...
        self.ui.action_print_prescription_bag.triggered.connect(self._print_prescription_bag)
        ...
        self.ui.action_print_cases2.triggered.connect(lambda: self._print_cases(print_self_prescript=True))
        self.ui.action_print_cases3.triggered.connect(lambda: self._print_cases(print_patient=False))
        self.ui.action_export_cases_pdf.triggered.connect(self._print_cases)
        self.ui.action_export_cases_pdf2.triggered.connect(lambda: self._print_cases(print_self_prescript=True))
        ...
        self.ui.action_set_traditional_health_case.triggered.connect(self._set_traditional_health_case)
        ...
        self.ui.action_print_marked_registration_form.triggered.connect(self._print_marked_registration_form)

        self.ui.action_print_marked_ins_prescript.triggered.connect(self._print_marked_ins_prescript)
        self.ui.action_print_marked_ins_receipt.triggered.connect(self._print_marked_ins_receipt)

        self.ui.action_print_marked_self_prescript.triggered.connect(self._print_marked_self_prescript)
        self.ui.action_print_marked_self_receipt.triggered.connect(self._print_marked_self_receipt)
        self.ui.action_print_marked_self_receipt1.triggered.connect(self._print_marked_self_receipt1)
        ...
        self.ui.action_change_pharmacy_type.triggered.connect(self._change_pharmacy_type)
        ...
        self.ui.tableWidget_medical_record_list.doubleClicked.connect(lambda: self.open_medical_record(case_key=None))
        self.ui.tableWidget_medical_record_list.keyPressEvent = self._table_widget_medical_record_key_press
        self.ui.tableWidget_medical_record_list.horizontalHeader().sectionClicked.connect(self._header_clicked)
        ...
        self.ui.action_print_correction_reg_income.triggered.connect(self._print_correction_reg_income)
        self.ui.action_print_care_income.triggered.connect(self._print_correction_reg_income)
        self.ui.radioButton_normal.clicked.connect(lambda: self._set_view_state('詳細檢視'))
        self.ui.radioButton_simple.clicked.connect(lambda: self._set_view_state('精簡顯示'))
        ...
        self.ui.action_rewrite_ic_prescript.triggered.connect(self._rewrite_ic_prescript)
        self.ui.action_cancel_ic_card.triggered.connect(self._cancel_ic_card)
        self.ui.action_print_medical_certificate.triggered.connect(self._print_medical_certificate)
    ...
    def _set_table_width(self):
        width = [
            70, 10, 40, 160, 50, 40, 40, 40, 50, 80, 80, 40, 120, 50, 50, 90, 80, 80, 70, 40, 40, 80, 200, 200,
            80, 80, 80, 80, 80, 80, 80, 80, 80, 400,
        ]
    ...
    def open_dialog(self):
        ...
        dialog = dialog_utils.get_dialog_medical_record_list(self, self.database, self.system_settings)
        if self.dialog_setting['dialog_executed']:
            ...
            dialog.ui.comboBox_regist_type.setCurrentText(self.dialog_setting['regist_type'])
            ...
            dialog.ui.comboBox_injury_type.setCurrentText(self.dialog_setting['injury_type'])
            ...
            dialog.ui.comboBox_registrar.setCurrentText(self.dialog_setting['registrar'])
            dialog.ui.comboBox_room.setCurrentText(self.dialog_setting['room'])
            dialog.ui.checkBox_enable_end_date.setChecked(self.dialog_setting['enable_end_date'])
            dialog.ui.radioButton_archive_database.setChecked(self.dialog_setting['archive_database'])
        ...
        if dialog.exec_():
            ...
            self.dialog_setting['regist_type'] = dialog.ui.comboBox_regist_type.currentText()
            self.dialog_setting['treat_type'] = dialog.ui.comboBox_treat_type.currentText()
            self.dialog_setting['share_type'] = dialog.ui.comboBox_share_type.currentText()
            self.dialog_setting['injury_type'] = dialog.ui.comboBox_injury_type.currentText()
            self.dialog_setting['apply_type'] = dialog.ui.comboBox_apply_type.currentText()
            self.dialog_setting['person'] = dialog.ui.comboBox_doctor.currentText()
            self.dialog_setting['registrar'] = dialog.ui.comboBox_registrar.currentText()
            self.dialog_setting['room'] = dialog.ui.comboBox_room.currentText()
            self.dialog_setting['enable_end_date'] = dialog.ui.checkBox_enable_end_date.isChecked()
            self.dialog_setting['archive_database'] = dialog.ui.radioButton_archive_database.isChecked()
    ...
    def _get_archive_database(self):
        ...
        if self.dialog_setting['archive_database'] and archive_database_name not in ['', None]:
    ...
    def _read_medical_record_list(self, sql, medicine_list, page=1):
        ...
        self.ui.label_record_count.setText(f'共 {self.ui.tableWidget_medical_record_list.rowCount()} 筆')

        if self.ui.tableWidget_medical_record_list.rowCount() > 0 and \
                self.system_settings.field('病歷查詢顯示合計') == 'Y':
    ...
    def _set_table_data(self, row_no, row):
        ...
        pres_days = case_utils.get_pres_days(self.database, row['CaseKey'], medicine_set)
        if pres_days <= 0:
            pres_days = None
            pharmacy_type = None
        else:
        ...
        if self.system_settings.field('日期格式') == '民國年':
            ...
            try:
                birthday = date_utils.date_to_zh_tw_date(string_utils.xstr(row['Birthday']))
            except Exception:
                birthday = None
        ...
        self.ui.tableWidget_medical_record_list.setCellWidget(row_no, self.column['TreatType'], None)
        ...
        for col_no in range(len(medical_record)):
            ...
            if self_total_fee > 0 or self_prescript_count > 0 or ins_type == '自費':
                if treat_type == '民俗調理':
                    self.ui.tableWidget_medical_record_list.item(
                        row_no, col_no).setForeground(QtGui.QColor('brown'))
                else:
                    self.ui.tableWidget_medical_record_list.item(
                        row_no, col_no).setForeground(QtGui.QColor('blue'))

            if string_utils.xstr(row['TreatType']) == '自購':
                self.ui.tableWidget_medical_record_list.item(
                    row_no, col_no).setForeground(QtGui.QColor('darkGreen'))
            ...
            if string_utils.xstr(row['ApplyType']) == '不申報':
                self.ui.tableWidget_medical_record_list.item(
                    row_no, col_no).setForeground(QtGui.QColor('gray'))

            if string_utils.xstr(row['TreatType']) in nhi_utils.SPECIAL_CODE_DICT:
                self.ui.tableWidget_medical_record_list.item(
                    row_no, col_no).setForeground(QtGui.QColor('magenta'))
        ...
        if color is not None:
            self.ui.tableWidget_medical_record_list.item(row_no, col_no).setForeground(QtGui.QColor(color))
    ...
    def _set_treat_type(self, row_no, treat_type, *args):
        ...
        self.ui.tableWidget_medical_record_list.setCellWidget(row_no, col_no, treat_type_label)
    ...
    def _set_done_status(self, row, row_no):
        ...
        if is_closed:
            case_utils.set_close_case_icon(
                self.ui.tableWidget_medical_record_list, row_no, self.column['DoctorDone'], is_closed
            )
            return
    ...
    def _done_button_clicked(self):
        ...
        dialog = dialog_utils.get_dialog_medical_record_done(
            ...
            self.table_widget_medical_record_list.field_value(self.column['CaseKey']), property_name,
        )
        if dialog.exec_():
    ...
    def _set_version_history(self, row, row_no):
        ...
        if len(rows) <= 0:
            self.ui.tableWidget_medical_record_list.setCellWidget(
                row_no, self.column['VersionHistory'],
                None
            )
            return
        ...
        property_value = True
        ui_utils.set_table_widget_field_icon(
            self.ui.tableWidget_medical_record_list, row_no, self.column['VersionHistory'], gtk_icon_file,
            'has_version_history', property_value, self._version_history_button_clicked)
    ...
    def delete_medical_record(self):
        ...
        row = self.database.select_record(f'SELECT IsClosed FROM cases WHERE CaseKey = {case_key}')[0]
    ...
    def _open_marked_records(self):
        for row_no in range(self.ui.tableWidget_medical_record_list.rowCount()):
            ...
            try:
                check_box_print_mark = self.ui.tableWidget_medical_record_list.cellWidget(row_no, self.column['PrintMark'])
    ...
    def open_medical_record(self, case_key=None):
        if self.user_name == '超級使用者':
            pass
        elif personnel_utils.get_permission(self.database, self.program_name, '調閱病歷', self.user_name) != 'Y':
            if case_key is None:
                system_utils.show_message_box(
                    QMessageBox.Warning,
                    '權限不足',
    ...
    # 列印費用收據
    def print_receipt_form(self, case_key, print_type, print_dosage=True, print_only_medicine_set2=False):
        database = self._get_archive_database()

        printer_utils.print_receipt_form(
            self, database, self.system_settings, case_key, print_type,
            print_dosage=print_dosage, print_only_medicine_set2=print_only_medicine_set2)
    ...
    def _set_block_check(self):
        ...
        end_no, ok = input_dialog.getInt(
            self, '區段註記', '請輸入區段註記結束號', row_count, 1, row_count, self.column['PrintMark'])
        if not ok:
            return
    ...
    def _print_cases(self, print_self_prescript=False, print_patient=True):
        ...
        if sender_name in ['action_print_cases', 'action_print_cases2', 'action_print_cases3']:
            if not print_patient:
                patient_key = None

            printer_utils.print_form_medical_records(
                self, database, self.system_settings,
                patient_key, sql, None, None, print_self_prescript=print_self_prescript,
            )
            ...
            printer_utils.print_form_medical_records(
                self, database, self.system_settings,
                patient_key, sql, None, None, print_type='pdf_by_dialog',
    ...
    def _print_fees(self):
        ...
        if sender_name == 'action_print_fees':
            printer_utils.print_form_medical_fees(
                self, database, self.system_settings, patient_key, sql,
            )
        else:
            printer_utils.print_form_medical_fees(
                self, database, self.system_settings, patient_key, sql, 'pdf_by_dialog',
            )
    ...
    def _export_marked_medical_record_to_excel(self):
        ...
        export_utils.export_table_widget_to_excel(
            ...
            f'{start_date}至{end_date}{person}註記病歷資料', calc_total=False, mark_col_no=1
        )
        system_utils.show_message_box(
            QMessageBox.Information,
            '資料匯出完成',
    ...
    def _get_medical_record_rows(self):
        ...
        for row_no in range(self.ui.tableWidget_medical_record_list.rowCount()):
            ...
            case_date = self.ui.tableWidget_medical_record_list.item(row_no, self.column['CaseDate']).text()[:10]
            period = self.ui.tableWidget_medical_record_list.item(row_no, self.column['Period']).text()[:1]
            next_case_date = self.ui.tableWidget_medical_record_list.item(row_no + 1, self.column['CaseDate'])
            if next_case_date is not None:
                next_case_date = next_case_date.text()[:10]

            next_period = self.ui.tableWidget_medical_record_list.item(row_no + 1, self.column['Period'])
            if next_period is not None:
                next_period = next_period.text()[:1]

            ins_type = self.ui.tableWidget_medical_record_list.item(row_no, self.column['InsType']).text()
            try:
                treat_type = self.ui.tableWidget_medical_record_list.item(row_no, self.column['TreatType']).text()
            except Exception:
                treat_type = ''

            card = self.ui.tableWidget_medical_record_list.item(row_no, self.column['Card']).text()
            if card in ['免卡', None]:
                card = ''

            regist_fee = number_utils.get_integer(
                self.ui.tableWidget_medical_record_list.item(row_no, self.column['RegistFee']).text()
            )
            diag_share_fee = number_utils.get_integer(
                self.ui.tableWidget_medical_record_list.item(row_no, self.column['DiagShareFee']).text()
            )
            drug_share_fee = number_utils.get_integer(
                self.ui.tableWidget_medical_record_list.item(row_no, self.column['DrugShareFee']).text()
            )
            total_fee = number_utils.get_integer(
                self.ui.tableWidget_medical_record_list.item(row_no, self.column['TotalFee']).text()
            )
            doctor = self.ui.tableWidget_medical_record_list.item(row_no, self.column['Doctor']).text()
            patient_key = self.ui.tableWidget_medical_record_list.item(row_no, self.column['PatientKey']).text()
            name = self.ui.tableWidget_medical_record_list.item(row_no, self.column['Name']).text()
            regist_no = self.ui.tableWidget_medical_record_list.item(row_no, self.column['RegistNo']).text()
            course = self.ui.tableWidget_medical_record_list.item(row_no, self.column['Course']).text()
    ...
    def _export_to_excel(self):
        ...
        export_utils.export_daily_medical_records_to_excel(
            database, self.system_settings, excel_file_name,
            medical_record_rows,
        )
    ...
    def _export_to_json(self):
        ...
        db_utils.export_medical_record_to_json(self, database, json_file_name, case_key_list)
    ...
    def _table_widget_medical_record_key_press(self, event):
        ...
        return QtWidgets.QTableWidget.keyPressEvent(self.ui.tableWidget_medical_record_list, event)
    ...
    def _change_ins_type(self):
        ...
        for row_no in range(self.ui.tableWidget_medical_record_list.rowCount()):
            ...
            try:
                change_mark = self.ui.tableWidget_medical_record_list.cellWidget(row_no, self.column['PrintMark'])
    ...
    def _change_apply_type(self):
        ...
        apply_type, ok = QInputDialog.getItem(
            self, "醫療費用申報方式", "請選擇醫療費用申報方式, 更改前請先註記病歷", items, 0, False
        )
    ...
    def _change_pharmacy_type(self):
        ...
        pharmacy_type, ok = QInputDialog.getItem(
            self, "調劑費申報方式", "請選擇調劑費申報方式, 更改前請先註記病歷", items, 0, False
        )
    ...
    def _change_injury_type(self):
        ...
        injury_type, ok = QInputDialog.getItem(
            self, "主訴職災申報方式", "請選擇主訴職災申報方式, 更改前請先註記病歷", items, 0, False
        )
    ...
    def _change_doctor(self):
        ...
        doctor, ok = QInputDialog.getItem(
            self, "變更主治醫師", "請選擇主治醫師姓名, 更改前請先註記病歷", items, 0, False
        )
    ...
    def _header_clicked(self, col_no):
        ...
        for row_no in range(row_count):
            try:
                check_box = self.ui.tableWidget_medical_record_list.cellWidget(row_no, col_no)
    ...
    def _set_traditional_health_case(self):
        ...
        if ins_type == '健保':
            case_utils.write_traditional_health_care(
                database, self.system_settings, case_key,
                traditional_health_care_fee=None, massager=None
            )
            self._read_medical_record_list(self.sql, self.medicine_list)
    ...
    def _print_correction_reg_income(self):
        printer_utils.print_correction_reg_income(
            ...
            self.ui.tableWidget_medical_record_list, self.column)
    ...
    def _export_correction_reg_income_txt(self):
        ...
        export_utils.export_correction_reg_income_txt(
            self.database, self.system_settings, txt_filename,
            self.ui.tableWidget_medical_record_list, self.column)
    ...
    def _calculate_total(self):
        ...
        for row_no in range(row_count):
            ...
            try:
                discount_fee += number_utils.get_integer(
                    self.ui.tableWidget_medical_record_list.item(row_no, self.column['DiscountFee']).text())
            except Exception:
                pass

            try:
                diag_fee += number_utils.get_integer(
                    self.ui.tableWidget_medical_record_list.item(row_no, self.column['DiagFee']).text())
            except Exception:
                pass

            try:
                inter_drug_fee += number_utils.get_integer(
                    self.ui.tableWidget_medical_record_list.item(row_no, self.column['InterDrugFee']).text())
            except Exception:
                pass

            try:
                pharmacy_fee += number_utils.get_integer(
                    self.ui.tableWidget_medical_record_list.item(row_no, self.column['PharmacyFee']).text())
            except Exception:
                pass

            try:
                treat_fee += number_utils.get_integer(
                    self.ui.tableWidget_medical_record_list.item(row_no, self.column['TreatFee']).text())
            except Exception:
                pass

            try:
                ins_apply_fee += number_utils.get_integer(
                    self.ui.tableWidget_medical_record_list.item(row_no, self.column['InsApplyFee']).text())
            except Exception:
                pass

        total_record = [
            None, None, None, None, '合計', None, None, None, None, None, None,
        ...
        for col_no in range(len(total_record)):
            ...
            self.ui.tableWidget_medical_record_list.item(row_count, col_no).setFont(font)
    ...
    def _show_all_cases(self):
        ...
        self.sql = self.sql.replace('AND (Position1 IS NULL OR LENGTH(Position1) = 0)', '')
        rows = self.database.select_record(self.sql)
        self.medical_record_rows = len(rows)
        self._read_medical_record_list(self.sql, self.medicine_list)
    ...
    def open_ins_medical_record(self):
        ...
        database = self._get_archive_database()
        self.parent.open_medical_record(case_key, '病歷查詢健保病歷', archive_database=database)

    def _print_referral_form(self):
        database = self._get_archive_database()
        patient_key = self.table_widget_medical_record_list.field_value(self.column['PatientKey'])
        case_key = self.table_widget_medical_record_list.field_value(self.column['CaseKey'])

        printer_utils.print_referral_form(self, database, self.system_settings, patient_key, case_key, None)

    def _is_today(self):
        today = date_utils.date_to_str()
        case_date = self.table_widget_medical_record_list.field_value(self.column['CaseDate'])[:10]
        if case_date != today:
            system_utils.show_message_box(
                QMessageBox.Critical,
                'ic卡作業錯誤',
    ...
    def _rewrite_ic_card(self):
        ...
        case_key = self.table_widget_medical_record_list.field_value(self.column['CaseKey'])
        patient_key = self.table_widget_medical_record_list.field_value(self.column['PatientKey'])
        share_type = self.table_widget_medical_record_list.field_value(self.column['Share'])
        card = self.table_widget_medical_record_list.field_value(self.column['Card'])
    ...
    def _export_case_to_word(self):
        ...
        filename = dialog_utils.get_save_dialog_filename(self, '匯出病歷', default_filename)
        ...
        export_utils.export_case_to_word(self.database, filename, case_key_list=case_key_list)
    ...
    def _set_case(self, option):
        ...
        for row_no in range(self.ui.tableWidget_medical_record_list.rowCount()):
            ...
            try:
                check_box_close_mark = self.ui.tableWidget_medical_record_list.cellWidget(
                    row_no, self.column['PrintMark'])
            ...
            self.database.exec_sql(sql)
            case_utils.set_close_case_icon(
                self.ui.tableWidget_medical_record_list, row_no, self.column['DoctorDone'], True,
            )
            self.ui.tableWidget_medical_record_list.setCellWidget(
