...
class PurchaseList(QtWidgets.QMainWindow):
    ...
    def _set_signal(self):
        ...
        self.ui.tableWidget_purchase_list.doubleClicked.connect(
    ...
    def _set_permission(self):
        ...
        if (
            personnel_utils.get_permission(
                self.database, self.program_name, "購買商品", self.user_name
            )
            != "Y"
        ):
        ...
        if (
            personnel_utils.get_permission(
                self.database, self.program_name, "購藥明細", self.user_name
            )
            != "Y"
        ):
    ...
    # 讀取病歷
    def _get_sql(self):
        dialog = dialog_utils.get_dialog_purchase_list(
    ...
    def _set_table_data(self, row_no, row):
        ...
        for col_no in range(len(purchase_row)):
            ...
            self.ui.tableWidget_purchase_list.setItem(row_no, col_no, item)
            if col_no in [3, 6, 7, 8]:
                self.ui.tableWidget_purchase_list.item(row_no, col_no).setTextAlignment(
    ...
    def _export_to_excel(self):
        ...
        export_utils.export_table_widget_to_excel(
            excel_file_name, self.ui.tableWidget_purchase_list, [0], [3, 6, 7, 8], title
        )
