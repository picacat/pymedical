...
class StatisticsDoctorMedicinePercent(QtWidgets.QMainWindow):
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_STATISTICS_DOCTOR_MEDICINE_PERCENT, self)
        system_utils.set_css(self, self.system_settings)
        system_utils.center_window(self)
        self.table_widget_doctor = class_utils.get_table_widget(self.ui.tableWidget_doctor, self.database)
        self.table_widget_commission = class_utils.get_table_widget(self.ui.tableWidget_commission, self.database)
    ...
    def _set_table_width(self):
        width = [
            100, 100,
            120, 60, 90, 80, 60, 200, 60, 70, 60, 90, 90, 60, 60, 90,
        ]
    ...
    def _calculate_data(self):
        ...
        for row_no, row in enumerate(rows):
            ...
            ui_utils.set_combo_box(combo_box_medicine_type, list(self.medicine_type_dict.keys()), None)
            if medicine_type in self.medicine_type_dict:
                combo_box_medicine_type.setCurrentText(medicine_type)
                commission_rate = number_utils.get_integer(self.medicine_type_dict[medicine_type])
            ...
            self._set_item_data(
                row_no, 2, row['CaseDate'].date().strftime('%Y-%m-%d'))
            ...
            self.ui.tableWidget_doctor.setCellWidget(row_no, 12, combo_box_medicine_type)
    ...
    def _set_commission(self, combo_box_medicine_type, medicine_type):
        ...
        commission_rate = number_utils.get_integer(self.medicine_type_dict[medicine_type])
        amount = number_utils.get_integer(self.ui.tableWidget_doctor.item(row_no, 11).text())
        commission =  round(amount * (commission_rate / 100))        
    ...
    def _set_item_data(self, row_no, col_no, data, align='left', table_widget_commission=False):
        ...
        if col_no > 0 and number_utils.get_integer(data) < 0:
            tableWidget.item(row_no, col_no).setForeground(
                QtGui.QColor('red')
            )
    ...
    def _calculate_commission_total(self):
        ...
        self._set_item_data(row_count, 1, total_fee, align='right', table_widget_commission=True)
        self._set_item_data(row_count, 3, total_commission, align='right', table_widget_commission=True)
    ...
    def _calculate_medicine_name_total(self):
        for row_no in range(self.ui.tableWidget_doctor.rowCount()):
            ...
            amount = number_utils.get_integer(self.ui.tableWidget_doctor.item(row_no, 11).text())
            quantity = number_utils.get_integer(self.ui.tableWidget_doctor.item(row_no, 8).text())
            pres_days = number_utils.get_integer(self.ui.tableWidget_doctor.item(row_no, 10).text())
            ...
            self._set_item_data(
                commission_row_no, 1, string_utils.xstr(total_quantity + last_quantity),
                align='right', table_widget_commission=True)
            
            last_amount = self._get_commission_cell_value(commission_row_no, 2)
            self._set_item_data(
                commission_row_no, 2, string_utils.xstr(amount + last_amount),
                align='right', table_widget_commission=True)
    ...
    def _get_commission_row_no(self, medicine_name):
        for row_no in range(self.ui.tableWidget_commission.rowCount()):
            try:
                currnet_medicine_name = self.ui.tableWidget_commission.item(row_no, 0).text()
            except Exception:
                continue
    ...
    def _calculate_medicine_type(self, medicine_type):
        ...
        self._set_item_data(row_count, 1, total_fee, align='right', table_widget_commission=True)
        self._set_item_data(row_count, 2, commission_rate, align='right', table_widget_commission=True)        
        self._set_item_data(row_count, 3, total_commission, align='right', table_widget_commission=True)
