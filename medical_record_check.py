...
from libs import (case_utils, date_utils, dialog_utils, nhi_utils,
                  number_utils, prescript_utils, registration_utils,
...
class MedicalRecordCheck(QtWidgets.QDialog):
    ...
    def _get_card(self):
        try:
            card = self.parent.tab_registration.comboBox_card.currentText().split(' ')[0]
        except Exception:
    ...
    def _check_disease1_error(self, treat_type, disease_code1, disease_code2=None):
        ...
        if treat_type == '腦血管疾病':
            ...
            if disease_code1[:3] == 'J45':  #
                ...
                error_message = '* ICD-10-CM主診斷碼非小兒氣喘病名<br>小兒氣喘主診斷碼範圍: J45~'
            ...
            if disease_code1[:3] in [
               'C18', 'C19', 'C20', 'C21', 'C22', 'C23', 'C24', 'C33', 'C34', 'C50']:
                pass
            ...
            if nhi_utils.is_breast_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_breast_cancer(disease_code2):
                pass
            else:
            ...
            if nhi_utils.is_liver_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_liver_cancer(disease_code2):
                pass
            else:
            ...
            if nhi_utils.is_lung_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_lung_cancer(disease_code2):
                pass
            else:
            ...
            if nhi_utils.is_colorectal_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_colorectal_cancer(disease_code2):
                pass
            else:
            ...
            if nhi_utils.is_stomach_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_stomach_cancer(disease_code2):
                pass
            else:
                error_message = '* ICD-10-CM主診斷碼非胃癌病名<br>胃癌主診斷碼範圍: C16~<br>'
        elif treat_type == '攝護腺癌照護':
            if nhi_utils.is_prostate_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_prostate_cancer(disease_code2):
                pass
            else:
            ...
            if nhi_utils.is_oral_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_oral_cancer(disease_code2):
                pass
            else:
                error_message = '* ICD-10-CM主診斷碼非口腔癌病名<br>口腔癌主診斷碼範圍: C01~C10<br>'
        elif treat_type == '子宮頸癌照護':
            if nhi_utils.is_cervical_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_cervcial_cancer(disease_code2):
                pass
            else:
                error_message = '* ICD-10-CM主診斷碼非子宮頸癌病名<br>子宮頸癌主診斷碼範圍: C53<br>'
        elif treat_type == '子宮體癌照護':
            if nhi_utils.is_endometrial_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_endometrial_cancer(disease_code2):
                pass
            else:
                error_message = '* ICD-10-CM主診斷碼非子宮體癌病名<br>子宮體癌主診斷碼範圍: C54<br>'
        elif treat_type == '甲狀腺癌照護':
            if nhi_utils.is_thyroid_cancer(disease_code1):
                pass
            elif nhi_utils.is_main_cancer(disease_code1) and nhi_utils.is_thyroid_cancer(disease_code2):
                pass
            else:
                error_message = '* ICD-10-CM主診斷碼非甲狀腺癌病名<br>甲狀腺癌主診斷碼範圍: C73<br>'
        elif treat_type == '慢性腎病照護':
            if disease_code1[:4] in ['N182', 'N183', 'N184', 'N185', 'N186']:  # 慢性腎病
                pass
            else:
    ...
    def _check_pres_days_error(treat_type, pres_days):
        ...
        if treat_type in nhi_utils.PREGNANT_CARE_TREAT and pres_days < 7:
            error_message = f'* {treat_type}未開立七天以上的內服藥, 請開立內服藥至少七天'
            ...
            error_message = f'* {treat_type}照護未開立五天以上的內服藥, 請開立內服藥至少五天'
    ...
    def _check_child_care(self):
        ...
        error = self._check_disease1_error(self.treat_type, self.disease_code1, self.disease_code2)
    ...
    def _check_no_pharmacy(self):
        ...
        if total_medicine_set == 1:  # 只有一組藥品不正確
            system_utils.show_message_box(
                QMessageBox.Warning,
                '健保開藥調劑檢查',
                f'<b><font size="5" color="red">錯誤! 本次病歷只有健保處方，且設定為不調劑</font></b>',
                '請注意! 請確認調劑方式是否設定為不調劑.',
                button_text='了解'
            )
            return False
    ...
    def _check_general(self):
        ...
        if self.system_settings.field('檢查損傷診斷碼') == 'Y' and self.treat_type not in ['內科', '醫療諮詢']:
    ...
    def _check_deposit_fee(self):
        ...
        if self.ins_apply_fee > self.deposit_fee:
            ...
            msg_box.setInformativeText("請注意! 可以考慮調漲欠卡費或減少開藥天數以降低損失")
    ...
    def _check_treat_type(self):
        ...
        if self.treatment in [None, ''] and self.second_treatment in [None, ''] and course >= 2:
            card = string_utils.xstr(self.medical_record['Card'])
            warning_message.append(f'本次掛號卡序為 {card}-{course} 但病歷為內科病歷，請重新取得內科卡序!')
    ...
    def _check_dosage(self):
        ...
        for row_no in range(row_count):
            ...
            if unit not in ['', None] and unit.text() not in ['', None] and (dosage is None or dosage.text().strip() == ''):
                error_message.append(f'{medicine_name} 無劑量')
                break

            try:
                dosage_value = number_utils.get_float(dosage.text().strip())
                if self.system_settings.field('健保開藥劑量必須大於0') == 'Y' and dosage_value <= 0:
        ...
        if self.medical_record['Injury'] in nhi_utils.INFECTIOUS_TYPE or \
           self.medical_record['Share'] in nhi_utils.INFECTIOUS_TYPE:  # 確診病歷不設限
            return check_ok
        ...
        minimum_dosage2 = number_utils.get_integer(self.system_settings.field('6歲以下最低劑量'))
        ...
        elif number_utils.get_integer(age_year) >= 6 and \
                minimum_dosage is not None and 0 < total_dosage < minimum_dosage:  # 6歲以上低於最低劑量
            error_message.append(f'6歲以上用藥少於系統設定內的最低劑量{minimum_dosage}克')
        elif number_utils.get_integer(age_year) < 6 and \
                minimum_dosage2 is not None and 0 < total_dosage < minimum_dosage2:  # 6歲以下低於最低劑量
            error_message.append(f'6歲以下用藥少於系統設定內的最低劑量{minimum_dosage2}克')
    ...
    def _check_empty_disease(self):
        ...
        if self.disease_code1 == '' and self.disease_code2 == '' and \
                self.disease_code3 == '' and self.disease_code4 == '':
    ...
    def _check_unavailable_disease(self):
        ...
        if self.disease_code1 in ['U099', 'Z8616']:
            error_message.append(f'* 主診斷碼不可申報{self.disease_code1}, 請輸入在次診斷碼!')
            ...
            if self.disease_code1[:1] in ['V', 'W', 'X', 'Y', 'Z']:
                error_message.append(f'主診斷碼不可申報{self.disease_code1}, 請選擇其他病名.')
    ...
    def _check_course_disease(self):
        ...
        if disease_code1 != '' and self.disease_code1[:3] != last_disease_code1[:3]:  # 只檢查前三碼
        ...
        if len(error_message) > 0:
            ...
            if self.system_settings.field('療程不同病名不能存檔') == 'Y':
                msg_box.addButton(QPushButton(f"請修改主診斷碼為 {disease_code1} 後再存檔"), QMessageBox.NoRole)
                msg_box.exec_()
                return False
            else:
                msg_box.addButton(QPushButton("繼續存檔"), QMessageBox.YesRole)
    ...
    def _check_same_disease_course2(self):
        ...
        case_date = self.medical_record['CaseDate']
        last_treat_date = (case_date - datetime.timedelta(days=30)).strftime('%Y-%m-01 00:00:00')
        case_key = self.medical_record['CaseKey']
        patient_key = self.medical_record['PatientKey']
        sql = f'''
            SELECT
    ...
    def _check_course_pres_days_once(self):
        ...
        if number_utils.get_integer(self.medical_record['Continuance']) <= 1:  # 療程首次不用檢查
            return check_ok
    ...
    def _check_prescript(self):
        ...
        total_dosage, infectious_drug = prescript_utils.get_total_dosage(self.table_widget_ins_prescript)
        if infectious_drug:
            return check_ok
    ...
    def _check_prescript_same_three_times(self):
        ...
        case_date = self.medical_record['CaseDate'].date()
        end_date = (datetime.date(case_date.year, case_date.month, 1) - datetime.timedelta(days=1)).replace(day=1)
        ...
        if sorted(prescript_list[0][1]) == sorted(prescript_list[1][1]) == sorted(prescript_list[2][1]):
    ...
    def _check_disease_neat(self):
        ...
        case_date =self.medical_record['CaseDate'].strftime('%Y-%m-%d')  # 2024-10-01以前不要檢查
        ...
        for disease_code in disease_code_list:
            ...
            if not case_utils.is_disease_code_neat(self.database, disease_code):
                ...
                if self.medical_record['CaseDate'].year >= 2016:  # ICD-10-CM 2016年開始實施
    ...
    def _check_special_code(self):
        ...
        if self.medical_record['Share'] in nhi_utils.INFECTIOUS_INJURY_TYPE:  # 清冠一號開藥不受限制
            return check_ok
    ...
    def _check_complicated_acupuncture(self):
        ...
        if self.disease_code1 in complicated_acupuncture_list1 or \
           self.disease_code2 in complicated_acupuncture_list1 or \
           self.disease_code3 in complicated_acupuncture_list1 or \
           self.disease_code4 in complicated_acupuncture_list1:
            return check_ok
    ...
    def _check_moderate_complicated_acupuncture(self):
        ...
        if not case_utils.is_moderate_complicated_acupuncture_ok(
           self.disease_code1, self.disease_code2, self.disease_code3, self.disease_code4,
           self.parent.parent.moderate_complicated_acupuncture_list, self.parent.parent.special_disease_list):
            error_message.append('診斷碼無中度複雜性針灸適應症')

        if '中度針灸合併中度傷科' in self.treatment:
            if not case_utils.is_moderate_complicated_massage_ok(
               self.disease_code1, self.disease_code2, self.disease_code3, self.disease_code4,
               self.parent.parent.moderate_complicated_massage_list, self.parent.parent.special_disease_list):
    ...
    def _check_highly_complicated_acupuncture(self):
        ...
        # 2025-05-02 陳立德
        if 'G82' in self.disease_code2 or 'G83' in self.disease_code2 or \
            'G82' in self.disease_code3 or 'G83' in self.disease_code3 or \
            'G82' in self.disease_code4 or 'G83' in self.disease_code4:
            ...
            if 'B91' not in self.disease_code2:
                error_message.append('診斷碼G82或G83需合併B91<br>急性脊髓灰白質炎之後期影響併有提及麻痺性徵候(小兒麻痺症)')
    ...
    def _check_complicated_massage(self):
        ...
        if self.disease_code1 in complicated_massage_list1 or \
                self.disease_code2 in complicated_massage_list1 or \
                self.disease_code3 in complicated_massage_list1 or \
                self.disease_code4 in complicated_massage_list1:
            return check_ok
    ...
    def _check_massage_treatment_once(self, treatment):
        ...
        if len(rows) > 0:
            row = rows[0]
            error_message.append(f'已經在 {row["CaseDate"].date()} 執行過{treatment}處置')
    ...
    def _check_ins_medicine(self):
        ...
        if self.pres_days < 2:
            if (self.medical_record['Share'] in nhi_utils.INFECTIOUS_TYPE or
            ...
            regist_type = self.parent.tab_registration.comboBox_reg_type.currentText()
            if self.medical_record['RegistType'] in nhi_utils.SPECIAL_PHARMACY_TYPE or \
                    regist_type in nhi_utils.SPECIAL_PHARMACY_TYPE:
                pass
            else:
    ...
    def _check_complicated_acupuncture_treats(self):
        ...
        for row_no in range(self.table_widget_ins_treat.rowCount()):
            ...
            if self.treatment in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_LIST and '治療時間:' in treat_name:
    ...
    def _check_complicated_massage_treats(self):
        ...
        for row_no in range(self.table_widget_ins_treat.rowCount()):
            ...
            if self.treatment in nhi_utils.HIGHLY_COMPLICATED_MASSAGE_TREAT and '治療時間:' in treat_name:
                treat_name = string_utils.removeprefix(treat_name, '治療時間:')
                treat_name = string_utils.removesuffix(treat_name, '分鐘')
                if number_utils.get_integer(treat_name) < 20:
    ...
    def _check_merge_treats(self):
        if self.treatment in ['一般針灸', '電針'] and self.second_treatment in ['一般傷科']:
            return True
        ...
        for row_no in range(self.table_widget_ins_treat.rowCount()):
            ...
            if treat_times and treat_name is not None and '治療時間' in treat_name:
                ...
                if self.treatment in ['一般針灸', '電針', '中度複雜性針灸'] and \
                        self.second_treatment in ['一般傷科', '中度複雜性傷科'] and \
                        number_utils.get_integer(treat_name) < 10:
                    error_message.append('治療時間不足10分鐘')
                elif self.treatment in ['一般針灸', '電針', '中度複雜性針灸'] and \
                        self.second_treatment in ['高度複雜性傷科'] and \
                        number_utils.get_integer(treat_name) < 20:
                    error_message.append('治療時間不足20分鐘')
                elif self.treatment in ['高度複雜性針灸'] and \
                        self.second_treatment in ['一般傷科', '中度複雜性傷科'] and \
                        number_utils.get_integer(treat_name) < 20:
                    error_message.append('治療時間不足20分鐘')
                elif self.treatment in ['高度複雜性針灸'] and \
                        self.second_treatment in ['高度複雜性傷科', '中度複雜性傷科合併特殊疾病', '脫臼整復復位', '骨折復位'] and \
                        number_utils.get_integer(treat_name) < 20:
                    error_message.append('治療時間不足20分鐘')
    ...
    def _check_acupuncture_level(self):
        ...
        if self.treatment in nhi_utils.MODERATE_COMPLICATED_ACUPUNCTURE_LIST and \
                previous_treatment in nhi_utils.GENERAL_ACUPUNCTURE_TREAT:
            error_message.append('上次針灸治療為一般針灸，本次針灸治療不能為中度複雜性針灸')
        elif self.treatment in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_LIST and \
                previous_treatment in nhi_utils.GENERAL_ACUPUNCTURE_TREAT:
            error_message.append('上次針灸治療為一般針灸，本次針灸治療不能為高度複雜性針灸')
        elif self.treatment in nhi_utils.HIGHLY_COMPLICATED_ACUPUNCTURE_LIST and \
                previous_treatment in nhi_utils.MODERATE_COMPLICATED_ACUPUNCTURE_LIST:
            error_message.append('上次針灸治療為中度複雜性針灸，本次針灸治療不能為高度複雜性針灸')
