...
class StatisticsDoctorMedicine(QtWidgets.QMainWindow):
    '''醫師處方類別抽成統計 2025.08.04 仁聿.'''
    
    def __init__(self, parent=None, *args):
        super(StatisticsDoctorMedicine, self).__init__(parent)
        self.parent = parent
    ...
    def _set_tab_widget(self, start_date, end_date, period, ins_type, doctor, option, weekday_list):
        ...
        self._add_statistic_doctor_medicine_percent(start_date, end_date, period, ins_type, doctor, option, weekday_list)

    # 醫師門診人數統計
    def _add_statistic_doctor_medicine_percent(self, start_date, end_date, period, ins_type, doctor, option, weekday_list):
        self.tab_statistics_doctor_medicine_percent = module_utils.get_statistics_doctor_medicine_percent(
            self, self.database, self.system_settings, start_date, end_date, period, ins_type,
            doctor, option, weekday_list
        )
        self.tab_statistics_doctor_medicine_percent.start_calculate()
        self.ui.tabWidget_statistics_doctor.addTab(self.tab_statistics_doctor_medicine_percent, '醫師處方類別抽成統計')
