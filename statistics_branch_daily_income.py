...
class StatisticsBranchDailyIncome(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(
            ui_utils.UI_STATISTICS_BRANCH_DAILY_INCOME, self
        )
    ...
    def _calculate_regist_fee(self):
        ...
        self.ui.tableWidget_regist_fee.setColumnCount(
            clinic_count * col_data_length + 3
        )
        ...
        self._set_table_item(
            ...
            0,
            self.ui.tableWidget_regist_fee.columnCount() - 1,
            "總計",
        )
        ...
        for i, clinic_name in enumerate(self.database_list):
            ...
            for j, col_data in enumerate(col_data_list):
                self._set_table_item(
                    self.ui.tableWidget_regist_fee, 1, col_no + j, col_data
                )
            ...
            for row_no in range(start_row_no, start_row_no + len(self.period_list)):
                ...
                ins_regist_fee, self_regist_fee, deposit_fee = (
                    self._calculate_total_regist_fee(rows, period)
                )
                ...
                self._set_table_item(
                    self.ui.tableWidget_regist_fee, row_no, col_no, ins_regist_fee
                )
                self._set_table_item(
                    self.ui.tableWidget_regist_fee, row_no, col_no + 1, deposit_fee
                )
                self._set_table_item(
                    self.ui.tableWidget_regist_fee, row_no, col_no + 2, self_regist_fee
                )

            self._set_table_item(
                self.ui.tableWidget_regist_fee,
                start_row_no + len(self.period_list),
                1,
                "小計",
            )
            self._set_table_item(
    ...
    def _calculate_diag_share_fee(self):
        ...
        self.ui.tableWidget_diag_share_fee.setColumnCount(
            clinic_count * col_data_length + 3
        )
        ...
        self._set_table_item(
            ...
            0,
            self.ui.tableWidget_diag_share_fee.columnCount() - 1,
            "總計",
        )
        ...
        for i, clinic_name in enumerate(self.database_list):
            ...
            col_no = col_data_length * i + 2
            self._set_table_item(
                self.ui.tableWidget_diag_share_fee, 0, col_no, clinic_name
            )
            if col_data_length >= 2:
                self.ui.tableWidget_diag_share_fee.setSpan(
                    0, col_no, 1, col_data_length
                )

            for j, col_data in enumerate(col_data_list):
                self._set_table_item(
                    self.ui.tableWidget_diag_share_fee, 1, col_no + j, col_data
                )
            ...
            for row_no in range(start_row_no, start_row_no + len(self.period_list)):
                ...
                self._set_table_item(
                    self.ui.tableWidget_diag_share_fee, row_no, 1, period
                )
                self._set_table_item(
                    self.ui.tableWidget_diag_share_fee, row_no, col_no, diag_share_fee
                )
    ...
    def _calculate_drug_share_fee(self):
        ...
        self.ui.tableWidget_drug_share_fee.setColumnCount(
            clinic_count * col_data_length + 3
        )
        ...
        self._set_table_item(
            ...
            0,
            self.ui.tableWidget_drug_share_fee.columnCount() - 1,
            "總計",
        )
        ...
        for i, clinic_name in enumerate(self.database_list):
            ...
            col_no = col_data_length * i + 2
            self._set_table_item(
                self.ui.tableWidget_drug_share_fee, 0, col_no, clinic_name
            )
            if col_data_length >= 2:
                self.ui.tableWidget_drug_share_fee.setSpan(
                    0, col_no, 1, col_data_length
            ...
            for j, col_data in enumerate(col_data_list):
                self._set_table_item(
                    self.ui.tableWidget_drug_share_fee, 1, col_no + j, col_data
                )
            ...
            for row_no in range(start_row_no, start_row_no + len(self.period_list)):
                ...
                self._set_table_item(
                    self.ui.tableWidget_drug_share_fee, row_no, 1, period
                )
                self._set_table_item(
                    self.ui.tableWidget_drug_share_fee, row_no, col_no, drug_share_fee
                )
    ...
    def _calculate_self_total_fee(self):
        ...
        self.ui.tableWidget_self_total_fee.setColumnCount(
            clinic_count * col_data_length + 3
        )
        ...
        self._set_table_item(
            ...
            0,
            self.ui.tableWidget_self_total_fee.columnCount() - 1,
            "總計",
        )
        ...
        for i, clinic_name in enumerate(self.database_list):
            ...
            col_no = col_data_length * i + 2
            self._set_table_item(
                self.ui.tableWidget_self_total_fee, 0, col_no, clinic_name
            )
            if col_data_length >= 2:
                self.ui.tableWidget_self_total_fee.setSpan(
                    0, col_no, 1, col_data_length
            ...
            for j, col_data in enumerate(col_data_list):
                self._set_table_item(
                    self.ui.tableWidget_self_total_fee, 1, col_no + j, col_data
                )
            ...
            for row_no in range(start_row_no, start_row_no + len(self.period_list)):
                ...
                self._set_table_item(
                    self.ui.tableWidget_self_total_fee, row_no, 1, period
                )
                self._set_table_item(
                    self.ui.tableWidget_self_total_fee, row_no, col_no, self_total_fee
                )
    ...
    def _calculate_project_fee(self):
        ...
        for clinic_name in self.database_list:
            ...
            for row in rows:
                ...
                if row_no is None:
                    row_no = self._append_table_item(
                        self.ui.tableWidget_project, 0, project_name
                    )
                ...
                cell_value = self._get_cell_value(
                    self.ui.tableWidget_project, row_no, col_no
                )
                project_fee = amount * pres_days - discount_fee
                cell_value += project_fee

                self._set_table_item(
                    self.ui.tableWidget_project, row_no, col_no, cell_value
                )
    ...
    def _calculate_total_income(self):
        ...
        for row_no in range(2, self.ui.tableWidget_regist_fee.rowCount()):
            ...
            self._set_table_item(
                self.ui.tableWidget_total_income, row_no - 2, 0, regist_fee
            )
        ...
        for row_no in range(2, self.ui.tableWidget_diag_share_fee.rowCount()):
            ...
            self._set_table_item(
                self.ui.tableWidget_total_income, row_no - 2, 1, diag_share_fee
            )
        ...
        for row_no in range(2, self.ui.tableWidget_drug_share_fee.rowCount()):
            ...
            self._set_table_item(
                self.ui.tableWidget_total_income, row_no - 2, 2, drug_share_fee
            )
        ...
        for row_no in range(2, self.ui.tableWidget_self_total_fee.rowCount()):
            ...
            self._set_table_item(
                self.ui.tableWidget_total_income, row_no - 2, 3, self_total_fee
            )
        ...
        for col_no in range(1, self.ui.tableWidget_project.columnCount()):
            ...
            self._set_table_item(
                self.ui.tableWidget_total_income, col_no - 1, 4, project_fee
            )
    ...
    def _append_table_item(tableWidget, col_no, data):
        ...
        tableWidget.item(row_count, col_no).setTextAlignment(
