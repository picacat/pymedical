...
from libs import (case_utils, charge_utils, date_utils, nhi_utils,
...
class InsApplyXML(QtWidgets.QMainWindow):
    ...
    def _add_tdata(self, root):
        ...
        t29.text = string_utils.xstr(self.ins_total_fee['tcm_count'])             # 中醫案件件數小計
        t30 = ET.SubElement(tdata, 't30')
        t30.text = string_utils.xstr(self.ins_total_fee['tcm_amount'])            # 中醫案件點數小計

        t33 = ET.SubElement(tdata, 't33')
        t33.text = string_utils.xstr(self.ins_total_fee['chronical_count'])             # 慢箋件數小計
        t34 = ET.SubElement(tdata, 't34')
        t34.text = string_utils.xstr(self.ins_total_fee['chronical_amount'])            # 慢箋點數小計
    ...
    def _add_dbody(self, ddata, row):
        ...
        for i in range(1, nhi_utils.MAX_DISEASE_CODE + 1):
            ...
            if disease_code != '':
                if not self.pre_ins_apply and case_date.year <= 2024 and self.dict_icd_map is not None:  # 2024以舊版申報
                    try:
        ...
        if string_utils.xstr(row['CaseType']) == '30':  # 腦血管疾病
            default_pharmacy_fee = charge_utils.get_ins_pharmacy_fee(
                self.database, self.system_settings, drug_fee, '申報',
            )
            if number_utils.get_integer(row['PharmacyFee']) > default_pharmacy_fee:
                pharmacy_code = ''
            elif number_utils.get_integer(row['PharmacyFee']) > 0 and pharmacy_code.strip() == '':
            ...
            pharmacy_code = charge_utils.get_ins_code_from_charge_settings(self.database, '照護費', '中醫門診延長照護藥品調劑費')
        ...
        if self.system_settings.field('資源類別') in nhi_utils.AT_LACK_AREA or regist_type in nhi_utils.AT_LACK_AREA:
            lack_area = '01'
            d52 = ET.SubElement(dbody, 'd52')
            d52.text = self._get_name(lack_area)
        elif self.system_settings.field('資源類別') in nhi_utils.GOTO_LACK_AREA or regist_type in nhi_utils.GOTO_LACK_AREA:
            lack_area = '02'
            d52 = ET.SubElement(dbody, 'd52')
            d52.text = self._get_name(lack_area)
            try:
                ...
                pass
        elif regist_type in nhi_utils.TOUR_MOUNTAIN_ISLAND and treat_type in nhi_utils.HOME_CARE:  # 山地離島居家醫療訪視費 2022.09.09 龍潭安聲
            lack_area = '04'
            d52 = ET.SubElement(dbody, 'd52')
            d52.text = self._get_name(lack_area)
            try:
    ...
    def _add_pdata(self, dbody, row):
        ...
        for course in range(1, nhi_utils.MAX_COURSE+1):
            ...
            case_type = string_utils.xstr(row['CaseType'])
            if case_type not in ['28'] and case_utils.get_case_extend(self.database, case_key, '整合醫療照護') == 'Y':  # 慢箋不能報
            ...
            try:
                ...
                if case_row['CaseDate'].strftime('%Y-%m-%d') >= '2023-03-01' and age_year < 7 and course == 1 and \
                        string_utils.xstr(case_row['Treatment']) in nhi_utils.MASSAGE_TREAT:
            ...
            if treat_type in nhi_utils.PREGNANT_CARE_TREAT:  # 孕產照護不放針傷處置代碼  2024-12-18 陳立德 岐伯齋
                pass
            elif case_type not in ['28'] and treat_code != '':
                if string_utils.xstr(row['CaseType']) == '22':  # 特定照護: 孕產照護，肝乳癌照護, 癌症中醫門診延長照護
                ...
                self._set_treatment(dbody, row, case_row, course, treat_code, order_type, identifier)
            ...
            if case_type == '25' and string_utils.xstr(row['SpecialCode1']) == 'EC':
                ...
                self._set_treatment(dbody, row, case_row, course, diag_code, '居家醫療', identifier)

            prescript_rows = self._get_prescript_rows(case_key)
            if len(prescript_rows) > 0:
                self._set_prescript(dbody, row, case_row, prescript_rows, case_key, course, identifier)
    ...
    def _set_integrate_care(self, dbody, row, case_row, identifier):
        ...
        case_date = date_utils.west_date_to_nhi_date(row['CaseDate'])
        try:
            start_time, end_time = case_utils.get_integrate_case_time(self.database, case_key)
        except Exception:
            start_time = '0000'
            end_time = '0000'
    ...
    def _get_treat_datetime(self, case_key, case_row):
        ...
        try:
            start_time = prescript_utils.get_treat_time(self.database, case_key, '治療開始:')
            end_time = prescript_utils.get_treat_time(self.database, case_key, '治療結束:')
        except Exception:
            start_time = '0000'
            end_time = '0000'
    ...
    def _set_treatment(self, dbody, row, case_row, course, treat_code, order_type, identifier):
        pdata = ET.SubElement(dbody, 'pdata')
        p17_code = '2'  # 同一療程
        p19_code = ''   # 事前審查受理編號
        total_dosage = 1  # 總量
        ...
        try:
            if order_type == '居家醫療':  # redefine
                ...
                unit_price = charge_utils.get_ins_fee_from_ins_code(
                    self.database, 'E5012C', case_date=case_date)  # 台灣清冠一號補助費
                total_dosage = case_utils.get_pres_days(self.database, case_row['CaseKey'])
                amount = unit_price * total_dosage
                percent = 100
                p17_code = '4'
                infectious_drug_code = prescript_utils.get_infectious_drug_code(self.database, case_key)
                if infectious_drug_code not in ['', None]:
                    p19_code = infectious_drug_code
                else:
                    p19_code = charge_utils.get_ins_code_from_infectious_drug(self.database, '清冠一號')
            elif order_type == '遠距診療費':  # redefine
                order_type = '2'
                amount = charge_utils.get_ins_fee_from_ins_code(self.database, 'E5204C', case_date=case_date)  # 遠距診療費
                percent = 100
                unit_price = amount
                p17_code = '4'
            else:
                ...
                treat_code = string_utils.xstr(row[f'TreatCode{course}'])
                unit_price = number_utils.get_integer(
                    charge_utils.get_ins_fee_from_ins_code(self.database, treat_code, case_date=case_date)
                )
                if string_utils.xstr(case_row['TreatType']) in \
                        nhi_utils.MISC_CARE_TREAT + nhi_utils.PREGNANT_CARE_TREAT + nhi_utils.CANCER_CARE_TREAT:
                    total_dosage = 0
                    amount = 0
        except KeyError:
            amount = 0
        ...
        if treat_code in nhi_utils.COMPLICATED_TREAT_CODE:
            treat_position = prescript_utils.get_treat_position(self.database, case_key, '治療部位:')
            if treat_position != '':
        ...
        start_date, end_date, start_time, end_time = self._get_treat_datetime(case_key, case_row)
    ...
    def _set_auxiliary_treat(self, dbody, row, case_row, field_value, identifier):
        ...
        for prescript_row in prescript_rows:
            ...
            self._set_auxiliary_treat_row(dbody, row, case_row, ins_code, start_date, end_date, identifier)

    def _set_auxiliary_treat_row(self, dbody, row, case_row, ins_code, start_date, end_date, identifier):
        pdata = ET.SubElement(dbody, 'pdata')
    ...
    def _set_prescript(self, dbody, row, case_row, prescript_rows, case_key, course, identifier, set_A21=True):
        ...
        if number_utils.get_integer(row['PharmacyFee']) > 0:
            if string_utils.xstr(row['CaseType']) in ['30', '31']:  # 腦血管疾病加強照護, 居家照護
                self._set_auxiliary_pharmacy(dbody, row, case_row, pres_days, course, identifier)
            else:
                self._set_pharmacy(dbody, row, case_row, pres_days, course, identifier)

        for prescript_row in prescript_rows:
            self._set_medicine(
                dbody, row, case_row, prescript_row, pres_days, packages, instruction, identifier,
            )
    ...
    def _set_infectious_virtual_code(self, dbody, row, case_row, identifier):
        infectious_date = case_utils.get_case_extend(self.database, case_row['CaseKey'], '確診日期')
    ...
    def _set_auxiliary_pharmacy(self, dbody, row, case_row, pres_days, course, identifier):
        if pres_days <= 0:
            return
        ...
        on_duty, pharmacist = nhi_utils.pharmacist_schedule_on_duty(self.database, case_row['CaseKey'])
        if on_duty:
            item_name = '藥師調劑'
        else:
            item_name = '醫師調劑'
        ...
        if pharmacist is not None:
            p16.text = personnel_utils.get_person_field_value(self.database, pharmacist, 'ID')
        else:
            p16.text = string_utils.xstr(case_row['DoctorID'])
    ...
    def _set_medicine(self, dbody, row, case_row, prescript_row, pres_days, packages, instruction, identifier):
    ...
    def _add_auxiliary_case(self, dbody, row):
        ...
        self.sequence = 0
        identifier = case_utils.get_identifier(self.database, case_key, '就醫識別碼')
        self._set_auxiliary_case(dbody, row, rows[0], '2', identifier)  # order_type = 2 診療明細, 4 = 不另計價
        ...
        for case_row in rows:
            ...
            if len(prescript_rows) > 0:
                self._set_prescript(
                    ...
                    case_key, number_utils.get_integer(case_row['Continuance']), identifier,
                    set_A21=set_A21,
                )
    ...
    def _add_infectious_case(self, dbody, row):
        ...
        infectious_drug_fee = charge_utils.get_ins_fee_from_ins_code(
            self.database, 'E5012C', case_date=case_row['CaseDate'])  # 台灣清冠一號補助費
        pres_days = case_utils.get_pres_days(self.database, case_row['CaseKey'])

        if ins_total_fee == (infectious_drug_fee * pres_days):
            self._set_treatment(dbody, row, case_row, course, 'E5012C', '台灣清冠一號藥品補助費', identifier)
            ...
            if treat_fee > 0:
                self._set_treatment(dbody, row, case_row, course, 'E5204C', '遠距診療費', identifier)

            self._set_infectious_virtual_code(dbody, row, case_row, identifier)
            self._set_covid19(dbody, row, case_row, pres_days, identifier, ins_code='ViT-COVID19')
