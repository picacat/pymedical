...
class DictTongue(QtWidgets.QMainWindow):
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_DICT_TONGUE, self)
        ...
        self.table_widget_dict_tongue = class_utils.get_table_widget(self.ui.tableWidget_dict_tongue, self.database)
    ...
    def dict_groups_name_changed(self):
        ...
        self.ui.groupBox_dict_tongue.setTitle(f'{self.dict_type}資料 - [{dict_groups_name}]')
    ...
    def _remove_tongue(self):
        tongue_name = self.table_widget_dict_tongue.field_value(3)
        msg_box = dialog_utils.get_message_box(
            f'刪除{self.dict_type}資料',
            QMessageBox.Warning,
            f'<font size="5" color="red"><b>確定刪除{self.dict_type}: "{tongue_name}"?</b></font>',
            '注意！資料刪除後, 將無法回復!'
        )
        remove_record = msg_box.exec_()
        if not remove_record:
    ...
    def _edit_tongue(self):
        clinic_key = self.table_widget_dict_tongue.field_value(0)
