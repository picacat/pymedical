...
# 執行業務所得 2024.07.19 天地精進
class StatisticsBusinessIncome(QtWidgets.QMainWindow):
    # 初始化
    def __init__(self, parent=None, *args):
        super(StatisticsBusinessIncome, self).__init__(parent)
        self.parent = parent
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_STATISTICS_BUSINESS_INCOME, self)
    ...
    def _set_tab_widget(self, start_date, end_date, period, ins_type, doctor, option, weekday_list):
        self.ui.tabWidget_statistics_business_income.clear()

        self.ui.statusbar.showMessage(
            f' 統計期間: 從 {start_date[:10]} 至 {end_date[:10]} {period} 保險: {ins_type} 醫師: {doctor}'
        )

        self._add_statistic_business_income_list(start_date, end_date, period, ins_type, doctor, option, weekday_list)

    # 醫師門診人數統計
    def _add_statistic_business_income_list(self, start_date, end_date, period, ins_type, doctor, option, weekday_list):
        self.tab_statistics_business_income_list = module_utils.get_statistics_business_income_list(
            self, self.database, self.system_settings, start_date, end_date, period, ins_type,
            doctor, option, weekday_list
        )
        self.tab_statistics_business_income_list.start_calculate()
        self.ui.tabWidget_statistics_business_income.addTab(self.tab_statistics_business_income_list, '執行業務所得統計')
