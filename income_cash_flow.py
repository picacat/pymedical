...
class IncomeCashFlow(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_registration = class_utils.get_table_widget(
            self.ui.tableWidget_registration, self.database
        )
        self.table_widget_registration.set_column_hidden([0])

        self.table_widget_charge = class_utils.get_table_widget(
            self.ui.tableWidget_charge, self.database
        )
        self.table_widget_charge.set_column_hidden([0])

        self.table_widget_total = class_utils.get_table_widget(
            self.ui.tableWidget_total, self.database
        )
        self.table_widget_payment_type = class_utils.get_table_widget(
            self.ui.tableWidget_payment_type, self.database
        )
    ...
    def _set_payment_type_table(self):
        ...
        for row_no, payment_type in enumerate(payment_type_list):
            self.ui.tableWidget_payment_type.setItem(
                row_no, 0, QtWidgets.QTableWidgetItem(payment_type)
            )
            self.ui.tableWidget_payment_type.setItem(
                row_no, 1, QtWidgets.QTableWidgetItem("0")
            )
    ...
    def _set_registration_fees(self):
        ...
        self.table_widget_registration.set_db_data(
            sql, self._set_registration_table_data
        )
    ...
    def _set_registration_table_data(self, row_no, row):
        ...
        for col_no in range(len(medical_record)):
            ...
            if col_no in [3, 10, 11, 12, 13, 14, 15, 16, 19]:
                self.ui.tableWidget_registration.item(row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
            elif col_no in [2]:
                self.ui.tableWidget_registration.item(row_no, col_no).setTextAlignment(
            ...
            if string_utils.xstr(row["InsType"]) == "自費":
                self.ui.tableWidget_registration.item(row_no, col_no).setForeground(
                    QtGui.QColor("blue")
                )
    ...
    def _set_refund_table_data(self, row_no, row):
        ...
        for col_no in range(len(medical_record)):
            self.ui.tableWidget_registration.setItem(
                row_no, col_no, QtWidgets.QTableWidgetItem(medical_record[col_no])
            )
    ...
    def _set_repayment_table_data(self, row_no, row):
        ...
        if row["ReturnDate1"] is not None:
            if case_date == string_utils.xstr(row["ReturnDate1"].strftime("%Y-%m-%d")):
                if period == "全部" or (
                    period != "全部" and period == string_utils.xstr(row["Period1"])
                ):
        ...
        if row["ReturnDate2"] is not None:
            if case_date == string_utils.xstr(row["ReturnDate2"].strftime("%Y-%m-%d")):
                if period == "全部" or (
                    period != "全部" and period == string_utils.xstr(row["Period2"])
                ):
        ...
        if row["ReturnDate3"] is not None:
            if case_date == string_utils.xstr(row["ReturnDate3"].strftime("%Y-%m-%d")):
                if period == "全部" or (
                    period != "全部" and period == string_utils.xstr(row["Period3"])
                ):
    ...
    def _calculate_registration_total(self):
        ...
        for row_no in range(row_count):
            regist_fee += number_utils.get_integer(
                self.ui.tableWidget_registration.item(row_no, 10).text()
            )
            diag_share_fee += number_utils.get_integer(
                self.ui.tableWidget_registration.item(row_no, 11).text()
            )
            deposit_fee += number_utils.get_integer(
                self.ui.tableWidget_registration.item(row_no, 12).text()
            )
            refund_fee += number_utils.get_integer(
                self.ui.tableWidget_registration.item(row_no, 13).text()
            )
            debt_fee += number_utils.get_integer(
                self.ui.tableWidget_registration.item(row_no, 14).text()
            )
            repayment += number_utils.get_integer(
                self.ui.tableWidget_registration.item(row_no, 15).text()
            )
            subtotal += number_utils.get_integer(
                self.ui.tableWidget_registration.item(row_no, 16).text()
            )
    ...
    def _set_charge_table_data(self, row_no, row):
        ...
        for col_no in range(len(medical_record)):
            ...
            if col_no in [3, 10, 12, 13, 14, 15, 16, 19]:
                self.ui.tableWidget_charge.item(row_no, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
            elif col_no in [2]:
                self.ui.tableWidget_charge.item(row_no, col_no).setTextAlignment(
            ...
            if total_fee > 0:
                self.ui.tableWidget_charge.item(row_no, col_no).setForeground(
                    QtGui.QColor("blue")
                )
        ...
        if debt_fee < 0:
            self.ui.tableWidget_charge.item(row_no, 15).setForeground(
                QtGui.QColor("red")
            )
        if drug_share_fee + total_fee + debt_fee != subtotal:
            self.problem_records.append([name, total_fee, debt_fee, receipt_fee])
    ...
    def _calculate_charge_total(self):
        ...
        for row_no in range(row_count):
            drug_share_fee += number_utils.get_integer(
                self.ui.tableWidget_charge.item(row_no, 12).text()
            )
            total_fee += number_utils.get_integer(
                self.ui.tableWidget_charge.item(row_no, 13).text()
            )
            receipt_fee += number_utils.get_integer(
                self.ui.tableWidget_charge.item(row_no, 14).text()
            )
            debt_fee += number_utils.get_integer(
                self.ui.tableWidget_charge.item(row_no, 15).text()
            )
            subtotal += number_utils.get_integer(
                self.ui.tableWidget_charge.item(row_no, 16).text()
            )
        ...
        for col_no in range(len(total_record)):
            ...
            if col_no in [12, 13, 14, 15, 16]:
                self.ui.tableWidget_charge.item(row_count, col_no).setTextAlignment(
                    QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                )
    ...
    def _calculate_total(self):
        ...
        total_fee = number_utils.get_integer(
            self.ui.tableWidget_registration.item(
                self.ui.tableWidget_registration.rowCount() - 1, 16
            ).text()
        ) + number_utils.get_integer(
            self.ui.tableWidget_charge.item(
                self.ui.tableWidget_charge.rowCount() - 1, 16
            ).text()
        )
        ...
        total_rows = [
            [
                "掛號費",
                self.ui.tableWidget_registration.item(
                    self.ui.tableWidget_registration.rowCount() - 1, 10
                ).text(),
            ],
            [
                "門診負擔",
                self.ui.tableWidget_registration.item(
                    self.ui.tableWidget_registration.rowCount() - 1, 11
                ).text(),
            ],
            [
                "藥品負擔",
                self.ui.tableWidget_charge.item(
                    self.ui.tableWidget_charge.rowCount() - 1, 12
                ).text(),
            ],
            [
                "欠卡費",
                self.ui.tableWidget_registration.item(
                    self.ui.tableWidget_registration.rowCount() - 1, 12
                ).text(),
            ],
            [
                "還卡費",
                self.ui.tableWidget_registration.item(
                    self.ui.tableWidget_registration.rowCount() - 1, 13
                ).text(),
            ],
            [
                "自費還款",
                self.ui.tableWidget_registration.item(
                    self.ui.tableWidget_registration.rowCount() - 1, 15
                ).text(),
            ],
            [
                "自費金額",
                self.ui.tableWidget_charge.item(
                    self.ui.tableWidget_charge.rowCount() - 1, 13
                ).text(),
            ],
            [
                "批價收費",
                self.ui.tableWidget_charge.item(
                    self.ui.tableWidget_charge.rowCount() - 1, 14
                ).text(),
            ],
            [
                "掛號欠款",
                self.ui.tableWidget_registration.item(
                    self.ui.tableWidget_registration.rowCount() - 1, 14
                ).text(),
            ],
            [
                "批價欠款",
                self.ui.tableWidget_charge.item(
                    self.ui.tableWidget_charge.rowCount() - 1, 15
                ).text(),
            ],
        ...
        for row_no, row in enumerate(total_rows):
            ...
            for col_no in range(len(row)):
                ...
                if col_no in [1]:
                    self.ui.tableWidget_total.item(row_no, col_no).setTextAlignment(
                        QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
                    )
                if row_no in [4, 8, 9]:
                    self.ui.tableWidget_total.item(row_no, col_no).setForeground(
                        QtGui.QColor("red")
                    )
    ...
    def _calculate_payment_type(self):
        ...
        for row_no in range(self.ui.tableWidget_payment_type.rowCount() - 1):
            total_fee = number_utils.get_integer(
                self.ui.tableWidget_payment_type.item(row_no, 1).text()
            )
