    ...
    def _check_data(self):
        ...
        for row_no in range(row_count):
            ...
            try:
                ...
                next_disease_code = self.ui.tableWidget_errors.item(
                    row_no + 1, 8
                ).text()
            ...
            if patient_key != next_patient_key:  # 換人
                ...
                if share_type in ["山地離島"]:
                    ...
                    if (
                        ...
                        is_new_course = False
                        for i in range(
                            row_no, self.ui.tableWidget_errors.rowCount() + 1
                        ):
                            check_next_patient_key = self.ui.tableWidget_errors.item(
                                i + 1, 3
                            )
                            ...
                            if (
                                self.ui.tableWidget_errors.item(i + 1, 7) is not None
                                and number_utils.get_integer(
                                    self.ui.tableWidget_errors.item(i + 1, 7).text()
                                )
                                > course
                            ):
                                is_new_course = True
                        ...
                        if (
                            is_new_course
                            and next_treat_type not in nhi_utils.ALL_CARE_TREAT
                        ):
                            error_message.append("療程中刷卡")
                    elif (
                        treat_type in nhi_utils.INS_TREAT and next_treat_type == "內科"
                    ) or (
                    ...
                    if (
                        treat_type in nhi_utils.INS_TREAT
                        and next_treat_type == "內科"
                        and disease_code == next_disease_code
                    ):
                        if (
                            treat_type in nhi_utils.ALL_CARE_TREAT
                            or next_treat_type in nhi_utils.ALL_CARE_TREAT
                        ):
                            pass
                        else:
                            if (
                                ...
                                and next_case_date is not None
                                and case_date[5:7] != next_case_date[5:7]
                            ):  # 不同月份不要檢查
                                pass
                    ...
                    if course <= 1 and next_course <= 1:
                        delta = date_utils.str_to_date(
                            next_case_date
                        ) - date_utils.str_to_date(case_date)
                        if delta.days == 1:
            ...
            if date_utils.str_to_date(case_date).month != self.apply_month:
                ...
                for i in range(1, 32):
                    try:
                        ...
                        next_patient_key = self.ui.tableWidget_errors.item(
                            row_no + i, 3
                        ).text()
                        next_card = self.ui.tableWidget_errors.item(
                            row_no + i, 6
                        ).text()
                        next_course = number_utils.get_integer(
                            self.ui.tableWidget_errors.item(row_no + i, 7).text()
