...
class ChargeSettingsShare(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_diag_share = class_utils.get_table_widget(
            self.ui.tableWidget_diag_share, self.database
        )
        self.table_widget_diag_share.set_column_hidden([0, 1])
        self.table_widget_drug_share = class_utils.get_table_widget(
            self.ui.tableWidget_drug_share, self.database
        )
    ...
    def _set_diag_share_data(self, rec_no, rec):
        ...
        for column in range(len(diag_share_rec)):
            self.ui.tableWidget_diag_share.setItem(
                rec_no, column, QtWidgets.QTableWidgetItem(diag_share_rec[column])
            )
            if column in [7]:
    ...
    def _diag_share_add(self):
        dialog = dialog_utils.get_dialog_input_share(
            self, self.database, self.system_settings, None, "門診負擔"
        )
        result = dialog.exec_()
        if result != 0:
    ...
    def _diag_share_delete(self):
        ...
        msg_box.setText(
            "<font size='4' color='red'><b>確定刪除此筆門診負擔資料?</b></font>"
        )
        ...
        self.ui.tableWidget_diag_share.removeRow(
            self.ui.tableWidget_diag_share.currentRow()
        )
    ...
    def _set_drug_share_data(self, rec_no, rec):
        ...
        for column in range(len(drug_share_rec)):
            self.ui.tableWidget_drug_share.setItem(
                rec_no, column, QtWidgets.QTableWidgetItem(drug_share_rec[column])
            )
            if column in [5]:
    ...
    def _drug_share_add(self):
        dialog = dialog_utils.get_dialog_input_share(
            self, self.database, self.system_settings, None, "藥品負擔"
        )
        result = dialog.exec_()
        if result != 0:
    ...
    def _drug_share_delete(self):
        ...
        msg_box.setText(
            "<font size='4' color='red'><b>確定刪除此筆藥品負擔資料?</b></font>"
        )
        msg_box.setInformativeText("注意！資料刪除後, 將無法回復!")
        msg_box.addButton(QPushButton("取消"), QMessageBox.NoRole)
        msg_box.addButton(QPushButton("確定"), QMessageBox.YesRole)
        ...
        self.database.delete_record("charge_settings", "ChargeSettingsKey", key)
        self.ui.tableWidget_drug_share.removeRow(
            self.ui.tableWidget_drug_share.currentRow()
        )
