...
class DialogExportEMRXml(QtWidgets.QDialog):
    ...
    def _get_xml_file_name(self, xml_file_path, row):
        ...
        xml_file_name = f'{doctor_part}-{foxconn_id}- {nhi_utils.INS_CLASS}-{patient_part}.xml'
    ...
    def _add_registration_data(self, encounter, row):
        ...
        if doc_name != '':
            doc_id = personnel_utils.get_person_field_value(self.database, doc_name, 'ID')
    ...
    def _add_drugs(self, root, row):
        ...
        for prescript_row in rows:
            ...
            packages = case_utils.get_packages(self.database, case_key, medicine_set=medicine_set)
            ...
            instruction = case_utils.get_instruction(self.database, case_key, medicine_set=medicine_set)
            ...
            drug_start_date.text = f'{start_date.year}{start_date.month:0>2}{start_date.day:0>2}'
            drug_end_date = ET.SubElement(drug, 'DrugEndDate')
            drug_end_date.text = f'{end_date.year}{end_date.month:0>2}{end_date.day:0>2}'
