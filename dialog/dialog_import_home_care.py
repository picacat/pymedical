...
class DialogImportHomeCare(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None, *args):
        ...
        items = ['請求居家輕量藍牙就醫檔案', '下載居家輕量藍牙就醫資料', '匯入居家輕量藍牙就醫檔案']
    ...
    def _nhi_dowload_b(self, file_date):
        ...
        local_id, nhi_id = nhi_utils.NHI_DownloadB(self.system_settings, type_code, dest_file)
        json_dict = {
            'local_id': local_id,
            'nhi_id': nhi_id,
        }
    ...
    def _append_patient(self, json_dict):
        ...
        fields = ['Name', 'ID', 'Gender', 'Birthday', 'CardNo', 'InsType', 'Nationality', 'InitDate']
        data = [
            patient_name, patient_id, gender, birthday, card_no, share_type, nationality, init_date]
    ...
    def _get_patient_html(self, row):
        ...
        disease_name1 = f'主診斷: {case_utils.get_disease_name(self.database, disease_code1)}'
        disease_name2 = f'次診斷1: {case_utils.get_disease_name(self.database, disease_code2)}'
        disease_name3 = f'次診斷2: {case_utils.get_disease_name(self.database, disease_code3)}'
    ...
    def _set_medical_record(self, json_row):
        ...
        if pos >= 0:
            sam_id = json_dict['ic_xml'][pos+len(sam_id_tag):pos+len(sam_id_tag)+12]
        else:
            sam_id = None
    ...
    def _write_treatment_signature(self, case_key, json_row):
        ...
        data = [
            case_key, '處置簽章', json_row['treatment_signature'],
        ]
    ...
    def _write_ins_fee(self, case_key):
        ...
        fields = [
            ...
            'InsTotalFee', 'DiagShareFee', 'DrugShareFee',
            'InsApplyFee', 'AgentFee',
        ]
        data = [
            ins_fee['diag_fee'], ins_fee['drug_fee'], ins_fee['pharmacy_fee'],
            ins_fee['acupuncture_fee'], ins_fee['massage_fee'], ins_fee['dislocate_fee'],
            ins_fee['ins_total_fee'], ins_fee['diag_share_fee'], ins_fee['drug_share_fee'],
            ins_fee['ins_apply_fee'], ins_fee['agent_fee'],
        ]
    ...
    def _get_security(self, json_row):
        ...
        security = case_utils.update_xml_doc(
            security, 'seq_number', json_row['card'])
        security = case_utils.update_xml_doc(
        ...
        security = case_utils.update_xml_doc(
            security, 'sam_id', json_row['sam_id'])
        security = case_utils.update_xml_doc(
    ...
    def _write_cases(self, json_row):
        ...
        fields = [
            'CaseDate', 'DoctorDate', 'ChargeDate', 'PatientKey', 'Name', 'Visit', 'RegistType', 'Injury',
            'TreatType', 'Share', 'InsType', 'Card', 'Period', 'ChargePeriod', 'RegistNo',
            'ApplyType', 'PharmacyType',
            'Symptom', 'Treatment',
            'DiseaseCode1', 'DiseaseName1',
            'DiseaseCode2', 'DiseaseName2',
            'DiseaseCode3', 'DiseaseName3',
            'Doctor', 'Security', 'DoctorDone', 'ChargeDone',
        ]

        data = [
            case_date, case_date, case_date,
            patient_row['PatientKey'],
            patient_row['Name'],
            '複診', '一般門診',
            json_row['injury_type'], json_row['treat_type'], json_row['share_type'],
            '健保', json_row['card'],
            period, period, 0,
            '申報', '申報' if self.system_settings.field('申報藥事服務費') == 'Y' else '不申報',
            json_row['symptom'], json_row['treatment'],
            disease_code1, disease_name1,
            disease_code2, disease_name2,
            disease_code3, disease_name3,
            json_row['doctor'], security,
            'True', 'True',
        ]
