...
try:
    ...
    system_utils.pip3_install('PyQtWebEngine')
    from PyQt5.QtWebEngineWidgets import QWebEngineView
...
# 主視窗
class DialogMedicineLibrary(QtWidgets.QDialog):
    # 初始化
    def __init__(self, parent=None, *args):
        super(DialogMedicineLibrary, self).__init__(parent)
        self.parent = parent
        self.database = args[0]
        self.system_settings = args[1]
        self.medicine_name = args[2]
        self.medicine_type = args[3]
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_DIALOG_BROWSER, self)
        self.setFixedSize(self.size())  # non resizable dialog
        system_utils.set_css(self, self.system_settings)
        self.web_widget = QWebEngineView(self)
        self.placeholder = self.findChildren(QWidget)[0]
        self.layout = self.placeholder.layout()
    ...
    def _search_medicine(self):
        url = self.get_medicine_url()
        print(url)
        if url:
            self.web_widget.setUrl(QUrl(url))
            self.web_widget.setZoomFactor(1.5)  # 放大到 150%
            self.layout.addWidget(self.web_widget)

    def get_medicine_url(self):
        if self.medicine_type in ['單方', '水藥']:
        ...
        try:
            clean_name = re.sub(r"\s*\(.*\)", "", self.medicine_name)
