...
class DialogCertificatePayment(QtWidgets.QDialog):
    ...
    def _set_signal(self):
        ...
        self.ui.spinBox_certificate_fee.valueChanged.connect(self._calculate_certificate_fee)
        self.ui.spinBox_certificate_quantity.valueChanged.connect(self._calculate_certificate_fee)
    ...
    def _set_combo_box(self):
        ui_utils.set_combo_box(self.ui.comboBox_ins_type,
                               nhi_utils.INS_TYPE + ['自費(含健保內的自費)'], '全部')
    ...
    def accepted_button_clicked(self):
        ...
        end_date = self.ui.tableWidget_medical_record.item(self.ui.tableWidget_medical_record.rowCount() - 1, 2).text()

        if certificate_utils.check_certificate_duplicate(self.database, '收費證明', patient_key, start_date, end_date):
            msg_box = dialog_utils.get_message_box(
                '重複開立證明', QMessageBox.Warning,
    ...
    def _read_medical_record(self):
        ...
        if self.table_widget_medical_record.row_count() > 0 and self.ui.checkBox_accept.isChecked():
    ...
    def _save_files(self):
        ...
        if self.ui.checkBox_create_medical_record.isChecked() or self.ui.checkBox_print_cert_fee_only.isChecked():
        ...
        if self.ui.checkBox_create_medical_record.isChecked():
            ...
            certificate_utils.insert_prescript(self.database, case_key, unit_price, quantity)
            certificate_utils.insert_wait(self.database, self.system_settings, case_key, patient_key, name)
    ...
    def _is_merge_case(self, row, certificate_key):
        ...
        cert_massage_fee = number_utils.get_integer(certificate_items_row['SMassageFee']) + s_massage_fee
        self_total_fee = number_utils.get_integer(certificate_items_row['SelfTotalFee']) + s_massage_fee
        total_fee = number_utils.get_integer(certificate_items_row['TotalFee']) + s_massage_fee
        receipt_fee = number_utils.get_integer(certificate_items_row['ReceiptFee']) + s_massage_fee
        sql = f'''
    ...
    def _auto_create_certificate_payment(self):
        try:
            ...
            start_date = date_utils.str_to_date(date_utils.date_to_west_date(self.auto_create_list[4]))
            end_date = date_utils.str_to_date(date_utils.date_to_west_date(self.auto_create_list[5]))
    ...
    def _calculate_certificate_fee(self):
        certificate_fee = self.ui.spinBox_certificate_fee.value() * self.ui.spinBox_certificate_quantity.value()
        self.ui.lineEdit_certificate_fee.setText(string_utils.xstr(certificate_fee))
