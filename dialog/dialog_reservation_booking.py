...
from libs import (alleypin_utils, date_utils, dialog_utils, hainachuan_utils,
                  nhi_utils, number_utils, patient_utils, personnel_utils,
                  printer_utils, registration_utils, string_utils,
...
class DialogReservationBooking(QtWidgets.QDialog):
    ...
    def _set_ui(self):
        ...
        self.ui.buttonBox.button(QtWidgets.QDialogButtonBox.Yes).setText('存檔列印預約單')
    ...
    def _set_reserve_type(self):
        reserve_type_list = ['現場預約', '初診預約', '視訊預約', '視訊初診預約', '特殊預約']
        try:
    ...
    def check_validation(self):
        ...
        if patient_id.strip() != '' and \
                self.ui.lineEdit_name.text().strip() != '':
            button_enabled = True
        else:
            button_enabled = False

        self.ui.buttonBox.button(QtWidgets.QDialogButtonBox.Yes).setEnabled(button_enabled)
    ...
    def _dialog_button_clicked(self, sender):
        ...
        if self.system_settings.field('alleypin') == 'Y':
            alleypin_utils.add_reservation_alleypin_appointments(self.database, self.system_settings, reserve_key)
    ...
    def _auto_get_reservation_no(self):
        ...
        last_reservation_no = self._get_last_reservation_no(start_date, end_date, period)
        start_no = number_utils.get_integer(self.system_settings.field(f'{period}起始號'))
    ...
    def _save_reserve_record(self):
        ...
        room = registration_utils.get_room(self.database, period, doctor, weekday=weekday)
    ...
    def _write_temp_patient(self):
        fields = [
            'Name', 'ID', 'Birthday', 'PhoneNo', 'Cellphone', 'Address',
        ]
    ...
    def _set_patient_data(self, rows):
        ...
        if self.reservation_date[:10] == today and \
                registration_utils.is_today_already_visited(self.database, patient_key) and \
                self.system_settings.field('同日預約兩次') != 'Y':
            msg_box = QMessageBox()
        ...
        if registration_utils.is_in_permission_list(
                self.database, patient_key, '黑名單', self.ui.lineEdit_reservation_date.text()[:10]):
            msg_box = QMessageBox()
            msg_box.setIcon(QMessageBox.Critical)
    ...
    def _check_reservation_status(self, patient_key, name):
        ...
        message = registration_utils.check_prescription_finished(       # 檢查上次健保給藥是否服藥完畢
            self.database, self.system_settings, None, patient_key, in_date=reservation_date,
        )
    ...
    def _check_reservation_limit(self, patient_key, name):
        ...
        if registration_utils.is_in_permission_list(
                self.database, patient_key, '白名單', self.ui.lineEdit_reservation_date.text()[:10]):
            return is_ok

        reservation_limit = number_utils.get_integer(self.system_settings.field('預約次數限制'))

        if self.system_settings.field('預約次數不同醫師分別計算') == 'Y':
            reservation_list = self._get_reservation_list(patient_key, name, self.doctor)
        else:
            reservation_list = self._get_reservation_list(patient_key, name)  # 2023-09-11 馥林: 所有醫師一起統計
        ...
        if self.doctor in ['三伏貼', '三九貼']:  # 三伏貼不要限制次數  2025-06-13 安聲
            ...
            msg_box.setText(f'<h3>此人預約已超過預約次數{reservation_limit}次的限制, 無法再次預約掛號.</h4>')
    ...
    def _get_week_day_name(self, region='zh_TW'):
        date_obj = datetime.datetime.strptime(self.reservation_date[:10], "%Y-%m-%d").date()
