    ...
    def read_dictionary(self, medicine_type=None):
        ...
        if self.system_settings.field("處方詞庫僅列出水藥") == "Y":
            ...
            if current_row > 0:  # 第一筆不要限制
                herb_type_item = self.table_widget_prescript.item(
                    0, prescript_utils.PRESCRIPT_COL_NO["MedicineType"]
                )
                unit_item = self.table_widget_prescript.item(
                    0, prescript_utils.SELF_PRESCRIPT_COL_NO["Unit"]
                )
                if herb_type_item is not None:
                    herb_type = herb_type_item.text()

                if unit_item is not None:
                    unit_type = unit_item.text()

        if (herb_type is not None and herb_type == "水藥") or (
            unit_type is not None and unit_type == "錢"
        ):
            # medicine_type = f'AND (MedicineType IN ("{herb_type}", "成方"))'
            medicine_type = f'AND (MedicineType IN ("{herb_type}"))'
        ...
        #     medicine_type = 'AND (MedicineType NOT IN ("穴道", "處置", "檢驗"))'
            ...
            if (
                ...
                in nhi_utils.ACUPUNCTURE_TREAT
            ):
                medicine_type = """
                    AND (MedicineType in ("穴道", "處置", "外用") OR (MedicineType = "成方" AND Unit = "次"))
                """
            elif (
                self.parent.comboBox_treatment.currentText() in nhi_utils.MASSAGE_TREAT
            ):
                medicine_type = """
            ...
            medicine_type = (
                "AND (MedicineType IS NOT NULL AND LENGTH(MedicineType) > 0)"
            )
