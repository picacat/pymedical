...
class DialogMedicalRecordList(QtWidgets.QDialog):
    ...
    def _set_signal(self):
        ...
        self.ui.toolButton_select_patient.clicked.connect(
            lambda: self._select_patient(None)
        )
        self.ui.toolButton_select_patient_list.clicked.connect(
            self._select_patient_list
        )
        ...
        self.ui.groupBox_advance_search.toggled.connect(
            self._group_box_advance_search_clicked
        )
    ...
    def _set_combo_box(self):
        ...
        ui_utils.set_combo_box(
            self.ui.comboBox_share_type, nhi_utils.SHARE_TYPE, "全部"
        )
        ui_utils.set_combo_box(
            self.ui.comboBox_injury_type, nhi_utils.INJURY_TYPE, "全部"
        )
        ui_utils.set_combo_box(
            self.ui.comboBox_apply_type, nhi_utils.APPLY_TYPE, "全部"
        )
    ...
    def _get_condition(self, order_by=False):
        ...
        #     if self.ui.checkBox_enable_end_date.isChecked():
        #         condition.append(
        #             f'(cases.CaseDate BETWEEN "{start_date}" AND "{end_date}")'
        #         )
        #     else:
        ...
        if self.ui.radioButton_range_date.isChecked():
            if self.ui.checkBox_enable_end_date.isChecked():
                condition.append(
                    f'(cases.CaseDate >= "{start_date}" AND cases.CaseDate <= "{end_date}")'
                )
            else:
                # 原本是 DATE(cases.CaseDate) = "..."。把欄位包在函式裡，
                # CaseDate 的索引就完全用不到，每次查詢都要掃描整張 cases。
                # 改成半開區間 [當日 00:00:00, 次日 00:00:00)，語意完全相同
                # 但可以直接走索引範圍掃描。
                next_day = (
                    self.ui.dateEdit_start_date.date()
                    .addDays(1)
                    .toString("yyyy-MM-dd 00:00:00")
                )
                condition.append(
                    f'(cases.CaseDate >= "{start_date}" '
                    f'AND cases.CaseDate < "{next_day}")'
                )
            ...
            if len(weekday_list) > 0:
                condition.append(
                    f"WEEKDAY(cases.CaseDate) IN({','.join(weekday_list)})"
                )

        if self.system_settings.field("病歷查詢預設健保") == "Y":
            condition.append('cases.InsType = "健保"')
        elif ins_type != "全部":
        ...
        if keyword != "":
            if os.path.exists(keyword):
                ...
                if pattern.match(keyword):
                    ...
                    condition.append(
                        f"cases.PatientKey BETWEEN {patient_key_list[0]} AND {patient_key_list[1]}"
                    )
                    order_method = "patient_key"
                else:
