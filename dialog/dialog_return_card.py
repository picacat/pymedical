...
class DialogReturnCard(QtWidgets.QDialog):
        ...
        if card in nhi_utils.ABNORMAL_CARD:
            ...
            if self.doctor_done:
                if self.system_settings.field('讀卡機控制軟體版本') == 'cshis6':
                    ic_card.write_ic_medical_record(self.case_key, cshis_utils.NORMAL_CARD, reset_vhc_card=False)
                else:
    ...
    def _write_ic_card(self, treat_after_check):
        ...
        if use_vhc_card:
            case_utils.set_case_extend(self.database, self.case_key, '健保卡種類', '虛擬健保卡')
    ...
    def update_cases_by_ic_card(self, ic_card, card=None):
        ...
        if card in [None, '', '自動產生', '欠卡']:
            ...
            self.database.exec_sql(f'UPDATE cases SET Card = "{card}" WHERE CaseKey = {self.case_key}')
