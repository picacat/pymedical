...
class DialogInputDrug(QtWidgets.QDialog):
    ...
    def _edit_medicine(self):
        ...
        bonus = medicine_utils.get_medicine_extend(self.database, self.medicine_key, '療程實現贈送')
    ...
    def _update_medicine(self):
        ...
        self.database.update_record('medicine', fields, 'MedicineKey', self.medicine_key, data)

        bonus = self.ui.lineEdit_bonus.text().strip()
        extend_type = '療程實現贈送'
        if bonus == '':
            medicine_utils.remove_medicine_extend(self.database, self.medicine_key, extend_type)
        else:
            medicine_utils.set_medicine_extend(self.database, self.medicine_key, extend_type, bonus)
    ...
    def _get_person_list(self):
        ...
        extra_list = ['醫師', '推拿師父', '櫃台', '醫師分成', '推拿師父分成', '櫃台分成', '廠房']
    ...
    def _add_commission(self):
        ...
        dialog = dialog_utils.get_dialog_commission(self, self.database, self.system_settings, person_list)
        if not dialog.exec_():
            dialog.deleteLater()
            return
