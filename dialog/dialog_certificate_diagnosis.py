...
class DialogCertificateDiagnosis(QtWidgets.QDialog):
    ...
    def _set_signal(self):
        ...
        self.ui.textEdit_doctor_comment.textChanged.connect(
    ...
    def _read_medical_record(self):
        ...
        doctor_clause = " Doctor IS NOT NULL AND LENGTH(Doctor) > 0 "
        ...
        if self.ui.checkBox_include_purchase.isChecked():
            ...
            doctor_condition = f"""
                AND (
                    ({doctor_clause}) OR
                    TreatType IN ("自購", "開立證明")
                )
            """
        else:
            purchase_condition = ' AND TreatType NOT IN ("自購", "開立證明")'
            doctor_condition = f" AND ({doctor_clause}) "
        ...
        if self.ui.tableWidget_medical_record.rowCount() <= 0:
            ...
            self.ui.tableWidget_medical_record.setItem(
                0, 4, QtWidgets.QTableWidgetItem("查無病歷")
            )
    ...
    def add_order(self, doctor_comment):
        comment_line = []
        comment = self.ui.textEdit_doctor_comment.toPlainText().replace(
            "(以下空白 This space intentionally left blank)", ""
        )
        if comment != "":
    ...
    def _check_diagnosis_completed(self):
        ...
        if (
            diagnosis != ""
            and doctor_comment != ""
            and self.ui.tableWidget_medical_record.rowCount() > 0
        ):
    ...
    def _read_certificate(self):
        ...
        self.ui.spinBox_certificate_fee.setValue(
            number_utils.get_integer(row["CertificateFee"])
        )
