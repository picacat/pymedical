...
class DictDrug(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        ...
        self.table_widget_dict_drug = class_utils.get_table_widget(
            self.ui.tableWidget_dict_drug, self.database
        )
        self.table_widget_in_price = class_utils.get_table_widget(
            self.ui.tableWidget_in_price, self.database
        )
    ...
    def _set_signal(self):
        ...
        self.ui.tableWidget_dict_drug.itemSelectionChanged.connect(
        ...
        self.ui.lineEdit_search_drug.keyPressEvent = (
            self._line_edit_search_drug_package_key_press
        )
    ...
    def dict_groups_changed(self):
        dict_groups_type = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_name"]
        )
        self.ui.toolButton_remove_dict_groups.setEnabled(True)
        self.ui.toolButton_edit_dict_groups.setEnabled(True)
        if dict_groups_type in ["單方", "複方", "水藥", "外用", "高貴", "器材"]:
    ...
    def _read_dict_drug(self, dict_groups_type, keyword=None):
        ...
        medicine_key = self.table_widget_dict_drug.field_value(
            self.medicine_col_no["medicine_key"]
        )
    ...
    def _add_dict_groups(self):
        ...
        field = ["DictGroupsType", "DictOrderNo", "DictGroupsName", "DictGroupsLevel2"]
    ...
    def _remove_dict_groups(self):
        dict_groups = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_name"]
        )
        ...
        key = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_key"]
        )
        self.database.delete_record("dict_groups", "DictGroupsKey", key)
        self.ui.tableWidget_dict_groups.removeRow(
            self.ui.tableWidget_dict_groups.currentRow()
        )

    # 更改處方類別抽成
    def _edit_percent(self):
        dict_groups_key = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_key"]
        )
        if dict_groups_key == "":
            return
        ...
        old_percent = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_percent"]
        )
    ...
    def _edit_dict_groups(self):
        old_groups = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_name"]
        )
        ...
        self.database.update_record(
            ...
            fields,
            "DictGroupsKey",
            self.table_widget_dict_groups.field_value(
                self.groups_col_no["dict_groups_key"]
            ),
            data,
        )
    ...
    def _add_charge_field(self):
        ...
        charge_field = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_field_name"]
        )
    ...
    def _remove_single_drug(self):
        drug_name = self.table_widget_dict_drug.field_value(
            self.medicine_col_no["medicine_name"]
        )
        msg_box = dialog_utils.get_message_box(
            f"刪除{self.dict_type}資料",
            QMessageBox.Warning,
        ...
        key = self.table_widget_dict_drug.field_value(
            self.medicine_col_no["medicine_key"]
        )
        self.database.delete_record("medicine", "MedicineKey", key)
        self.database.delete_record("commission", "MedicineKey", key)
        self.ui.tableWidget_dict_drug.removeRow(
            self.ui.tableWidget_dict_drug.currentRow()
        )
        self._set_drug_tool_buttons()
    ...
    def _read_drug_description(self, medicine_key):
        ...
        try:
            self.ui.textEdit_description.setText(
                string_utils.get_str(rows[0]["Description"], "utf8")
            )
        except TypeError:
            pass
    ...
    def _import_medicine(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(
            self, "開啟處方JSON檔", "*.json", "json 檔 (*.json);;", options=options
        )
        if not file_name:
            return
    ...
    def _filter_deactivate_medicine(self):
        for row_no in range(self.ui.tableWidget_dict_drug.rowCount(), -1, -1):
            deactivate_item = self.ui.tableWidget_dict_drug.item(
                row_no, self.medicine_col_no["deactivate"]
            )
    ...
    def _filter_location_medicine(self):
        for row_no in range(self.ui.tableWidget_dict_drug.rowCount(), -1, -1):
            location_item = self.ui.tableWidget_dict_drug.item(
                row_no, self.medicine_col_no["location"]
            )
            self.ui.tableWidget_dict_drug.showRow(row_no)
    ...
    def export_all_medicine_to_excel(self):
        ...
        export_utils.export_multiple_table_widgets_to_excel(
            excel_file_name,
            self.ui.tableWidget_dict_groups,
            self.ui.tableWidget_dict_drug,
            None,
            [7, 12, 13, 14, 15, 16],
    ...
    def _import_excel(self):
        ...
        excel_filename, _ = QFileDialog.getOpenFileName(
            self, "開啟藥品檔", "*.xlsx", "xlsx 檔 (*.xlsx)", options=options
        )
    ...
    def _append_excel(self):
        ...
        dict_medicine_type = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_name"]
        )
        start_no = 2
        i = 0
    ...
    def _sync_medicine(self, workbook, medicine_type):
        ...
        max_progress = sheet.max_row
        progress_dialog = QtWidgets.QProgressDialog(
            f"正在匯入{medicine_type}資料Excel中, 請稍後...",
            "取消",
            0,
            max_progress,
            self,
    ...
    def _no_purchase(self):
        ...
        no_purchase = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_no_purchase"]
        )
        if no_purchase == "Y":
    ...
    def _sync_drug(self):
        ...
        workbook = openpyxl.load_workbook(excel_filename, data_only=True)
        medicine_type = self.table_widget_dict_groups.field_value(
            self.groups_col_no["dict_groups_name"]
        )
    ...
    def _sync_drug_data(self, workbook, medicine_type):
        ...
        max_progress = sheet.max_row
        progress_dialog = QtWidgets.QProgressDialog(
            f"正在同步{medicine_type}資料Excel中, 請稍後...",
            "取消",
            0,
            max_progress,
            self,
