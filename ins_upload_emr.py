...
class InsUploadEMR(QtWidgets.QMainWindow):
    ...
    def _create_pdf_files(self, row):
        ...
        if string_utils.xstr(row['Visit']) == '初診照護':
            printer_utils.print_form_patient_new_care(
                self, self.database, self.system_settings,
                patient_key, case_key, self.apply_date, 'pdf'
            )
            patient_new_care_file = f'{self.EXPORT_DIR}/patient_new_care_{patient_key:0>6}.pdf'
        ...
        if self.system_settings.field('健保業務') != '台北業務組':
            ...
            ins_order_file = f'{self.EXPORT_DIR}/ins_order_{case_type}{sequence:0>6}.pdf'
            pdfs += [ins_order_file]
    ...
    def _get_sequence(self, row_no):
        # self.start_no = 6000  # 測試用, 用完要comment
