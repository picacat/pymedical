...
class StatisticsInsDiscountRegistFee(QtWidgets.QMainWindow):
    ...
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(
            ui_utils.UI_STATISTICS_INS_DISCOUNT_REGIST_FEE, self
        )
    ...
    def _read_data(self):
        self.basic_regist_fee = number_utils.get_integer(
            charge_utils._get_basic_regist_fee(self.database, "健保")
        )
    ...
    def _get_basic_regist_fee(self, row):
        ...
        course_type = nhi_utils.get_course_type(
    ...
    def _set_table_data(self, row_no, row):
        ...
        case_date = date_utils.date_to_zh_tw_date(
    ...
    def _calculate_total(self):
        ...
        for cell in row:
            self._set_item_data(total_case_count, cell[0], string_utils.xstr(cell[1]))
    ...
    def export_to_csv(self):
        ...
        with open(csv_file_name, "w", newline="", encoding="utf-8-sig") as csv_file:
            ...
            for row_no in range(self.ui.tableWidget_medical_record.rowCount()):
                try:
                    case_date = self.ui.tableWidget_medical_record.item(
                        row_no, 1
                    ).text()
                ...
                try:
                    birthday = self.ui.tableWidget_medical_record.item(
                        row_no, 13
                    ).text()
                ...
                try:
                    patient_id = self.ui.tableWidget_medical_record.item(
                        row_no, 14
                    ).text()
                except Exception:
                    patient_id = ""
                ...
                try:
                    discount_fee = self.ui.tableWidget_medical_record.item(
                        row_no, 9
                    ).text()
                except Exception:
                    discount_fee = ""
    ...
    def _list_regist_fee(self):
        ...
        for row_no, row in enumerate(rows):
            regist_fee = number_utils.get_integer(row["RegistFee"])
            person_count, total_regist_Fee, discount_type = self._get_regist_fee_count(
                regist_fee
            )
            ...
            for col_no in range(len(cell)):
                ...
                if col_no in [0, 1, 2]:
                    self.ui.tableWidget_regist_fee_list.item(
                        row_no, col_no
                    ).setTextAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
    ...
    def _get_regist_fee_count(self, regist_fee):
        ...
        basic_regist_fee = number_utils.get_integer(
            charge_utils._get_basic_regist_fee(self.database, "健保")
        )
