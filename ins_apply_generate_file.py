...
from libs import (case_utils, charge_utils, nhi_utils, number_utils,
                  personnel_utils, prescript_utils, string_utils)
...
class InsApplyGenerateFile(QtWidgets.QMainWindow):
    ...
    def _create_ins_records(self, rows):
        ...
        for row_no, row in enumerate(rows):
            ...
            if ins_apply_row is None:
                ...
                if case_type in ['C5']:  # 2022.05.10 法定傳染病多寫一筆清冠一號藥品補助費
                    self._write_ins_record(row, case_c5=True)

                if case_type in ['24', '29'] and number_utils.get_integer(pres_days) > 30:  # 2023.10.18 慢性病連續處方箋
    ...
    def _check_case_error(self, row, first_visit, share_code):
        ...
        doctor_id = personnel_utils.get_person_field_value(self.database, doctor_name, 'ID')
        doctor_count = personnel_utils.get_person_field_count(self.database, doctor_name)
        if doctor_count >= 2:
        ...
        if first_visit == '初診照護' and not self._is_patient_new_care_record_exists(row['PatientKey']):
    ...
    def _write_ins_record(self, row, case_c5=False, case_28=False):
        ...
        special_code = nhi_utils.get_special_code(self.database, self.system_settings, case_key)
        ...
        diag_fee = charge_utils.get_ins_fee_from_ins_code(self.database, diag_code, case_date=case_start_date)
        diag_fee = charge_utils.check_markup_diag_fee(diag_fee, string_utils.xstr(row['RegistType']))  # 檢查診察費是否需要加成
        ...
        # 非山地離島居家醫療
        if treat_type in nhi_utils.HOME_CARE and \
           regist_type not in nhi_utils.TOUR_TYPE and \
           share_type in ['基層醫療']:
        ...
        if case_c5:
            ...
            drug_fee = charge_utils.get_ins_drug_fee(self.database, pres_days, case_date=case_start_date)
            ins_total_fee = drug_fee
        else:
            case_type = nhi_utils.get_case_type(self.database, self.system_settings, row, diag_fee, ins_total_fee)
            ...
            if not case_c5:  # 遠距診療費
                ...
                if isolation_position in ['防疫旅館', '醫院', '集檢所']:
                    ...
                    treat_fee = charge_utils.get_ins_fee_from_ins_code(
                        self.database, 'E5204C', case_date=case_start_date)  # 遠距診療費
        ...
        doctor_id = personnel_utils.get_person_field_value(self.database, doctor_name, 'ID')
        pharmacist_id = nhi_utils.get_pharmacist_id(self.database, self.system_settings, row)
        ...
        if number_utils.get_integer(row['Continuance']) >= 2:
            original_security = case_utils.get_first_course_field(
                self.database, row['CaseDate'], row['PatientKey'], card, 'Security',
            )
            original_identifier = case_utils.extract_security_xml(original_security, '就醫識別碼')

        actual_identifier = case_utils.get_case_extend(self.database, case_key, '原就醫識別碼')
        ...
        fields = [
            ...
            'Visit', 'DiseaseCode1', 'DiseaseCode2', 'DiseaseCode3', 'DiseaseCode4',
            'PresDays', 'PresType',
            'DoctorName', 'DoctorID', 'PharmacistID',
            'DrugFee', 'TreatFee', 'DiagCode', 'DiagFee', 'PharmacyCode', 'PharmacyFee',
            'InsTotalFee', 'ShareFee', 'DiagShareFee', 'DrugShareFee', 'InsApplyFee', 'AgentFee',
            'PatientKey', 'Name',
            'Identifier', 'OriginalIdentifier', 'ActualIdentifier',
            ...
            'CaseKey7', 'TreatCode7', 'TreatFee7', 'Percent7',
            'CaseKey8', 'TreatCode8', 'TreatFee8', 'Percent8',
            'CaseKey9', 'TreatCode9', 'TreatFee9', 'Percent9',
            'CaseKey10', 'TreatCode10', 'TreatFee10', 'Percent10',
            'CaseKey11', 'TreatCode11', 'TreatFee11', 'Percent11',
            'CaseKey12', 'TreatCode12', 'TreatFee12', 'Percent12',
            'CaseKey13', 'TreatCode13', 'TreatFee13', 'Percent13',
            'CaseKey14', 'TreatCode14', 'TreatFee14', 'Percent14',
            'CaseKey15', 'TreatCode15', 'TreatFee15', 'Percent15',
            'Message'
        ]
        ...
        data = [
            ...
            nhi_utils.INS_CLASS, case_start_date, row['CaseDate'].date(),
            ...
            doctor_name, doctor_id, pharmacist_id,
            drug_fee, treat_fee, diag_code, diag_fee,
            pharmacy_code, pharmacy_fee,
            ins_total_fee, share_fee, diag_share_fee, drug_share_fee, ins_apply_fee,

            agent_fee,  # number_utils.get_integer(row['AgentFee']), 2022.09.11 新制

            number_utils.get_integer(row['PatientKey']),
            string_utils.xstr(row['Name']),
            identifier, original_identifier, actual_identifier,
            ...
            treat_records[6]['CaseKey'], treat_records[6]['TreatCode'], treat_records[6]['TreatFee'],
            treat_records[6]['Percent'],

            treat_records[7]['CaseKey'], treat_records[7]['TreatCode'], treat_records[7]['TreatFee'],
            treat_records[7]['Percent'],

            treat_records[8]['CaseKey'], treat_records[8]['TreatCode'], treat_records[8]['TreatFee'],
            treat_records[8]['Percent'],

            treat_records[9]['CaseKey'], treat_records[9]['TreatCode'], treat_records[9]['TreatFee'],
            treat_records[9]['Percent'],

            treat_records[10]['CaseKey'], treat_records[10]['TreatCode'], treat_records[10]['TreatFee'],
            treat_records[10]['Percent'],

            treat_records[11]['CaseKey'], treat_records[11]['TreatCode'], treat_records[11]['TreatFee'],
            treat_records[11]['Percent'],

            treat_records[12]['CaseKey'], treat_records[12]['TreatCode'], treat_records[12]['TreatFee'],
            treat_records[12]['Percent'],

            treat_records[13]['CaseKey'], treat_records[13]['TreatCode'], treat_records[13]['TreatFee'],
            treat_records[13]['Percent'],

            treat_records[14]['CaseKey'], treat_records[14]['TreatCode'], treat_records[14]['TreatFee'],
        ...
        if row['CaseDate'].strftime('%Y-%m-%d') >= '2023-03-20' and \
                infectious_drug in ['台灣清冠一號及科學中藥', '台灣清冠一號']:
            case_type = 'C5'
    ...
    def _rewrite_ins_record(self, ins_apply_row, case_row):
        ...
        if string_utils.xstr(case_row['TreatType']) in nhi_utils.AUXILIARY_CARE_TREAT:
            ...
            if string_utils.xstr(case_row['TreatType']) in nhi_utils.HOME_CARE:
                treat_fee += number_utils.get_integer(case_row['DiagFee'])  # home care redefine diag_fee --> treat_fee
        ...
        diag_share_fee = (
                number_utils.get_integer(ins_apply_row['DiagShareFee']) +
                number_utils.get_integer(case_row['DiagShareFee'])
        )
        drug_share_fee = (
                number_utils.get_integer(ins_apply_row['DrugShareFee']) +
        ...
        if string_utils.xstr(case_row['TreatType']) in nhi_utils.HOME_CARE:
            pharmacy_code = nhi_utils.get_home_care_pharmacy_code(
                self.database, self.system_settings, treat_records,
            )
        else:
            pharmacy_code = nhi_utils.get_pharmacy_code(
                self.system_settings,
        ...
        if case_start_date.year < 2023 or (case_start_date.year == 2023 and case_start_date.month < 7):  # 112.07 以前沿用舊制
            agent_fee = charge_utils.get_ins_agent_fee(
                self.database, self.system_settings, share_type, treatment, course, drug_fee,
            )
        else:
            share_code = nhi_utils.get_final_share_code(share_code, diag_share_fee, drug_share_fee)  # 2022.10 新制
            agent_fee = nhi_utils.get_agent_fee(share_code, diag_share_fee, drug_share_fee)

        if treat_type in nhi_utils.HOME_CARE and share_type in ['基層醫療']:  # 基層醫療居家醫療部份負擔 S10, S20 --> K10, K20
        ...
        if number_utils.get_integer(ins_apply_row['CaseKey1']) <= 0:  # 如果療程首次不在本月, 抓最後一次醫師 2020.12.08 禾生堂
            doctor_name = string_utils.xstr(case_row['Doctor']).replace(',', '')
            doctor_id = personnel_utils.get_person_field_value(
                self.database, doctor_name, 'ID'
            )
