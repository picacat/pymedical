...
class DetectPaymentCardThread(QtCore.QThread):
    ...
    def __init__(self, parent, ic_card, cpay):
    ...
    def run(self):
        ...
        while True:
            ...
            payment_done = self.cpay.is_payment_done(self.total_fee)
            if payment_done:
                self.cancel_payment.emit(self.payment_type)
                break
            else:
                receipt_fee = self.cpay.get_payment()
            ...
            if error_code == 0:
                self._stop = False
                self.ic_card.close_com()
                if receipt_fee is not None and receipt_fee > 0:  # todo
...
class PyCashierPayment(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, *args):
        ...
        self.detect_payment_thread = DetectPaymentCardThread(self, self.ic_card, self.cpay)
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_PYCASHIER2_PAYMENT, self)
        style = '''
    ...
    def _charge_payment(self, case_key):
        ...
        if total_fee > 0:  # todo
            self.cpay.start_payment(total_fee)
        else:
            self.cpay.cancel_payment()
    ...
    def set_payment_data(self, patient_key, regist_type, keyword):
        ...
        if doctor not in ['陳士源'] and regist_fee > 0:
            regist_fee -= 100
        ...
        self.cpay.machine_deduct(total_fee)
        self.detect_payment_thread.set_total_fee(total_fee)
        self.detect_payment_thread.set_label_received_fee(self.ui.label_received_fee)
