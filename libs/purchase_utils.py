...
def set_purchase_list_table(database, in_table_widget):
    table_widget_purchase_list = class_utils.get_table_widget(in_table_widget, database)
...
def check_single_compound(database, case_key, medicine_type):
    ...
    single_compound = medicine_utils.get_medicine_extend(
        database, medicine_key, "成方單項"
    )
    title_compound = medicine_utils.get_medicine_extend(
        database, medicine_key, "成方抬頭"
    )
...
def set_purchase_list_data(
    ...
    if "(退貨)" in medicine_name or "(換貨)" in medicine_name:
        ...
        if "(退貨)" in medicine_name and (
            "自費水藥" in medicine_name
            or "自費粉藥" in medicine_name
            or medicine_name in "代煎水藥"
        ):
            pres_days = dosage
            dosage = 1
    else:
    ...
    massage_referrer = prescript_utils.get_pres_extend_value(
        database, prescript_key, "傷助推薦"
    )
    if massage_referrer in [None, ""]:
        massage_referrer = string_utils.xstr(row["MassageReferrer"])

    nursing_assistant = prescript_utils.get_pres_extend_value(
        database, prescript_key, "護佐"
    )
    ...
    if debt > 0:
        ...
        if (
            query_start_date is not None
            and query_end_date is not None
            and repayment_date is not None
            and (
                repayment_date[:10] < query_start_date
                or repayment_date[:10] > query_end_date
            )
        ):
    ...
    if treat_type in ["療程實現", "療程實現贈送"]:
        ...
        massager_commission = (
            get_commission(massager_commission_rate, unit_price) * dosage
        )
        cashier_commission = (
            get_commission(cashier_commission_rate, unit_price) * dosage
        )
    else:
        ...
        massager_commission = get_commission(
            massager_commission_rate, commision_receipt
        )
    ...
    if "(退貨)" in medicine_name or "(換貨)" in medicine_name:
        if (
            query_start_date is not None
            and query_end_date is not None
            and sale_date != case_date
            and (case_date < query_start_date or case_date > query_end_date)
        ):
            return
    ...
    for col_no in range(len(prescript_record)):
        ...
        if (
            discount_fee > doctor_commission
            or discount_fee > massager_commission
            or discount_fee > cashier_commission
        ) and col_no == PURCHASE_COL_NO["discount"]:
            cell_item.setForeground(QtGui.QColor("red"))

        if (
            "湯包" in medicine_type
            and "(贈)" in medicine_name
            and dosage > 1
            and col_no
            in [PURCHASE_COL_NO["medicine_name"], PURCHASE_COL_NO["quantity"]]
        ):
...
def get_commission_rate(
    ...
    if (
        (position == "醫師" and doctor == "")
        or (position == "推拿師父" and massage_referrer == "")
        or (position == "櫃台" and nursing_assistant == "")
    ):
        return None
    ...
    if medicine_key in ["", None]:
        if medicine_name in ["自費水藥", "自費粉藥"]:
            ...
            if position == "醫師":
                if massage_referrer not in ["", None] or nursing_assistant not in [
                    "",
                    None,
                ]:
                    commission_type = "分成率"
    ...
    if (
        "%" not in string_utils.xstr(commission_rate)
        and number_utils.get_float(commission_rate) < 1
    ):
...
def calculate_purchase_list_total(
    table_widget, start_date=None, end_date=None, no_zero_bonus=False
):
    (
        total_discount,
        total_fee,
    ...
    total_dosage = 0
    total_doctor_commission, total_massager_commission, total_cashier_commission = (
        0,
        0,
        0,
    )
    ...
    for row_no in range(table_widget.rowCount()):
        ...
        doctor_commission = number_utils.get_integer(
            table_widget.item(row_no, PURCHASE_COL_NO["doctor_commission"]).text()
        )
        massager_commission = number_utils.get_integer(
            table_widget.item(row_no, PURCHASE_COL_NO["massager_commission"]).text()
        )
        cashier_commission = number_utils.get_integer(
            table_widget.item(row_no, PURCHASE_COL_NO["cashier_commission"]).text()
        )

        if (
            no_zero_bonus
            and doctor_commission <= 0
            and massager_commission <= 0
            and cashier_commission <= 0
        ):
            continue

        case_date = table_widget.item(
            row_no, PURCHASE_COL_NO["case_date"]
        ).text()  # 銷售日期
        return_date = table_widget.item(
            row_no, PURCHASE_COL_NO["return_date"]
        ).text()  # 退換貨日期
        ...
        if check_date is None or (
            ...
            and (case_date == check_date or return_date == check_date)
        ):
        ...
        total_doctor_commission += doctor_commission
        total_massager_commission += massager_commission
        total_cashier_commission += cashier_commission
...
def _check_prescript_exists(tableWidget_prescript, medicine_row):
    ...
    for row_no in range(row_count):
        medicine_key = tableWidget_prescript.item(
            row_no, PRESCRIPT_COL_NO["MedicineKey"]
        )
...
def insert_prescript_row(
    ...
    if cancel_button:
        ...
        bonus = medicine_utils.get_medicine_extend(
            database, medicine_key, "療程實現贈送"
        )
        course_count = number_utils.get_integer(
            row["Dosage"]
        ) + number_utils.get_integer(bonus)
    ...
    for col_no in range(len(prescript_row)):
        ...
        if col_no in [PRESCRIPT_COL_NO["Unit"]]:
            tableWidget_prescript.item(row_no, col_no).setTextAlignment(
                QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter
            )
        elif col_no in [
            PRESCRIPT_COL_NO["Quantity"],
    ...
    tableWidget_prescript.setCellWidget(
        row_no, PRESCRIPT_COL_NO["Promotion"], check_box
    )
    ...
    button.clicked.connect(
        lambda: remove_prescript_row(tableWidget_prescript, extra_process)
    )
    if cancel_button:
        remove_col = 9
    else:
...
def prescript_item_changed(tableWidget_prescript, item):
    ...
    tableWidget_prescript.item(row_no, PRESCRIPT_COL_NO["Amount"]).setTextAlignment(
        QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter
    )
...
def is_returned_goods(database, tableWidget_prescript, row_no):
    medicine_key = tableWidget_prescript.item(
        row_no, PURCHASE_COL_NO["medicine_key"]
    ).text()
    if medicine_key == "":
        return False
