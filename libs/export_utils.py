...
from libs import (case_utils, date_utils, nhi_utils, number_utils,
                  personnel_utils, string_utils, system_utils)
...
alternate_row_color = PatternFill(start_color='FFC7CE', end_color='FFC7CE', fill_type='solid')
...
def export_table_widget_to_excel(excel_file_name, in_table_widget,
                                 hidden_column=None, numeric_cell=None,
                                 title=None, column_width=None, calc_total=False,
                                 mark_col_no=None):
...
def export_tab_widget_to_excel(excel_file_name, in_tab_widget,
                               hidden_column=None, numeric_cell=None,
                               title=None, column_width=None):
    if numeric_cell is None:
        numeric_cell = []
...
def export_multiple_table_widgets_to_excel(
        excel_file_name, table_widget_master, table_widget_detail,
        hidden_column=None, numeric_cell=None, title=None, column_width=None):
    if numeric_cell is None:
        numeric_cell = []
...
def export_daily_medical_records_to_excel(
    ...
    header_row = [
        '日期', '班別', '醫師', '保險', '病歷號', '病患姓名', '診號', '卡序', '療程', '就醫類別',
...
def add_nursing_home_sheet(database, system_settings, row, workbook, apply_year, apply_month, clinic_id):
    ...
    header_row = [
        '日期', case_date, None, '時間', '08:00 - 12:00', None,
    ]
    sheet.append(header_row)
    ...
    # --- 關鍵修改：排序與輸出合計 ---
    summary_row_no = len(rows) + 9  # 合計字樣所在列
    ...
    # 3. 依照排序後的順序寫入 Excel (從 L 欄開始，即 column 12)
    current_col = 12
    for code, count in sorted_codes:
        # 第一列：代碼名稱 (例如 D07)
        cell_code = sheet.cell(row=summary_row_no, column=current_col)
        cell_code.value = code
        cell_code.font = bold
        cell_code.alignment = align_center
        
        # 第二列：統計數量 (例如 1)
        cell_count = sheet.cell(row=summary_row_no + 1, column=current_col)
    ...
    # 4. 金額總計保持不變 (T, U, V 欄)
    sheet.cell(row=summary_row_no, column=20).value = f"=SUM(T9:T{summary_row_no-1})"
    sheet.cell(row=summary_row_no, column=21).value = f"=SUM(U9:U{summary_row_no-1})"
    sheet.cell(row=summary_row_no, column=22).value = f"=SUM(V9:V{summary_row_no-1})"
...
# ====== 小工具：設定 A4 橫向 + 縮放單頁 ======
def set_sheet_to_a4_onepage(sheet, last_row=None, last_col=None,
    ...
    sheet.page_margins = PageMargins(left=m, right=m, top=m, bottom=m,
...
#  綜合業績報表 2020.03.17
def export_multiple_performance_to_excel(clinic_name, year, month, excel_file_name, tab_widget):
    workbook = Workbook()
    sheet = workbook.active
    workbook.remove_sheet(sheet)

    _export_week_table(
        workbook, '週人數統計表', clinic_name, year, month, tab_widget.widget(0).ui.tableWidget_medical_record
    )
    _export_week_table(
        workbook, '自費收入統計表', clinic_name, year, month, tab_widget.widget(1).ui.tableWidget_medical_record
    )
    _export_week_project(
        workbook, '週專案統計表', clinic_name, year, month, tab_widget.widget(2).ui.tableWidget_medical_record
    )
    _export_week_daily(
        workbook, '週人數日報表', clinic_name, year, month, tab_widget.widget(0).ui.tableWidget_week
    )
    _export_week_daily(
        workbook, '週自費收入日報表', clinic_name, year, month, tab_widget.widget(1).ui.tableWidget_week
    )
    _export_week_doctor(
        workbook, '週醫師業績日報表', clinic_name, year, month, tab_widget.widget(3).ui.tableWidget_medical_record
    )
...
def _export_week_table(workbook, title, clinic_name, year, month, table_widget):
    ...
    for i in range(table_widget.columnCount()):
        header_row.append(
            '' if table_widget.horizontalHeaderItem(i) is None else table_widget.horizontalHeaderItem(i).text()
        )
    sheet.append(header_row)
    ...
    for row_no in range(1, table_widget.rowCount()):
        ...
        for col_no in range(table_widget.columnCount()):
            row.append(
                '' if table_widget.item(row_no, col_no) is None else table_widget.item(row_no, col_no).text(),
            )
        sheet.append(row)
...
def _export_week_daily(workbook, title, clinic_name, year, month, table_widget):
    ...
    header_row = [
        None,
        '' if table_widget.item(0, 1) is None else table_widget.item(0, 1).text(), None, None,
        '' if table_widget.item(0, 4) is None else table_widget.item(0, 4).text(), None, None,
        '' if table_widget.item(0, 7) is None else table_widget.item(0, 7).text(), None, None,
        '' if table_widget.item(0, 10) is None else table_widget.item(0, 10).text(), None, None,
        '' if table_widget.item(0, 13) is None else table_widget.item(0, 13).text(), None, None,
        '' if table_widget.item(0, 16) is None else table_widget.item(0, 16).text(), None, None,
        '' if table_widget.item(0, 19) is None else table_widget.item(0, 19).text(), None, None,
    ]
    sheet.append(header_row)
    sheet.merge_cells(f'B{row_index}:D{row_index}')
    sheet.merge_cells(f'E{row_index}:G{row_index}')
...
def export_medical_record_diagnosis_excel(
    ...
    header_row = ['序', '就醫日期', '病歷號', '年齡', '性別', '主訴', '舌診', '脈象', '辨證', '治則', '病名']
    ...
    for row_no in range(table_widget.rowCount()):
        ...
        row.append(string_utils.get_str(case_row['Distincts'], 'utf8').replace('\n', ''))
...
def export_certificate_payment_to_excel(**kwargs):
    ...
    sheet.append([])
    header_row = [
        '門診日期', '保險', '掛號費', '門診負擔', '藥品負擔', '小計', '健保申報',
        '自費藥費', '處置費', '其他費用', '折扣', '小計', '合計'
    ]
    sheet.append(header_row)
...
def export_infectious_list(system_settings, excel_file_name, tableWidget_infectious):
    ...
    header_row = [
        None, '醫事機構代碼', '醫療機構名稱', '申報費用年月', '申請費用', '姓名', '出生日期',
        '藥品品名', '用藥起始日期', '用藥天數', '開立總克數', 'COVID-19檢驗結果陽性', '同意書回傳院所',
    ]
    sheet.append(header_row)
...
def export_income_list(system_settings, excel_file_name, tableWidget_income, columns):
    ...
    header_row = [
        '序號', '門診日期', '班別', '病歷號', '姓名', '保險', '負擔類別', '就醫類別', '優待類別', '卡序', '藥日',
        '掛號費', '門診負擔', '藥品負擔', '欠卡費', '還卡費', '還款', '自費金額', '欠款',
        '掛號欠款', '民俗調理', '實收現金', '掛號人員', '批價人員',
    ]
    sheet.append(header_row)
...
def export_certificate_diagnosis(database, system_settings, excel_file_name, certificate_key):
    sql = f'''
        SELECT
            certificate.*,
    ...
    cell_title.value = f"{system_settings.field('院所名稱')} 診斷證明書\nCERTIFICATE OF DIAGNOSIS"
    ...
    physician_cert_no = personnel_utils.get_person_field_value(database, physician, 'Certificate')
...
def export_case_to_word_by_case_key(database, doc, case_key):
    ...
    age, _ = date_utils.get_age(
        row['Birthday'], row['CaseDate'])
    doc.add_heading(
        f'''門診日{row["CaseDate"].date()}\t保險: {row["InsType"]} {row["Share"]} {card}\t醫師: {row["Doctor"]}''', level=1)
    doc.add_heading(
        f'姓名: {row["Name"]}\t性別: {row["Gender"]}\t生日: {row["Birthday"]}\t年齡: {age}歲', level=2)
    ...
    if disease_code1 != '':
        doc.add_heading(
            f'''主診斷碼:{disease_code1} {row["DiseaseName1"]}''', level=3)
    if disease_code2 != '':
        doc.add_heading(
            f'''次斷碼1:{disease_code2} {row["DiseaseName2"]}''', level=3)
    if disease_code3 != '':
        doc.add_heading(
            f'''次斷碼2:{disease_code3} {row["DiseaseName3"]}''', level=3)
...
def export_prescript_to_word_by_case_key(database, doc, case_key, treatment):
    ...
    for row in rows:
        ...
        if packages > 0 or pres_days > 0:
            ...
            if medicine_set == 1:
                merged_cell = instruction_row[0].merge(instruction_row[1]).merge(instruction_row[2])
            else:
                merged_cell = instruction_row[0].merge(instruction_row[1]).merge(instruction_row[4])
...
def export_correction_reg_income_txt(database, system_settings, txt_filename,
                                     tableWidget_medical_record_list, column):
    ...
    for row_no in range(tableWidget_medical_record_list.rowCount()):
        ...
        case_key = tableWidget_medical_record_list.item(row_no, column['CaseKey']).text()
        sql = f'''
            SELECT
        ...
        share_fee = number_utils.get_integer(case_row['SDiagShareFee']) + \
