...
def extract_ins_compound(
    database, prescript_row, table_widget_prescript, table_widget_treat=None
):
...
def extract_compound(
    prescript_tab, database, system_settings, medicine_key, table_widget_medicine=None
):
    sql = f"""
    ...
    row = rows[0]
    is_unit_price = False
    compound_single = medicine_utils.get_medicine_extend(
        database, medicine_key, "成方單項"
    )
    compound_title = medicine_utils.get_medicine_extend(
        database, medicine_key, "成方抬頭"
    )
    if prescript_tab.objectName() == "SelfPrescript" and (
        compound_title == "Y" or compound_single == "Y"
    ):
    ...
    if (
        compound_single == "Y" and compound_title != "Y"
    ):  # 單項true 抬頭false 不匯入成方內容
        return
    ...
    for compound_row in compound_rows:
        ...
        if (
            medicine_type in ["穴道", "處置"]
            and number_utils.get_integer(prescript_tab.medicine_set) <= 1
        ):
            add_treat(prescript_tab, table_widget_medicine, row)

            current_row = prescript_tab.tableWidget_prescript.currentRow()
            medicine_key = prescript_tab.tableWidget_prescript.item(
                current_row, INS_PRESCRIPT_COL_NO["MedicineKey"]
            )
            medicine_name = prescript_tab.tableWidget_prescript.item(
                current_row, INS_PRESCRIPT_COL_NO["MedicineName"]
            )
    ...
    for row_no in range(table_widget_prescript.rowCount()):
        ...
        try:
            item = table_widget_prescript.item(
                row_no, INS_PRESCRIPT_COL_NO["MedicineKey"]
            )
            if (
                item is not None
                and item.text() != ""
                and database is not None
                and medicine_set is None
            ):
                medicine_key = item.text()
                sql = f"""
...
def get_default_complicated_acupuncture_time(system_settings):
    ...
    return number_utils.get_integer(moderate_time), number_utils.get_integer(
        highly_time
    )
...
def check_extend_ins_drug(ins_tab, self_tab):
    ...
    extend_ins_drug = is_prescript_exists(
        self_tab.tableWidget_prescript,
        SELF_PRESCRIPT_COL_NO["MedicineName"],
        "健保合包",
    )
...
def get_herbal_net_weight(is_herbal_decocation_service, table_widget_prescript):
    ...
    for row_no in range(table_widget_prescript.rowCount()):
        ...
        if (
            not is_herbal_decocation_service and "生薑" in medicine_name
        ):  # 自煎 (無代煎)要扣掉生薑的重量
            continue
...
def duplicate_ins_table_widget(database, src_table, dst_table, ratio=None):
            ...
            for c in range(cols):
                ...
                if c in [
                    ...
                    if medicine_key is not None:
                        price = number_utils.get_float(
                            get_medicine_field(database, medicine_key, "SalePrice")
                        )
                        try:
