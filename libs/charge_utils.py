...
from libs import (case_utils, date_utils, nhi_utils, number_utils,
...
def _get_basic_regist_fee(database, ins_type, visit='複診'):
    ...
    if visit == '初診':
        ...
        json_regist_remark_fee = get_json_remark_fee(json_data=json_data, field_name='初診')
...
def get_regist_fee(database, system_settings,
                   birthday, discount_type, ins_type, share_type, treat_type,  course=None, visit='複診'):
    if share_type == '中低收入戶':
        share_type = '基層醫療'
    ...
    if ins_type == '自費':
        ...
        if string_utils.xstr(discount_type) != '':  # 掛號費優待優先取得
            discount_fee = _get_self_regist_discount_fee(database, discount_type=discount_type)
    ...
    if len(rows) > 0:
        ...
        if remark == '排除其他優待':
            ...
            json_regist_fee = get_json_fee(system_settings, birthday, regist_fee, discount_type, remark)
    ...
    if string_utils.xstr(discount_type) != '':  # 掛號費優待優先取得
        ...
        if share_type != '基層醫療' and discount_type != '':
            discount_fee = _get_compound_discount_fee(database, share_type, discount_type)

        if discount_fee is None and course_type == '療程':
            discount_fee = _get_regist_discount_fee(database, discount_type+course_type)
        ...
        old_man_regist_fee = get_old_man_regist_fee(database, system_settings, birthday, course_type)
        child_regist_fee = get_child_regist_fee(database, system_settings, birthday, course_type)
        try:
            other_age_regist_fee = get_other_age_regist_fee(database, system_settings, birthday, course_type)
        except Exception:
        ...
        if old_man_regist_fee is not None:
            ...
            if share_type != '基層醫療':
                special_regist_fee = get_old_man_special_regist_fee(database, share_type)
...
def get_custom_herb_fee1(database, case_key, table_widget_prescript):
    ...
    is_herbal_decocation_service = prescript_utils.is_herbal_decocation_service(table_widget_prescript)
...
def set_normal_price(database, table_widget_prescript):
    for row_no in range(table_widget_prescript.rowCount()):
        ...
        table_widget_prescript.setItem(
            row_no, prescript_utils.SELF_PRESCRIPT_COL_NO['Price'], item,
        )
...
def get_herb_fee_by_dosage(database, total_dosage):
    ...
    for row in rows:
        try:
            min = number_utils.get_integer(string_utils.xstr(row['ItemName']).split('-')[0])
            max = number_utils.get_integer(string_utils.xstr(row['ItemName']).split('-')[1])
        except Exception:
            continue
...
# 取得自費水藥費
def get_herb_fee(database, system_settings, item_name, total_dosage,
                 case_key=None, table_widget_prescript=None):
...
def get_diag_share_fee(database, system_settings, share_type, treat_type, course, reg_type=None):
    diag_share_fee = 0
    ...
    if treat_type in nhi_utils.HOME_CARE:
        ...
        return number_utils.round_up(diag_fee * 5 / 100)  # 門診掛號居家醫療部份負擔: 申報費用 5%
    ...
    if reg_type in nhi_utils.TOUR_FAR or \
       reg_type in nhi_utils.LACK_AREA or \
       system_settings.field('資源類別') in nhi_utils.TOUR_FAR or \
       system_settings.field('資源類別') in nhi_utils.LACK_AREA:
        diag_share_fee -= diag_share_fee * nhi_utils.LACK_RESOURCE_DISCOUNT_RATE / 100  # 資源不足及偏遠地區減免20%
...
# 取得藥品負擔
def get_drug_share_fee(database, system_settings, share_type, ins_drug_fee, reg_type=None):
    if share_type == '中低收入戶':
        share_type = '基層醫療'
    ...
    if reg_type in nhi_utils.TOUR_FAR or \
       reg_type in nhi_utils.LACK_AREA or \
       resource_type in nhi_utils.TOUR_FAR or \
       resource_type in nhi_utils.LACK_AREA:
        drug_share_fee -= drug_share_fee * nhi_utils.LACK_RESOURCE_DISCOUNT_RATE / 100  # 資源不足及偏遠地區減免20%
...
# 取得民俗條理費
def get_traditional_health_care_fee(database, system_settings, ins_type, course, massager):
...
def get_ins_drug_fee(database, pres_days, case_date=None):
    ...
    drug_code = 'A21'
    ins_drug_fee = get_ins_fee_from_ins_code(database, drug_code, case_date=case_date) * pres_days
...
# 調劑費 = 0: 不申報調劑費, 沒有申報藥費
# 調劑費 > 0: 藥師調劑, 醫師調劑
def get_ins_pharmacy_fee(database, system_settings, ins_drug_fee, pharmacy_type='申報', reg_type='一般門診'):
    ins_pharmacy_fee = 0
...
# 取得健保針灸費
def get_ins_acupuncture_fee(database, treatment, ins_drug_fee, course=None, case_date=None):
    ...
    ins_acupuncture_fee = get_ins_fee_from_ins_code(database, acupuncture_code, case_date=case_date)
...
def get_ins_exam_fee(database, case_key, treatment, integrate_care=None):
    ...
    if patient_key is not None:
        ...
        if len(rows) > 0:
            ...
            if case_date >= '2023-03-01' and age_year is not None and age_year < 7 and \
                    treatment in nhi_utils.MASSAGE_TREAT and course <= 1:
...
# 取得健保代辦費
def get_ins_agent_fee(database, system_settings, share_type, treatment, course, ins_drug_fee, reg_type=None):
    ins_agent_fee = 0
...
def get_ins_fee(database, system_settings, table_widget_ins_care=None, **kwargs):
    ...
    if kwargs['treat_type'] in nhi_utils.CARE_TREAT:
        ins_fee = get_ins_special_care_fee(
            database, system_settings, table_widget_ins_care,
            case_key=case_key,
        ...
        ins_fee['exam_fee'] = get_ins_exam_fee(database, case_key, kwargs['treatment'], integrate_care)
        return ins_fee
    ...
    if kwargs['treat_type'] in nhi_utils.HOME_CARE:
        ...
        ins_fee['diag_fee'] = get_ins_diag_fee(database, system_settings, kwargs['course'], diag_code)
    else:
        ins_fee['diag_fee'] = get_ins_diag_fee(database, system_settings, kwargs['course'])
    ...
    ins_fee['pharmacy_fee'] = get_ins_pharmacy_fee(
        database, system_settings,
        ins_fee['drug_fee'], kwargs['pharmacy_type'], kwargs['reg_type'],
    )

    ins_fee['acupuncture_fee'] = get_ins_acupuncture_fee(
        database, kwargs['treatment'],
        ins_fee['drug_fee'], kwargs['course'], case_date=case_date.date())
    ...
    if kwargs['share'] in nhi_utils.INFECTIOUS_INJURY_TYPE:  # 法定傳染病通報隔離
        if isolation_position in [None, '', '居家']:
            ins_fee['diag_fee'] = get_ins_fee_from_ins_code(database, 'E5204C')  # 遠距診療費
        ...
        if infectious_drug in ['台灣清冠一號及科學中藥', '科學中藥']:
            ...
            ins_fee['pharmacy_fee'] = get_ins_pharmacy_fee(
                ...
                kwargs['pharmacy_type'], kwargs['reg_type'],
            )
    ...
    elif case_date.strftime('%Y-%m-%d') >= '2023-03-20' and infectious_drug in ['台灣清冠一號及科學中藥', '台灣清冠一號']:
        amount = get_ins_fee_from_ins_code(database, 'E5012C')  # 台灣清冠一號補助費
        infectious_drug_fee = amount * pres_days
    ...
    if kwargs['treat_type'] in nhi_utils.HOME_CARE and \
            kwargs['reg_type'] not in nhi_utils.TOUR_TYPE and \
            kwargs['share'] not in nhi_utils.AGENT_SHARE:
        treat_fee = (ins_fee['acupuncture_fee'] + ins_fee['massage_fee'] + ins_fee['dislocate_fee'])
        ...
        ins_fee['diag_share_fee'] = get_diag_share_fee(
            database, system_settings, kwargs['share'], kwargs['treatment'], kwargs['course'], kwargs['reg_type'],
        )

    drug_fee = ins_fee['drug_fee']
    ins_fee['drug_share_fee'] = get_drug_share_fee(
        database, system_settings, kwargs['share'], drug_fee, kwargs['reg_type'],
    )
    if case_date.strftime('%Y-%m-%d') >= '2023-03-20' and infectious_drug in ['台灣清冠一號及科學中藥']:
    ...
    ins_fee['agent_fee'] = get_ins_agent_fee(
        database, system_settings, kwargs['share'], kwargs['treatment'], kwargs['course'], drug_fee,
    )
...
# 取得各項照護申報費用
def get_ins_special_care_fee(database, system_settings, table_widget_ins_care=None, **kwargs):
    ins_fee = {}
    ...
    if table_widget_ins_care is None:
        care_fee = get_ins_care_fee_from_case_key(database, kwargs['case_key'])  # 小兒氣喘, 小兒腦麻為包套, 照護費已包含藥費, 調劑費與針傷處置費
    else:
        care_fee = get_ins_care_fee_from_table_widget(table_widget_ins_care)  # 小兒氣喘, 小兒腦麻為包套, 照護費已包含藥費, 調劑費與針傷處置費

    if kwargs['treat_type'] in ['腦血管疾病', '兒童鼻炎']:  # 腦血管疾病, 兒童鼻炎可申報藥費及調劑費
        drug_fee = get_ins_drug_fee(database, pres_days)
        pharmacy_fee = get_ins_pharmacy_fee(
            database, system_settings, drug_fee, kwargs['pharmacy_type'], kwargs['reg_type'])
    elif kwargs['treat_type'] in ['癌症中醫門診延長照護']:
        drug_code = get_ins_code_from_charge_settings(database, '照護費', '中醫門診延長照護每日藥費')
        pharmacy_code = get_ins_code_from_charge_settings(database, '照護費', '中醫門診延長照護藥品調劑費')
        ...
        if treatment in nhi_utils.GENERAL_ACUPUNCTURE_TREAT:
            treat_code = get_ins_code_from_charge_settings(database, '照護費', '中醫門診延長照護針灸治療處置費')
            acupuncture_fee = get_ins_fee_from_ins_code(database, treat_code)
        elif treatment in nhi_utils.GENERAL_MASSAGE_TREAT:
            treat_code = get_ins_code_from_charge_settings(database, '照護費', '中醫門診延長照護傷科治療處置費')
            massage_fee = get_ins_fee_from_ins_code(database, treat_code)
        elif treatment in nhi_utils.ACUPUNCTURE_TREAT:
            acupuncture_fee = get_ins_acupuncture_fee(database, treatment, drug_fee, course)
    ...
    if kwargs['treat_type'] in ['兒童鼻炎']:  # 兒童鼻炎可申報診察費, 針灸費, 傷科費
        ...
        acupuncture_fee = get_ins_acupuncture_fee(database, kwargs['treatment'], drug_fee)
    ...
    ins_fee['diag_share_fee'] = get_diag_share_fee(
        database, system_settings, kwargs['share'], kwargs['treat_type'], kwargs['course']
    )
    if kwargs['treat_type'] in \
            nhi_utils.AUXILIARY_CARE_TREAT + \
            nhi_utils.CANCER_CARE_TREAT + \
            nhi_utils.PREGNANT_CARE_TREAT + ['慢性腎病照護']:  # 照護要申報藥品負擔
        virtual_drug_fee = get_ins_drug_fee(database, kwargs['pres_days'])
        ins_fee['drug_share_fee'] = get_drug_share_fee(database, system_settings, kwargs['share'], virtual_drug_fee)
    else:
        ins_fee['drug_share_fee'] = get_drug_share_fee(database, system_settings, kwargs['share'], drug_fee)
...
def calculate_ins_fee(database, system_settings, case_key, ins_type=None):
    ...
    isolation_position = case_utils.get_case_extend(database, row['CaseKey'], '隔離處所')
    ...
    fields = [
        'DiagFee', 'InterDrugFee', 'PharmacyType', 'PharmacyFee',
...
# 計算自費批價
def calculate_self_fee(parent, database, system_settings,
                       table_widget_prescript, pres_days, packages, self_fee):
    ...
    for row_no in range(row_count):
        ...
        amount = get_table_widget_item_fee(
            table_widget_prescript, row_no, prescript_utils.SELF_PRESCRIPT_COL_NO['Amount'])

        unit_item = table_widget_prescript.item(row_no, prescript_utils.SELF_PRESCRIPT_COL_NO['Unit'])
        ...
        if treat_type != '自購' and instruction_item is not None and instruction_item.text() != '':
            try:
        ...
        if not single_item:
            if dosage_mode == '次劑量' or (by_package == 'Y' and unit in ['顆', '錠']):  # 2025-02-27 耀康
                subtotal *= packages
...
def get_markup_diag_fee(database, ins_apply_row, diag_fee):
    ...
    diag_fee = check_markup_diag_fee(diag_fee, regist_type=string_utils.xstr(case_row['RegistType']))  # 檢查診察費是否需要加成
...
def set_nhi_basic_data(database):
    ...
    rows = [
        ...
        ('處置費', '一般傷科', 'E02', 227,
         '''標準作業程序:
            ...
            適應症:
                ...
                (2)肌腱炎: 網球肘、棒球肩、腕部橈側腱鞘炎等。
                      關節病變: 類風濕性關節炎、退化性關節炎、僵直性關節炎、痛風、冰凍肩(凝肩)等。'''),
        ('處置費', '中度複雜性傷科治療-療程第一次-另開內服藥', 'E03', 427,
         '''支付規範:
                (1)須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
                (2)治療時間合計十分鐘以上。(3)療程第二次-第六次以一般傷科處置(E01, E02)申報。'''),
        ('處置費', '中度複雜性傷科治療-療程第一次', 'E04', 427,
         '''支付規範:
                (1)須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
                (2)治療時間合計十分鐘以上。(3)療程第二次-第六次以一般傷科處置(E01, E02)申報。'''),
        ('處置費', '高度複雜性傷科治療-多部位損傷-起始次-另開內服藥', 'E05', 877,
         '''通則:
                ...
                4.治療時間合計二十分鐘以上。'''),
        ('處置費', '高度複雜性傷科治療-多部位損傷-起始次', 'E06', 877,
         '''通則:
                1.起始次: 係指該病人受傷部位初次到醫療院所做之處理治療。脫臼整復、骨折之起始次處置，含再次復位、再次接骨。
                2.後續治療處置以一般傷科治療處置(E01, E02)申報。
                3.須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
                4.治療時間合計二十分鐘以上。'''),
        ('處置費', '中度複雜性傷科合併特殊疾病-起始次-另開內服藥', 'E07', 877,
         '''通則:
                1.起始次: 係指該病人受傷部位初次到醫療院所做之處理治療。脫臼整復、骨折之起始次處置，含再次復位、再次接骨。
                2.後續治療處置以一般傷科治療處置(E01, E02)申報。
                3.須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
                4.治療時間合計二十分鐘以上。'''),
        ('處置費', '中度複雜性傷科合併特殊疾病-起始次', 'E08', 877,
         '''通則:
                1.起始次: 係指該病人受傷部位初次到醫療院所做之處理治療。脫臼整復、骨折之起始次處置，含再次復位、再次接骨。
                2.後續治療處置以一般傷科治療處置(E01, E02)申報。
                3.須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
        ...
        ('處置費', '脫臼整復復位-起始次-另開內服藥', 'E09', 1177,
         '''通則:
                1.起始次: 係指該病人受傷部位初次到醫療院所做之處理治療。脫臼整復、骨折之起始次處置，含再次復位、再次接骨。
                2.後續治療處置以一般傷科治療處置(E01, E02)申報。
                3.須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
                4.治療時間合計二十分鐘以上。'''),
        ('處置費', '脫臼整復復位-起始次', 'E10', 1177,
         '''通則:
                1.起始次: 係指該病人受傷部位初次到醫療院所做之處理治療。脫臼整復、骨折之起始次處置，含再次復位、再次接骨。
                2.後續治療處置以一般傷科治療處置(E01, E02)申報。
                3.須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
        ...
        ('處置費', '骨折復位-起始次-另開內服藥', 'E11', 1277,
         '''通則:
                1.起始次: 係指該病人受傷部位初次到醫療院所做之處理治療。脫臼整復、骨折之起始次處置，含再次復位、再次接骨。
                2.後續治療處置以一般傷科治療處置(E01, E02)申報。
                3.須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
                4.治療時間合計二十分鐘以上。'''),
        ('處置費', '骨折復位-起始次', 'E12', 1277,
         '''通則:
                1.起始次: 係指該病人受傷部位初次到醫療院所做之處理治療。脫臼整復、骨折之起始次處置，含再次復位、再次接骨。
                2.後續治療處置以一般傷科治療處置(E01, E02)申報。
                3.須合併以下任一輔助治療: 藥薰治療、拔罐治療、刮痧治療、電療、熱療(含紅外線治療)、膏布治療或夾板固定治療。
        ...
        ('照護費', '中醫助孕照護處置費(不含藥費)(同療程第1次)', 'P39005', 900,
         '中醫四診診察費[排卵期評估、女性須含基礎體溫(BBT)、濾泡期、排卵期、黃體期之月經週期療法之診療]、針灸治療處置費、衛教、營養飲食指導，單次門診須全部執行方能申請本項點數。'),
        ('照護費', '中醫保胎照護處置費(不含藥費)(同療程第1次)', 'P39006', 900,
         '中醫四診診察費[排卵期評估、女性須含基礎體溫(BBT)、濾泡期、排卵期、黃體期之月經週期療法之診療]、針灸治療處置費、衛教、營養飲食指導，單次門診須全部執行方能申請本項點數。'),
        ('照護費', '中醫助孕照護針灸處置費(不含藥費)(同療程第2-6次)', 'P39007', 300,
         '限與P39005合併申報、每週限申報3次。'),
        ('照護費', '中醫保胎照護針灸處置費(不含藥費)(同療程第2-6次)', 'P39008', 300,
        ...
        ('照護費', '中醫慢性腎臟病加強照護費（給藥日數 7 天以下）', 'P64001', 900,
         '1.包括中醫醫療四診診察費、口服藥費、調劑費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病加強照護費（給藥日數 8-14 天）', 'P64002', 1250,
         '1.包括中醫醫療四診診察費、口服藥費、調劑費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病加強照護費（給藥日數 15-21 天）', 'P64003', 1600,
         '1.包括中醫醫療四診診察費、口服藥費、調劑費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病加強照護費（給藥日數 22-28 天）', 'P64004', 1950,
         '1.包括中醫醫療四診診察費、口服藥費、調劑費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病加強照護費（給藥日數 7 天以下、針灸處置）', 'P64005', 1300,
         '1.包括中醫醫療四診診察費、口服藥費、調劑費、針灸處置費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病加強照護費（給藥日數 8-14 天、針灸處置）', 'P64006', 1650,
         '1.包括中醫醫療四診診察費、口服藥費、調劑費、針灸處置費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病加強照護費（給藥日數 15-21 天、針灸處置）', 'P64007', 2000,
         '1.包括中醫醫療四診診察費、口服藥費、調劑費、針灸處置費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病加強照護費（給藥日數 22-28 天、針灸處置）', 'P64008', 2350,
         '1.包括中醫醫療四診診察費、口服藥費、調劑費、針灸處置費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病加強照護費（未給口服藥、針灸處置同療程第1次）', 'P64009', 800,
         '1.包括中醫醫療四診診察費、針灸處置費、穴位按摩指導。2.第一次就診須檢附相關檢查數據，應併入病患病歷記錄備查。'),
        ('照護費', '中醫慢性腎臟病針灸照護費（同療程第 2~6 次）', 'P64010', 300,
...
def get_commission_rate(
        database, medicine_key, name, treat_type=None, doctor=None, only_doctor=True, medicine_name=None):
    ...
    if only_doctor and name not in ['醫師', '醫師分成']:  # 以下只有醫師才能讀取medicine的醫師抽成
...
def get_discount_fee(system_settings, self_total_fee, discount_rate):
    ...
    discount_fee = number_utils.get_integer(self_total_fee - (self_total_fee * discount_rate / 100))
