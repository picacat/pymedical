...
ERROR_MESSAGE = {
    ...
    3245: ["簽章已取得就醫資料", None],
    3246: ["病患健保身份已變更", "請執行健保讀卡機作業的「更新病患健保卡內容」"],
    3300: ["金鑰不存在", None],
    3301: ["無虛擬卡模型資料", None],
    3302: ["無 SAM 資料", None],
    ...
    4000: [
        "讀卡機timeout",
        "請檢查讀卡機連接埠是否接妥, 或是系統設定->讀卡機連接埠是否正確",
    ],
    4012: [
        "未置入安全模組卡",
        "安全模組可能未正確安裝至讀卡機內, 請關掉讀卡機電源, 打開螺絲背蓋, 檢查安全模組卡是否安裝妥當.",
    ],
    ...
    4061: [
        "網路不通",
        "請檢查電腦網路接頭是否鬆脫或中華電信VPN網路數據機是否正常, 如有檢查困難, 請致電中華電信 0800-080-128 查詢",
    ],
    4071: ["健保IC卡與IDC認證失敗", None],
    5001: [
        "就醫可用次數不足",
        "請執行[更新病患健保卡內容]作業, 若仍然無法取得可用次數, 請確認病患健保卡加保狀態",
    ],
    5002: ["卡片已註銷", None],
    5003: ["卡片已過有限期限", None],
    ...
    9005: [
        "要求的函式無法執行",
        "1.請確定元件是否為最新版, 2.讀卡機問題, 請更換讀卡機試試看",
    ],
    ...
    9201: [
        "初始安全模組檔讀取異常，請在C:\\NHI\\SAM\\COMX1目錄下放置健保署正確安全模組檔",
        None,
    ],
    9202: [
        "安全模組檔讀取異常，已在其它電腦使用過，請在C:\\NHI\\SAM\\COMX1目錄下放置健保署正確安全模組檔。",
        None,
    ],
...
def decode_basic_data(buffer):
    ...
    try:
        basic_data_info["birthday"] = date_utils.nhi_date_to_west_date(
            s.read(7).decode("ascii")
        )
    except Exception:
        basic_data_info["birthday"] = None

    basic_data_info["gender"] = patient_utils.get_gender(
        s.read(1).decode("ascii").strip()
    )

    try:
        basic_data_info["card_date"] = date_utils.nhi_date_to_west_date(
            s.read(7).decode("ascii")
        )
    except Exception:
...
def decode_register_basic_data(buffer):
    ...
    basic_data_info["insured_code"] = (
        s.read(2).decode("ascii").strip()
    )  # Reserved, not use
    basic_data_info["insured_mark"] = get_insured_mark(
        s.read(1).decode("ascii").strip()
    )
    try:
        basic_data_info["card_valid_date"] = date_utils.nhi_date_to_west_date(
            s.read(7).decode("ascii")
        )
    except Exception:
        basic_data_info["card_valid_date"] = ""

    basic_data_info["card_available_count"] = number_utils.get_integer(
        s.read(2).decode("ascii").strip()
    )
    try:
        basic_data_info["new_born_date"] = date_utils.nhi_date_to_west_date(
            s.read(7).decode("ascii").strip()
        )
    except Exception:
...
def decode_treat_data(buffer):
    ...
    s = io.BytesIO(buffer)
    treat_data_info["registered_date"] = date_utils.nhi_datetime_to_west_datetime(
        s.read(13).decode("ascii")
    )
...
def decode_xml_data(buffer):
    ...
    xml_feedback_data_info["upload_time"] = (
    ...
    minute = s.read(2).decode("ascii").strip()
    second = s.read(2).decode("ascii").strip()
    xml_feedback_data_info["receive_time"] = (
        f"{year}-{month}-{day} {hour}:{minute}:{second}"
    )
...
def need_write_ic_card(database, system_settings, case_key, write_position):
    ...
    if (
        (ins_type == "健保")
        and (system_settings.field("產生醫令簽章位置") == write_position)
        and (system_settings.field("使用讀卡機") == "Y")
        and (card not in nhi_utils.ABNORMAL_CARD)
        and (xcard not in nhi_utils.ABNORMAL_CARD)
        and (card != "欠卡")
    ):
        return True
