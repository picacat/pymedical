...
from libs import (case_utils, date_utils, dialog_utils, nhi_utils,
...
def set_paper_size(printer, system_settings, width_or_enum, length=None, size_unit=None, print_type=None):
...
# 自費處方箋格式3使用 專用
def get_case_html(database, case_key, ins_type, background_color=None,
                  birthday_mask=True, tw_date=False, id_mask=True, medicine_set=None):
...
# 處方箋格式1,2使用
def get_case_html_1(database, case_key, ins_type, background_color=None, print_time_label=False):
    rows = get_case_row(database, case_key)
...
# 處方箋格式3使用
def get_case_html_utec(database, case_key, ins_type, background_color=None,
                       birthday_mask=True, tw_date=False, id_mask=True, medicine_set=None):
    rows = get_case_row(database, case_key)
...
# 其他收據80mm熱感紙
def get_case_html_13(database, case_key, ins_type, background_color=None, print_time_label=False):
    rows = get_case_row(database, case_key)
...
# 處方箋格式3使用
def get_case_html_2_1(database, case_key, ins_type, background_color=None,
                      birthday_mask=True, tw_date=False, id_mask=True):
    rows = get_case_row(database, case_key)
...
# 處方箋格式7使用
def get_case_html_9(database, case_key, ins_type, background_color=None,
                    birthday_mask=True, tw_date=False, id_mask=True):
    rows = get_case_row(database, case_key)
...
# 一般格式使用
def get_case_html_6(database, case_key, ins_type, medicine_set,
                    birthday_mask=True, id_mask=True, tw_date=True, background_color=None):
    rows = get_case_row(database, case_key)
...
# 其他收據格式9藥袋標示使用
def get_case_html_8(database, case_key, ins_type, background_color=None, mask_name=False):
    rows = get_case_row(database, case_key)
...
def get_disease(database, case_key):
    ...
    if case_date.year <= 2024 and dict_icd_map is not None:  # 2023年版ICD預檢 更新
        try:
            disease_code1 = dict_icd_map[disease_code1]  # 申報月份2025年以前只能申報2014年版本ICD-10
        except Exception:
            pass
...
def get_instruction_condition(database, system_settings, case_key, medicine_set, instruction=None):
...
def get_prescript_html(database, system_setting, case_key,
                       medicine_set, print_type, blocks, max_length=None,
                       instruction=None, print_total_dosage=None, print_treat_item=True):
    if medicine_set is None:
        prescript = '''
            <tr>
              <td>無處方</td>
    ...
    treat_condition = ''
    if (print_type == '費用收據' and system_setting.field('列印穴道處置') == 'N' and medicine_set == 1) or \
            (print_type == '過去病歷' and not print_treat_item):
    ...
    instruction_condition = get_instruction_condition(database, system_setting, case_key, medicine_set, instruction)
    sql = f'''
    ...
    if medicine_set == 1 and treatment in nhi_utils.INS_TREAT and \
            instruction not in ['健保另包', '健保檢驗', '自費檢驗']:
    ...
    print_location_before_medicine = system_setting.field('列印藥品存放位置在處方名稱前面')

    if print_total_dosage == 'Y':
        block_width = {
            3: {'medicine_name_width': 19,
                'dosage_width': 6,
        ...
        block_width = {
            3: {'medicine_name_width': 24,
                'dosage_width': 8,
                'total_dosage_width': 0,
                'separator_width': 1},
            2: {'medicine_name_width': 39,
                'dosage_width': 10,
                'total_dosage_width': 0,
                'separator_width': 1},
            1: {'medicine_name_width': 50,
    ...
    for row_no in range(1, row_count+1):
        ...
        for i in range(blocks):
            prescript_block = get_medicine_detail(
                medicine_set, rows, (row_no-1)*blocks + i, pres_days, packages, print_alias)
...
def get_prescript_html2(database, system_setting, case_key, medicine_set,
                        print_type, blocks, instruction=None, max_line=6, wide_dosage=False, is_print_dosage=True):
    print_location = system_setting.field('列印藥品存放位置')
    print_location_before_medicine = system_setting.field('列印藥品存放位置在處方名稱前面')
    ...
    if pres_days <= 0 or system_setting.field('列印針傷處置名稱') == 'Y':
        if medicine_set == 1 and treatment not in ['', None] and \
                instruction not in ['健保另包', '健保檢驗', '自費檢驗']:
            if treatment in nhi_utils.ACUPUNCTURE_TREAT:
                medicine_type = '穴道'
            else:
    ...
    if print_dosage in ['總量', '日量'] or (medicine_set >= 2 and self_daily_dosage in ['Y']):
        block_width = {
            3: {'medicine_name_width': 34,
                'dosage_width': 15,
                'total_dosage_width': 0,
                'separator_width': 1},
            2: {'medicine_name_width': 34,
                'dosage_width': 15,
                'total_dosage_width': 0,
                'separator_width': 1},
            1: {'medicine_name_width': 70,
                'dosage_width': 15,
                'total_dosage_width': 0,
                'separator_width': 1},
        }
        ...
        block_width = {
            3: {'medicine_name_width': 29,
                'dosage_width': 11,
                'total_dosage_width': 10,
                'separator_width': 1},
            2: {'medicine_name_width': 29,
                'dosage_width': 11,
                'total_dosage_width': 10,
                'separator_width': 1},
            1: {'medicine_name_width': 70,
    ...
    for row_no in range(1, row_count+1):
        ...
        for i in range(blocks):
            ...
            if medicine_set >= 2 and dosage_mode == '次劑量' and unit in ['顆', '錠']:  # 悅兒親子 2024-08-23
            ...
            if instruction in ['健保檢驗', '自費檢驗']:
                prescript_line += f'''
                    ...
                    <td align="right" width="{dosage_width}%">{dosage}{unit}</td>
                '''
            elif print_dosage in ['總量', '日量'] or (medicine_set >= 2 and self_daily_dosage == 'Y'):
                dosage_str = total_dosage  # 預設列印總量
                if print_dosage in ['日量'] or (medicine_set >= 2 and self_daily_dosage == 'Y'):
                    dosage_str = dosage
...
def get_prescript_html7(database, system_setting, case_key,
                        medicine_set, print_type, blocks, max_length=None,
                        instruction=None, print_total_dosage=None):
    if medicine_set is None:
        prescript = '''
            <tr>
              <td>無處方</td>
...
def get_prescript_block3_html(
        ...
        print_alias, blocks=3, instruction=None):
    if medicine_set is None:
        prescript = '''
            <tr>
              <td>無處方</td>
...
def get_medicine_detail(medicine_set, rows, row_no, pres_days, packages, print_alias='N', by_packages=False):
    try:
    ...
    try:
        if medicine_set == 1 and medicine_type in ['穴道', '處置']:
            ...
            if dosage_mode == '次劑量':
                ...
                total_dosage = dosage
            elif by_packages and medicine_set >= 2 and unit not in ['克', '錢', '帖', '次']:  # 2025-02-06 耀康 非科中或水藥要乘以包數
                total_dosage *= packages
    ...
    return medicine_name, location, dosage, unit, total_dosage, ins_code, instruction, medicine_type


def get_instruction_html_one_line(database, system_settings, case_key, medicine_set, additional=None):
    sql = f'''
...
def get_instruction_html(database, system_settings, case_key, medicine_set, additional=None):
    sql = f'''
        SELECT CaseDate, Doctor, DrugShareFee, TotalFee FROM cases
        WHERE
            CaseKey = {case_key}
    ...
    prescript_total_fee = number_utils.get_integer(case_utils.get_total_fee(database, case_key, medicine_set))
    drug_share_fee = number_utils.get_integer(row['DrugShareFee'])
    total_fee = number_utils.get_integer(row['TotalFee'])
    if total_fee <= 0:
...
def get_instruction_html1(
        database, system_settings, case_key, medicine_set,
        additional=None, print_total_fee=True, print_pharmacy_date=False):
    sql = f'''
...
def get_instruction_html_utec(database, system_settings, case_key, medicine_set, additional=None, print_total_fee=True):
    sql = f'''
        SELECT CaseDate, RegistNo, Doctor, RegistFee, SDiagShareFee, SDrugShareFee, TotalFee FROM cases
        WHERE
            CaseKey = {case_key}
...
def get_instruction_html1_1(database, system_settings, case_key, medicine_set, additional=None):
    sql = f'''
    ...
    _, total_dosage, _, single_day_dosage = case_utils.get_prescript_html_data(database, system_settings, case_key, medicine_set)
    total_dosage = f'{total_dosage:.1f}'
...
def get_instruction_html2(database, system_settings, case_key, medicine_set, additional=None, ins_exam=False):
    sql = f'''
        SELECT * FROM cases
        WHERE
            CaseKey = {case_key}
...
def get_instruction_html3(database, system_settings, case_key, medicine_set, additional=None):
    sql = f'''
        SELECT * FROM cases
        WHERE
            CaseKey = {case_key}
...
def get_instruction_html4(database, system_settings, case_key, medicine_set, additional=None):
    sql = f'''
        SELECT CaseDate, Doctor, DrugShareFee, TotalFee FROM cases
        WHERE
            CaseKey = {case_key}
...
# 取得列印健保或自費處方dialog
def get_medicine_set_items(parent, database, system_settings, case_key, form_type, print_type='選擇列印'):
    if case_key in ['', None]:
        return
    ...
    if row_count <= 0:
        ...
        dialog = dialog_utils.get_dialog_select_medicine_set(parent, database, system_settings, case_key, form_type)
        if print_type == '選擇列印':
            if not dialog.exec_():
                items = None
            else:
...
# 列印門診掛號單
def print_regist_form(parent, database, system_settings, case_key, print_option='系統設定'):
    ...
    if printable == '不印':
        ...
        msg_box.setInformativeText("<font size='4'>注意！掛號收據將會列印至系統設定的印表機.</font>")
...
# 列印民俗調理單
def print_massage_form(parent, database, system_settings, case_key, print_option='系統設定'):
    ...
    if printable == '不印':
        ...
        msg_box.setInformativeText("<font size='4'>注意！民俗調理單將會列印至系統設定的印表機.</font>")
        msg_box.addButton(QPushButton("否"), QMessageBox.NoRole)  # 0
        msg_box.addButton(QPushButton("是"), QMessageBox.AcceptRole)  # 1
    ...
    if system_settings.field('民俗調理單印表機2') not in ['', None]:
        ...
        print_form2 = print_massage_form_func(parent, database, system_settings, case_key, printer2)
...
def get_print_prescription(parent, database, system_settings, case_key, print_mode):
    ...
    print_module = module.PrintPrescription(parent, database, system_settings, case_key, print_mode)
...
# 列印費用收據
def get_print_receipt(parent, database, system_settings, case_key, print_mode,
                      print_dosage=True, print_only_medicine_set2=False):
    from printer import print_receipt

    module = importlib.reload(print_receipt)
    print_module = module.PrintReceipt(
        parent, database, system_settings, case_key, print_mode, print_dosage, print_only_medicine_set2)
...
def print_receipt_form(
    ...
    print_form = get_print_receipt(
        parent, database, system_settings, case_key, print_option, print_dosage,
...
def get_print_misc(parent, database, system_settings, case_key, print_mode):
    ...
    print_module = module.PrintMisc(parent, database, system_settings, case_key, print_mode)
...
def get_print_misc2(parent, database, system_settings, case_key, print_mode):
    ...
    print_module = module.PrintMisc2(parent, database, system_settings, case_key, print_mode)
...
def print_misc_form2(parent, database, system_settings, case_key, print_type):
    print_form = get_print_misc2(parent, database, system_settings, case_key, print_type)
    print_form.print()
...
def get_print_misc3(parent, database, system_settings, case_key, print_mode):
    ...
    print_module = module.PrintMisc3(parent, database, system_settings, case_key, print_mode)
...
def print_misc_form3(parent, database, system_settings, case_key, print_type):
    print_form = get_print_misc3(parent, database, system_settings, case_key, print_type)
    print_form.print()
...
def get_print_prescription_bag(parent, database, system_settings, case_key, print_mode):
    ...
    print_module = module.PrintPrescriptionBag(parent, database, system_settings, case_key, print_mode)

    return print_module


# 列印藥袋
def print_prescription_bag_form(parent, database, system_settings, case_key, print_option):
...
# 列印健保處方箋
def print_ins_prescript(parent, database, system_settings, case_key, print_type, print_option='系統設定'):
    ...
    if printable == '不印':
        ...
        msg_box.setInformativeText("<font size='4'>注意！處方箋將會列印至系統設定的印表機.</font>")
        msg_box.addButton(QPushButton("否"), QMessageBox.NoRole)  # 0
        msg_box.addButton(QPushButton("是"), QMessageBox.AcceptRole)  # 1
    ...
    print_form = print_prescription_ins_form(parent, database, system_settings, case_key)
...
def print_self_prescript(parent, database, system_settings, case_key, medicine_set,
    ...
    prescript_count = get_prescript_count(database, case_key, medicine_set=0)  # medicine_set = 0 全部自費
    ...
    print_form = print_prescription_self_form(parent, database, system_settings, case_key, medicine_set)
    if print_type == 'print':
        print_form.print()
    else:
        print_form.preview()
...
# 列印健保醫療收據
def print_ins_receipt(parent, database, system_settings, case_key, print_type,
                      print_option='系統設定', print_dosage=True, print_note=False):
    ...
    if printable == '不印':
        ...
        msg_box.setText("<font size='4' color='red'><b>是否列印健保醫療收據?</b></font>")
        msg_box.setInformativeText("<font size='4'>注意！醫療收據將會列印至系統設定的印表機.</font>")
        msg_box.addButton(QPushButton("否"), QMessageBox.NoRole)  # 0
        msg_box.addButton(QPushButton("是"), QMessageBox.AcceptRole)  # 1
    ...
    print_form = print_receipt_ins_form(parent, database, system_settings, case_key, print_dosage)
...
def print_self_receipt(parent, database, system_settings, case_key, medicine_set,
                       print_type, print_option='系統設定', print_dosage=True):
    ...
    if printable == '不印':
        ...
        msg_box.setText("<font size='4' color='red'><b>是否列印自費醫療收據?</b></font>")
        msg_box.setInformativeText("<font size='4'>注意！醫療收據將會列印至系統設定的印表機.</font>")
        msg_box.addButton(QPushButton("否"), QMessageBox.NoRole)  # 0
        msg_box.addButton(QPushButton("是"), QMessageBox.AcceptRole)  # 1
    ...
    if print_option == '系統設定' and form[:2] in ['09'] and medicine_set >= 3:  # 格式09 只印自費1(已經包含全部自費金額), 其他不印
        return

    if print_option == '系統設定' and medicine_set == 2 and \
            prescript_utils.is_folk_massage(database, system_settings, case_key) and \
            system_settings.field('列印民俗調理') != 'Y':  # 民俗調理是否列印收據
        return

    print_form = print_receipt_self_form(parent, database, system_settings, case_key, medicine_set, print_dosage)
...
# 列印藥袋
def print_prescript_bag(parent, database, system_settings, case_key, print_type, medicine_set, print_option='系統設定'):
    ...
    if printable == '不印':
        ...
        msg_box.setInformativeText("<font size='4'>注意！藥袋將會列印至系統設定的印表機.</font>")
        msg_box.addButton(QPushButton("否"), QMessageBox.NoRole)  # 0
        msg_box.addButton(QPushButton("是"), QMessageBox.AcceptRole)  # 1
    ...
    if form[:2] not in ['05', '07', '08'] and medicine_set >= 2:  # 自費2只印藥袋5, 7, 8 其他不印
        return

    pres_days1 = case_utils.get_pres_days(database, case_key, medicine_set=1)
    current_ins_prescript_rows = prescript_utils.get_ins_prescript_rows(database, case_key, medicine_set)
    ...
    print_form = print_prescription_bag_form(parent, database, system_settings, case_key, medicine_set)
...
# 列印其他收據
def print_misc(parent, database, system_settings, case_key, print_type, print_option='系統設定'):
    ...
    if form_no in ['30']:  # Form30: 領藥單
        ...
        if len(rows) <= 0:
            return
    elif form_no in ['1', '2', '10', '11', '12', '13', '15', '16']:  # Form12, 13 自費費用收據
        total_fee = 0
        sql = f'''
            SELECT TotalFee FROM cases
            WHERE
    ...
    if printable == '不印':
        ...
        msg_box.setInformativeText(f"<font size='4'>注意！{form}將會列印至系統設定的印表機.</font>")
        msg_box.addButton(QPushButton("否"), QMessageBox.NoRole)  # 0
        msg_box.addButton(QPushButton("是"), QMessageBox.AcceptRole)  # 1
...
# 列印其他收據2
def print_misc2(parent, database, system_settings, case_key, print_type, print_option='系統設定'):
...
# 列印其他收據
def print_misc3(parent, database, system_settings, case_key, print_type, print_option='系統設定'):
...
def print_reservation(parent, database, system_settings, reservation_key,
    ...
    print_form = print_reservation_form(parent, database, system_settings, reservation_key)
...
def get_print_ins_apply_total_fee(parent, database, system_settings, ins_total_fee):
    ...
    print_module = module.PrintInsApplyTotalFee(parent, database, system_settings, ins_total_fee)

    return print_module


# 列印申請總表
def print_form_ins_apply_total_fee(parent, database, system_settings, ins_total_fee, print_type='print'):
    if print_type == 'pdf':
        pass
...
def get_print_html(parent, database, system_settings, html, orientation):
    ...
    print_module = module.PrintHtml(parent, database, system_settings, html, orientation)
...
# 列印html
def print_form_html(
        parent, database, system_settings, html, orientation='portrait', print_type='print', filename=None):
    if print_type == 'print':
...
def get_print_ins_apply_schedule_table(parent, database, system_settings, html, apply_date):
    from printer import print_ins_apply_schedule_table

    module = importlib.reload(print_ins_apply_schedule_table)
    print_module = module.PrintInsApplyScheduleTable(parent, database, system_settings, html, apply_date)

    return print_module


# 列印排班表
def print_form_ins_apply_schedule_table(parent, database, system_settings, html, apply_date, print_type='print'):
    if print_type == 'pdf':
        pass
    elif system_settings.field('列印報表') == '詢問':
        dialog = QtPrintSupport.QPrintDialog()
    ...
    print_form = get_print_ins_apply_schedule_table(parent, database, system_settings, html, apply_date)
...
def get_print_ins_apply_order(parent, database, system_settings, apply_year, apply_month, apply_type, ins_apply_key):
    ...
    print_module = module.PrintInsApplyOrder(
        parent, database, system_settings, apply_year, apply_month, apply_type, ins_apply_key
    )
...
# 列印醫令明細
def print_form_ins_apply_order(parent, database, system_settings,
...
def get_print_medical_records(parent, database, system_settings, patient_key,
                              sql, start_date, end_date, print_self_prescript=False, print_treat_item=True):
    ...
    print_module = module.PrintMedicalRecords(
        parent, database, system_settings, patient_key, sql, start_date, end_date,
        print_self_prescript, print_treat_item,
    )
...
# 列印實體病歷
def print_form_medical_records(parent, database, system_settings,
                               patient_key, sql, start_date, end_date, print_type=None,
                               print_self_prescript=False, print_treat_item=True):
...
def get_print_referral_form(parent, database, system_settings, patient_key, case_key):
    ...
    print_module = module.PrintReferralForm(parent, database, system_settings, patient_key, case_key)

    return print_module


# 列印轉診單
def print_referral_form(parent, database, system_settings, patient_key, case_key, print_type):
    print_referral = get_print_referral_form(parent, database, system_settings, patient_key, case_key)
...
def get_print_medical_fees(parent, database, system_settings, patient_key, sql):
    ...
    print_module = module.PrintMedicalFees(
        parent, database, system_settings, patient_key, sql,
    )
...
# 列印收費明細
def print_form_medical_fees(parent, database, system_settings, patient_key, sql, print_type=None):
...
def get_print_medical_chart(parent, database, system_settings, patient_key, apply_date):
    ...
    print_module = module.PrintMedicalChart(
        parent, database, system_settings, patient_key, apply_date,
    )
...
# 列印就醫證明
def print_form_medical_certificate(parent, database, system_settings, case_key, print_type=None):
...
# 列印雙月病歷首頁
def print_form_medical_chart(parent, database, system_settings, patient_key, apply_date, print_type=None):
...
def get_print_simple_medical_chart(parent, database, system_settings, patient_key):
    ...
    print_module = module.PrintSimpleMedicalChart(parent, database, system_settings, patient_key)

    return print_module


# 列印簡易病歷首頁
def print_form_simple_medical_chart(parent, database, system_settings, patient_key, print_type=None):
    print_simple_medical_chart = get_print_simple_medical_chart(parent, database, system_settings, patient_key)
    print_form(system_settings, print_simple_medical_chart, print_type)


def get_print_patient_new_care(parent, database, system_settings, patient_key, case_key, apply_date):
...
# 列印初診照護病歷
def print_form_patient_new_care(parent, database, system_settings, patient_key, case_key, apply_date, print_type=None):
...
def get_print_diagnosis_proof(parent, database, system_settings, certificate_key, title):
    from printer import print_diagnosis_proof

    module = importlib.reload(print_diagnosis_proof)
    print_module = module.PrintDiagnosisProof(parent, database, system_settings, certificate_key, title)

    return print_module


# 列印就醫證明書
def print_form_diagnosis_proof(parent, database, system_settings, certificate_key, title, print_type=None):
    print_diagnosis_proof = get_print_diagnosis_proof(parent, database, system_settings, certificate_key, title)
    print_form(system_settings, print_diagnosis_proof, print_type)


def get_print_certificate_diagnosis(parent, database, system_settings, certificate_key, title):
    from printer import print_certificate_diagnosis

    module = importlib.reload(print_certificate_diagnosis)
    print_module = module.PrintCertificateDiagnosis(parent, database, system_settings, certificate_key, title)

    return print_module


# 列印診斷證明書
def print_form_certificate_diagnosis(parent, database, system_settings, certificate_key, title, print_type=None):
...
def get_print_certificate_payment(parent, database, system_settings, certificate_key, show_tax_declare):
...
def print_form_certificate_payment(
        parent, database, system_settings, certificate_key, show_tax_declare, print_type=None):
    print_certificate_payment = get_print_certificate_payment(
        parent, database, system_settings, certificate_key, show_tax_declare)
    print_form(system_settings, print_certificate_payment, print_type)


def get_print_certificate_cash_payment(parent, database, system_settings, certificate_key):
    from printer import print_certificate_cash_payment

    module = importlib.reload(print_certificate_cash_payment)
    print_module = module.PrintCertificateCashPayment(parent, database, system_settings, certificate_key)

    return print_module


def get_print_certificate_cash_payment2(parent, database, system_settings, certificate_key, form_type=None):
...
def get_print_certificate_ins_fee(parent, database, system_settings, certificate_key):
    ...
    print_module = module.PrintCertificateInsFee(parent, database, system_settings, certificate_key)
...
# 列印醫療費用證明書明細
def print_form_certificate_cash_payment(parent, database, system_settings, certificate_key, print_type=None):
...
def get_print_certificate_payment_total(parent, database, system_settings, certificate_key, print_ins_fee):
...
def print_form_certificate_total(
        parent, database, system_settings, certificate_key, print_type=None, print_ins_fee=True):
...
def get_print_certificate_payment_receipt(parent, database, system_settings, certificate_key):
    from printer import print_certificate_payment_receipt

    module = importlib.reload(print_certificate_payment_receipt)
    print_module = module.PrintCertificatePaymentReceipt(parent, database, system_settings, certificate_key)
...
def get_print_certificate_payment_prescript(parent, database, system_settings, certificate_key, form_type):
...
# 列印醫療費用證明書處方明細
def print_form_certificate_prescript(parent, database, system_settings, certificate_key, form_type=None):
    print_certificate_payment_prescript = get_print_certificate_payment_prescript(
        parent, database, system_settings, certificate_key, form_type,
    )
    print_form(system_settings, print_certificate_payment_prescript, form_type=form_type)


def get_print_certificate_payment_self_prescript(parent, database, system_settings, certificate_key):
...
# 列印醫療費用證明書自費處方明細
def print_form_certificate_self_prescript(parent, database, system_settings, certificate_key, print_type=None):
    print_certificate_payment_self_prescript = get_print_certificate_payment_self_prescript(
        parent, database, system_settings, certificate_key,
    )
...
def get_prescript_count(database, case_key, medicine_set):
    if medicine_set == 1:
        medicine_type_script = 'MedicineSet = 1 AND MedicineType NOT IN ("穴道", "處置", "檢驗") '
    else:
        medicine_type_script = 'MedicineSet >= 2 AND MedicineType NOT IN ("穴道", "處置", "檢驗") '
...
def get_print_correction_area_income_list(parent, database, system_settings,
                                          table_widget_medical_record_list, column_name):
...
def print_correction_reg_income(
        parent, database, system_settings, table_widget_medical_record_list, column_name, print_type=None):
...
def get_print_ins_regist_fee_discount(parent, database, system_settings,
                                      start_date, end_date, table_widget_medical_record):
...
def print_regist_fee_discount(
        parent, database, system_settings, start_date, end_date, table_widget_medical_record, print_type=None):
...
def get_print_income(parent, database, system_settings, orientation, income_date, income_period,
                     tableWidget_income, tableWidget_total, income_list_columns):
    from printer import print_income

    module = importlib.reload(print_income)
    print_module = module.PrintIncome(
        parent, database, system_settings, orientation, income_date, income_period,
        tableWidget_income, tableWidget_total, income_list_columns)
...
# 掛號櫃台結帳直式日報表1
def print_income(parent, database, system_settings, orientation, income_date, income_period,
                 tableWidget_income, tableWidget_total, income_list_columns, print_type=None):
...
def get_print_income2(parent, database, system_settings, orientation,
                      tab_income_cash_flow, tab_income_list, income_list_columns):
    from printer import print_income2

    module = importlib.reload(print_income2)
    print_module = module.PrintIncome2(
        parent, database, system_settings, orientation,
        tab_income_cash_flow, tab_income_list, income_list_columns)
...
# 掛號櫃台結帳直式日報表1
def print_income2(parent, database, system_settings, orientation,
                  tab_income_cash_flow, tab_income_list, income_list_columns, print_type=None):
    print_income2 = get_print_income2(
        parent, database, system_settings, orientation, tab_income_cash_flow, tab_income_list, income_list_columns)
    print_form(system_settings, print_income2, print_type)


def get_print_income_ins_list(parent, database, system_settings, start_date, period,
                              tableWidget_income, tableWidget_free_count, tableWidget_summary):
    ...
    print_module = module.PrintIncomeInsList(
        parent, database, system_settings, start_date, period,
        tableWidget_income, tableWidget_free_count, tableWidget_summary)
...
# 掛號櫃台健保收費明細
def print_income_ins_list(parent, database, system_settings, start_date, period,
                          tableWidget_income, tableWidget_free_count, tableWidget_summary, print_type=None):
...
# 列印預約名單
def get_print_reservation_list(parent, database, system_settings, start_date, end_date, period,
                               doctor, tableWidget_reservation_list):
    ...
    print_module = module.PrintReservationList(
        parent, database, system_settings, start_date, end_date, period, doctor, tableWidget_reservation_list)

    return print_module


# 列印預約名單
def print_reservation_list(parent, database, system_settings, start_date, end_date, period,
                           doctor, tableWidget_reservation_list, print_type=None):
...
# 列印矯正機關內門診預約名單
def get_print_correction_area_reservation_list(parent, database, system_settings, start_date, end_date, period,
                                               doctor, tableWidget_reservation_list):
...
# 列印矯正機關內門診預約名單
def print_correction_area_reservation_list(parent, database, system_settings, start_date, end_date, period,
                                           doctor, tableWidget_reservation_list, print_type=None):
...
def get_print_purchase_list(
        ...
        tableWidget_self_prescript, tableWidget_self_prescript_agent, no_zero_bonus):
...
# 自費管理-自費銷售記錄-列印自費銷售記錄
def print_purchase_list(parent, database, system_settings, start_date, end_date,
                        tableWidget_self_prescript, tableWidget_self_prescript_agent=None,
                        print_type=None, no_zero_bonus=False):
...
# 自費管理-自費銷售記錄-列印自費銷售總表
def print_seller(parent, database, system_settings, start_date, end_date,
                 tableWidget_seller, print_type=None):
...
# 自費管理-自費銷售抽成總表-列印
def get_print_doctor_sale_summary(parent, database, system_settings, start_date, end_date, tableWidget_prescript):
...
# 自費管理-自費銷售抽成總表-列印
def print_doctor_sale_summary(parent, database, system_settings, start_date, end_date,
                              tableWidget_prescript, print_type=None):
...
# 醫師統計-門診收入總覽-列印
def get_print_doctor_summary(parent, database, system_settings, start_date, end_date, doctor,
    ...
    print_module = module.PrintDoctorSummary(
        parent, database, system_settings, start_date, end_date, doctor, tableWidget_doctor_summary
    )
...
# 醫師統計-門診收入總覽-列印
def print_doctor_summary(parent, database, system_settings, start_date, end_date, doctor,
                         tableWidget_doctor_summary, print_type=None):
...
# 醫師月報表-收入統計-列印
def get_print_doctor_monthly_income(parent, database, system_settings, start_date, end_date, doctor,
...
# 醫師月報表-收入統計-列印
def print_doctor_monthly_income(parent, database, system_settings, start_date, end_date, doctor,
                                tableWidget_doctor_monthly, print_type=None):
    print_list = get_print_doctor_monthly_income(
        parent, database, system_settings, start_date, end_date, doctor, tableWidget_doctor_monthly)
    print_form(system_settings, print_list, print_type)
...
def get_print_growth_income(parent, database, system_settings, tableWidget_income_list):
    ...
    print_module = module.PrintGrowthIncome(parent, database, system_settings, tableWidget_income_list)

    return print_module


# 列印業績成長-年度收入統計
def print_growth_income(parent, database, system_settings, tableWidget_income_list, print_type=None):
    print_income = get_print_growth_income(parent, database, system_settings, tableWidget_income_list)
    print_form(system_settings, print_income, print_type)


# 天地精進格式使用
def get_case_html_10(database, case_key, ins_type, medicine_set,
                     birthday_mask=True, id_mask=True, tw_date=True, background_color=None):
    rows = get_case_row(database, case_key)
...
def get_prescript_html23(database, system_setting, case_key,
                         medicine_set, print_type, blocks, max_length=None,
                         instruction=None, print_total_dosage=None, print_treat_item=True,
                         print_dosage=True):
    if medicine_set is None:
    ...
    if print_total_dosage == 'Y':
        block_width = {
            3: {'medicine_name_width': 15,
                'dosage_width': 6,
                'total_dosage_width': 5,
                'separator_width': 1},
            2: {'medicine_name_width': 26,
                'dosage_width': 10,
                'total_dosage_width': 9,
                'separator_width': 1},
            1: {'medicine_name_width': 55,
                'dosage_width': 20,
                'total_dosage_width': 20,
                'separator_width': 4},
        }
    ...
    for row_no in range(1, row_count+1):
        ...
        for i in range(blocks):
            prescript_block = get_medicine_detail(
                medicine_set, rows, (row_no-1)*blocks + i, pres_days, packages, print_alias, by_packages=by_packages)
            ...
            if print_location != 'Y':
                ...
                try:
                    if medicine_type in ['單方', '複方']:
                        ...
                        if current_location not in ['', None] and location != current_location:
...
def get_prescript_html27(database, system_setting, case_key,
                       medicine_set, print_type, blocks, max_length=None,
                       instruction=None, print_total_dosage=None, print_treat_item=True):
    if medicine_set is None:
        prescript = '''
    ...
    block1_rows, block2_rows = split_rows_into_blocks(rows, num_blocks=blocks, block_size=max_length)
    print_location = system_setting.field('列印藥品存放位置')
    print_location_before_medicine = system_setting.field('列印藥品存放位置在處方名稱前面')
    ...
    for row_no in range(max_length):
        ...
        if print_location == 'Y':
            if print_location_before_medicine == 'Y':
                medicine_name_block1 = string_utils.xstr(block1_rows[row_no]['Location']) + medicine_name_block1
                medicine_name_block2 = string_utils.xstr(block2_rows[row_no]['Location']) + medicine_name_block2
            else:
                medicine_name_block1 += string_utils.xstr(block1_rows[row_no]['Location'])
                medicine_name_block2 += string_utils.xstr(block2_rows[row_no]['Location'])
...
# 處方箋格式27使用
def get_case_html_27(database, case_key, medicine_set, ins_type, background_color=None,
                     birthday_mask=False, tw_date=False, id_mask=True):
    rows = get_case_row(database, case_key)
...
# 處方箋格式3使用
def get_case_html_23(database, case_key, ins_type, background_color=None,
                     birthday_mask=True, tw_date=False, id_mask=True):
    rows = get_case_row(database, case_key)
...
# 自費費用
def get_self_fees_html_dynamic(database, system_settings, case_key, medicine_set, width=15):
    sql = f'''
        SELECT * FROM cases
        WHERE
            CaseKey = {case_key}
    ...
    if system_settings.field('列印所有收費收據費用明細') == 'Y' and \
            system_settings.field('列印所有收費收據各自金額') == 'Y':
        ...
        if medicine_set == 2 and total_medicine_set >= 2:  # 自費有兩帖以上, 而且各帖的自費折扣加總不等於總折扣
            this_discount_fee = 0
    ...
    if system_settings.field('列印所有收費收據費用明細') == 'Y' and \
            system_settings.field('列印所有收費收據各自金額') == 'Y' and \
            medicine_set >= 3:  # 自費2 以後都不印
        pass
    elif regist_fee > 0:
...
# 自費費用
def get_self_fees_html_dynamic29(database, system_settings, case_key, medicine_set,
                                 print_cash_fees=False, width=15):
    sql = f'''
        SELECT * FROM cases
        WHERE
            CaseKey = {case_key}
...
def get_title_image(clinic_name, clinic_id, clinic_telephone, clinic_address, title='醫療費用收據'):
    import os
    ...
    if os.path.exists(image_file):
        if title == '門診掛號單':
            br_line = ''
        elif clinic_name in ['帖人中醫診所', '仲明堂中醫診所', '青田中醫診所', '明醫中醫診所']:
...
def get_case_html_24(database, system_settings, case_key, ins_type, background_color=None,
                    birthday_mask=True, tw_date=False, id_mask=True):
    rows = get_case_row(database, case_key)
...
def get_prescript_html24(database, system_setting, case_key,
                         medicine_set, print_type, blocks, max_length=None,
                         instruction=None, print_total_dosage=None, print_treat_item=True,
                         td_width=250, border=1, block_len=10, header=True, print_sequence=True,):
    if medicine_set is None:
        prescript_td = '''
            <tr>
              <td>無處方</td>
    ...
    for row_no in range(1, row_count+1):
        for i in range(blocks):
            ...
            if print_sequence:
                current_line = f'<td align="center" width="15%">{seq_str}</td>' + current_line
...
def get_prescript_html29(database, system_setting, case_key,
                         medicine_set, print_type, blocks, max_length=None,
                         instruction=None, print_total_dosage=None, print_treat_item=True,
                         td_width=250, border=1, block_len=10, header=True, print_sequence=True,):
    if medicine_set is None:
