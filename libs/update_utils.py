...
def update_nhi_payment(database):
    ...
    title = '下載健保支付標準檔'
    message = '<font size="5" color="red"><b>正在下載健保支付標準檔, 請稍後...</b></font>'
    hint = '正在與更新檔資料庫連線, 會花費一些時間.'

    download_file = dropbox_utils.download_dropbox_file(filename, url, title, message, hint)
...
def update_chronic_condition(parent, database):
    ...
    for i, key in enumerate(json_data):
        ...
        database.exec_sql(f'UPDATE icd10 SET SpecialCode = "{chronic_code}" WHERE ICDCode = "{icd_code}"')
...
def update_2023_icd10(parent, database):
    ...
    progress_dialog.setValue(0)
    fields = [
        'ICDCode', 'ChineseName', 'EnglishName',
    ]
    for i, item in enumerate(json_data):
        ...
        rows = database.select_record(f'SELECT ICD10Key FROM icd10 WHERE ICDCode = "{icd_code}"')
        if len(rows) > 0:
            database.exec_sql(f'''
                UPDATE icd10 SET
            ...
            data = [
                icd_code, chinese_name, english_name,
            ]
