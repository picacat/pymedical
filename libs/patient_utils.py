...
# 尋找病患資料
def search_patient(ui, database, settings, keyword, verify_keyword=None,
                   use_patient_key=False):
...
def is_two_years_ago_visit(database, patient_key):
    ...
    start_date = two_years_ago.replace(year=last_month_year, month=last_month, day=1).strftime('%Y-%m-%d')
...
def is_no_return_days(database, patient_key, no_return_days):
    no_return = False
    start_date = (datetime.datetime.now() - datetime.timedelta(days=no_return_days)).date()
...
def get_visit(database, patient_key):
    ...
    if row['InitDate'].year == current_date.year and \
       row['InitDate'].month == current_date.month and \
       row['InitDate'].day == current_date.day:
        visit = '初診'
...
def get_init_date(database, system_settings, patient_key):
    ...
    if init_date != '' and \
       ins_judge_init_date is not None and ins_judge_init_date != '' and \
       init_date >= ins_judge_init_date:
...
def write_patient_new_care(database, patient_key, field, value):
    fields = [
        'PatientKey', 'Field', 'Value',
    ]

    data = [
        patient_key, field, value
    ]
...
def set_patient_extension_settings(database, patient_key, extension_type, content):
    ...
    fields = [
        'PatientKey', 'ExtensionType', 'Content',
    ]

    data = [
        patient_key, extension_type, content
    ]
