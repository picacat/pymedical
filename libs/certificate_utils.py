...
from libs import number_utils, registration_utils, string_utils
...
def insert_medical_record(
    database, system_settings, patient_key, name, certificate_fee
):
    charge_date = None
    charge_period = None
    charge_done = "False"
...
# 檢查證明書是否重複開立
def check_certificate_duplicate(
    database, certificate_type, patient_key, start_date, end_date
):
    duplicate = False
...
def get_total_certificate_fees(database, certificate_key):
    ...
    for row in rows:
        ...
        self_treat_fee = (
            self_diag_fee + self_acupuncture_fee + self_massage_fee + self_dislocate_fee
        )

        self_misc_fee = number_utils.get_integer(
            row["SMaterialFee"]
