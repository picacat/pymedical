...
from libs import (case_utils, date_utils, db_utils, nhi_utils, number_utils,
                  patient_utils, personnel_utils, prescript_utils,
...
def get_medical_record_html(database, system_settings, case_key, show_separator=True):
    ...
    if row['InsType'] == '健保' and max_medicine_set >= 2:
        medical_record += f' <b><font color="blue">(含自費, 金額: {total_fee})</font></b>'
...
def get_prescript_record(database, system_settings, case_key, display_total_dosage=False, display_location=False):
    sql = f'''
    ...
    html_str = get_prescript_medicine_record(
        database, system_settings, case_key, 1,
        display_total_dosage=display_total_dosage, display_location=display_location)
...
def get_prescript_html_data(
        ...
        treatment=None, display_total_dosage=False, display_location=False):
    prescript_data = ''
    ...
    if medicine_set == 1:  # 健保才過濾處方或處置
        if treatment is None:
            treatment_script = ' AND prescript.MedicineType NOT IN ("穴道", "處置", "檢驗") '
        else:
            treatment_script = ' AND prescript.MedicineType IN ("穴道", "處置", "檢驗") '
    ...
    for row in rows:
        if string_utils.xstr(row['MedicineName']) in ['', '優待', '自費藥費', '自費粉藥', '自費水藥']:
            continue
    ...
    return prescript_data, round(total_pres_dosage, 3), total_fee, round(single_day_dosage, 3)
...
def get_dosage_html(database, system_settings, case_key, medicine_set, total_dosage, total_fee):
    dosage_data = ''
...
def get_prescript_medicine_record(database, system_settings, case_key, medicine_set,
                                  display_total_dosage=False, display_location=False):
    prescript_data, total_dosage, total_fee, single_day_dosage = get_prescript_html_data(
        database, system_settings, case_key, medicine_set,
        display_total_dosage=display_total_dosage, display_location=display_location
    )
    ...
    prescript_data += get_dosage_html(database, system_settings, case_key, medicine_set, total_dosage, total_fee)
    if medicine_set == 1:
...
def get_ins_prescript_treat_record(database, system_settings, case_key):
    ...
    '''
    prescript_data, _, _, _ = get_prescript_html_data(database, system_settings, case_key, 1, True)
...
def get_self_prescript_medicine_record(database, system_settings, case_key, display_total_dosage=False):
    prescript_html = ''
    ...
    for medicine_set in range(2, max_medicine_set+1):
        prescript_html += get_prescript_medicine_record(
            database, system_settings, case_key, medicine_set,
            display_total_dosage=display_total_dosage, display_location=True)
...
def copy_past_medical_record(
        database, system_settings, medical_record, case_key, copy_diagnostic, copy_remark, copy_disease,
        copy_ins_prescript, copy_ins_prescript_to, copy_ins_treat,
        copy_self_prescript, copy_self_prescript_to_ins, not_overwrite=False):
    sql = f'''
        SELECT * FROM cases
        WHERE
            CaseKey = {case_key}
    ...
    if copy_disease:
        ...
        if len(error_message) > 0:
            system_utils.show_message_box( QtWidgets.QMessageBox.Critical,
                'ICD10資料已作廢',
    ...
    if copy_self_prescript:
        ...
        for row in rows:
            ...
            if system_settings.field('健保自費分開') == 'Y':
                pass
            elif medical_record.tab_list[tab_index] is None or \
                    medical_record.tab_list[tab_index].tableWidget_prescript.item(
                        0, prescript_utils.SELF_PRESCRIPT_COL_NO['MedicineName']) is None:
    ...
    if system_settings.field('健保自費分開') == 'Y':
        pass
    elif medical_record.tab_list[1] is None:  # 2019.04.27 拷貝完, 清除所有自費處方後, 自動新增自費處方1
...
# 拷貝處方集
def copy_collection(
        database, medical_record, case_date, medical_row,
        copy_diagnostic, copy_disease, copy_prescript):
    ui = medical_record.ui
    if copy_diagnostic:
    ...
    if copy_prescript:
        ...
        for prescript_row in prescript_rows:
            ...
            if medicine_type in ['穴道', '處置']:
                ...
                medical_record.tab_list[0].append_prescript(row, prescript_row['Dosage'])
                medical_record.tab_list[0].append_null_medicine()


# 拷貝經驗方
def copy_experience(
        database, medical_record, case_date, experience_row,
        medicine_set, copy_diagnostic, copy_disease, copy_prescript):
    ui = medical_record.ui
    if copy_diagnostic:
        ui.textEdit_symptom.setText(string_utils.get_str(experience_row['ExpSymptom'], 'utf8'))
        ui.textEdit_tongue.setText(string_utils.get_str(experience_row['ExpTongue'], 'utf8'))
        ui.textEdit_pulse.setText(string_utils.get_str(experience_row['ExpPulse'], 'utf8'))
        ui.lineEdit_distinguish.setText(string_utils.xstr(experience_row['ExpDistincts']))
...
def backup_medical_record(database, case_key, deleter, delete_datetime, editor=None):
    backup_records_key = backup_cases(database, case_key, deleter, delete_datetime, editor)
...
def is_special_disease_ok(disease_code1, disease_code2, disease_code3, disease_code4, special_disease_list):
    if (disease_code1 in special_disease_list or
       disease_code2 in special_disease_list or
       disease_code3 in special_disease_list or
       disease_code4 in special_disease_list):
        check_ok = True
    else:
        check_ok = False
...
def is_moderate_complicated_acupuncture_ok(
        ...
        moderate_complicated_acupuncture_list, special_disease_list):
    if (disease_code1 in moderate_complicated_acupuncture_list or
       disease_code2 in moderate_complicated_acupuncture_list or
       disease_code3 in moderate_complicated_acupuncture_list or
       disease_code4 in moderate_complicated_acupuncture_list):
        check_ok = True
    elif is_special_disease_ok(disease_code1, disease_code2, disease_code3, disease_code4, special_disease_list):
        check_ok = True
    else:
        check_ok = False
...
def is_moderate_complicated_acupuncture_with_special_disease_ok(
    ...
    if (disease_code1 in moderate_complicated_acupuncture_list or
       disease_code2 in moderate_complicated_acupuncture_list or
       disease_code3 in moderate_complicated_acupuncture_list or
       disease_code4 in moderate_complicated_acupuncture_list) and \
       is_special_disease_ok(disease_code1, disease_code2, disease_code3, disease_code4, special_disease_list):
        check_ok = True
    else:
        check_ok = False
...
def is_highly_complicated_acupuncture_ok(
        disease_code1, disease_code2, disease_code3, disease_code4,
        highly_complicated_acupuncture_list, moderate_complicated_acupuncture_list, special_disease_list):
    if (disease_code1 in highly_complicated_acupuncture_list or
       disease_code2 in highly_complicated_acupuncture_list or
       disease_code3 in highly_complicated_acupuncture_list or
       disease_code4 in highly_complicated_acupuncture_list):
        check_ok = True
...
def is_moderate_complicated_massage_ok(
        disease_code1, disease_code2, disease_code3, disease_code4,
        moderate_complicated_massage_list, special_disease_list):
    if (disease_code1 in moderate_complicated_massage_list or
       disease_code2 in moderate_complicated_massage_list or
       disease_code3 in moderate_complicated_massage_list or
       disease_code4 in moderate_complicated_massage_list):
        check_ok = True
    elif is_special_disease_ok(disease_code1, disease_code2, disease_code3, disease_code4, special_disease_list):
        check_ok = True
    else:
...
def is_moderate_complicated_massage_with_special_disease_ok(
    ...
    if (disease_code1 in moderate_complicated_massage_list or
       disease_code2 in moderate_complicated_massage_list or
       disease_code3 in moderate_complicated_massage_list or
       disease_code4 in moderate_complicated_massage_list) and \
       is_special_disease_ok(disease_code1, disease_code2, disease_code3, disease_code4, special_disease_list):
        check_ok = True
    else:
        check_ok = False
...
def is_highly_complicated_massage_ok(
        disease_code1, disease_code2, disease_code3, disease_code4,
        highly_complicated_massage_list, moderate_complicated_massage_list, special_disease_list):
    if (disease_code1 in highly_complicated_massage_list or
       disease_code2 in highly_complicated_massage_list or
       disease_code3 in highly_complicated_massage_list or
       disease_code4 in highly_complicated_massage_list):
        check_ok = True
...
def is_dislocate_ok(disease_code1, disease_code2, disease_code3, disease_code4, dislocate_list):
    if (disease_code1 in dislocate_list or
       disease_code2 in dislocate_list or
       disease_code3 in dislocate_list or
       disease_code4 in dislocate_list):
        check_ok = True
    else:
        check_ok = False
...
def is_fracture_ok(disease_code1, disease_code2, disease_code3, disease_code4, fracture_list):
    if (disease_code1 in fracture_list or
       disease_code2 in fracture_list or
       disease_code3 in fracture_list or
       disease_code4 in fracture_list):
        check_ok = True
    else:
        check_ok = False
...
def check_treatment_disease(
        ...
        moderate_complicated_massage_list, highly_complicated_massage_list,
        dislocate_list, fracture_list, special_disease_list,
        row):
    ...
    if treatment == '中度複雜性針灸':
        ...
        if acupuncture_ok and massage_ok:
            return True
    elif treatment in ['中度針灸合併中度傷科合併特殊疾病起始次', '中度針灸合併中度傷科合併特殊疾病後續治療']:
        acupuncture_ok = is_moderate_complicated_acupuncture_ok(
           disease_code1, disease_code2, disease_code3, disease_code4,
           moderate_complicated_acupuncture_list, special_disease_list)
        ...
        if acupuncture_ok and massage_ok:
            return True
    elif treatment in ['高度針灸合併中度傷科合併特殊疾病起始次', '高度針灸合併中度傷科合併特殊疾病後續治療']:
        acupuncture_ok = is_highly_complicated_acupuncture_ok(
           disease_code1, disease_code2, disease_code3, disease_code4,
           highly_complicated_acupuncture_list, moderate_complicated_acupuncture_list, special_disease_list)
        massage_ok = is_moderate_complicated_massage_with_special_disease_ok(
        ...
        if is_dislocate_ok(disease_code1, disease_code2, disease_code3, disease_code4, dislocate_list):
            return True
    elif treatment == '骨折復位':
        if is_fracture_ok(disease_code1, disease_code2, disease_code3, disease_code4, fracture_list):
            return True
    else:
        return True
...
def update_traditional_health_care(database, system_settings, in_case_key, traditional_health_care_fee):
    sql = f'''
        SELECT
    ...
    fields = [
        ...
        'SMassageFee', 'SelfTotalFee',
        'TotalFee', 'ReceiptFee',
        'DoctorDate', 'DoctorDone',
        'ChargeDate', 'ChargeDone', 'ChargePeriod',
    ]
    data = [
        treat_type,
    ...
    database.exec_sql(f'DELETE FROM prescript WHERE CaseKey = {in_case_key}')
    _insert_traditional_health_care_prescript(database, system_settings, in_case_key, traditional_health_care_fee)


def write_traditional_health_care(database, system_settings, in_case_key,
                                  traditional_health_care_fee=None, massager=None):
    case_key = insert_traditional_health_care_cases(database, in_case_key, traditional_health_care_fee, massager)

    if case_key is not None:
        insert_traditional_health_care_prescript(database, system_settings, case_key, traditional_health_care_fee)


def insert_traditional_health_care_cases(database, in_case_key, traditional_health_care_fee, massager):
    sql = f'''
        SELECT
    ...
    fields = [
        'CaseDate', 'DoctorDate', 'ChargeDate', 'PatientKey', 'Name', 'Visit',
        'RegistType', 'Injury', 'Share',
        'InsType', 'TreatType', 'Period', 'ChargePeriod',
        'Room', 'RegistNo', 'Massager', 'Register', 'DesignatedMassager',
        'SMassageFee', 'SelfTotalFee', 'TotalFee', 'ReceiptFee',
        'DoctorDone', 'ChargeDone', 'Position1',
...
def insert_traditional_health_care_prescript(
        database, system_settings, case_key, traditional_health_care_fee, folk_massage_name=None):
    medicine_set = 2
...
def insert_prescript(database, case_key, medicine_type, medicine_name, dosage, unit, fee):
    medicine_set = 2
...
def get_first_course_field(database, case_date, patient_key, card, field, treat_item=None):
    ...
    if treat_item == 'AH':  # home care
        ...
        last_month = (datetime.date(case_date.year, case_date.month, 1) - datetime.timedelta(1)).replace(day=1)
        start_date = f'{last_month} 00:00:00'        
        sql = f'''
...
def get_prescript_record_from_json(database, system_settings, backup_records_key, json_medical_record):
    sql = f'''
        SELECT * FROM backup_records
        WHERE
            TableName = "prescript" AND
    ...
    html = get_prescript_medicine_record_from_json(
        database, system_settings, backup_records_key, json_rows, dosage_mode, medicine_set=1)
...
def get_prescript_medicine_record_from_json(
    ...
    prescript_data, total_dosage, total_fee, single_day_dosage = get_prescript_html_data_from_json(
...
def get_dosage_html_from_json(database, system_settings, backup_records_key, medicine_set, total_dosage, total_fee):
    dosage_data = ''
...
def get_regist_type_sql(regist_type):
    if regist_type == '全部':
        ...
        nhi_regist_type = set(nhi_utils.REG_TYPE).difference(nhi_utils.LONG_TERM_CARE + nhi_utils.TOUR_TYPE)
        sql = f'''
...
def get_last_highly_complicated_massage_date(
        database, case_date, patient_key, disease_code1,
        disease_code2=None, disease_code3=None, disease_code4=None):
    ...
    for i in range(2):
        ...
        if len(rows) > 0:
            ...
            if (disease_code1 == string_utils.xstr(row['DiseaseCode1']) and days == 180) or days == 90:
...
def get_prescript_medicine_json_record(database, system_settings, row, medicine_set):
    ...
    prescript_data += get_dosage_json_html(database, row, medicine_set, total_dosage, total_fee)
    if medicine_set == 1:
        prescript_heading = '健保處方'
    else:
        prescript_heading = f'自費處方{medicine_set-1}'
...
def get_prescript_json_html_data(database, system_settings, row, medicine_set, treatment=None):
    prescript_data = ''
    total_dosage = 0.0
    total_fee = 0.0
...
def get_ins_prescript_treat_json_record(database, system_settings, row):
    ...
    '''
    prescript_data, _, _ = get_prescript_json_html_data(database, system_settings, row, 1, treatment)
...
def copy_medical_record_json(
        database, system_settings, medical_record, extension_json_key, copy_diagnostic, copy_remark, copy_disease,
        copy_ins_prescript, copy_ins_prescript_to, copy_ins_treat, copy_self_prescript, not_overwrite=False):
    ...
    if copy_self_prescript:
        ...
        for json_row in rows:
            ...
            tab_index = medicine_set - 1
            if medical_record.tab_list[tab_index] is None or \
                    medical_record.tab_list[tab_index].tableWidget_prescript.item(
                        0, prescript_utils.SELF_PRESCRIPT_COL_NO['MedicineName']) is None:
                medical_record.add_prescript_tab(medicine_set)
            else:
...
def get_regist_type_exclude_sql(option, exclude=True):
    ...
    if '資源不足' in option:
        regist_condition.append(f' {tuple(nhi_utils.TOUR_FAR + nhi_utils.AT_LACK_AREA + nhi_utils.GOTO_LACK_AREA)} ')

    if '巡迴醫療' in option:
        regist_condition.append(f' {tuple(nhi_utils.TOUR_MOUNTAIN_ISLAND)} ')

    if '法定傳染病' in option:
        regist_condition.append(f' {tuple(nhi_utils.INFECTIOUS_TYPE + nhi_utils.INFECTIOUS_TYPE)} ')

    if '視訊門診' in option:
        regist_condition.append(f' {tuple(nhi_utils.TELECOM_TYPE)} ')

    if '照護機構' in option:
        regist_condition.append(f' {tuple(nhi_utils.LONG_TERM_CARE + nhi_utils.LONG_TERM_CARE)} ')
...
def set_disease_tool_tip(database, disease_code, disease_name, complicated_treat_list=None):
...
def extract_treatment(treatment):
    ...
    if treatment in ['中度針灸合併高度傷科', '高度針灸合併高度傷科']:  # 轉先前舊的高度傷科
        treatment += '起始次'
...
def copy_reference_prescript(
        database, system_settings, medical_record, reference_key, copy_diagnostic, copy_prescript):
    sql = f'''
...
def get_medical_record_code_data(database, system_settings, case_key):
    ...
    case_type = nhi_utils.get_case_type(database, system_settings, row, diag_fee, ins_total_fee)
    disease_code = string_utils.xstr(row['DiseaseCode1'])
    pharmacy_times = 1
    pres_type = 'A'  # A: 一般處方箋, B: 慢性病處方箋 C: 慢性病連續處方箋

    qr_code = f'{clinic_id}|{patient_id}|{name}|{gender_code}|{birth_date}|{doctor_id}|' + \
...
def get_prescript_code_data(database, case_key):
    ...
    for row_no, row in enumerate(rows):
        ...
        qr_code += f'|{order_type}|{order_seq}|{ins_code}|{medicine_name}|{ingredient}|' + \
