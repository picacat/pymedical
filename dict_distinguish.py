...
class DictDistinguish(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None, *args):
        super(DictDistinguish, self).__init__(parent)
        self.parent = parent
    ...
    def _set_ui(self):
        ...
        self.table_widget_dict_distinguish = class_utils.get_table_widget(
            self.ui.tableWidget_dict_distinguish, self.database
        )
        self.table_widget_dict_distinguish.set_column_hidden([0])
    ...
    def _set_signal(self):
        ...
        self.ui.toolButton_add_distinguish.clicked.connect(self._add_distinguish)
        self.ui.toolButton_remove_distinguish.clicked.connect(self._remove_distinguish)
        self.ui.toolButton_edit_distinguish.clicked.connect(self._edit_distinguish)
        self.ui.tableWidget_dict_distinguish.doubleClicked.connect(self._edit_distinguish)
    ...
    def dict_groups_name_changed(self):
        ...
        self.ui.groupBox_dict_distinguish.setTitle(f'{self.dict_type}資料 - [{dict_groups_name}]')
        self._read_dict_distinguish(dict_groups_name)
        self.ui.tableWidget_dict_groups_name.setFocus(True)

    def _read_dict_distinguish(self, dict_groups_name):
        sql = f'''
            SELECT * FROM clinic
            WHERE
        ...
        '''
        self.table_widget_dict_distinguish.set_db_data(sql, self._set_dict_distinguish_data)
    ...
    def _set_dict_distinguish_data(self, rec_no, rec):
        ...
        for column in range(len(dict_distinguish_rec)):
            self.ui.tableWidget_dict_distinguish.setItem(
                rec_no, column,
                QtWidgets.QTableWidgetItem(dict_distinguish_rec[column])
            )
    ...
    def _remove_distinguish(self):
        distinguish_name = self.table_widget_dict_distinguish.field_value(3)
        msg_box = dialog_utils.get_message_box(
            f'刪除{self.dict_type}資料',
            QMessageBox.Warning,
            f'<font size="5" color="red"><b>確定刪除{self.dict_type}: "{distinguish_name}"?</b></font>',
            '注意！資料刪除後, 將無法回復!'
        )
        remove_record = msg_box.exec_()
        if not remove_record:
        ...
        key = self.table_widget_dict_distinguish.field_value(0)
        self.database.delete_record('clinic', 'ClinicKey', key)
        self.ui.tableWidget_dict_distinguish.removeRow(self.ui.tableWidget_dict_distinguish.currentRow())

    # 更改主訴
    def _edit_distinguish(self):
        clinic_key = self.table_widget_dict_distinguish.field_value(0)
