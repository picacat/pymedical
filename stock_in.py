...
# 進貨 2022.11.19 誠泰
class StockIn(QtWidgets.QMainWindow):
    # 初始化
    def __init__(self, parent=None, *args):
        super(StockIn, self).__init__(parent)
        self.parent = parent
        self.args = args
    ...
    # 設定GUI
    def _set_ui(self):
        self.ui = ui_utils.load_ui_file(ui_utils.UI_STOCK_IN, self)
        system_utils.set_css(self, self.system_settings)
        system_utils.center_window(self)

        self.tab_stock_in_list = module_utils.get_stock_in_list(self, *self.args)
        self.ui.tabWidget_stock.addTab(self.tab_stock_in_list, '進貨資料')
    ...
    def add_stock_in(self, order_no, stock_in_key):
        tab_stock_in_data = module_utils.get_stock_in_data(
            self, self.database, self.system_settings, stock_in_key)

        if stock_in_key is None:
            self.ui.tabWidget_stock.addTab(tab_stock_in_data, '輸入進貨單')
        else:
            tab_name = f'進貨單{order_no}'
            if self._tab_exists(tab_name):
                return

            self.ui.tabWidget_stock.addTab(tab_stock_in_data, tab_name)

        self.ui.tabWidget_stock.setCurrentWidget(tab_stock_in_data)
    ...
    def update_stock(self):
        self.tab_stock_in_list.update_stock()
