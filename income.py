...
from libs import (dialog_utils, module_utils, personnel_utils, printer_utils,
...
class Income(QtWidgets.QMainWindow):
    ...
    def _set_signal(self):
        ...
        self.ui.action_print_portrait.triggered.connect(lambda: self._print_income('portrait'))
        self.ui.action_print_portrait2.triggered.connect(lambda: self._print_income2('portrait'))
        self.ui.action_print_landscape.triggered.connect(lambda: self._print_income('landscape'))
        self.ui.action_print_portrait_pdf.triggered.connect(lambda: self._print_income('portrait', 'pdf'))
        self.ui.action_print_landscape_pdf.triggered.connect(lambda: self._print_income('landscape', 'pdf'))
    ...
    def open_dialog(self):
        dialog = dialog_utils.get_dialog_income(self, self.database, self.system_settings, '掛號櫃台結帳')
        ...
        if dialog.exec_():
            ...
            self.dialog_setting['regist_type'] = dialog.comboBox_regist_type.currentText()
    ...
    def start_calculate(self):
        ...
        self.tab_income_cash_flow = module_utils.get_income_cash_flow(
            ...
            regist_type, therapist, room, cashier,
            income_source, calculate_by_cashier,
        )
        self.tab_income_list = module_utils.get_income_list(
            self, self.database, self.system_settings, start_date, end_date, period, regist_type, therapist, room,
            self.tab_income_cash_flow.ui.tableWidget_registration,
            self.tab_income_cash_flow.ui.tableWidget_charge,
            income_source, self.income_list_columns
        )
    ...
    def _check_unpaid_cases(self, start_date, end_date, period, regist_type, doctor, room, cashier):
    ...
    def _check_unpaid_record(self, start_date, end_date, period, doctor, room, cashier, regist_type):
        sql = f'''
    ...
    def _print_income(self, orientation, print_type=None):
        printer_utils.print_income(
            self, self.database, self.system_settings, orientation,
            ...
            self.income_list_columns, print_type
        )

    def _print_income2(self, orientation, print_type=None):
        printer_utils.print_income2(
            self, self.database, self.system_settings, orientation,
            self.tab_income_cash_flow, self.tab_income_list, self.income_list_columns, print_type
        )
