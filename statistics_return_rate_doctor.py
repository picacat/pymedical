...
class StatisticsReturnRateDoctor(QtWidgets.QMainWindow):
    ...
    def _show_return_rate(self):
        ...
        label_return_rate.setText(
            f'''
                <center>
                    {self.doctor}醫師回診率 = 歸戶回診人數 / 歸戶總人數 <br>
                    {self.numerator} / {self.denominator} = <b>{return_rate: .2f}%</b> ({self.doctor_return_days}日內回診)
                </center>
            '''
        )
    ...
    def _plot_chart(self):
        ...
        chart.setTitle(f'{self.doctor}醫師回診率')
        chart.legend().hide()
    ...
    def export_to_excel(self):
        ...
        excel_file_name, _ = QFileDialog.getSaveFileName(
            ...
            f'{start_date}至{end_date}{self.doctor}醫師回診率統計表.xlsx',
        ...
        system_utils.show_message_box(
            ...
            '資料匯出完成',
            f'<h3>醫師回診率統計檔{excel_file_name}匯出完成.</h3>',
            'Microsoft Excel 格式.'
        )
